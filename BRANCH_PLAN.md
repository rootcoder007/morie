# feat/native-specializations — branch plan (morie r-package)

Ported from the rmorie branch (engines identical; benchmarks measured
there on L14 apply — same code, same machine). morie-specific deltas:
labels are "(morie native)", morie has no `morie_estimate_dr_forest`
export so module 11's forest engine is not carried (add with the
export if morie grows one), and the duplicate `morie_estimate_irm`
definition (causal.R shadowed by irm.R) existed here too and is fixed
the same way.
...
| 11 | (not exported in morie) | grf | N/A here — see rmorie branch |
