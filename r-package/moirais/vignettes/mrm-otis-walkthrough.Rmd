...
The **MRM (McNamara--Ruhela--Medina)** framework is a coordinated set
...
provincial Ontario Tracking Information System (OTIS)
...
OTIS is shipped with the package; the `moirais_load_dataset()` loader
hides the SQLite-backed indirection.

```{r load-otis, eval = FALSE}
library(moirais)
otis <- moirais_load_dataset("otis-2025-a01")
str(otis)
```
...
The returned object summarises the IPW (Hájek), AIPW
...
`moirais.tps_csi` and `moirais.siu_iap`.
...
- The full MRM framework paper, including all ten estimators,
  ...
  [10.5281/zenodo.20096075](https://doi.org/10.5281/zenodo.20096075).
- The MOIRAIS package paper covers the broader toolkit:
