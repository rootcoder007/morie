---
title: "IPW deep-dive (Hájek and Horvitz--Thompson)"
output:
...
blocks that MOIRAIS exposes: the Horvitz--Thompson and the
Hájek-stabilised IPW estimators, propensity-score modelling,
...
The `estimate_ate()` machinery fits a logistic propensity model
...
# Hájek-stabilised IPW

`estimate_ate()` defaults to the Hájek estimator, which divides
...
```{r hajek}
ate_hajek <- estimate_ate(df, "y", "treat", c("X1", "X2"),
                          propensity_col = "ps")
ate_hajek$estimate
ate_hajek$se
```
