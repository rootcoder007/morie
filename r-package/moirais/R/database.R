...
#' Get path to the built-in MOIRAIS datasets database
#'
#' Returns the path to \code{moirais.db} that ships with the package.
#' This database contains all CPADS, CCS, CSADS, CSUS, HealthInfobase, and
#' CIHI datasets pre-loaded as SQLite tables.
#'
#' @return File path string.
#' @export
moirais_builtin_db <- function() {
  root <- find_project_root()
  file.path(root, "data", "cache", "moirais.db")
}

#' Connect to the MOIRAIS SQLite cache database
#'
#' Opens (or creates) the shared cache at \code{data/cache/moirais.db}.
#' Both R (DBI/RSQLite) and Python (sqlite3) read/write this same file.
#'
#' @param db_path Path to the SQLite file. Defaults to
#'   \code{MOIRAIS_CACHE_DB} env var or \code{data/cache/moirais.db}.
#' @return A DBI connection object.
#' @export
moirais_db_connect <- function(db_path = NULL) {
  ...
  if (is.null(db_path)) {
    db_path <- Sys.getenv("MOIRAIS_CACHE_DB", "data/cache/moirais.db")
  }
...
#' Store a data frame in the MOIRAIS cache
#'
...
#' @export
moirais_cache_store <- function(data, table_name, db_path = NULL) {
  con <- moirais_db_connect(db_path)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
  DBI::dbWriteTable(con, table_name, data, overwrite = TRUE)
  invisible(nrow(data))
}

#' Load a table from the MOIRAIS cache
#'
#' @param table_name Name of the SQLite table.
#' @param db_path Optional override for the database path.
#' @return A data.frame, or \code{NULL} if the table does not exist.
#' @export
moirais_cache_load <- function(table_name, db_path = NULL) {
  con <- moirais_db_connect(db_path)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
...
#' List all tables in the MOIRAIS cache
#'
#' @param db_path Optional override for the database path.
#' @return A data.frame with columns \code{table} and \code{rows}.
#' @export
moirais_cache_list <- function(db_path = NULL) {
  con <- moirais_db_connect(db_path)
  on.exit(DBI::dbDisconnect(con), add = TRUE)
...
#' @export
moirais_cache_file <- function(path, table_name, db_path = NULL) {
  ...
  moirais_cache_store(data, table_name, db_path)
}
...
#'   \item SQLite cache (\code{data/cache/moirais.db})
...
#' @export
moirais_load_cpads <- function(db_path = NULL, use_ckan = TRUE) {
  # 1. Local files.
  local_paths <- c(
  ...
  for (p in local_paths) {
    if (file.exists(p)) {
      ...
      tryCatch(moirais_cache_store(data, "cpads_canonical", db_path), error = function(e) NULL)
      return(data)
    }
  }

  # 2. SQLite cache.
  cached <- moirais_cache_load("cpads_canonical", db_path)
  if (!is.null(cached)) {
  ...
  if (use_ckan) {
    ...
    data <- moirais_fetch_ckan("cpads", db_path = db_path)
    return(data)
  }
...
#' @return A data.frame.
#' @export
moirais_fetch_ckan <- function(dataset_key = "cpads", limit = 32000L, db_path = NULL) {
  ...
  tryCatch(moirais_cache_store(records, table_name, db_path), error = function(e) {
...
#' Supports fuzzy matching: \code{moirais_load_dataset("cpads_2021")} resolves
...
#' @return A data.frame.
#' @export
moirais_load_dataset <- function(key, db_path = NULL) {
  matched <- .fuzzy_match_key(key)
  if (is.null(matched)) {
    stop("Unknown dataset key: '", key, "'. See moirais_dataset_catalog().", call. = FALSE)
  }
  catalog <- moirais_dataset_catalog()
  entry <- catalog[catalog$key == matched, ]

  # 1. Built-in database (ships with package).
  builtin_path <- tryCatch(moirais_builtin_db(), error = function(e) NULL)
  ...
  # 2. User cache.
  cached <- moirais_cache_load(entry$table_name, db_path)
  if (!is.null(cached)) {
  ...
  if (file.exists(entry$local_path)) {
    ...
    moirais_cache_store(data, entry$table_name, db_path)
    return(data)
  }
  ...
  if (nzchar(entry$ckan_resource_id)) {
    ...
    data <- moirais_fetch_ckan(entry$survey, db_path = db_path)
    return(data)
  }
...
#' @export
moirais_list_datasets <- function(db_path = NULL) {
  catalog <- moirais_dataset_catalog()
  cached_tables <- tryCatch({
    cl <- moirais_cache_list(db_path)
...
#' @export
moirais_dataset_info <- function(key) {
  matched <- .fuzzy_match_key(key)
...
#' @export
#' Get path to an MOIRAIS userguide
#'
...
#' @export
moirais_userguide <- function(name = NULL) {
  if (is.null(name)) {
    dir(system.file("extdata", "userguides", package = "moirais"))
  } else {
    system.file("extdata", "userguides", name, package = "moirais", mustWork = TRUE)
  }
}

moirais_download_bootstrap <- function(survey = "all", limit = 32000L, db_path = NULL) {
  bootstrap_keys <- list(
  ...
  for (key in targets) {
    ...
    if (file.exists(entry$local_path)) {
      ...
      moirais_cache_file(entry$local_path, entry$table_name, db_path)
    ...
    if (nzchar(entry$ckan_resource_id)) {
      ...
      tryCatch({
        data <- moirais_fetch_ckan(entry$survey, limit = limit, db_path = db_path)
        moirais_cache_store(data, entry$table_name, db_path)
