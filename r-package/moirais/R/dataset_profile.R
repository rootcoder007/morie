...
#'
#' Mirrors the Python `moirais.infer_measurement_level()`. Heuristically
...
#'
#' Mirrors the Python `moirais.profile_dataset()`. Returns a list of
...
profile_dataset <- function(df) {
  ...
  cols <- lapply(names(df), function(nm) {
    ...
    base <- list(
      ...
      measurement_level = infer_measurement_level(x),
...
#'
#' Mirrors the Python `moirais.suggest_analysis_plan()`. Inspects the output
#' of [profile_dataset()] and returns plain-English recommendations for
#' candidate analyses.
#'
#' @param profile A list returned by [profile_dataset()].
#'
