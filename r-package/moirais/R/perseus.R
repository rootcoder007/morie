...
#' @param question User question.
#' @param context Optional context string.
#' @param python_bin Python executable to use. Defaults to `MOIRAIS_PYTHON_BIN` or `python3`.
#' @return Agent text response.
#' @export
ask_percy <- function(question, context = NULL, python_bin = Sys.getenv("MOIRAIS_PYTHON_BIN", "python3")) {
  prompt <- build_prompt(question, context = context)

  code <- paste(
    "import json, sys",
    "from moirais.perseus import ask_percy",
  ...
  if (!is.null(status) && status != 0) {
    stop(
      "Perseus agent call failed. Ensure the Python MOIRAIS package is available ",
...
moirais_assistant_query <- ask_percy
