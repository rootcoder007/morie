...
#'   `"moirais_legacy"` reproduces previous EML legacy column names.
#' @return Named character vector.
#' @export
default_synthetic_name_map <- function(profile = c("generic", "moirais_legacy")) {
...
generate_synthetic_data <- function(
  ...
  profile = c("generic", "moirais_legacy"),
  name_map = NULL
) {
  n <- as.integer(n)
  if (is.na(n) || n < 100L) {
