...
#'
#' DETERMINISM.  Subsets are enumerated in lexicographic order rather than
#' drawn at random, so both language arms examine the same candidates.
#'
...
Mvedet <- function(X, h = NULL, n_starts = 100000L) {
  ...
  for (J in .rscombos(n, p + 1L, as.integer(n_starts))) {
    mc <- .rsmeancov(Xm, J)
