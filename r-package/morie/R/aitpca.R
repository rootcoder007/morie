...
#'
#' Formula: clr(x) = log x - mean(log x); eigendecompose the clr covariance; proportion
#' retained = (s_1^2+...+s_r^2)/sum s^2
#'
...
#' @param X See Usage.
#' @param k See Usage.
#' @return List with ``values``, ``loadings``, ``scores``, ``prop_var``, ``cum_prop``,
#' ``k``, ``n``, ``D``.
#' @references Aitchison, A Concise Guide to Compositional Data Analysis, Chapter 2.
#' Verified against the text: the centre estimate is xi-hat = C(g_1, ..., g_D) with g_i
#' the geometric mean of the ith component, and totvar(x) = trace(Gamma) = (1/D)
#' sum_\{i<j\} var\{log(x_i/x_j)\}.
#' @export
#' @examples
