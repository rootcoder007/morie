...
#' .w4a_need
#'
#' A step of the geron_w4a_native implementation. Called by
#' \code{morie_beta_schedule_values}, \code{morie_check_buffer}, \code{morie_dueling_q}
#' and 35 others in the module.
...
#'
#' A step of the geron_w4a_native implementation. Called by
#' \code{morie_geron_dataloader}, \code{morie_geron_dropout_alt},
#' \code{morie_lcg_normal}.
#' See the file header for the source the module follows.
#' the source it follows.
#'
...
morie_geron_block_params <- function(d_model, d_ff = NULL, cross_attention = FALSE) {
  ...
  if (cross_attention) { out$cross_attention <- attn
...
morie_geron_encoder_only <- function(X, n_layers = 12, n_heads = 12, d_model = 768,
  ...
  per <- morie_geron_block_params(d, d_ff = d_ff, cross_attention = FALSE)
  emb <- V * d + M * d + S * d
  head <- if (is.null(n_classes)) 0L else { .w4a_need(as.integer(n_classes) >= 2L, "n_classes must be >= 2.")
...
morie_geron_flash_attention <- function(Q, K, V, block_size = 2, causal = FALSE) {
  ...
  S <- (Qa %*% t(Ka)) * scale
  if (causal) { mask <- outer(seq_len(N), seq_len(M), function(a, b) b > a)
  S[mask] <- -Inf }
  Sm <- S - apply(S, 1, max)
  Ex <- exp(Sm)
...
morie_geron_dino <- function(images, student, teacher, center = NULL, tau_s = 0.1, tau_t = 0.04,
  ...
  softmax <- function(Z) { Z <- Z - apply(Z, 1, max)
  E <- exp(Z)
  E / rowSums(E) }
...
#' @param pi,pi_ref Log-probs (B, 2) under policy / reference; col 0 = chosen unless
#' `preferences` given.
#' @param preferences 0/1 winner index per row (default all 0).
#' @param beta Inverse temperature (>0).
#' @return List with `loss`, `per_pair_loss`, `margin`, `reward_chosen`,
#' `reward_rejected`, `accuracy`.
#' @export
#' @examples
...
#' @return List with `quantized`, `scales`, `zero_points`, `dequantized`,
#' `max_abs_error`, `compression`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
morie_geron_deep_rnn <- function(X, hidden_sizes = 4, n_layers = NULL, weights = NULL, seed = 0, activation = "tanh") {
  ...
  if (!is.null(weights)) {
    ...
    draw <- function(n, sd) {
      u <- numeric(n)
      for (i in seq_len(n)) { s <<- (1664525 * s + 1013904223) %% 2^32
      u[i] <- (s + 0.5) / 2^32 }
      (2 * u - 1) * sqrt(3) * sd
    }
    layers <- list()
...
#' @return List with `variance`, `bias2`, `root_splits`, `structural_instability`,
#' `ensemble_prediction`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
#' @return List with `tree`, `n_leaves`, `baseline_leaves`, `leaves_saved`,
#' `train_score`, `train_score_cost`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
#' @param criterion,max_depth CART controls.
#' @return List with `predictions_match`, `thresholds`, `scaled_thresholds`,
#' `thresholds_match`, `knn_match`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
morie_geron_early_stopping_alt <- function(X_train, y_train, X_val, y_val, n_iter = 100, eta = 0.01,
  ...
  for (it in seq_len(T_)) {
    ...
    v <- rmse(Bm, theta, yv)
    va_hist <- c(va_hist, v)
    if (v < best$val - 1e-15) { best <- list(val = v, iter = it, theta = theta)
    since <- 0L }
    else { since <- since + 1L
...
#' @return List with `total_params`, `encoder_params`, `decoder_params`, `src_mask`,
#' `tgt_mask`, `cross_mask`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
#' @return List with `explained_variance_ratio`, `explained_variance`, `singular_values`,
#' `cumulative`, `n_for_95`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
#' @return List with `predictions`, `tree_predictions`, `train_score`,
#' `single_tree_score`, `disagreement`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
morie_geron_extra_trees <- function(X, y, n_estimators = 10, max_features = NULL, seed = 0,
  ...
  unif <- function(n) { u <- numeric(n)
  for (i in seq_len(n)) { st <<- (1664525 * st + 1013904223) %% 2^32
  u[i] <- (st + 0.5) / 2^32 }
  u }
  ...
  grow <- function(Xs, ys, depth) {
    ...
    if (pure || (!is.null(max_depth) && depth >= max_depth) || length(ys) < 2L * msl) {
      val <- if (criterion == "mse") mean(ys) else { tb <- table(ys)
...
morie_geron_fcn <- function(image, model, upsample = 1, activation = "relu") {
  ...
  for (li in seq_along(model)) {
    layer <- model[[li]]
    if (is.list(layer) && length(layer) == 3) { K <- layer[[1]]
    bias <- layer[[2]]
    stride <- layer[[3]] }
    else { K <- layer
    bias <- 0
  ...
  up_map <- if (up > 1) { a <- scores[, rep(seq_len(dim(scores)[2]), each = up), , drop = FALSE]
  a[, , rep(seq_len(dim(scores)[3]), each = up), drop = FALSE] } else scores
...
#' @return List with `output`, `visual_tokens`, `cross_attention`, `gate_value`,
#' `is_identity_at_init`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
#' @return List with `value`, `sign`, `exponent`, `mantissa_field`, `rel_error`,
...
morie_geron_fp16_quant <- function(x) {
  ...
  for (i in seq_len(n)) {
    ...
    mant <- av / 2^e
    if (mant >= 2) { mant <- mant / 2
    e <- e + 1 }
    if (mant < 1) { mant <- mant * 2
    e <- e - 1 }
    eb <- e + 15L
    if (eb >= 31L) { ef[i] <- 31L
    mf[i] <- 0L
...
morie_geron_few_shot <- function(model, examples, query, k = NULL, separator = "\n",
  ...
  fmt <- function(xx, yy) { s <- sub("\\{x\\}", xx, template)
  sub("\\{y\\}", yy, s) }
...
morie_geron_gan <- function(X, G = NULL, D = NULL, z_dim = 1, epochs = 200, lr = 0.05, seed = 0, non_saturating = TRUE) {
  ...
  if (is.null(G)) { Wg <- matrix(0, k, d)
  bg <- rep(0, d) } else { Wg <- .morie_gr_mat(G[[1]], "Wg")
  bg <- as.numeric(G[[2]]) }
  if (is.null(D)) { wd_ <- rep(0, d)
  bd <- 0.0 } else { wd_ <- as.numeric(D[[1]])
  bd <- as.numeric(D[[2]]) }
...
#' @return List with `branch_1x1`, `branch_3x3`, `branch_5x5`, `branch_pool`,
#' `out_channels`, `params`.
#' @export
#' @examples
...
#' @return List with `layers`, `modules`, `total_params`, `final_feature_map`,
#' `total_reduction_saving`.
#' @export
#' @examples
...
#' @param ... Other decoder_only args.
#' @return List with `total_params`, `non_embedding_params`, `config`, `params_vs_small`,
#' `all_sizes`.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
...
#' @return List with `total_parameters`, `parameters_per_layer`, `breakdown`,
#' `shape_trace`, `kv_cache_bytes`.
#' @export
#' @examples
...
#' Exhaustive grid search scored by K-fold CV, delegated to
#' `morie_geron_cross_validation_score` (hmgrs)
...
morie_geron_dueling_dqn_alt <- function(env, V, A, buffer, epochs = 10, lr = 0.1, gamma = 0.95, target_sync = 5) {
  ...
  for (ep in seq_len(E)) {
    ...
    if (ep %% sync == 0L) { Qt <- morie_dueling_q(Vv, Av)
    syncs <- c(syncs, ep) }
  }
  Q <- morie_dueling_q(Vv, Av)
  denom <- mean(abs(Q))
