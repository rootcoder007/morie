...
# they had chosen, with no warning. .morie_local_seed() sets the seed
...
.morie_restore_seed <- function(old) {
  if (is.null(old)) {
...
.morie_local_seed <- function(seed, envir = parent.frame()) {
  if (is.null(seed)) {
  ...
  do.call(on.exit,
          list(bquote((.(.morie_restore_seed))(.(old))),
...
.morie_local_det_rng <- function(name, seed, envir = parent.frame()) {
  old <- if (exists(".Random.seed", envir = globalenv(), inherits = FALSE)) {
    get(".Random.seed", envir = globalenv(), inherits = FALSE)
  } else {
    NULL
