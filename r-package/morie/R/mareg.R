...
#' where it comes from. The residual \code{tau^2} is the part the
...
#'
#' Formula: \code{y_i = x_i' beta + u_i + e_i}, \code{Var(u) = tau^2},
...
Mareg <- function(yi, vi, X) {
  ...
  wls <- function(Xd, w, pp) {
    A <- crossprod(Xd * w, Xd)
    bb <- as.numeric(crossprod(Xd * w, y))
    beta <- as.numeric(.s03ridgesolve(A, bb, 1e-12))
