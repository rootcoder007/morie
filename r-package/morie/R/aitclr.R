...
#' row in EXTERNAL_SOURCES.md.  clr(x) = (ln(x_1/g(x)), ..., ln(x_D/g(x))).  The
#' transform maps S^D onto the hyperplane of vectors summing to zero, so ``sum_check`` is
#' zero up to rounding and is reported as a check on the caller's data rather than as a
#' result.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
