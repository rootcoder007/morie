...
morie_crypto_slhdsa_sign <- function(sk, message) {
  ...
  .Call(`_morie_morie_crypto_slhdsa128s_sign`, sk, message)
}
...
morie_crypto_slhdsa_verify <- function(pk, message, signature) {
  ...
  if (is.character(message)) message <- charToRaw(message)
  .Call(`_morie_morie_crypto_slhdsa128s_verify`, pk, message, signature)
}
...
morie_crypto_hqc_encaps <- function(pk) {
  stopifnot(is.raw(pk))
  .Call(`_morie_morie_crypto_hqc128_encaps`, pk)
}
...
morie_crypto_hqc_decaps <- function(sk, ct) {
  ...
  .Call(`_morie_morie_crypto_hqc128_decaps`, sk, ct)
}
...
morie_crypto_lamport_keygen <- function() {
  ...
  for (b in 1:2) {
    for (i in 1:256) {
      ...
      sk[[b, i]] <- s
      pk[b, i] <- .morie_sha256_hex_impl(s)
    }
  }
...
morie_crypto_lamport_sign <- function(keypair, message) {
  ...
  structure(list(reveal = reveal, digest_hex = .morie_sha256_hex_impl(message)),
...
morie_crypto_lamport_verify <- function(pk, message, signature) {
  ...
  if (is.character(message)) message <- charToRaw(message)
  dg <- .morie_sha256_impl(message)
  if (!identical(.morie_sha256_hex_impl(message), signature$digest_hex)) {
    return(FALSE)
  }
  bits <- as.integer(rawToBits(dg))
  for (i in 1:256) {
    if (!identical(
      .morie_sha256_hex_impl(signature$reveal[[i]]),
      pk[bits[i] + 1L, i]
    )) {
      return(FALSE)
    }
