...
.morie_gr_init <- function(nrow, ncol, seed, scale = 0.1) {
  matrix(.morie_gr_lcg_w(nrow * ncol, seed, scale), nrow = nrow,
...
morie_geron_adam <- function(grads, m = NULL, v = NULL, b1 = 0.9, b2 = 0.999,
  ...
  for (nm in c("m", "v", "theta")) {
    arr <- switch(nm, m = mm, v = vv, theta = th)
    .morie_gr_need(length(arr) == length(g),
                   sprintf("geron_adam: %s length %d but grads length %d",
                           nm, length(arr), length(g)))
  }
  .morie_gr_need(b1 >= 0 && b1 < 1 && b2 >= 0 && b2 < 1,
...
#' @export
#' @examples
#' morie_geron_adam_update(1, 0.1, 0, 0, t = 1, eta = 0.001)
morie_geron_adam_update <- function(theta, grad, m, s, t, eta, b1 = 0.9,
                                    b2 = 0.999, eps = 1e-8) {
  theta <- as.numeric(theta); grad <- as.numeric(grad)
  m <- as.numeric(m); s <- as.numeric(s)
  for (nm in c("grad", "m", "s")) {
    arr <- switch(nm, grad = grad, m = m, s = s)
    .morie_gr_need(length(arr) == length(theta),
                   sprintf("%s length %d != theta length %d", nm, length(arr),
                           length(theta)))
  }
  .morie_gr_need(length(theta) > 0L, "theta is empty.")
  .morie_gr_need(all(is.finite(theta)) && all(is.finite(grad)),
  ...
  list(theta_new = theta - step, m_new = m_new, s_new = s_new,
...
morie_geron_adamw <- function(grads, m = NULL, v = NULL, b1 = 0.9, b2 = 0.999,
  ...
  list(theta = th + step, theta_next = th + step, step = step,
...
morie_geron_adamax <- function(grads, m = NULL, u = NULL, b1 = 0.9, b2 = 0.999,
  ...
  th <- if (is.null(theta)) numeric(length(g)) else as.numeric(theta)
  .morie_gr_need(length(mm) == length(g) && length(uu) == length(g) &&
                   length(th) == length(g),
  ...
  list(theta = th + step, theta_next = th + step, step = step, m = m_new,
...
#' @export
#' @examples
#' morie_geron_adagrad_update(c(1, 2), c(2, -1), c(0, 3), 0.1, eps = 1e-10)
morie_geron_adagrad_update <- function(theta, grad, s, eta, eps = 1e-10) {
  theta <- as.numeric(theta); grad <- as.numeric(grad); s <- as.numeric(s)
  ...
  eff <- eta / denom
  step <- eff * grad
  list(theta_new = theta - step, s_new = s_new, effective_lr = eff,
...
#' @export
#' @examples
#' morie_geron_1cycle_schedule(0.1, 0.5, t = 1, T = 7)
morie_geron_1cycle_schedule <- function(eta_min, eta_max, t, T, mom_max = 0.95,
  ...
  .morie_gr_need(mom_min >= 0 && mom_min < mom_max && mom_max < 1,
  ...
  if (peak == 0L) {
    ...
    knots <- c(0, peak, T - 1)
    lr <- stats::approx(knots, c(eta_min, eta_max, eta_min), xout = steps,
                        rule = 2)$y
    mom <- stats::approx(knots, c(mom_max, mom_min, mom_max), xout = steps,
                         rule = 2)$y
  }
  list(lr_schedule = lr, momentum_schedule = mom, peak_step = as.integer(peak),
...
#' @export
#' @examples
#' morie_geron_batch_size_heuristic(n_train = 5L)
morie_geron_batch_size_heuristic <- function(n_train,
  ...
  list(batch_size = batch, steps_per_epoch = steps, candidates = cand,
...
morie_geron_aic <- function(log_lik, k, n = NULL) {
  ll <- as.numeric(log_lik); kk <- as.numeric(k)
  ...
  .morie_gr_need(length(ll) == length(kk),
  ...
  .morie_gr_need(all(kk >= 0) && all(kk == floor(kk)),
  ...
  scalar <- length(ll) == 1L
  list(aic = if (scalar) aic[1] else aic,
...
#' @export
#' @examples
#' morie_geron_bic(log_lik = c(1, 2, 3, 4, 5, 6, 7, 8), k = 5L, n = 5L)
morie_geron_bic <- function(log_lik, k, n) {
  ...
  scalar <- length(ll) == 1L
  list(bic = if (scalar) bic[1] else bic,
       delta = if (scalar) delta[1] else delta,
       weights = if (scalar) weights[1] else weights,
...
#' @export
#' @examples
#' morie_geron_gmm_n_params(k = 5L, d = 5L)
morie_geron_gmm_n_params <- function(k, d, covariance_type = "full") {
  k <- as.integer(k); d <- as.integer(d)
...
#' @export
#' @examples
#' morie_geron_aic_gmm(log_likelihood = 5L, n_params = 5L)
morie_geron_aic_gmm <- function(log_likelihood, n_params) {
  ...
  penalty <- 2 * p
  list(aic = penalty - 2 * ll, log_likelihood = ll, n_params = p,
...
#' @export
#' @examples
#' morie_geron_bic_gmm(log_likelihood = 5L, n = 5L, n_params = 5L)
morie_geron_bic_gmm <- function(log_likelihood, n, n_params) {
  ...
  per <- log(n)
  penalty <- per * p
  list(bic = penalty - 2 * ll, log_likelihood = ll, n_params = p,
...
morie_geron_auc_roc <- function(y_true, scores, pos_label = 1) {
  ...
  pos <- y == pos_label
  n_pos <- sum(pos); n_neg <- length(y) - n_pos
  .morie_gr_need(n_pos > 0L && n_neg > 0L,
  ...
  list(auc = auc, fpr = fpr, tpr = tpr, thresholds = thr,
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_auc_roc_trapezoid(V, V)
morie_geron_auc_roc_trapezoid <- function(y_true, y_scores, pos_label = 1) {
  y <- as.vector(y_true); s <- as.numeric(y_scores)
  .morie_gr_need(length(y) == length(s),
  ...
  ss <- s[ord]; p <- as.numeric(pos[ord])
  tp <- cumsum(p); fp <- cumsum(1 - p)
  keep <- c(diff(ss) != 0, TRUE)
  tp <- c(0, tp[keep]); fp <- c(0, fp[keep])
  thresholds <- c(Inf, ss[keep])
  tpr <- tp / n_pos; fpr <- fp / n_neg
  auc <- sum(diff(fpr) * (utils::head(tpr, -1) + utils::tail(tpr, -1)) / 2)
  list(auc = auc, fpr = fpr, tpr = tpr, thresholds = thresholds,
       n_pos = as.integer(n_pos), n_neg = as.integer(n_neg),
       estimate = auc, n = length(y),
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_confusion_matrix(V, V)
morie_geron_confusion_matrix <- function(y_true, y_pred, n_classes = NULL) {
  ...
  .morie_gr_need(all(yt == floor(yt)) && all(yp == floor(yp)),
                 "labels must be whole numbers used as class indices.")
  yt <- as.integer(yt); yp <- as.integer(yp)
  .morie_gr_need(min(yt) >= 0L && min(yp) >= 0L,
  ...
  .morie_gr_need(max(yt) < K && max(yp) < K,
  ...
  list(matrix = cm, accuracy = acc, precision = prec, recall = rec, f1 = f1,
...
#' morie_geron_cart_split_cost(X, c(0, 0, 1, 1), 0, 1.5)
morie_geron_cart_split_cost <- function(X, y, feature, threshold,
  ...
  .morie_gr_need(feature >= 0L && feature < ncol(X),
  ...
  imp <- switch(criterion,
    gini = function(v) if (!length(v)) 0 else {
      p <- as.numeric(table(v)) / length(v); 1 - sum(p * p) },
    entropy = function(v) if (!length(v)) 0 else {
      p <- as.numeric(table(v)) / length(v); -sum(p * log2(p)) },
    mse = function(v) if (!length(v)) 0 else {
      v <- as.numeric(v); mean((v - mean(v))^2) })
  if (criterion == "mse") {
    y <- as.numeric(y)
  ...
  left <- col <= threshold
  yl <- y[left]; yr <- y[!left]
  ml <- length(yl); mr <- length(yr)
  gl <- imp(yl); gr <- imp(yr)
  cost <- (ml / m) * gl + (mr / m) * gr
  parent <- imp(y)
  list(cost = cost, impurity_left = gl, impurity_right = gr,
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_binary_classification(V, V)
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_bias_variance_tradeoff(V, V)
morie_geron_bias_variance_tradeoff <- function(preds, y, f_true = NULL) {
  ...
  .morie_gr_need(ncol(P) == length(yv),
                 "geron_bias_variance_tradeoff: preds points != y entries")
  .morie_gr_need(all(is.finite(P)) && all(is.finite(yv)),
  ...
  .morie_gr_need(length(target) == length(yv),
  ...
  list(bias2 = bias2, variance = variance, noise = noise, mse = mse,
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_batch_learning(V, V)
morie_geron_batch_learning <- function(X, y, fit_intercept = FALSE, ridge = 0) {
  ...
  .morie_gr_need(length(yv) == nrow(Xm),
                 "geron_batch_learning: X rows != y entries")
  .morie_gr_need(all(is.finite(Xm)) && all(is.finite(yv)),
  ...
  k <- ncol(D)
  theta <- if (lam == 0) .morie_gr_lstsq(D, yv) else {
    P <- diag(lam, k)
  ...
  list(theta = theta, predict = predict_fn, fitted = fitted, residuals = resid,
...
morie_geron_batch_gd_grad <- function(X, y, theta, eta = NULL) {
  ...
  .morie_gr_need(all(is.finite(Xm)) && all(is.finite(yv)) && all(is.finite(th)),
...
#' @export
#' @examples
#' morie_geron_batch_gradient_descent(matrix(c(1, 2), ncol = 1), 
...
morie_geron_batch_gradient_descent <- function(X, y, theta, eta, n_iter) {
  ...
  mse <- function(t) { r <- as.numeric(Xm %*% t) - yv; sum(r * r) / m }
  path <- list(th)
  losses <- mse(th)
  ...
  for (i in seq_len(n_iter)) {
    ...
    th <- th - eta * grad
    .morie_gr_need(all(is.finite(th)),
                   sprintf("gradient descent diverged; eta=%g exceeds the stability bound %g.",
                           eta, eta_max))
    path[[length(path) + 1L]] <- th
    losses <- c(losses, mse(th))
  }
  list(theta = th, theta_path = path, loss_history = losses, gradient = grad,
...
morie_geron_conv_output_size <- function(in_size, kernel, padding = 0,
  ...
  vec <- function(v, name, nd = NULL) {
    ...
    if (!is.null(nd)) .morie_gr_need(length(a) == nd,
                                     paste0(name, " must have 1 or ", nd, " entries."))
    a
  }
  ins <- vec(in_size, "in_size")
  nd <- length(ins)
  ks <- vec(kernel, "kernel", nd); ps <- vec(padding, "padding", nd)
  ss <- vec(stride, "stride", nd); ds <- vec(dilation, "dilation", nd)
...
#' morie_geron_conv2d_forward(M, M)
morie_geron_conv2d_forward <- function(X, W, b = 0, stride = 1, padding = 0) {
  to3 <- function(A) {
    if (is.matrix(A)) array(as.numeric(A), dim = c(1L, nrow(A), ncol(A))) else {
      .morie_gr_need(length(dim(A)) == 3L, "X and W must be 2-D or 3-D.")
      A
    }
  }
  X <- to3(X); W <- to3(W)
  storage.mode(X) <- "double"; storage.mode(W) <- "double"
  ...
  s <- pr(stride, "stride"); p <- pr(padding, "padding")
  ...
  kh <- dim(W)[2]; kw <- dim(W)[3]
  Hp <- H + 2L * p[1]; Wp <- Wd + 2L * p[2]
  ...
  list(Y = Y, out_shape = c(oh, ow), padded_shape = c(Hp, Wp),
...
morie_geron_bahdanau_attention <- function(h, s_prev, W, U, v, b = NULL) {
  ...
  list(alpha = alpha, weights = alpha, scores = scores, context = context,
...
#' @export
#' @examples
#' morie_geron_cross_attention(matrix(c(1, 2, 0, 1), 2, 2, byrow = TRUE), 
#'     matrix(c(1, 0, 0, 2, 1, 1), 3, 2, byrow = TRUE), WQ = diag(2), 
#'     WK = matrix(c(0.5, 0, 0, 1), 2, 2, byrow = TRUE), WV = matrix(c(1, 
#'         2), ncol = 1))
morie_geron_cross_attention <- function(X_dec, X_enc, WQ, WK, WV, mask = NULL) {
  Xd <- .morie_gr_mat(X_dec, "X_dec"); Xe <- .morie_gr_mat(X_enc, "X_enc")
  WQ <- .morie_gr_mat(WQ, "WQ"); WK <- .morie_gr_mat(WK, "WK")
  WV <- .morie_gr_mat(WV, "WV")
  .morie_gr_need(length(Xd) > 0L && length(Xe) > 0L,
...
#' @export
#' @examples
#' morie_geron_bidirectional_rnn(matrix(c(1, 1, 0.5), ncol = 1), 
#'     matrix(1), matrix(0.5), matrix(1), matrix(-0.5))
morie_geron_bidirectional_rnn <- function(X, Wx_f, Wh_f, Wx_b, Wh_b,
  ...
  Wxf <- as.matrix(Wx_f); Whf <- as.matrix(Wh_f)
  Wxb <- as.matrix(Wx_b); Whb <- as.matrix(Wh_b)
  storage.mode(Wxf) <- "double"; storage.mode(Whf) <- "double"
  storage.mode(Wxb) <- "double"; storage.mode(Whb) <- "double"
  h <- ncol(Wxf)
  .morie_gr_need(nrow(Wxf) == d && nrow(Wxb) == d,
                 "geron_bidirectional_rnn: X features do not match Wx_f/Wx_b")
  .morie_gr_need(ncol(Wxb) == h,
                 "geron_bidirectional_rnn: forward/backward hidden size mismatch")
  .morie_gr_need(all(dim(Whf) == c(h, h)) && all(dim(Whb) == c(h, h)),
  ...
  for (nm in c("b_f", "b_b", "h0_f", "h0_b")) {
    vv <- switch(nm, b_f = bf, b_b = bb, h0_f = hf, h0_b = hb)
    .morie_gr_need(length(vv) == h,
  ...
  out <- cbind(H_f, H_b)
  list(output = out, h_fwd = H_f, h_bwd = H_b,
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_bidirectional_combine(V, V)
morie_geron_bidirectional_combine <- function(h_forward, h_backward,
  ...
  .morie_gr_need(length(F) > 0L && length(B) > 0L,
  ...
  h <- switch(combine,
              concat = cbind(F, B),
              sum = { .morie_gr_need(ncol(F) == ncol(B), "sum needs equal widths."); F + B },
              mean = { .morie_gr_need(ncol(F) == ncol(B), "mean needs equal widths."); (F + B) / 2 },
              stop("combine must be one of 'concat', 'sum', 'mean'.", call. = FALSE))
  list(h = h, output_dim = ncol(h), n_steps = nrow(F), forward_dim = ncol(F),
...
morie_geron_biological_neuron <- function(x, w, b, activation = "step") {
  ...
  fires <- z >= 0
  scalar <- length(z) == 1L
  list(a = if (scalar) a[1] else a, z = if (scalar) z[1] else z,
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_batch_normalization(V)
morie_geron_batch_normalization <- function(x, gamma = 1, beta = 0, eps = 1e-5,
  ...
  .morie_gr_need(length(g) %in% c(1L, d) && length(b) %in% c(1L, d),
                 "geron_batch_normalization: gamma and beta must be scalars or length-d")
  g <- rep(g, length.out = d); b <- rep(b, length.out = d)
  mu <- colMeans(X)
  ...
  .morie_gr_need(length(rm_) == d && length(rv) == d,
                 "geron_batch_normalization: running statistics must have length d")
  list(y = y, x_hat = x_hat, mu = mu, var = v, running_mean = rm_,
...
#' @export
#' @examples
#' morie_geron_batch_normalization_paper(X = c(1, 2, 3, 4, 5, 6, 7, 8), gamma = 0.5, beta = 0.5)
morie_geron_batch_normalization_paper <- function(X, gamma, beta, eps = 1e-5,
  ...
  g <- rep(g, length.out = d); bt <- rep(bt, length.out = d)
  eps <- as.numeric(eps)
  .morie_gr_need(eps >= 0, "eps must be non-negative.")
  mu <- colMeans(X)
  v <- apply(X, 2, .morie_gr_pvar)
  ...
  list(Y = Y, x_hat = x_hat, batch_mean = mu, batch_var = v,
...
#' W1 <- matrix(c(1, -1), 1, 2)
#' W2 <- matrix(c(2, 3), 2, 1)
#' morie_geron_backpropagation(matrix(c(1, 2), ncol = 1), matrix(c(0, 
#'     1), ncol = 1), list(W1, W2), c("relu", "identity"))
morie_geron_backpropagation <- function(X, y, weights, activations,
  ...
  .morie_gr_need(loss %in% c("mse", "ce"),
                 "geron_backpropagation: loss must be 'mse' or 'ce'")
  Ws <- list(); bs <- list()
  for (i in seq_along(weights)) {
    layer <- weights[[i]]
    if (is.list(layer) && length(layer) == 2L && !is.matrix(layer)) {
      W <- as.matrix(layer[[1]]); bvec <- as.numeric(layer[[2]])
    } else {
      W <- as.matrix(layer); bvec <- numeric(ncol(as.matrix(layer)))
    }
    storage.mode(W) <- "double"
    .morie_gr_need(length(bvec) == ncol(W),
  ...
  if (loss == "mse") {
    ...
    .morie_gr_need(nrow(Y) == n && ncol(Y) == d_out,
    ...
    if (is.matrix(y) && nrow(y) == n && ncol(y) == d_out) {
      ...
      .morie_gr_need(length(idx) == n,
                     "geron_backpropagation: y must have n class labels")
      .morie_gr_need(min(idx) >= 0L && max(idx) < d_out,
                     "geron_backpropagation: class labels out of range")
      Y <- matrix(0, n, d_out)
      Y[cbind(seq_len(n), idx + 1L)] <- 1
    }
  }
  fwd <- function(z, kind) switch(kind,
    identity = z, relu = pmax(z, 0), sigmoid = 1 / (1 + exp(-z)),
    tanh = tanh(z), softmax = .morie_al_softmax_rows(z))
  dact <- function(a, z, kind) switch(kind,
    identity = matrix(1, nrow(z), ncol(z)), relu = (z > 0) * 1,
    sigmoid = a * (1 - a), tanh = 1 - a * a,
    stop("geron_backpropagation: activation has no elementwise derivative",
         call. = FALSE))
  a <- A0; As <- list(A0); Zs <- list()
  for (i in seq_len(L)) {
  ...
  grads_W <- vector("list", L); grads_b <- vector("list", L)
  ...
  list(grads_W = grads_W, grads_b = grads_b, deltas = deltas, loss = total,
...
#' @export
#' @examples
#' morie_geron_backpropagation_gradient(list(matrix(c(1, 2), 1, 
#'     2), matrix(3, 1, 1)), list(matrix(c(1, 1), 2, 1)), matrix(1, 
...
morie_geron_backpropagation_gradient <- function(activations, weights, y_true,
  ...
  .morie_gr_need(length(acts) == length(Ws) + 1L,
  ...
  for (l in seq_along(Ws)) {
    .morie_gr_need(nrow(Ws[[l]]) == ncol(acts[[l]]) &&
                     ncol(Ws[[l]]) == ncol(acts[[l + 1L]]),
                   "weight shape mismatch.")
  }
  .morie_gr_need(all(dim(y) == dim(acts[[length(acts)]])),
                 "y_true shape must match the output activation shape.")
  dact <- function(name, a) switch(name,
    sigmoid = a * (1 - a), tanh = 1 - a * a, relu = (a > 0) * 1,
    matrix(1, nrow(a), ncol(a)))
  L <- length(Ws)
  deltas <- vector("list", L)
  err <- acts[[L + 1L]] - y
  deltas[[L]] <- err * dact(out_act, acts[[L + 1L]])
  if (L >= 2L) for (l in seq.int(L - 1L, 1L)) {
  ...
  list(grad_weights = grads, grad_biases = gbias, deltas = deltas, loss = lo,
...
#' @export
#' @examples
#' morie_geron_backprop_through_time(matrix(c(1, 0.5, 0.5, 1), 2, 
#'     2, byrow = TRUE), matrix(c(0.2, -0.1, 0.4, 0.3), 2, 2, byrow = TRUE), 
#'     matrix(c(1, 0, 0, 2), 2, 2, byrow = TRUE), W_h = matrix(c(0.5, 
#'         0.1, -0.2, 0.3), 2, 2, byrow = TRUE), h_init = c(0.1, 
#'         0.2))
morie_geron_backprop_through_time <- function(loss_grads, hiddens, inputs,
  ...
  list(grad_Wx = grad_Wx, grad_Wh = grad_Wh, grad_b = grad_b, deltas = deltas,
...
#' @export
#' @examples
#' morie_geron_autograd_chain_rule(graph = c(1, 2, 3, 4, 5, 6, 7, 8), grad_output = 5L)
morie_geron_autograd_chain_rule <- function(graph, grad_output) {
  ...
  list(grad_input = g, intermediate_grads = intermediates,
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_clip_contrastive_loss(V, V)
morie_geron_clip_contrastive_loss <- function(image_embeddings,
  ...
  B <- nrow(I)
  if (normalize) {
    ni <- sqrt(rowSums(I^2)); nt <- sqrt(rowSums(T_^2))
    .morie_gr_need(!any(ni == 0) && !any(nt == 0),
  ...
  list(loss = loss, loss_i2t = loss_i2t, loss_t2i = loss_t2i,
...
#' @export
#' @examples
#' morie_geron_contrastive_infonce(anchors = matrix(c(1, 2, 3, 4, 5, 6), nrow = 2), positives = matrix(c(1, 2, 3, 4, 5, 6), nrow = 2), negatives = matrix(c(1, 2, 3, 4, 5, 6), nrow = 2))
morie_geron_contrastive_infonce <- function(anchors, positives, negatives,
  ...
  if (is.matrix(negatives) || is.data.frame(negatives)) {
    ...
    .morie_gr_need(dim(N)[1] == B && dim(N)[3] == d,
  ...
  list(loss = loss, per_anchor_loss = per_anchor, pos_sim = pos,
...
morie_geron_blip <- function(images, texts, temperature = 1,
  ...
  list(itc_loss = itc, itc_i2t = loss_i2t, itc_t2i = loss_t2i,
...
#' morie_geron_blip_itm_itc(matrix(c(1, 0, 0, 1), 2, 2, byrow = TRUE), 
#'     matrix(c(1, 0.2, 0.1, 1), 2, 2, byrow = TRUE), cl, matrix(c(0, 
#'         1, 1, -1), 2, 2, byrow = TRUE), tau = 0.5)
morie_geron_blip_itm_itc <- function(image_emb, text_emb, caption_logits,
                                     caption_targets, tau = 0.07,
                                     lam_itc = 1, lam_itm = 1, lam_lm = 1,
                                     normalize = TRUE) {
  I <- .morie_gr_mat(image_emb, "image_emb"); T_ <- .morie_gr_mat(text_emb, "text_emb")
  ...
  list(loss = loss, itc = itc, itm = itm, lm = lm, similarity = sim,
...
morie_geron_blip2 <- function(image, text, n_query = 4, d_query = 8,
  ...
  Q <- as.integer(n_query); dq <- as.integer(d_query)
  .morie_gr_need(Q >= 1L && dq >= 1L,
  ...
  n_patches <- nrow(V); dv <- ncol(V); dt <- ncol(Tt)
  ...
  list(query_output = qout, llm_input = llm_input, attention = attn,
...
#' @export
#' @examples
#' morie_geron_deit_distillation_loss(matrix(c(1, 0, 0, 2), 2, 2, 
#'     byrow = TRUE), matrix(c(0.5, 0.5, 1, 0), 2, 2, byrow = TRUE), 
#'     c(0, 1), matrix(c(1, 0, 1, 0), 2, 2, byrow = TRUE), alpha = 0.3)
morie_geron_deit_distillation_loss <- function(logits_cls, logits_dist, y,
  ...
  .morie_gr_need(all(dim(Lc) == dim(Ld)),
  ...
  list(loss = loss, loss_cls = loss_cls, loss_dist = loss_dist,
...
#' morie_geron_dalle_autoregressive_token(c(0, 1), c(2, 0), skew, 
#'     temperature = 0.5)
morie_geron_dalle_autoregressive_token <- function(text_tokens,
  ...
  .morie_gr_need(is.function(logits_fn),
                 "logits_fn must be callable(context) -> logits.")
  temperature <- as.numeric(temperature)
  .morie_gr_need(is.finite(temperature) && temperature > 0,
  ...
  list(log_likelihood = ll, token_logprobs = logprobs, perplexity = ppl,
...
#' @export
#' @examples
#' pb <- matrix(c(0, 0, 1, 1, 10, 10, 11, 11, 0, 0, 2, 2), 3, 4, 
#'     byrow = TRUE)
#' pc <- matrix(c(10, 0, 0, 10, 1, 1), 3, 2, byrow = TRUE)
#' gb <- matrix(c(0, 0, 1, 1, 10, 10, 11, 11), 2, 4, byrow = TRUE)
#' morie_geron_detr_hungarian_matching(pb, pc, gb, c(0, 1), no_object_class = 1)
morie_geron_detr_hungarian_matching <- function(pred_boxes, pred_classes,
  ...
  to_xyxy <- function(B, fmt) {
    ...
    out <- if (fmt == "xyxy") B else if (fmt == "cxcywh") {
      cx <- B[, 1]; cy <- B[, 2]; w <- B[, 3]; h <- B[, 4]
      .morie_gr_need(all(w >= 0) && all(h >= 0),
                     "cxcywh boxes must have non-negative width and height.")
      cbind(cx - w / 2, cy - h / 2, cx + w / 2, cy + h / 2)
    } else stop("box_format must be 'xyxy' or 'cxcywh'.", call. = FALSE)
    .morie_gr_need(!any(out[, 3] < out[, 1]) && !any(out[, 4] < out[, 2]),
                   "boxes have x2 < x1 or y2 < y1 after conversion.")
    out
  }
  P <- to_xyxy(pred_boxes, box_format); G <- to_xyxy(gt_boxes, box_format)
  N <- nrow(P); M <- nrow(G)
  .morie_gr_need(N > 0L && M > 0L,
  ...
  lam_bbox <- as.numeric(lam_bbox); lam_giou <- as.numeric(lam_giou)
  .morie_gr_need(lam_bbox >= 0 && lam_giou >= 0,
  ...
  prob <- exp(logp)
  l1 <- matrix(0, N, M); giou <- matrix(0, N, M)
  ...
  list(matching = matching, cost_matrix = cost, total_cost = total_cost,
...
.morie_gr_lsa <- function(cost) {
  ...
  for (j in seq_len(m)) {
    if (p[j + 1L] != 0L) { rows <- c(rows, p[j + 1L] - 1L); cols <- c(cols, j - 1L) }
  }
  ord <- order(rows, method = "radix")
  rows <- rows[ord]; cols <- cols[ord]
  if (transposed) {
...
#' @export
#' @examples
#' morie_geron_actor_critic_advantage(c(0, 1, 2), c(0, 1, 2), c(1, 
#'     2, 0), c(1, -1, 0.5), 0.9, done = c(FALSE, TRUE, FALSE))
morie_geron_actor_critic_advantage <- function(V, s, s_next, r, gamma,
  ...
  s <- as.integer(as.vector(s)); s_next <- as.integer(as.vector(s_next))
  r <- as.numeric(r)
  .morie_gr_need(length(s) == length(s_next) && length(s) == length(r),
                 "s, s_next and r must have equal length.")
  .morie_gr_need(length(s) > 0L, "no transitions supplied.")
  .morie_gr_need(min(s) >= 0L && max(s) < length(V) &&
  ...
  .morie_gr_need(length(mask) == length(s),
                 "done must have one flag per transition.")
  v_s <- V[s + 1L]; v_next <- V[s_next + 1L]
  target <- r + gamma * (1 - mask) * v_next
  adv <- target - v_s
  list(advantage = adv, td_target = target, value_s = v_s,
...
#' @export
#' @examples
#' morie_geron_double_dqn_target(matrix(c(0, 1, 2, 0), 2, 2, byrow = TRUE), 
#'     matrix(c(10, -10, 1, 3), 2, 2, byrow = TRUE), s_next = c(0, 
#'         1, 0), r = c(0, 1, 2), gamma = 0.9, done = c(FALSE, FALSE, 
#'         TRUE))
morie_geron_double_dqn_target <- function(Q_online, Q_target, s_next, r, gamma,
                                          done = NULL) {
  Qo <- .morie_gr_mat(Q_online, "Q_online"); Qt <- .morie_gr_mat(Q_target, "Q_target")
  .morie_gr_need(all(dim(Qo) == dim(Qt)), "Q_online shape must match Q_target.")
  .morie_gr_need(length(Qo) > 0L, "Q tables are empty.")
  S <- nrow(Qo)
  sn <- as.integer(as.vector(s_next)); rew <- as.numeric(r)
  ...
  .morie_gr_need(length(cont) == length(sn),
                 "done must have one flag per transition.")
  ...
  list(target = target, selected_action = a_star, q_eval = q_eval,
...
#' R <- matrix(c(1, 0.5, 0.2, 0), 2, 2)
#' r <- morie_geron_value_iteration(c(0, 0), P, R, gamma = 0.9)
#' str(r, max.level = 1)
morie_geron_value_iteration <- function(V, P, R, gamma, tol = 1e-10,
                                        max_iter = 10000) {
  Vv <- as.numeric(V)
  Pm <- P
  .morie_gr_need(length(dim(Pm)) == 3L,
                 "geron_bellman_optimality: P must be 3-D (S, A, S)")
  storage.mode(Pm) <- "double"
  S <- dim(Pm)[1]; A <- dim(Pm)[2]; S2 <- dim(Pm)[3]
  .morie_gr_need(S > 0L && A > 0L,
  ...
  list(V = Vv, policy = policy, Q = Q, iterations = it, residual = residual,
...
#' morie_geron_q_value_iteration(matrix(0, 2, 2), P, R, 0.9)
morie_geron_q_value_iteration <- function(Q, transitions, rewards, gamma,
  ...
  S <- nrow(Q); A <- ncol(Q)
  T_ <- transitions
  .morie_gr_need(length(dim(T_)) == 3L && all(dim(T_) == c(S, A, S)),
  ...
  .morie_gr_need(all(dim(R) == c(S, A, S)),
  ...
  .morie_gr_need(all(abs(rowsum - 1) < 1e-8 | abs(rowsum) < 1e-8),
                 "each (s, a) row must sum to 1 (available) or 0 (unavailable).")
  .morie_gr_need(all(is.finite(R)) && all(is.finite(Q)),
  ...
  Vv <- apply(Q, 1, max)
  policy <- apply(Q, 1, which.max) - 1L
  list(Q = Q, V = Vv, policy = policy, residual = residual, iterations = it,
...
#' @export
#' @examples
#' morie_geron_ddim_sampling_step(c(1, -0.5), t = 3, t_prev = 1, 
#'     eps_pred = c(0.2, 0.1), alpha_bar = c(1, 0.9, 0.8, 0.5))
morie_geron_ddim_sampling_step <- function(x_t, t, t_prev, eps_pred, alpha_bar,
                                           clip_x0 = NULL) {
  x_t <- as.numeric(x_t); eps <- as.numeric(eps_pred)
...
#' env <- list(reset = function() 1, step = function(a) list(1, 
#'     if (a == 0) 1 else 0, TRUE))
#' morie_geron_a2c(env, matrix(c(0, 0), 2, 1), 0, epochs = 25, lr = 0.5, 
#'     seed = 1)
morie_geron_a2c <- function(env, actor, critic, epochs = 100, lr = 0.1,
  ...
  alpha <- as.numeric(lr)
  .morie_gr_need(is.finite(alpha) && alpha > 0,
  ...
  draw <- function() { s_state <<- .morie_al_lcg(s_state); (s_state + 0.5) / 2^32 }
  ep_returns <- numeric(E)
  last_adv <- numeric(0)
  for (ep in seq_len(E)) {
    states <- list(); acts <- integer(0); rewards <- numeric(0)
    s <- as.numeric(reset())
    ...
    Tn <- length(rewards)
    G <- numeric(Tn); accum <- 0
    for (t in seq.int(Tn, 1L)) { accum <- rewards[t] + g * accum; G[t] <- accum }
    ...
    for (t in seq_len(length(acts))) {
      ...
      onehot <- numeric(n_actions); onehot[acts[t] + 1L] <- 1
  ...
  list(actor = A, critic = V, returns = ep_returns, policy = policy,
...
#' env <- list(reset = function() 1, step = function(a) list(1, 
#'     if (a == 0) 1 else 0, TRUE))
#' morie_geron_a3c(env, matrix(c(0, 0), 2, 1), 0, n_workers = 3, 
...
morie_geron_a3c <- function(env, actor, critic, n_workers = 4, lr = 0.1,
  ...
  envs <- lapply(seq_len(W) - 1L, function(w) {
    ...
    .morie_gr_need(is.function(e$reset) && is.function(e$step),
                   "geron_a3c: env must provide callable reset() and step(action)")
    e
  })
  states_seed <- (as.numeric(seed) + 7919 * (seq_len(W) - 1L)) %% 2^32
  shared_A <- A; shared_V <- V
  worker_returns <- matrix(0, W, E)
  updates <- 0L
  for (ep in seq_len(E)) {
    for (w in seq_len(W)) {
      reset <- envs[[w]]$reset; step <- envs[[w]]$step
      localA <- shared_A; localV <- shared_V
      states <- list(); acts <- integer(0); rewards <- numeric(0)
      s <- as.numeric(reset())
  ...
  list(actor = shared_A, critic = shared_V, returns = returns,
...
#' @export
#' @examples
#' morie_geron_dbscan_core_point(X = c(1, 2, 3, 4, 5, 6, 7, 8), eps = 0.5, min_samples = 5L)
morie_geron_dbscan_core_point <- function(X, eps, min_samples,
  ...
  list(is_core = core, is_border = border, is_noise = noise,
...
morie_geron_agglomerative <- function(X, n_clusters = 2, linkage = "single") {
  ...
  .morie_gr_need(k >= 1L && k <= n,
                 "geron_agglomerative: n_clusters must lie in [1, n]")
  .morie_gr_need(linkage %in% c("single", "complete", "average", "centroid"),
  ...
  active <- seq_len(n)
  merges <- list(); heights <- numeric(0)
  cluster_dist <- function(a, b) {
    sub <- D[members[[a]], members[[b]], drop = FALSE]
    switch(linkage,
           single = min(sub), complete = max(sub), average = mean(sub),
           centroid = {
  ...
  while (length(active) > k) {
    ...
    for (ii in seq_len(length(active) - 1L)) {
      for (jj in seq.int(ii + 1L, length(active))) {
        a <- active[ii]; b <- active[jj]
        dab <- cluster_dist(a, b)
        if (dab < best_d) { best_d <- dab; best <- c(a, b) }
      }
    }
    a <- best[1]; b <- best[2]
    merges[[length(merges) + 1L]] <- c(min(members[[a]]) - 1L,
                                       min(members[[b]]) - 1L)
  ...
  list(labels = labels, merges = merges, heights = heights,
...
morie_geron_birch <- function(X, n_clusters = 3, threshold = 0.5,
  ...
  for (i in seq_len(n)) {
    x <- A[i, ]
    best_leaf <- NA_integer_; best_entry <- NA_integer_; best_d <- Inf
    for (li in seq_along(leaves)) {
      ...
      for (ei in seq_along(leaf)) {
        ...
        if (d < best_d && radius_if_added(cf, x) <= t_) {
          best_d <- d; best_leaf <- li; best_entry <- ei
        }
      }
    }
    if (!is.na(best_leaf)) {
      cf <- leaves[[best_leaf]][[best_entry]]
      cf$n <- cf$n + 1L; cf$ls <- cf$ls + x; cf$ss <- cf$ss + sum(x * x)
      cf$members <- c(cf$members, i)
      leaves[[best_leaf]][[best_entry]] <- cf
    } else {
      target <- NA_integer_
      for (li in seq_along(leaves)) if (length(leaves[[li]]) < B) { target <- li; break }
      if (is.na(target)) { leaves[[length(leaves) + 1L]] <- list(); target <- length(leaves) }
  ...
  radii <- vapply(cfs, function(cf)
    sqrt(max(cf$ss / cf$n - sum(cf$ls * cf$ls) / (cf$n * cf$n), 0)), numeric(1))
  m <- length(cfs)
  if (is.null(k) || k >= m) {
    ...
    while (length(active) > k) {
      ...
      for (ii in seq_len(length(active) - 1L)) {
        for (jj in seq.int(ii + 1L, length(active))) {
          ...
          d <- sqrt(sum((ca - cb)^2))
          if (d < best_d) { best_d <- d; best <- c(a, b) }
        }
      }
      a <- best[1]; b <- best[2]
  ...
  list(labels = labels, subcluster_centers = centers,
...
#' morie_geron_backpropagation(matrix(c(1, 2), ncol = 1), matrix(c(0, 
#'     1), ncol = 1), list(W1, W2), c("relu", "identity"))
#' morie_geron_adaboost(X = W2, y = W1)
morie_geron_adaboost <- function(X, y, base_estimator = NULL,
  ...
  .morie_gr_need(length(classes) == 2L,
  ...
  fit_stump <- function(w) {
    ...
    for (j in seq_len(ncol(A))) {
      ...
      thrs <- if (length(vals) > 1L)
        (utils::head(vals, -1) + utils::tail(vals, -1)) / 2 else vals
      for (thr in thrs) {
        left <- A[, j] <= thr
        for (pol in c(1, -1)) {
  ...
  w <- rep(1 / n, n)
  predictors <- list(); alphas <- numeric(0); errs <- numeric(0)
  ...
  for (m in seq_len(M)) {
    ...
    alphas <- c(alphas, alpha); errs <- c(errs, err)
    F_ <- F_ + alpha * pred
  ...
  list(alphas = alphas, errors = errs, train_errors = train_errs,
...
#' @export
#' @examples
#' morie_geron_adaboost_weight_update(c(0, 0, 1, 1), c(0, 1, 1, 
#'     1), rep(0.25, 4), log(3))
morie_geron_adaboost_weight_update <- function(y_true, y_pred, weights,
                                               alpha_t) {
  y_true <- as.vector(y_true); y_pred <- as.vector(y_pred)
  w <- as.numeric(weights)
  .morie_gr_need(length(y_true) == length(y_pred),
                 "y_true and y_pred must have equal length.")
  .morie_gr_need(length(y_true) == length(w),
  ...
  total <- sum(w)
  .morie_gr_need(is.finite(total) && total > 0,
  ...
  new_total <- sum(w_new)
  .morie_gr_need(is.finite(new_total) && new_total > 0,
                 "weight update overflowed; alpha_t is too large.")
  w_new <- w_new / new_total
  list(weights_new = w_new, misclassified = wrong > 0,
...
morie_geron_bagging <- function(X, y, base_estimator = NULL, n_estimators = 10,
  ...
  .morie_gr_need(task %in% c("auto", "regression", "classification"),
  ...
  list(predict = predict_fn, train_pred = train_pred, train_mse = train_mse,
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_bagging_predictor(V)
morie_geron_bagging_predictor <- function(predictions, aggregate = "mean") {
  ...
  list(prediction = agg, per_instance_variance = vr,
...
#' @export
#' @examples
#' morie_geron_cross_validation_score(X = c(1, 2, 3, 4, 5, 6, 7, 8), y = c(1, 2, 3, 4, 5, 6, 7, 8), K = 3L)
morie_geron_cross_validation_score <- function(X, y, K, fit = NULL,
                                               predict = NULL, score = NULL,
                                               shuffle = FALSE,
                                               random_state = NULL) {
  X <- .morie_gr_mat(X, "X"); y <- as.numeric(y)
  ...
  for (k in seq_len(K)) {
    ...
    .morie_gr_need(length(yp) == length(test_idx),
    ...
    scores <- c(scores, s); sizes <- c(sizes, length(test_idx))
  }
  se <- if (K > 1L) stats::sd(scores) / sqrt(K) else NA_real_
  list(cv_score = mean(scores), fold_scores = scores, fold_sizes = sizes,
...
#' @export
#' @examples
#' morie_geron_autoencoder(matrix(c(0, 0, 1, 1.1, 2, 2.3, 3, 2.9), 
#'     4, 2, byrow = TRUE), 1)
morie_geron_autoencoder <- function(X, bottleneck, center = TRUE) {
  ...
  list(encoder = W1, decoder = W2, codes = codes, reconstruction = recon,
...
#' @export
#' @examples
#' morie_geron_autoencoder_reconstruction_loss(X = matrix(c(1, 2, 3, 4, 5, 6), nrow = 2), encoded = matrix(c(1, 2, 3, 4, 5, 6), nrow = 2), decoded = matrix(c(1, 2, 3, 4, 5, 6), nrow = 2))
morie_geron_autoencoder_reconstruction_loss <- function(X, encoded, decoded) {
  X <- .morie_gr_mat(X, "X"); decoded <- .morie_gr_mat(decoded, "decoded")
  ...
  list(loss = mean(per_sample), mse_per_element = mean(resid^2),
...
#' @export
#' @examples
#' morie_geron_denoising_autoencoder(x = c(1, 2, 3, 4, 5, 6, 7, 8), noise = c(1, 2, 3, 4, 5, 6, 7, 8), decoded = c(1, 2, 3, 4, 5, 6, 7, 8))
morie_geron_denoising_autoencoder <- function(x, noise, decoded,
  ...
  .morie_gr_need(all(dim(x) == dim(decoded)), "decoded shape must match x shape.")
  nb <- as.numeric(noise)
  .morie_gr_need(length(nb) == length(x) || length(nb) == 1L ||
                   length(nb) == ncol(x),
                 "noise is not broadcastable to x.")
  noise_b <- if (length(nb) == length(x)) matrix(as.numeric(noise), nrow(x), ncol(x)) else
    matrix(nb, nrow(x), ncol(x), byrow = TRUE)
  .morie_gr_need(all(is.finite(noise_b)), "noise must be finite.")
  x_tilde <- if (corruption == "additive") x + noise_b else if (corruption == "dropout") {
    .morie_gr_need(all(noise_b == 0 | noise_b == 1),
                   "with corruption='dropout', noise must be a 0/1 keep-mask.")
    x * noise_b
  } else stop("corruption must be 'additive' or 'dropout'.", call. = FALSE)
  resid <- x - decoded
  per_sample <- rowSums(resid^2)
  ...
  list(loss = loss, mse_per_element = mean(resid^2),
...
.morie_gr_conv2d_valid <- function(Z, K, s) {
  kh <- nrow(K); kw <- ncol(K); h <- nrow(Z); w <- ncol(Z)
  .morie_gr_need(h >= kh && w >= kw,
...
#' @export
#' @examples
#' morie_geron_convolutional_autoencoder(x = 5L, encoder_weights = c(1, 2, 3, 4, 5, 6, 7, 8), decoder_weights = c(1, 2, 3, 4, 5, 6, 7, 8))
morie_geron_convolutional_autoencoder <- function(x, encoder_weights,
  ...
  acts <- list(identity = function(a) a, tanh = tanh,
               sigmoid = function(a) 1 / (1 + exp(-a)))
  .morie_gr_need(output_activation %in% names(acts),
  ...
  .morie_gr_need(all(dim(x_hat) == dim(x)),
                 "the decoder reconstruction shape does not match the input; adjust kernels or stride.")
  resid <- x - x_hat
  loss <- sum(resid^2)
  list(x_hat = x_hat, code = code, code_shape = dim(code), loss = loss,
...
#' morie_geron_anomaly_autoencoder(zero, matrix(c(0, 3, 1, 5), ncol = 1), 
#'     quantile = 0.6)
morie_geron_anomaly_autoencoder <- function(model, X, threshold = NULL,
  ...
  recon <- if (is.function(model)) model(A) else
    if (is.function(model$reconstruct)) model$reconstruct(A) else
      if (is.function(model$predict)) model$predict(A) else
        if (is.function(model$encode) && is.function(model$decode))
          model$decode(model$encode(A)) else
  ...
  flags <- errors > thr
  list(errors = errors, is_anomaly = flags, threshold = thr,
...
#' @export
#' @examples
#' morie_geron_dcgan_generator(z = 5L, weights = c(1, 2, 3, 4, 5, 6, 7, 8))
morie_geron_dcgan_generator <- function(z, weights, seed_shape = NULL,
  ...
  if (is.null(seed_shape)) {
    ...
    .morie_gr_need(side * side == length(flat),
                   "projection size is not a perfect square; pass seed_shape.")
    seed_shape <- c(side, side)
  }
  h0 <- as.integer(seed_shape[1]); w0 <- as.integer(seed_shape[2])
  .morie_gr_need(h0 >= 1L && w0 >= 1L && h0 * w0 == length(flat),
  ...
  kernels <- ws[-1]
  bn <- function(M) {
    mu <- mean(M); sd_ <- .morie_gr_psd(as.numeric(M))
  ...
  image <- tanh(A)
  list(image = image, image_shape = dim(image), seed = seed_mat,
...
#' morie_geron_auxiliary_task_pretraining(NULL, list(Xa, c(1, 2, 
#'     3, 4)), list(Xt, c(3, 2)), aux_epochs = 50, epochs = 5)
morie_geron_auxiliary_task_pretraining <- function(model, aux_data,
                                                   target_data,
                                                   aux_epochs = 200,
                                                   epochs = 20, lr = 0.05) {
  pair <- function(name, data) {
    .morie_gr_need(is.list(data) && length(data) == 2L,
    ...
    .morie_gr_need(nrow(Xd) > 0L,
                   paste0("geron_auxiliary_task_pretraining: ", name, " X must be non-empty"))
    .morie_gr_need(length(yd) == nrow(Xd),
                   paste0("geron_auxiliary_task_pretraining: ", name, " row/target mismatch"))
    .morie_gr_need(all(is.finite(Xd)) && all(is.finite(yd)),
                   paste0("geron_auxiliary_task_pretraining: ", name, " must be finite"))
    list(X = Xd, y = yd)
  }
  aux <- pair("aux_data", aux_data); tgt <- pair("target_data", target_data)
  .morie_gr_need(ncol(aux$X) == ncol(tgt$X),
  ...
  mse <- function(th) { r <- as.numeric(tgt$X %*% th) - tgt$y; sum(r * r) / length(tgt$y) }
  target_loss <- mse(ft$theta); scratch_loss <- mse(sc$theta)
  list(theta = ft$theta, theta_pretrained = pre$theta, theta_scratch = sc$theta,
...
#' morie_geron_bert_finetune(ident, matrix(c(1, 0, 0, 1, 1, 1), 
#'     3, 2, byrow = TRUE), c(0, 1, 0), epochs = 25, lr = 0.5, l2 = 0.01)
morie_geron_bert_finetune <- function(bert, X, y, epochs = 100, lr = 0.1,
                                      l2 = 0) {
  .morie_gr_need(is.function(bert),
  ...
  list(W = W, b = b, losses = losses, accuracy = acc, predict = predict_fn,
...
morie_geron_bf16 <- function(x, rounding = "nearest_even") {
  .morie_gr_need(rounding %in% c("nearest_even", "truncate"),
  ...
  list(values = quant, bits = bits, abs_error = abs_err, rel_error = rel,
...
morie_geron_bf16_range <- function(x) {
  ...
  list(bf16 = b, abs_error = abs_err, rel_error = rel,
...
Math.morie_gvar <- function(x, ...) {
  switch(.Generic,
    exp = { e <- exp(x$value); .morie_gvar(e, list(list(x, e)), "exp") },
    log = {
      .morie_gr_need(x$value > 0, sprintf("log is undefined at %g", x$value))
      .morie_gvar(log(x$value), list(list(x, 1 / x$value)), "log")
    },
    sqrt = {
      .morie_gr_need(x$value > 0,
                     sprintf("derivative of sqrt is undefined at %g", x$value))
      s <- sqrt(x$value); .morie_gvar(s, list(list(x, 0.5 / s)), "sqrt")
    },
    tanh = { t_ <- tanh(x$value); .morie_gvar(t_, list(list(x, 1 - t_ * t_)), "tanh") },
...
morie_geron_autograd <- function(loss, params) {
  ...
  out <- loss(leaves)
  .morie_gr_need(inherits(out, "morie_gvar"),
  ...
  while (length(stack)) {
    ...
    node <- top[[1]]; expanded <- top[[2]]
    key <- as.character(node$id)
    if (expanded) { order_nodes[[length(order_nodes) + 1L]] <- node; next }
  ...
  list(grad = grad, value = out$value, tape_size = length(order_nodes),
...
morie_geron_arima <- function(y, p = 1, d = 0, q = 0, include_mean = TRUE) {
  ...
  if (Q == 0L) {
    ...
    k <- as.integer(min(max(P + Q + 1L, as.integer(ceiling(log(max(m, 2))^2))),
                        max(1L, (m - 1L) %/% 2L)))
    rows <- m - k
    .morie_gr_need(rows > k,
                   "geron_arima: series too short for the Hannan-Rissanen long autoregression")
    Xa <- do.call(cbind, lapply(seq_len(k) - 1L,
  ...
  forecast <- function(h = 1) {
    ...
    for (i in seq_len(H)) {
      ...
      if (length(ma)) for (b in seq_along(ma)) {
        val <- val + ma[b] * (if (length(errs) > b - 1L) errs[length(errs) - b + 1L] else 0)
      }
      hist <- c(hist, val); errs <- c(errs, 0)
      out <- c(out, val)
    }
    f <- out
  ...
  list(ar = ar, ma = ma, intercept = intercept, residuals = resid,
       sigma2 = sigma2, aic = aic, fitted = fitted, differenced = z,
       forecast = forecast, order = c(P, D, Q), estimate = sigma2, n = nobs,
       method = if (Q) sprintf("ARIMA(%d,%d,%d) by Hannan-Rissanen", P, D, Q)
                else sprintf("ARIMA(%d,%d,0) by OLS", P, D))
}
...
#' @export
#' @examples
#' morie_geron_arima_forecast(y = c(1, 2, 3, 4, 5, 6, 7, 8), phi = 0.5, theta = 0.5)
morie_geron_arima_forecast <- function(y, phi, theta, d = 0) {
  y <- as.numeric(y); phi <- as.numeric(phi); theta <- as.numeric(theta)
  .morie_gr_need(all(is.finite(y)), "y contains non-finite values.")
  .morie_gr_need(all(is.finite(phi)) && all(is.finite(theta)),
                 "phi and theta must be finite.")
  d <- as.integer(d)
  .morie_gr_need(d >= 0L, "d must be non-negative.")
  p <- length(phi); q <- length(theta)
  need <- d + max(p, 1L)
  .morie_gr_need(length(y) >= need,
                 "y is too short to form one forecast at this order.")
  last_levels <- numeric(0)
  w <- y
  if (d > 0L) for (i in seq_len(d)) { last_levels <- c(last_levels, w[length(w)]); w <- diff(w) }
  Tn <- length(w)
  eps <- numeric(Tn)
  for (t in seq_len(Tn)) {
    ar <- 0
  ...
  .morie_gr_need(is.finite(level),
                 "forecast is not finite; check phi/theta for explosive roots.")
  list(forecast = level, forecast_differenced = fc, residuals = eps,
...
#' morie_geron_beam_search(function(s, prefix) lp, NULL, beam_width = 2, 
#'     max_len = 3)
morie_geron_beam_search <- function(model, src, beam_width = 3, max_len = 10,
  ...
  for (i in seq_len(L)) {
    ...
    for (h in live) {
      lp <- score_next(h$seq)
      V <- length(lp)
      if (!is.null(eos)) .morie_gr_need(as.integer(eos) >= 0L && as.integer(eos) < V,
                                        "geron_beam_search: eos is outside the vocabulary")
      top <- order(-lp, method = "radix")[seq_len(min(K, V))]
      for (tok in top) {
        cand[[length(cand) + 1L]] <- list(seq = c(h$seq, tok - 1L),
                                          sc = h$sc + lp[tok])
      }
    }
  ...
  normed <- vapply(finished, function(it)
  ...
  list(sequence = best$seq, score = best$sc,
...
#' @export
#' @examples
#' morie_geron_beam_search_decoder(scores = c(1, 2, 3, 4, 5, 6, 7, 8), beam_width = 5L)
morie_geron_beam_search_decoder <- function(scores, beam_width, max_len = NULL,
  ...
  for (t in seq_len(steps)) {
    cand <- list()
    for (h in beams) for (yv in seq_len(V)) {
      cand[[length(cand) + 1L]] <- list(seq = c(h$seq, yv - 1L),
                                        sc = h$sc + S[t, yv])
    }
    ord <- order(-vapply(cand, function(c) c$sc, numeric(1)), method = "radix")
  ...
  list(best_sequence = best$seq, best_score = best$sc,
...
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' morie_geron_bpe_tokenizer(V)
morie_geron_bpe_tokenizer <- function(corpus, vocab_size = 100) {
  EOW <- "</w>"
  if (!is.null(names(corpus))) {
    words_ <- names(corpus); cnts <- as.integer(unlist(corpus))
    .morie_gr_need(all(cnts >= 0L),
  ...
  names(syms) <- words_
  vocab_keys <- character(0); vocab_vals <- numeric(0)
  for (wi in seq_along(words_)) for (s in syms[[wi]]) {
    pos <- match(s, vocab_keys)
    if (is.na(pos)) { vocab_keys <- c(vocab_keys, s); vocab_vals <- c(vocab_vals, cnts[wi]) }
    else vocab_vals[pos] <- vocab_vals[pos] + cnts[wi]
  }
  merges <- list()
  if (target >= length(vocab_keys)) {
    while (length(vocab_keys) < target) {
      pk <- character(0); pv <- numeric(0)
      for (wi in seq_along(words_)) {
        ss <- syms[[wi]]; cc <- cnts[wi]
        if (length(ss) < 2L) next
        for (i in seq_len(length(ss) - 1L)) {
          key <- paste0(ss[i], "\u001f", ss[i + 1L])
          pos <- match(key, pk)
          if (is.na(pos)) { pk <- c(pk, key); pv <- c(pv, cc) } else pv[pos] <- pv[pos] + cc
        }
      }
      if (!length(pk)) break
      best <- max(pv)
      ...
      new_sym <- paste0(a, b)
      for (wi in seq_along(words_)) {
        ss <- syms[[wi]]; out <- character(0); i <- 1L
        while (i <= length(ss)) {
          if (i + 1L <= length(ss) && ss[i] == a && ss[i + 1L] == b) {
            out <- c(out, new_sym); i <- i + 2L
          } else { out <- c(out, ss[i]); i <- i + 1L }
        }
        syms[[wi]] <- out
      }
      merges[[length(merges) + 1L]] <- c(a, b)
      vocab_keys <- c(vocab_keys, new_sym); vocab_vals <- c(vocab_vals, best)
    }
  }
  ...
  wt <- syms; names(wt) <- words_
  list(merges = ordered_merges, vocab = vocab, tokenize = tokenize,
...
#' @export
#' @examples
#' morie_geron_bpe_merge(corpus = c(1, 2, 3, 4, 5, 6, 7, 8), n_merges = 5L)
morie_geron_bpe_merge <- function(corpus, n_merges) {
  EOW <- "</w>"
  if (!is.null(names(corpus))) {
    words_ <- names(corpus); freqs <- as.integer(unlist(corpus))
  } else {
  ...
  for (it in seq_len(n_merges)) {
    ...
    a <- parts[ord[1], 1]; b <- parts[ord[1], 2]
    merged <- paste0(a, b)
    for (i in seq_along(words_)) {
      ss <- splits[[i]]; out <- character(0); j <- 1L
      while (j <= length(ss)) {
        if (j < length(ss) && ss[j] == a && ss[j + 1L] == b) {
          out <- c(out, merged); j <- j + 2L
        } else { out <- c(out, ss[j]); j <- j + 1L }
      }
      splits[[i]] <- out
    }
    merges[[length(merges) + 1L]] <- c(a, b)
  ...
  n_after <- sum(vapply(seq_along(words_),
                        function(i) length(splits[[i]]) * freqs[i], numeric(1)))
  vocab <- sort(unique(unlist(splits)))
  names(splits) <- words_
  list(merges = merges, vocab = vocab, splits = splits, merge_counts = counts,
...
.morie_gr_block_weights <- function(d_model, d_ff, seed) {
  list(Wq = .morie_gr_init(d_model, d_model, seed + 1),
...
morie_geron_albert <- function(X, n_layers = 4, n_heads = 2, d_model = 8,
  ...
  .morie_gr_need(all(ids == floor(ids)),
  ...
  B <- nrow(ids); Tn <- ncol(ids)
  L <- as.integer(n_layers); H <- as.integer(n_heads); d <- as.integer(d_model)
  .morie_gr_need(L >= 1L && H >= 1L && d >= 1L,
                 "geron_albert: n_layers, n_heads and d_model must all be >= 1")
  .morie_gr_need(d %% H == 0L,
  ...
  list(hidden = hidden, n_params = total,
...
morie_geron_bert <- function(X, n_layers = 2, n_heads = 2, d_model = 8,
  ...
  list(hidden = hidden, mlm_loss = mean(losses), mlm_losses = losses,
...
morie_geron_roberta <- function(X, n_layers = 2, n_heads = 2, d_model = 8,
  ...
  list(hidden = hidden, mlm_loss = epoch_losses[EP], epoch_losses = epoch_losses,
...
morie_geron_alexnet <- function(n_classes = 1000, input_size = 227,
  ...
  .morie_gr_need(p_drop >= 0 && p_drop < 1,
  ...
  spec <- list(
    ...
    list("fc", 4096L, NA_integer_, NA_integer_, NA_integer_))
  layers <- list()
  size <- S; channels <- ch; flatten_dim <- NA_integer_
  for (sp in spec) {
    kind <- sp[[1]]; units <- sp[[2]]; k <- sp[[3]]; s <- sp[[4]]; pad <- sp[[5]]
    if (kind %in% c("conv", "pool")) {
      out <- ((size - k + 2L * pad) %/% s) + 1L
      .morie_gr_need(out >= 1L,
      ...
      layers[[length(layers) + 1L]] <- list(kind = kind, filters = units,
      ...
      layers[[length(layers) + 1L]] <- list(kind = "fc", filters = units,
  ...
  layers[[length(layers) + 1L]] <- list(kind = "fc", filters = C,
  ...
  conv_p <- sum(vapply(layers, function(l)
    if (l$kind == "conv") as.numeric(l$params) else 0, numeric(1)))
  fc_p <- sum(vapply(layers, function(l)
    if (l$kind == "fc") as.numeric(l$params) else 0, numeric(1)))
  list(layers = layers, total_params = total, trainable_params = total,
