...
#' @param ref 1-based index of the reference part; the default D uses the last part, as
#' in Aitchison's own display.
#'
...
#' row in EXTERNAL_SOURCES.md.  alr(x) = (ln(x_1/x_D), ..., ln(x_\{D-1\}/x_D)), the
#' reference part being the last one.  ``ref`` generalises that to any part; ``parts``
#' reports which 1-based indices the returned coordinates belong to, in their original
#' order.  The index is 1-based in BOTH language arms so the two agree.
#' @export
#' @examples
#' V <- c(1, 2, 3, 4, 5, 6, 7, 8)
#' Alr(V)
