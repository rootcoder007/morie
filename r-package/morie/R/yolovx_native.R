...
morie_yolovx_encode_box <- function(box, cx, cy, stride = 1.0) {
  ...
  if (!(x0 <= px && px <= x1 && y0 <= py && py <= y1)) {
    stop(paste0(
      "yolovx: the location is outside the box, so it cannot ",
...
morie_yolovx_center_sampling <- function(box, grid_w, grid_h, stride = 1.0,
  ...
  list(
    in_box = inside, in_center = center, candidates = cand,
    n_candidates = length(cand), single_center = 1L,
    note = paste0(
      "one positive per object starves the model of ",
      "useful gradients"
    )
  )
}
...
#' @param max_k Optional; may be \code{NULL}. Coerced to integer by the body, with
#' \code{as.integer}.
...
morie_yolovx_simota_assign <- function(costs, ious, top_q = 10, max_k = NULL) {
  ...
  list(
    estimate = final, assignment = final, dynamic_k = ks, n_positives = n_pos,
    contested = as.integer(contested),
    method = "SimOTA dynamic top-k; Ge et al. (2021)",
    note = paste0(
      "k is DYNAMIC per ground truth, from the sum of its ",
