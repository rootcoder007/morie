...
#'   \code{threshold}, \code{alarm}, \code{phi}, \code{trend_coef},
...
Farsig <- function(counts, baseline_years = 5, reference_window = 3,
  ...
  .fit <- function(om) {
    f <- .irls(X, yb, om)
    phi <- sum(om * (yb - f$mu)^2 / f$mu) / (nb - p)
    if (phi < 1) phi <- 1
    list(b = f$b, mu = f$mu, inv = f$inv, phi = phi)
  }
  f <- .fit(om)
  ...
  se0 <- mu0 * sqrt(if (q0 > 0) q0 else 0)
  ...
  .t1_result(estimate = score, observed = y0, expected = mu0, threshold = U,
             alarm = if (y0 > U) 1 else 0, phi = f$phi,
