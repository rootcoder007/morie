...
SignlessL <- function(A) {
  ...
  nbip <- sum(bip)
  list(Q = Q, degree = deg, n = n, m = sum(deg) / 2, trace = sum(deg),
...
Dpll <- function(cnf) {
  ...
  found <- if (any(vapply(clauses, length, integer(1)) == 0L)) NULL
           else search(clauses, list())
  sat <- !is.null(found)
  model <- list()
  if (sat && nvars > 0L)
  ...
  list(satisfiable = sat, assignment = if (sat) found else list(),
       ...
       method = paste("DPLL: unit propagation, pure literal, split on the",
...
EmAlgo <- function(log_lik, Q, x0, steps) {
  ...
  list(theta = theta, loglik = trace[length(trace)], trace = trace,
       increments = inc, min_increment = mn, monotone = mn >= -1e-9,
       steps = steps,
       method = paste("EM driver with a Dempster-Laird-Rubin Theorem 1",
                      "monotonicity audit"))
}
...
SimplexLP <- function(c, A, b, max_iter = 1000L, tol = 1e-12) {
  ...
  if (any(b < 0))
    stop(paste("every entry of b must be non-negative; a negative",
  ...
  if (!done)
    stop(paste("pivot budget exhausted; Bland\'s rule should have",
               "terminated, so the problem is larger than max_iter"))

  sol <- numeric(total)
  for (i in seq_len(ncon)) sol[basis[i]] <- T[i, total + 1L]
  x <- sol[seq_len(nvar)]
  list(status = status, x = x, objective = sum(c * x),
