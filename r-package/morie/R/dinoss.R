...
#' @param C Current centre of length K; zeros if omitted.
...
#' @export
Dinocenter <- function(g_t, C = NULL, m = 0.9, tau_t = 0.04) {
  G <- as.matrix(g_t); B <- nrow(G); K <- ncol(G)
  c0 <- if (is.null(C)) rep(0, K) else .t1_vec(C)
  if (length(c0) != K) stop("C must have length K")
