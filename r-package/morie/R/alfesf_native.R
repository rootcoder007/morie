...
morie_alfesf_esmfold_confidence <- function(lddt_logits = NULL,
  ...
  if (!is.null(features) && !is.null(lddt)) {
    ...
    if (length(obs) != nrow(X)) {
      stop(sprintf(
        "alfesf: %d feature rows but %d lddt values",
        nrow(X), length(obs)
      ))
    }
    ...
    y <- pmin(
      as.integer(obs / 100.0 * .alfesf_LDDT_BINS),
      .alfesf_LDDT_BINS - 1L
    )
    ...
    if (nrow(W) != ncol(X)) {
      stop(sprintf(paste0(
        "alfesf: weights W has %d rows but the features ",
        "have %d columns"
      ), nrow(W), ncol(X)))
    }
    if (length(b) != ncol(W)) {
      stop(sprintf(paste0(
        "alfesf: bias length %d does not match the %d ",
        "output bins"
      ), length(b), ncol(W)))
    }
    lddt_logits <- X %*% W
    ...
    stop(paste0(
      "alfesf: features were given with neither `weights` to run ",
  ...
  if (is.character(temperature)) {
    ...
    if (is.null(lddt) || is.null(lddt_logits)) {
      stop(paste0(
        "alfesf: temperature='fit' needs observed lddt and the ",
        "logits to calibrate"
      ))
    }
    obs <- as.numeric(lddt)
  ...
  if (!is.null(pae_logits)) {
    ...
    if (n * n != nrow(flat)) {
      stop(sprintf(paste0(
        "alfesf: %d aligned-error rows is not a square ",
    ...
    if (!is.null(chain_id)) {
      ...
      if (length(ch) != n) {
        stop(sprintf(
          "alfesf: %d chain labels for %d residues",
          length(ch), n
        ))
      }
