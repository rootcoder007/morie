...
#'
#' @param x Numeric vector (length \code{\geq} 4).
...
#' Real cepstrum
#'
#' Real cepstrum \\code{c[n] = \\mathrm{IFFT}(\\log |\\mathrm{FFT}(x)|)}. Useful
...
#' @param n_fft FFT length (default: next power of 2 \\code{\\geq} `length(x)`).
...
#'
#' Complex cepstrum: inverse FFT of \\code{\\log X(\\omega)} using the unwrapped
...
#'
#' Estimates the DFA scaling exponent \\code{\\alpha}. White noise gives
#' \\code{\\alpha \\approx 0.5}; pink (1/f) noise \\code{\\alpha \\approx 1.0};
#' Brownian motion \\code{\\alpha \\approx 1.5}.
#'
...
#'
#' @param x Numeric vector (length \\code{\\geq} 16).
...
#'
#' Katz fractal dimension \\code{D = \\log_{10}(n - 1) / (\\log_{10}(n - 1) +
#' \\log_{10}(d / L))} of a 1-D signal. \\eqn{L} is total path length and
...
#'
#' Petrosian fractal dimension \\code{D = \\log_{10}(N) / (\\log_{10}(N) +
#' \\log_{10}(N / (N + 0.4 N_\\delta)))}, where \\code{N_\\delta} counts sign
...
#' signal, computes \\code{-x^2 \\log x^2}, then box-smooths over a 20 ms
