...
morie_permutation_lm_loss <- function(logits, targets, permutation,
  ...
  z <- as.integer(permutation)
  if (length(z) != T_) {
    stop(sprintf("permutation has length %d but logits has %d positions.",
                 length(z), T_), call. = FALSE)
  }
  m <- apply(L, 1L, max)
  d <- L - m
  ...
  sel <- nll[scored + 1L]
  loss <- switch(reduction, mean = mean(sel), sum = sum(sel), none = sel)
  list(loss = loss, estimate = loss, token_nll = nll,
...
morie_dp_mean <- function(y, C = NULL, epsilon = 1, n = NULL,
  ...
  if (mechanism == "laplace") {
    ...
    noise_sd <- scale
    eps_draw <- if (is.null(noise)) stats::rnorm(1L, 0, scale) else
      as.numeric(noise)
    if (epsilon >= 1) {
      warns <- c(warns, paste(
  ...
  noise_half <- if (mechanism == "laplace") scale * log(2 / alpha) else
    z_half * scale
  ...
  list(estimate = priv, non_private_mean = mu_raw, clipped_mean = mu_clip,
       ...
       method = paste("Differentially private clipped mean",
...
morie_minimax_regret_constant <- function(tol = 1e-14) {
  ...
  t <- (lo + hi) / 2
  list(t_star = t, constant = t * stats::pnorm(-t),
...
morie_efficiency_bound_ate <- function(y, D, X, family = c("gaussian",
                                                           "binomial"),
  ...
  if (length(d) != n || nrow(Xa) != n) {
    stop(sprintf("y, D and X must agree in length; got %d, %d and %d.",
                 n, length(d), nrow(Xa)), call. = FALSE)
  }
  if (n < 10L) {
  ...
  arm <- function(mask) {
    if (sum(mask) < ncol(Xd) + 1L) {
      stop(sprintf(paste("only %d observations in one treatment arm for %d",
                         "design columns; the outcome model is not",
                         "identified."), sum(mask), ncol(Xd)), call. = FALSE)
    }
    if (family == "binomial") {
  ...
  if (trim_binding > 0L) {
    warns <- c(warns, sprintf(paste(
      ...
      "from the ATE."), trim_binding, n, trim, 1 - trim))
  }
  ...
  zc <- stats::qnorm(1 - alpha / 2)
  se <- sqrt(var_aipw / n)
  list(estimate = tau, ate_aipw = tau, ate_ipw = tau_ipw, se = se,
       ...
       method = paste("Semiparametric efficiency and minimax regret",
                      "bounds for the ATE"))
}
