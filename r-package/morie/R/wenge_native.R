...
#' @param strategy One of \code{"all"}, \code{"em"}, \code{"ye"}, \code{"ym"}. Defaults
#' to \code{"em"}.
...
morie_wenge_mediation_functional <- function(Y, E, M, X, strategy = "em",
  ...
  if (!(strategy %in% .wenge_STRATEGIES)) {
    stop(sprintf(
      "mediation_functional: strategy must be one of %s, got %s",
  ...
  if (any(!(ev %in% c(0.0, 1.0)))) {
    stop(paste0(
      "mediation_functional: E must be binary 0/1; the mediation ",
  ...
  for (i in seq_len(n)) {
    ...
    if (p <= 0.0 || p >= 1.0) {
      stop(sprintf(paste0(
        "mediation_functional: f(E|X) is %g at observation ",
        "%d, so positivity fails in the sample and the ",
        "functional is not identified there"
      ), p, i))
    }
  }
  out <- list()
...
morie_wenge_weight_based_mediation <- function(X, M, C, Y, strategy = "em",
  ...
  yv <- .s03vec(Y)
  n <- length(yv)
  theta <- morie_wenge_mediation_functional(yv, ev, M, C,
    strategy = strategy,
    saturated = saturated
  )
  thetas <- if (is.list(theta)) {
    theta
  } else {
    stats::setNames(
      list(theta),
  ...
  ey1 <- sum(vapply(
    seq_len(n),
    function(i) yv[i] * ev[i] / fe1(i), numeric(1)
  )) / n
  ey0 <- sum(vapply(
    seq_len(n),
  ...
  out <- list(
    ...
    method = sprintf(paste0(
      "natural direct and indirect effects via the ",
      "mediation functional, Tchetgen Tchetgen & ",
      "Shpitser (2012) strategy %s"
    ), strategy)
  )
