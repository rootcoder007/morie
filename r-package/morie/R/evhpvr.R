...
.ht_nll <- function(xv, yv, a, b) {
  n <- length(xv)
  # accumulate in a loop, not with sum(): R sums in long double and
  # Python in double, and the golden-section comparisons below are
  # decided on the last bits, so a vectorised sum walks the two arms
  # onto different optima (seen at 1e-8 in parity).
  z <- (yv - a * xv) / xv^b
  sz <- 0
  for (v in z) sz <- sz + v
  m <- sz / n
  s2 <- 0
  for (v in z) s2 <- s2 + (v - m)^2
  s2 <- s2 / n
  if (s2 <= 0) return(list(f = Inf, mu = m, sd = 0))
  slx <- 0
  for (v in xv) slx <- slx + log(v)
  list(f = 0.5 * n * log(s2) + b * slx, mu = m, sd = sqrt(s2))
}
