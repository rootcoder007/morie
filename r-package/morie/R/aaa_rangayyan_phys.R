...
.bsaenvelope <- function(x, fs, win_s) {
  ...
  list(env = vapply(starts, function(i) .bsarms(xs[(i + 1L):(i + w)]),
                    numeric(1)),
       step_s = w / fs)
}
...
ApWave <- function(t, v_rest = -70, v_peak = 30, t_rise = 0.5, t_fall = 1,
  ...
  list(t_ms = ts, V_mV = Vs,
       ...
       method = paste("Idealised ramp-and-decay action potential;",
...
Nernst <- function(T = 310.15, z = 1, conc_out = 5, conc_in = 140,
  ...
  list(ion = as.character(ion),
       ...
       method = paste("Nernst equilibrium potential, Rangayyan (2024)",
...
#' @param dt Numeric; combined arithmetically in the body. Defaults to \code{0.01}.
#' @param I_ext Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{10}.
#' @param stim_start Numeric; combined arithmetically in the body. Defaults to \code{5}.
#' @param stim_stop Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{6}.
...
HhModel <- function(duration = 30, dt = 0.01, I_ext = 10, stim_start = 5,
  ...
  list(t_ms = ts, V_mV = Vs, m = ms, h = hs, n = ns,
       ...
       units = list(V = "mV", t = "ms", I = "uA/cm^2",
...
#' @param dt Numeric; combined arithmetically in the body. Defaults to \code{0.01}.
#' @param I_ext Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{0.5}.
...
Fhn <- function(duration = 200, dt = 0.01, I_ext = 0.5, a = 0.7, b = 0.8,
  ...
  period <- if (length(spikes) > 1L)
    (spikes[length(spikes)] - spikes[1L]) / (length(spikes) - 1L) else NULL
  list(t = ts, v = vs, w = ws,
       peak = max(vs), min = min(vs),
       n_spikes = length(spikes), spike_times = spikes, period = period,
       a = a, b = b, eps = eps, I_ext = I_ext,
       units = list(v = "dimensionless", w = "dimensionless",
                    t = "dimensionless model time units"),
       method = paste("FitzHugh (1961) Biophys J 1(6):445-466 / Nagumo et al.",
...
#'
#' @param t Coerced to numeric by the body, with \code{as.numeric}.
#' @param I_inj Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{0}.
...
RcMemb <- function(t, I_inj = 0, C_m = 0.2, R_m = 100, V_rest = -65) {
  ...
  list(t_ms = ts, V_mV = Vs, tau_ms = tau,
       V_steady_mV = V_rest + cur[length(cur)] * R_m,
       input_resistance_MOhm = R_m,
       peak_mV = max(Vs), final_mV = Vs[length(Vs)], V_rest_mV = V_rest,
       units = list(V = "mV", t = "ms", I = "nA", R = "MOhm",
                    C = "nF", tau = "ms"),
       method = paste("Passive RC membrane, leak-only reduction of",
...
#' field; Rangayyan (2024) eqs. (7.143)-(7.149), Section 7.8.2.
#'
#' @param n_nodes Coerced to integer by the body, with \code{as.integer}. Defaults to \code{100}.
#' @param dx_cm Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{0.02}.
#' @param duration_ms Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{60}.
#' @param dt_ms Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{0.005}.
#' @param sigma_i Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{1}.
#' @param sigma_e Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{2}.
#' @param C_m Numeric; combined arithmetically in the body. Defaults to \code{1}.
#' @param Sv Numeric; combined arithmetically in the body. Defaults to \code{1000}.
#' @param I_ion Optional; may be \code{NULL}. A function; the body checks with \code{is.function}.
#' @param stim_nodes Coerced to integer by the body, with \code{as.integer}. Defaults to \code{5}.
#' @param I_stim Numeric; combined arithmetically in the body. Defaults to \code{50}.
#' @param stim_ms Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{1}.
...
BiDomain <- function(n_nodes = 100, dx_cm = 0.02, duration_ms = 60,
  ...
  lim <- dx * dx / (2 * D)
  if (dt > lim)
    stop(sprintf(paste("dt_ms=%g exceeds the explicit stability limit %g ms",
                       "for D=%g cm^2/ms and dx=%g cm"), dt, lim, D, dx))
  tf <- as.numeric(threshold_frac)
  if (!(tf > 0 && tf < 0.5))
    stop(paste("threshold_frac must be strictly between 0 and 0.5;",
  ...
  list(x_cm = xs, Vm_mV = V, phi_e_mV = phie, phi_i_mV = phii,
       ...
       units = list(V = "mV", x = "cm", t = "ms", sigma = "mS/cm",
                    C_m = "uF/cm^2", Sv = "1/cm", I = "uA/cm^2",
                    D = "cm^2/ms", cv = "cm/ms"),
       method = paste("Rangayyan (2024) eqs. (7.143)-(7.149), Section 7.8.2,",
...
#' the source it follows.
#'
#' @param coronary_sound Coerced to numeric by the body, with \code{as.numeric}.
#' @param fs Numeric; combined arithmetically in the body.
...
CadAcou <- function(coronary_sound, fs, order = 8, hf_band = c(300, 900),
  ...
  rf <- .bsabandpow(freqs, psd, as.numeric(ref_band[1L]),
  ...
  c(mom, list(
    ...
    method = paste("Rangayyan (2024) Section 7.10 with AR modelling of",
...
#' Rangayyan (2024) eqs. (7.135)-(7.136), Section 7.7.2.
#'
#' @param diameter Coerced to numeric by the body, with \code{as.numeric}.
...
#' the source it follows.
#'
#' @param cry Coerced to numeric by the body, with \code{as.numeric}.
#' @param fs Numeric; combined arithmetically in the body.
...
InfantCry <- function(cry, fs, window_ms = 40, f0_range = c(200, 1000),
  ...
  if (w < as.integer(2 * fs / flo))
    stop(sprintf("a %g ms window holds fewer than two periods at %g Hz",
                 window_ms, flo))
  nwin <- length(xs) %/% w
  ...
  for (i in seq_len(nwin)) {
    ...
    if (!is.null(prev)) {
      ratio <- v / prev
      melody <- c(melody, if (abs(ratio - 1) < tol) 0L
  ...
  sf <- if (n > 1L)
    sqrt(.morie_fsum((voiced - mf) * (voiced - mf)) / (n - 1L)) else 0
  fmt <- numeric(0)
  if (!is.null(best_seg)) {
  ...
  list(f0_track_hz = track, t_track_s = tt, melody = melody,
       ...
       flat_tolerance = tol,
       units = list(frequency = "Hz", time = "s",
                    f0_range_semitones = "semitones"),
       method = paste("Rangayyan (2024) Section 8.13 after Varallyay: 40 ms",
...
#' the source it follows.
#'
#' @param egg Coerced to numeric by the body, with \code{as.numeric}.
#' @param fs Numeric; combined arithmetically in the body.
...
EggFeat <- function(egg, fs, normal_band = c(0.0333, 0.0667)) {
  ...
  c(mom, list(
    ...
    units = list(frequency = "Hz", dominant_freq_cpm = "cycles/minute",
                 fractions = "dimensionless", duration = "s"),
    method = paste("Rangayyan (2024) Section 1.2.8 with the PSD measures of",
...
#' the source it follows.
#'
#' @param eeg Coerced to numeric by the body, with \code{as.numeric}.
#' @param fs Numeric; combined arithmetically in the body.
...
SeizDet <- function(eeg, fs, epoch_s = 1, ratio_threshold = 2,
  ...
  if (w < 8L)
    stop(sprintf("epoch of %g s is only %d samples; use a longer epoch",
                 epoch_s, w))
  n_ep <- length(xs) %/% w
  if (n_ep < 2L) stop("need at least 2 whole epochs")
  binnie <- list(c(1, 2), c(2, 4), c(4, 6), c(6, 8), c(8, 11), c(11, 14),
                 c(14, fs / 2))
  trad <- list(delta = c(0.5, 4), theta = c(4, 8), alpha = c(8, 13.0001),
  ...
  for (e in seq_len(n_ep)) {
    ...
    row$slow_fraction <- (if (is.null(row$delta_fraction)) 0
                          else row$delta_fraction) +
                         (if (is.null(row$theta_fraction)) 0
                          else row$theta_fraction)
    row$form_factor <- .bsahjorth(seg)$form_factor
    rows[[e]] <- row
  }
  nb <- if (!is.null(baseline_epochs)) as.integer(baseline_epochs)
        else max(1L, n_ep %/% 4L)
  if (!(nb >= 1L && nb < n_ep))
    stop("baseline_epochs must be between 1 and n_epochs-1")
  base <- .morie_fsum(vapply(rows[seq_len(nb)],
                             function(r) r$slow_fraction, numeric(1))) / nb
  if (base <= 0)
  ...
  list(epochs = rows,
       ...
       method = paste("Rangayyan (2024) Section 8.17 with the spectral",
...
#' the source it follows.
#'
#' @param erp Coerced to numeric by the body, with \code{as.numeric}.
#' @param fs Numeric; combined arithmetically in the body.
...
ErpFeat <- function(erp, fs, t0 = 0, components = NULL,
  ...
  if (is.null(components))
    components <- list(N100 = list(50, 150, -1L), P200 = list(150, 250, 1L),
  ...
  for (nm in names(components)) {
    ...
    if (!length(win)) {
      rows[[nm]] <- list(latency_ms = NULL, amplitude_uV = NULL,
                         found = FALSE, window_ms = c(t1, t2))
      next
    }
    i <- win[which.max(pol * ys[win])]
    rows[[nm]] <- list(latency_ms = ts[i], amplitude_uV = ys[i],
  ...
  list(components = rows,
       ...
       method = paste("Rangayyan (2024) Section 1.2.7 (latency, duration and",
...
ErdErs <- function(eeg, fs, ref_window, active_window, band = c(8, 13)) {
  ...
  list(erd_percent = pct, ref_power = R, active_power = A,
       power_ratio = A / R, event = event,
       band_hz = c(lo, hi),
       ref_window_s = c(as.numeric(ref_window[1L]),
                        as.numeric(ref_window[2L])),
       active_window_s = c(as.numeric(active_window[1L]),
       ...
       units = list(power = "uV^2 per sample", erd_percent = "percent",
                    frequency = "Hz"),
       method = paste("Pfurtscheller & Aranibar (1979) / Pfurtscheller &",
...
CadSpec <- function(x, fs, bands = NULL) {
  ...
  c(mom, list(
    ...
    units = list(frequency = "Hz", power = "signal units^2",
                 fractions = "dimensionless"),
    method = paste("Rangayyan (2024) eqs. (6.32), (6.34), (6.35), (6.37),",
...
#' @param emg_ref Coerced to numeric by the body, with \code{as.numeric}.
#' @param fs Coerced to numeric by the body, with \code{as.numeric}.
#' @param n_taps Coerced to integer by the body, with \code{as.integer}. Defaults to \code{8}.
...
VagClean <- function(vag, emg_ref, fs, n_taps = 8, mu = 0.05, alpha = 0.02,
  ...
  if (adaptive_mu) {
    ...
    lim <- 1 / (M1 * rpow)
    if (mu >= lim)
      stop(sprintf(paste("mu=%g exceeds the stability limit %g for a",
  ...
  list(cleaned = out, artifact_estimate = art, weights = w,
       ...
       artifact_reduction_db = if (ra > 0 && rb > 0)
         20 * log(rb / ra, 10) else NULL,
       mu_trace = mus, n_taps = M1, alpha = alpha,
       adaptive_mu = as.logical(adaptive_mu), fs_hz = fs,
       units = list(signals = "input amplitude units (mV)",
                    reduction = "dB", mu = "dimensionless"),
       method = paste("Rangayyan (2024) eqs. (3.203), (3.204), (3.205),",
...
MuapModel <- function(t, n_fibers = 25, conduction_vel = 4, spread_mm = 3,
  ...
  list(t_ms = ts, muap_uV = wave,
       ...
       method = paste("MUAP as summed single-fibre potentials; morphology and",
...
#' @param fs Numeric; combined arithmetically in the body.
#' @param f1 Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{25}.
#' @param f2 Coerced to numeric by the body, with \code{as.numeric}. Defaults to \code{75}.
...
MurmSpec <- function(pcg, fs, f1 = 25, f2 = 75, f3 = 150) {
  ...
  c(mom, list(
    ...
    method = paste("Rangayyan (2024) eq. (6.45), Sections 6.2.2 and 6.4.2,",
                   "after Johnson et al.")))
}
...
OaeFeat <- function(oae, fs, noise_floor = NULL, bands = NULL) {
  ...
  for (bd in bands) {
    ...
    row <- list(lo_hz = lo, hi_hz = hi, power = p,
  ...
  out <- c(mom, list(
    ...
    method = paste("Rangayyan (2024) Section 1.2.16 with the PSD measures of",
                   "Section 6.4.1 and the band fraction of eq. (6.44)",
...
PdMonitor <- function(eeg, emg, gait, fs, tremor_band = c(3, 7)) {
  ...
  tremor <- function(sig, name) {
    ...
    list(xs = xs, fr = sp$freqs, ps = sp$power, tot = tot,
         frac = .bsabandpow(sp$freqs, sp$power, tlo, thi) / tot, fpk = fpk)
  }
  et <- tremor(eeg, "eeg")
  bands <- list(delta = c(0.5, 4), theta = c(4, 8), alpha = c(8, 13.0001),
  ...
  list(eeg_bands = eeg_bands,
       ...
       units = list(eeg = "uV", emg = "uV", frequency = "Hz",
                    fractions = "dimensionless", turns = "1/s"),
       method = paste("Rangayyan (2024) Section 10.14 with the EEG bands of",
...
PcgEeg <- function(pcg, eeg, fs, n_segments = 8, band = c(1, 100)) {
  ...
  list(freq_hz = freqs, coherence = coh, coherence_sq = c2, phase_rad = ph,
       ...
       fs_hz = fs,
       units = list(frequency = "Hz", coherence = "dimensionless [0,1]",
                    phase = "radians", delay = "ms"),
       method = paste("Rangayyan (2024) eq. (4.32), Section 4.5, magnitude",
...
MurmDet <- function(pcg, fs, threshold = 0.15, hf_band = c(150, 600)) {
  ...
  c(mom, list(
    ...
    method = paste("Rangayyan (2024) Section 10.2.4 with the band power",
...
#'
#' @param eeg Coerced to numeric by the body, with \code{as.numeric}.
#' @param eog Coerced to numeric by the body, with \code{as.numeric}.
...
PsgStage <- function(eeg, eog, emg, fs, epoch_len = 30) {
  ...
  for (e in seq_len(n_ep)) {
    ...
    emg_tone <- sqrt(.bsabandpow(spm$freqs, spm$power, 10, fs / 2) /
                       length(sl))
    rows[[e]] <- list(t_start_s = (e - 1L) * epoch_len, delta_fraction = d,
  ...
  for (e in seq_len(n_ep)) {
    r <- rows[[e]]
    rows[[e]]$stage <-
      if (r$delta_fraction > 0.5) "N3"
      else if (r$emg_tone < 0.5 * emg_ref && r$eog_activity > eog_ref &&
               r$beta_fraction + r$theta_fraction > 0.4) "REM"
      else if (r$alpha_fraction > 0.3 && r$emg_tone >= emg_ref) "Wake"
      else if (r$theta_fraction > r$alpha_fraction) "N2"
      else "N1"
  }
  ...
  list(epochs = rows, stage_sequence = seqv, stage_minutes = mins,
       ...
       heuristic = TRUE,
       units = list(signals = "uV", time = "s", stage_minutes = "minutes",
                    fractions = "dimensionless"),
       method = paste("Rangayyan (2024) Section 2.4.1 (polysomnography) with",
...
IeiStats <- function(event_times, T = NULL, n_bins = 20) {
  ...
  sr <- if (n > 1L)
    sqrt(.morie_fsum((rates - mr) * (rates - mr)) / (n - 1L)) else 0
  ...
  if (hi > lo) {
    ...
    hist <- Map(function(k, ct) c(lo + (k + 0.5) * width, ct),
                0:(n_bins - 1L), counts)
  } else {
    hist <- list(c(lo, n))
  }
  ...
  fano <- if (mc > 0)
    .morie_fsum((counts - mc) * (counts - mc)) / nw / mc else NULL
  cvr <- if (mr > 0) sr / mr else 0
  reg <- if (cvr < 0.1) "near-periodic"
         else if (cvr < 0.25) "regular" else "irregular"
  list(mean_ipi_s = mi, sd_ipi_s = si, cv_ipi = if (mi > 0) si / mi else 0,
       ...
       units = list(interval = "s", rate = "pps (pulses per second)",
                    cv = "dimensionless"),
       method = paste("Rangayyan (2024) Section 7.3, IPI statistics mu_r and",
...
ValvePcg <- function(pcg, fs, n_peaks = 3, order = NULL) {
  ...
  for (i in seq_along(found$freqs)) {
    ...
    peaks[[i]] <- list(freq_hz = found$freqs[i], power = found$powers[i],
                       bandwidth_3db_hz = qf$bandwidth_hz, q_factor = qf$q)
  }
  mom <- .bsapsdmom(freqs, psd)
  c(mom, list(
    ...
    freq_hz = freqs, ar_psd = psd, fs_hz = fs,
    units = list(frequency = "Hz", bandwidth = "Hz",
                 q_factor = "dimensionless"),
    method = paste("Rangayyan (2024) Section 6.5 after Durand et al., with",
...
RespFeat <- function(resp, fs, signal_type = "flow", min_breath_s = 1) {
  ...
  for (i in ons)
    if (!length(kept) || (i - kept[length(kept)]) / fs >= min_breath_s)
      kept <- c(kept, i)
  if (length(kept) < 2L)
    stop(paste("fewer than 2 breaths detected; check signal_type,",
               "fs and min_breath_s"))
  ...
  for (b in seq_len(length(kept) - 1L)) {
    ...
    breaths[[b]] <- list(t_start_s = (i0 - 1L) / fs,
                         period_s = (i1 - i0) / fs,
  ...
  list(rate_breaths_per_min = 60 / mp,
       mean_period_s = mp, sd_period_s = sp,
       regularity_cv = if (mp > 0) sp / mp else 0,
       depth = .morie_fsum(vapply(breaths, function(b) b$depth,
                                  numeric(1))) / n,
       ...
       units = list(rate = "breaths/min", time = "s",
                    depth = "litres if flow is L/s or volume is L",
                    ie_ratio = "dimensionless"),
       method = paste("Standard per-breath respiratory measures;",
...
RespSound <- function(length_m = 0.1, radius_m = 0.009, freqs = NULL,
  ...
  list(freq_hz = fs_hz, transfer_mag = mag,
       ...
       units = list(freq = "Hz", La = "kg/m^4", Ca = "m^3/Pa",
                    Ra = "Pa s/m^3", Ga = "m^3/(Pa s)", length = "m"),
       method = paste("Rangayyan (2024) eqs. (7.122), (7.127), (7.128),",
...
#' the source it follows.
#'
#' @param ecg Coerced to numeric by the body, with \code{as.numeric}.
#' @param spo2 Coerced to numeric by the body, with \code{as.numeric}.
#' @param snore Coerced to numeric by the body, with \code{as.numeric}.
#' @param fs Numeric; combined arithmetically in the body.
...
ApneaDet <- function(ecg, spo2, snore, fs, epoch_s = 60, desat_pct = 4) {
  ...
  sev <- if (idx < 5) "none" else if (idx < 15) "mild"
         else if (idx < 30) "moderate" else "severe"
  list(epochs = rows, n_flagged = as.integer(nflag),
       ...
       units = list(spo2 = "percent", ecg = "mV", hr = "bpm",
                    events_per_hour = "1/h", time = "s"),
       method = paste("Rangayyan (2024) Sections 10.2.5, 10.13 and 2.4",
...
SpeechFeat <- function(speech, fs, order = NULL, n_formants = 4,
  ...
  list(formants_hz = fmt, formant_bandwidths_hz = bws,
       formant_powers = pws,
       ...
       units = list(frequency = "Hz", period = "ms",
                    zero_crossing_rate = "1/s"),
       method = paste("Rangayyan (2024) Section 7.2.3 with all-pole",
...
VagFeat <- function(vag, fs, n_segments = 8) {
  ...
  means <- vapply(seq_len(ns), function(i)
    .morie_fsum(xs[((i - 1L) * seg + 1L):(i * seg)]) / seg, numeric(1))
  ...
  list(mean = mm[1L], variance = mm[2L], skewness = mm[3L],
       ...
       units = list(amplitude = "signal units (mV at the accelerometer)",
                    entropy = "bits", turns_per_second = "1/s",
                    form_factor = "dimensionless"),
       method = paste("Rangayyan (2024) Section 5.12.3 with eqs. (5.25),",
...
VagKnee <- function(vag, fs, weights = NULL, bias = NULL, n_segments = 8) {
  ...
  feats <- c(base$form_factor, base$kurtosis_excess,
  ...
  base$discriminant <- d
  base$features <- list(form_factor = feats[1L],
  ...
  base$method <- paste("Rangayyan (2024) Section 10.12 with the VAG feature",
...
CLogProd <- function(X, H, omega = NULL) {
  ...
  list(omega = om,
       ...
       max_abs_error = err,
       units = list(`log magnitude` = "nepers",
                    phase = "radians (unwrapped)"),
       method = paste("Rangayyan (2024) eq. (4.63), complex log of a product,",
...
#' @param a_k Coerced to complex by the body, with \code{as.complex}. Defaults to \code{complex(0)}.
...
CLogPz <- function(z, A = 1, r = 0, a_k = complex(0), b_k = complex(0),
  ...
  r <- as.integer(r)
  sets <- list(a_k = as.complex(a_k), b_k = as.complex(b_k),
  ...
  for (i in seq_len(n)) {
    ...
    xval[i] <- num / den
    if (is.null(first))
      first <- list(gain_and_delay = c(Re(gain), Im(gain)),
  ...
  recon <- complex(real = exp(Re(xhat)) * cos(Im(xhat)),
                   imaginary = exp(Re(xhat)) * sin(Im(xhat)))
  err <- max(0, Mod(recon - xval))
  list(z_real = Re(zs), z_imag = Im(zs),
       xhat_real = Re(xhat), xhat_imag = Im(xhat),
       X_real = Re(xval), X_imag = Im(xval),
       max_abs_error = err,
       terms = first,
       counts = list(M_I = length(ak), M_O = length(bk),
                     N_I = length(ck), N_O = length(dk)),
       units = list(xhat_real = "nepers (log magnitude)",
                    xhat_imag = "radians (phase)", z = "dimensionless"),
       method = paste("Rangayyan (2024) eq. (4.68) with eq. (4.67), complex",
