...
Manh2h <- function(yi, vi, design, edge) {
  ...
  v_dir <- 1 / sw
  nd <- .net_design(D[rest, , drop = FALSE])
  ...
  A <- crossprod(Xr * w, Xr)
  beta <- as.numeric(.s03ridgesolve(A, as.numeric(crossprod(Xr * w, yr)), 1e-12))
  cv <- vapply(seq_len(p), function(j) {
    ee <- numeric(p); ee[j] <- 1; as.numeric(.s03ridgesolve(A, ee, 1e-12))
  }, numeric(p))
  pos <- match(e, treats)
  cvec <- numeric(p)
