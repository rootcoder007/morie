...
EegAdapt <- function(x, fs, window = NULL, step = NULL, order = 4,
  ...
  N <- length(xs)
  p <- as.integer(order)
  w <- if (!is.null(window)) as.integer(window)
       else max(4L * (p + 1L), as.integer(0.5 * fsv))
  ...
  list(d = adaptive, d_fixed_reference = dists, times = times,
...
XCorr <- function(x, y, maxlag = NULL, normalize = FALSE, biased = TRUE) {
  ...
  list(ccf = vals, lags = lags, peak = vals[k], peak_lag = lags[k],
...
XCorrDisc <- function(x, y, delays = NULL) {
  ...
  list(ccf = if (scalar) out[[1L]] else out, lags = ks,
...
XCorrCont <- function(x, y, t, delays) {
  ...
  list(ccf = out, delays = dl, overlap_fraction = if (length(frac) == 1L)
         frac[[1L]] else frac,
...
XCorrProc <- function(x, y, lags = NULL, remove_mean = TRUE) {
  ...
  list(ccf = r$ccf, lags = r$lags, means = c(mx, my),
...
CorrConv <- function(x, y) {
  ...
  list(ccf = direct, via_convolution = via, lags = r$lags,
...
NccfTpl <- function(x, template) {
  ...
  k <- which.max(g)
  list(gamma = g, shifts = shifts, peak = g[[k]], peak_shift = shifts[[k]],
...
CorrDot <- function(x, y) {
  ...
  list(gamma = raw$gamma, dot_product = raw$dot_product,
       ...
       method = paste("Rangayyan (2024) Ch. 4 (correlation as a",
...
AlphaRhy <- function(x, fs, band = c(8, 13), threshold = 0.3) {
  ...
  peak <- max(vals)
  list(acf = a, band = c(lo, hi), lag_range = c(klo, khi),
...
CompSig <- function(g, shifts, scales = NULL, n = NULL) {
  ...
  list(x = x, n = nv, shifts = sh, scales = sc,
