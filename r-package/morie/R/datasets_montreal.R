...
morie_datasets_montreal_justice_safety_layers <- function(offline = TRUE) {
  if (isTRUE(offline)) {
    path <- .morie_extdata("montreal_justice_safety_catalog.csv")
...
#' @examples
#' \dontshow{if (nzchar(system.file("extdata", "montreal_sim_interventions_sample.csv", package = "morie")) || requireNamespace("rmoriedata", quietly = TRUE)) withAutoprint(\{ # examplesIf}
...
morie_datasets_montreal_sim_interventions <- function(offline = TRUE,
  ...
  if (!is.null(csv_path)) {
    ...
    path <- .morie_extdata("montreal_sim_interventions_sample.csv")
    if (!nzchar(path) && requireNamespace("rmoriedata", quietly = TRUE)) {
...
#'   `Description`.
#' @examples
#' \dontshow{if (nzchar(system.file("extdata", "montreal_sim_intervention_types.csv", package = "morie")) || requireNamespace("rmoriedata", quietly = TRUE)) withAutoprint(\{ # examplesIf}
...
morie_datasets_montreal_sim_intervention_types <- function() {
  path <- .morie_extdata("montreal_sim_intervention_types.csv")
  if (!nzchar(path) && requireNamespace("rmoriedata", quietly = TRUE)) {
