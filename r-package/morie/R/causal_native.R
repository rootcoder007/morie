...
morie_ate_potential_outcomes <- function(y1, y0, paired = TRUE, alpha = 0.05) {
  ...
  list(ate = ate, estimate = ate, se = se,
       ...
       method = paste0("ATE from potential outcomes, ",
...
morie_backdoor_adjustment <- function(x, y, z, at = NULL) {
  ...
  list(distribution = dist, strata = lev, p_z = pz, n = n,
...
.bd_paths <- function(x, y, ch, pa) {
  ...
  while (length(stack)) {
    ...
    for (nx in pa[[s$cur]]) if (!(nx %in% s$path)) {
      stack <- c(stack, list(list(cur = nx, path = c(s$path, nx), dirs = c(s$dirs, "<-"))))
    }
  }
  out
...
.bd_blocked <- function(p, d, Z, ch) {
  if (length(p) < 3L) return(FALSE)
  for (i in 2:(length(p) - 1L)) {
    node <- p[i]
    collider <- d[i - 1L] == "->" && d[i] == "<-"
    if (collider) {
      if (!(node %in% Z) && !length(intersect(.bd_desc(node, ch), Z))) return(TRUE)
    } else if (node %in% Z) {
      return(TRUE)
    }
  }
...
morie_backdoor_criterion <- function(dag, x, y, z = character(0)) {
  ...
  for (nd in g$nodes) if (nd %in% .bd_desc(nd, g$children)) {
  ...
  back <- Filter(function(s) length(s$dirs) && s$dirs[1] == "<-",
  ...
  reason <- if (ok) {
    ...
    paste0("Z contains descendants of X: ", paste(bad, collapse = ", "),
  ...
  list(satisfied = ok, descendant_violations = bad,
...
morie_baron_kenny <- function(y, x, m, alpha = 0.05) {
  ...
  list(c = cc, a = a, b = b, c_prime = cp, indirect = a * b,
...
morie_indirect_effect_sobel <- function(a, b, se_a = NULL, se_b = NULL, alpha = 0.05) {
  ...
  if (is.null(se_a) || is.null(se_b)) {
    return(list(estimate = ie, se = NULL, statistic = NULL, p_value = NULL,
  ...
  list(estimate = ie, se = se, statistic = z,
...
#' d <- rbinom(n, 1, plogis(0.8 * xx)); u <- rnorm(n, 0, 2)
...
morie_dr_did <- function(y_pre, y_post, d, x, trim = 0.995, alpha = 0.05) {
  ...
  if (length(post) != n || length(dd) != n || nrow(X) != n) {
    stop("y_pre, y_post, d and x must share a length; got ", n, ", ",
         length(post), ", ", length(dd), ", ", nrow(X), ".", call. = FALSE)
  }
  ...
  list(att = att, estimate = att, se = se,
...
morie_binary_mediation <- function(x, m, y, covariates = NULL, B = 0L, alpha = 0.05) {
  ...
  point <- function(idx) {
    ...
    dir <- stats::coef(suppressWarnings(
      stats::glm(ys ~ Xt, family = stats::binomial(), weights = w)))[2]
...
morie_anm_direction <- function(x, y, B = 200L) {
  ...
  pp <- function(a, r, obs) (1 + sum(vapply(seq_len(B), function(i) morie_hsic(a, sample(r)) >= obs,
                                            logical(1)))) / (1 + B)
  p_xy <- pp(x, r_xy, h_xy)
  p_yx <- pp(y, r_yx, h_yx)
