...
taken the fallback. **Its numbers change**: the stimulus positions are
now basicspace's, standardised to mean 0 and sd 1. On data with structure
to recover the two engines agree closely (stimuli correlate about 0.97,
intercepts 1.00) but not exactly; on structureless noise they do not
agree at all (0.04), as nothing constrains either. The respondent
intercepts and slopes are the per-respondent regression of the
placements on those positions on either engine, so they are defined for
every respondent; basicspace reports them only against a self-placement,
which this function does not take, and a 1-7 scale had given all-`NA`
respondent parameters. Non-finite stimuli from basicspace fall back to
the native solver.
...
# morie 1.3.0 - 2026-09-20

## Design and measurement for small or under-enumerated subpopulations

Fifteen functions, in the Python arm and in the vendored R arm, for studies
whose target group is a small share of the frame, is reached by a design that
is not simple random sampling, or is imperfectly identified by the frame
itself. The three problems compound, and the package previously covered only
the estimators, not the planning. Every value is verified against rmorie 1.3.0
to 1e-10.
...
New R vignette `subpopulation-design`, including the governance framework the
arithmetic sits inside.

## R arm dependencies

The R arm's DESCRIPTION now declares every package the vendored code calls.
`here` stays optional, and `cmdstanr`, which is not on CRAN, is no longer
declared where pak cannot resolve it.
...
misleads the reader. 182 example blocks were wrapped in `\dontrun` purely
...
* `man/` and `NAMESPACE` are generated from the `R/` roxygen again. The
  docs marathon had written about 2,300 `\examples{}` blocks straight into
...
* `morie` no longer pulls in 'jsonlite' or 'digest' through
...
* Python `mqtmpl`: every Haldane call now converts centiMorgan positions to Morgans (the HMM, imputation and EM paths all passed cM to a Morgans-only function).
