...
def gen_column_expr(var: dict) -> str:
    ...
    if dtype.startswith("Date"):
        return f"sample(2018:2024, n, replace = TRUE)"
...
def gen_fixture(year: str, kind: str, csv_filename: str,
                dict_name: str, dict_key: str,
                dicts: dict[str, dict]) -> str:
...
def main() -> int:
    ...
    for (year, kind, csv_filename, dict_name, dict_key) in REGISTRY:
        fixtures.append(
            gen_fixture(year, kind, csv_filename, dict_name, dict_key, dicts))

    dispatcher_rows = ",\n".join(
        f'  list(year = "{y}", kind = "{k}", csv_filename = "{c}", '
        f'helper = ".morie_arsau_{safe_key(y, k)}_panel")'
