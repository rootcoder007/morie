...
test_that("cross: native PLR agrees with DoubleML on a linear DGP", {
  ...
  expect_identical(ours$method, "PLR (morie native)")
  ...
  .gst <- morie:::.morie_dml_guard_begin()
  on.exit(morie:::.morie_dml_guard_end(.gst), add = TRUE)
...
test_that("cross: native IRM agrees with DoubleML IRM on a linear DGP", {
  ...
  ours <- morie_estimate_irm(df, treatment = "d", outcome = "y",
                             ...
                             random_state = 42L)
  expect_identical(ours$method, "IRM (morie native)")
