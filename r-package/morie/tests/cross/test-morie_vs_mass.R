...
test_that("native ginv matches MASS::ginv (square, rectangular, rank-deficient)", {
  ...
  expect_equal(morie:::.morie_ginv(A), MASS::ginv(A), tolerance = 1e-12)
  expect_equal(morie:::.morie_ginv(B), MASS::ginv(B), tolerance = 1e-12)
  expect_equal(morie:::.morie_ginv(C), MASS::ginv(C), tolerance = 1e-10)
})
