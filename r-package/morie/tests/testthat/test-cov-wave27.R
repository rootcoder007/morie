...
test_that("statistical callables run end to end on valid data", {
  ...
  w   <- { d <- as.matrix(dist(coords)); ww <- 1 / (d + diag(1, 120)); diag(ww) <- 0; ww }
  mk  <- matrix(rbinom(120 * 5, 2, 0.3), 120, 5); mk[, 5] <- 1L  # one constant col
  ...
  ok("rgcoh",  rgcoh(v, v)); ok("rgcrl", rgcrl(v)); ok("rgdfa", rgdfa(v))
  ok("rgeeg",  rgeeg(v, fs = 64)); ok("rghfd", rghfd(v))
  ok("rglyp",  rglyp(v)); ok("rgpsd", rgpsd(v)); ok("rgstf", rgstf(v))
  ok("rgfir",  rgfir(v, cutoff = 0.2))
  ok("fzlst",  fzlst(vp)); ok("polrz", polrz(v))
  ok("quntf",  morie:::quntf(v)); ok("nstat", nstat(v, coords))
...
test_that("Horowitz semiparametric callables run on adequate samples", {
  ...
  z  <- rbinom(n, 1, 0.5)
  ok("hrzb1", morie:::hrzb1(X, yb)); ok("hrzb2", morie:::hrzb2(X, yb))
  ok("hrzc1", morie:::hrzc1(X, abs(y) + 0.1))
  ok("hrzd1", morie:::hrzd1(abs(y) + 0.1, X, rbinom(n, 1, 0.6)))
  ok("hrzi1", morie:::hrzi1(X, yb)); ok("hrzi2", morie:::hrzi2(X, yb))
  ok("hrzk1", morie:::hrzk1(y)); ok("hrzk2", morie:::hrzk2(X[, 1], y))
...
test_that("ensemble/search callables normalise a vector-valued x", {
  set.seed(103)
  xv <- rnorm(80); yb <- rbinom(80, 1, 0.5); yc <- rnorm(80)
...
test_that("degenerate inputs reach the documented guard branches", {
  ...
  h2 <- morie:::hrzt2(NULL, rnorm(30), c(rep(1L, 28), 0L, 0L),
                      rbinom(30, 1, 0.5))
...
test_that("fwpas covers both transpose branches", {
  w <- matrix(rnorm(12), 3, 4); b <- rnorm(3)
...
test_that(".morie_cvm_pvalue mid-range + dataset_profile fallbacks", {
  ...
  expect_equal(morie:::infer_measurement_level(
    factor(sample(letters[1:4], 40, TRUE))), "nominal")
...
test_that(".cpads_default_csv + .resolve_cpads_csv cover their search paths", {
  wd <- tempfile("cpads-"); dir.create(wd)
...
test_that("RNG sync helpers cover the .Random.seed branches", {
  set.seed(1); invisible(runif(1))      # ensure .Random.seed exists
  morie_sync_rng(42L)
...
test_that("morie_load_dataset covers the unsupported-format stop", {
  ...
  local_dir <- tempfile("ld-"); dir.create(local_dir)
  bad <- file.path(local_dir, "x.parquet"); writeLines("x", bad)
  testthat::local_mocked_bindings(
    morie_builtin_db = function(...) tempfile(fileext = ".db"),
    morie_cache_load = function(...) NULL,
    morie_dataset_catalog = function(...) {
      c0 <- cat[1, , drop = FALSE]
      c0$key <- "covtest"; c0$local_path <- bad
      c0$ckan_resource_id <- ""; c0$download_url <- ""
...
test_that("predpol weak-calibration + ANOVA-failure branches execute", {
  set.seed(104)
  ar <- paste0("A", 1:16); g <- rep(c("x", "y"), 8)
  rk <- 1:16
...
test_that("entheo per-frame helpers hit the short-window skip", {
  set.seed(105)
  eeg <- matrix(rnorm(2 * 12), 2, 12); fmri <- matrix(rnorm(2 * 4), 2, 4)
