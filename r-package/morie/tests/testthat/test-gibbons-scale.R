...
test_that("the tagging vector marks the first sample's positions", {
  ...
  expect_equal(morie:::.gbTagged(c(1, 2), c(10, 20)), c(1, 1, 0, 0))
  expect_equal(morie:::.gbTagged(c(10, 20), c(1, 2)), c(0, 0, 1, 1))
  # a tie puts the first sample first, because the label breaks the tie
  expect_equal(morie:::.gbTagged(c(5), c(5)), c(1, 0))
  # the indicator sums to the first sample's size whatever the overlap
  set.seed(81)
  for (i in 1:5) {
    a <- stats::rnorm(7)
    b <- stats::rnorm(9)
    expect_equal(sum(morie:::.gbTagged(a, b)), 7)
    expect_length(morie:::.gbTagged(a, b), 16L)
  }
})
...
test_that("the linear-rank moments are their general formulas", {
  ...
  expect_equal(morie:::.gbLrMoments(rep(3, 6), 2L, 4L)$var, 0)
  expect_equal(morie:::.gbLrMoments(rep(3, 6), 2L, 4L)$mean, 6)
})
...
test_that("Mood's scale statistic matches stats::mood.test", {
  ...
  m <- morie:::Moodscale(stats::rnorm(5), stats::rnorm(7))
  nn <- 12
  ...
  expect_lt(morie:::Moodscale(stats::rnorm(60),
                               stats::rnorm(60, sd = 4))$p_value, 0.01)
  expect_gt(morie:::Moodscale(stats::rnorm(60),
                               stats::rnorm(60))$p_value, 0.01)
  expect_error(morie:::Moodscale(numeric(0), 1), "non-empty")

  # the moments in their own right
  for (mm in c(3L, 10L)) {
    for (nnn in c(4L, 15L)) {
      mo <- morie:::Moodmom(mm, nnn)
      N <- mm + nnn
  ...
  expect_error(morie:::Moodmom(0L, 3L), "at least 1")
})
...
test_that("the Ansari-Bradley statistic is ansari.test's, shifted", {
  ...
  mv <- morie:::.gbLrMoments(got$scores, 9L, 11L)
  ...
  expect_lt(morie:::Ansbrad(stats::rnorm(60),
                             stats::rnorm(60, sd = 4))$p_value, 0.01)
  expect_error(morie:::Ansbrad(1, numeric(0)), "non-empty")
})

test_that("Siegel-Tukey assigns its scores from both ends inward", {
  x <- c(1, 3, 5, 7)
  y <- c(2, 4, 6, 8)
  s <- morie:::Sgltukey(x, y)
  ...
  mv <- morie:::.gbLrMoments(s$scores, 4L, 4L)
  ...
  odd <- morie:::Sgltukey(c(1, 3, 5), c(2, 4))
  ...
  set.seed(84)
  expect_lt(morie:::Sgltukey(stats::rnorm(50),
                              stats::rnorm(50, sd = 5))$p_value, 0.05)
  expect_error(morie:::Sgltukey(numeric(0), 1), "non-empty")
  # dropping the middle must not empty a sample
  expect_error(morie:::Sgltukey(5, c(1, 9)), "emptied a sample")
})
...
test_that("the Klotz test uses squared normal scores", {
  ...
  # the moments and standardisation
  mv <- morie:::.gbLrMoments(k$scores, 8L, 10L)
  ...
  expect_lt(morie:::Klotzsc(stats::rnorm(60),
                             stats::rnorm(60, sd = 4))$p_value, 0.01)
  # and equal scales are not
  expect_gt(morie:::Klotzsc(stats::rnorm(60), stats::rnorm(60))$p_value,
            0.01)
  expect_error(morie:::Klotzsc(numeric(0), 1), "non-empty")
})
...
test_that("the percentile-modified scale test weights both tails", {
  ...
  # r defaults to s
  expect_equal(morie:::Pctranksc(x, y, s = 0.3)$R,
               morie:::Pctranksc(x, y, s = 0.3)$S)
  # a separate lower fraction is honoured
  asym <- morie:::Pctranksc(x, y, s = 0.3, r = 0.6)
  ...
  odd <- morie:::Pctranksc(stats::rnorm(9), stats::rnorm(10))
  ...
  expect_gt(morie:::Pctranksc(x, y, s = 1)$S,
            morie:::Pctranksc(x, y, s = 0.2)$S)
  # and s = 1 uses the whole sample
  expect_equal(morie:::Pctranksc(x, y, s = 1)$S, 20L)

  expect_error(morie:::Pctranksc(x, y, s = 0), "\\(0, 1\\]")
  expect_error(morie:::Pctranksc(x, y, s = 1.5), "\\(0, 1\\]")
  expect_error(morie:::Pctranksc(x, y, r = 0), "\\(0, 1\\]")
  expect_error(morie:::Pctranksc(numeric(0), 1), "non-empty")
})
...
test_that("all five scale tests agree on a large scale difference", {
  ...
  for (f in list(morie:::Moodscale, morie:::Ansbrad,
                 morie:::Sgltukey, morie:::Klotzsc)) {
  ...
  expect_gt(morie:::Moodscale(a, c2)$p_value, 0.001)
})
