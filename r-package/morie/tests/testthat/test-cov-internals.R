...
.cov_entheo_record <- function(seed = 1L, n_chan = 8L, n_tp = 80L,
  ...
  list(subject_id = "01", condition_order = c("DMT", "PCB"),
...
test_that("entheo_data: load_dmt_imaging covers root / subject branches", {
  res <- tryCatch(morie:::load_dmt_imaging(subject_id = "07",
                                           root = tempfile("envroot_")),
                  error = function(e) e)
  expect_true(inherits(res, "error") || is.list(res))
  res2 <- tryCatch(morie:::load_dmt_imaging(subject_id = NULL,
...
test_that("entheo_preprocess: preprocess_eeg / preprocess_fmri run", {
  ...
  r1 <- tryCatch(morie:::preprocess_eeg(rec, bandpass = c(0.5, 45),
                                        notch = 60, asr_threshold = 1.0),
                 error = function(e) e)
  expect_true(inherits(r1, "error") || is.list(r1))
  r2 <- tryCatch(morie:::preprocess_fmri(rec, motion_threshold_mm = 0.05,
...
test_that("regms: regime_switching at k = 2 and k = 3", {
  ...
  expect_true(inherits(r2, "error") || is.list(r2))
  set.seed(42)
  x3 <- c(stats::rnorm(60, -4, 1), stats::rnorm(60, 0, 1),
...
.cov_kulldorff_df <- function(seed = 51L, n_clust = 90L, n_back = 20L) {
  set.seed(seed)
  lat <- c(stats::rnorm(n_clust, 43.65, 0.004),
           stats::rnorm(n_back, 43.72, 0.05))
  lon <- c(stats::rnorm(n_clust, -79.38, 0.004),
...
test_that("mrm_kulldorff: full scan runs on clustered data", {
  ...
  res <- tryCatch(mrm_tps_kulldorff_scan(df, n_permutations = 9L,
                                         n_centers = 20L,
...
.cov_tps_df <- function(seed = 61L, n = 220L) {
  ...
  lat <- c(stats::rnorm(n %/% 2, 43.66, 0.01),
           stats::rnorm(n - n %/% 2, 43.70, 0.03))
  lon <- c(stats::rnorm(n %/% 2, -79.39, 0.01),
