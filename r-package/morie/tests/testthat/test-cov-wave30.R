...
test_that("GARCH / GJR-GARCH objectives: parameter-domain guard + valid", {
  ...
  expect_true(is.finite(morie:::.tgarch_negll(c(0.1, 0.05, 0.05, 0.8),
                                              r, 40L)))
})
...
test_that("Manski maximum-score objective returns a bounded score", {
  X <- matrix(rnorm(40), 20, 2); ys <- rep(c(-1, 1), 10)
...
test_that("variogram model + WLS objective cover every model branch", {
  ...
  expect_error(morie:::.vrgft_model(1, 0, 1, 2, "no-such-model"),
               "unknown model")
  expect_true(is.finite(
    morie:::.vrgft_obj(c(0, 1, 2), 1:3, c(0.4, 0.6, 0.8),
                       c(1, 1, 1), "exponential")))
})
