...
test_that("the runs statistic carries both of its standardisations", {
  ...
  g <- morie:::Runsz(10, n1, n2)
  expect_equal(g$mean_exact - g$mean, 1)
  # the continuity correction always moves the statistic toward its mean
  for (r in c(5, 15)) {
    expect_lte(abs(morie:::Runsz(r, n1, n2, correct = TRUE)$z),
               abs(morie:::Runsz(r, n1, n2)$z))
  }
  # at the mean the correction has nothing to do
  expect_equal(morie:::Runsz(g$mean, n1, n2, correct = TRUE)$z, 0)
  # too few runs is evidence of clustering, too many of alternation, and
  # both are significant at the extremes
  expect_lt(morie:::Runsz(2, 20L, 20L)$p_value, 0.001)
  expect_lt(morie:::Runsz(40, 20L, 20L)$p_value, 0.001)
  expect_error(morie:::Runsz(5, 0L, 3L), "at least 1")
})
...
test_that("runs up and down use Levene's moments", {
  ...
  n <- 25L
  mu <- (2 * 25 - 1) / 3
  at_mean <- morie:::Runsudvar(n, r = mu)
  ...
  expect_lt(morie:::Runsudvar(50L, r = 10)$p_value, 0.001)
  expect_lt(morie:::Runsudvar(50L, r = 60)$p_value, 0.001)
  # a stricter alpha needs a larger critical value
  expect_gt(morie:::Runsudvar(25L, alpha = 0.01)$zcrit,
            morie:::Runsudvar(25L, alpha = 0.05)$zcrit)
  expect_error(morie:::Runsudvar(1L), "at least 2")
})
...
test_that("the rank von Neumann ratio is Bartels' statistic", {
  ...
  alt <- morie:::Rvntest(c(1, 20, 2, 19, 3, 18, 4, 17, 5, 16))
  expect_gt(alt$statistic, 2)
  # the alternatives
  expect_equal(morie:::Rvntest(x, "less")$p_value, stats::pnorm(got$z))
  expect_equal(morie:::Rvntest(x, "greater")$p_value,
               1 - stats::pnorm(got$z))
  expect_error(morie:::Rvntest(c(1, 2)), "at least 3")
  expect_error(morie:::Rvntest(x, "sideways"),
  ...
  expect_lt(abs(morie:::Rvnmom(200L)$var - morie:::Rvnmom(200L)$var_approx),
            abs(morie:::Rvnmom(10L)$var - morie:::Rvnmom(10L)$var_approx))
  expect_error(morie:::Rvnmom(2L), "at least 3")
})

test_that("the exact runs table is a distribution with the right moments", {
  for (n1 in 2:6) {
    for (n2 in 2:6) {
      t <- morie:::Runstab(n1, n2)
      n <- n1 + n2
  ...
  t <- morie:::Runstab(5L, 5L)
  for (r in 2:10) {
    q <- morie:::Runstab(5L, 5L, r = r)
  ...
  expect_equal(morie:::Runstab(5L, 5L, r = 2L)$pmf_r, 2 / choose(10, 5))
  expect_error(morie:::Runstab(5L, 5L, r = 1L), "2\\.\\.n1\\+n2")
  expect_error(morie:::Runstab(5L, 5L, r = 11L), "2\\.\\.n1\\+n2")
  expect_error(morie:::Runstab(0L, 3L), "at least 1")
})

test_that("the exact critical region holds its stated level", {
  for (alpha in c(0.05, 0.10, 0.20)) {
    cr <- morie:::Runscrit(10L, 10L, alpha = alpha)
    t <- morie:::Runstab(10L, 10L)
  ...
  left <- morie:::Runscrit(10L, 10L, 0.05, tail = "left")
  two <- morie:::Runscrit(10L, 10L, 0.05, tail = "two-sided")
  expect_true(is.nan(left$upper))
  expect_gte(left$lower, two$lower)
  right <- morie:::Runscrit(10L, 10L, 0.05, tail = "right")
  expect_true(is.nan(right$lower))
  expect_lte(right$upper, two$upper)
  # a larger alpha admits a wider region
  loose <- morie:::Runscrit(10L, 10L, 0.20)
  expect_gte(loose$lower, two$lower)
  expect_error(morie:::Runscrit(10L, 10L, tail = "sideways"),
               "two-sided, left or right")
  expect_error(morie:::Runscrit(0L, 5L), "at least 1")
})
