...
test_that("the median-test statistic is hypergeometric under the null", {
  ...
  # can be
  expect_equal(morie:::Medtest(c(1, 2), c(3, 4))$statistic, 0)
  expect_equal(morie:::Medtest(c(3, 4), c(1, 2))$statistic, 2)
  expect_lt(morie:::Medtest(stats::rnorm(40),
                             stats::rnorm(40, 3))$p_value, 0.01)
  expect_gt(morie:::Medtest(stats::rnorm(40), stats::rnorm(40))$p_value,
            0.01)
  expect_error(morie:::Medtest(numeric(0), 1), "non-empty")
})
...
test_that("the median-test interval is built from order statistics", {
  ...
  # is wider
  wide <- morie:::Medtestci(x, y, c = 1L)
  expect_lte(wide$lower, ci$lower)
  expect_gte(wide$upper, ci$upper)
  # c at its maximum is the narrowest available
  expect_equal(morie:::Medtestci(x, y, c = 5L)$c, 5L)
  expect_error(morie:::Medtestci(x, y, c = 0L), "1\\.\\.min")
  expect_error(morie:::Medtestci(x, y, c = 6L), "1\\.\\.min")
  expect_error(morie:::Medtestci(numeric(0), y, 1L), "non-empty")
})
...
test_that("Simpson's rule reproduces the integrals it must", {
  ...
  expect_equal(morie:::.gbSimpson(function(u) 1), 1)
  expect_equal(morie:::.gbSimpson(function(u) u), 1 / 2)
  expect_equal(morie:::.gbSimpson(function(u) u^2), 1 / 3)
  expect_equal(morie:::.gbSimpson(function(u) u^3), 1 / 4)
  # and accurate beyond that
  expect_equal(morie:::.gbSimpson(function(u) u^4), 1 / 5,
               tolerance = 1e-10)
  expect_equal(morie:::.gbSimpson(function(u) exp(u)), exp(1) - 1,
               tolerance = 1e-10)
  expect_equal(morie:::.gbSimpson(function(u) sin(pi * u)), 2 / pi,
               tolerance = 1e-9)
  # a beta density integrates to one
  expect_equal(morie:::.gbSimpson(function(u) stats::dbeta(u, 3, 5)), 1,
               tolerance = 1e-8)
  # an even node count is bumped to odd, since the rule needs pairs of
  # intervals -- the answer is unaffected
  expect_equal(morie:::.gbSimpson(function(u) u^2, nodes = 100),
               morie:::.gbSimpson(function(u) u^2, nodes = 101))
  # more nodes is at least as accurate
  e1 <- abs(morie:::.gbSimpson(function(u) u^5, nodes = 11) - 1 / 6)
  e2 <- abs(morie:::.gbSimpson(function(u) u^5, nodes = 1001) - 1 / 6)
  expect_lt(e2, e1)
})
...
test_that("the precedence power distribution is a distribution", {
  ...
  # g(u) = u is the null
  null_g <- function(u) u
  p <- morie:::Medtestpow(8L, 6L, r = 4L, wcrit = 3L, g = null_g,
                           nodes = 401)
  ...
  shifted <- morie:::Medtestpow(8L, 6L, r = 4L, wcrit = 3L,
  ...
  expect_gte(morie:::Medtestpow(8L, 6L, 4L, 5L, null_g,
                                 nodes = 401)$power, p$power)
  # and a zero-width one has none
  expect_equal(morie:::Medtestpow(8L, 6L, 4L, 0L, null_g,
                                   nodes = 401)$power, 0)
  # g is clamped to [0, 1], so an out-of-range alternative cannot
  # produce a negative probability
  expect_true(all(morie:::Medtestpow(6L, 5L, 3L, 2L,
                                      function(u) 2 * u - 0.5,
                                      nodes = 401)$pmf >= -1e-12))

  expect_error(morie:::Medtestpow(0L, 5L, 1L, 1L, null_g), "at least 1")
  expect_error(morie:::Medtestpow(5L, 5L, 6L, 1L, null_g), "1 <= r <= m")
  expect_error(morie:::Medtestpow(5L, 5L, 0L, 1L, null_g), "1 <= r <= m")
})
...
test_that("the exact critical region respects the stated level", {
  ...
  x <- stats::rnorm(10)
  y <- stats::rnorm(10, 1)
  r <- morie:::Medtest2(x, y, alpha = 0.05)
  expect_equal(r$statistic, morie:::Medtest(x, y)$statistic)
  expect_equal(r$p_value, morie:::Medtest(x, y)$p_value)
  ...
  loose <- morie:::Medtest2(x, y, alpha = 0.20)
  ...
  tiny <- morie:::Medtest2(x, y, alpha = 1e-8)
  expect_true(is.nan(tiny$lower) || tiny$alpha_exact <= 1e-8)
  expect_error(morie:::Medtest2(x, y, alpha = 0), "strictly inside")
  expect_error(morie:::Medtest2(x, y, alpha = 1), "strictly inside")
})
...
test_that("the three tie conventions bracket one another", {
  ...
  clean <- morie:::Medties(c(1, 2), c(3, 4))
  ...
  expect_error(morie:::Medties(numeric(0), 1), "non-empty")
})
