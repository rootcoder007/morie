...
test_that("trfbl deterministic_seed is reproducible", {
  skip_if_no_hash()
  x <- diag(4)
  r1 <- trfbl_transformer_block(x, num_heads = 2L, deterministic_seed = 42L)
  r2 <- trfbl_transformer_block(x, num_heads = 2L, deterministic_seed = 42L)
  r3 <- trfbl_transformer_block(x, num_heads = 2L, deterministic_seed = 999L)
...
test_that("trfbl default (deterministic_seed = NULL) path is unchanged", {
  x <- diag(4)
  r1 <- trfbl_transformer_block(x, num_heads = 2L, seed = 0L)
  r2 <- trfbl_transformer_block(x, num_heads = 2L, seed = 0L)
  expect_equal(r1$output, r2$output)
})
