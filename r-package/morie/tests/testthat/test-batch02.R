...
test_that("bayes_cpi_genomic returns a well-formed list", {
  set.seed(11)
...
test_that("bayes_cpi_genomic respects pi_init and is finite", {
  set.seed(5)
...
test_that("bkprp_backpropagation sigmoid path returns gradients", {
  set.seed(1)
...
test_that("bkprp_backpropagation supports all activations", {
  set.seed(2)
  x <- matrix(rnorm(6), 3, 2)
  y <- matrix(rnorm(6), 3, 2)
  for (act in c("identity", "linear", "none", "sigmoid", "tanh", "relu")) {
    res <- bkprp_backpropagation(x, y, activation = act)
    expect_true(is.finite(res$loss))
    expect_true(all(is.finite(res$dW)))
  }
})

test_that("bkprp_backpropagation uses default w and b", {
  set.seed(3)
  x <- matrix(rnorm(6), 3, 2)
  y <- matrix(rnorm(6), 3, 2)
  res <- bkprp_backpropagation(x, y)
  expect_true(is.finite(res$loss))
  expect_equal(dim(res$z), c(3, 2))
})

test_that("bkprp_backpropagation errors on unknown activation", {
  x <- matrix(rnorm(6), 3, 2)
  y <- matrix(rnorm(6), 3, 2)
  expect_error(bkprp_backpropagation(x, y, activation = "bogus"))
})

test_that("backpropagation alias is identical to bkprp_backpropagation", {
  expect_identical(backpropagation, bkprp_backpropagation)
})

test_that("bayesian_lasso_full returns a well-formed list", {
  set.seed(3)
...
test_that("bayesian_lasso_full accepts a fixed lambda", {
  set.seed(8)
...
test_that("bnfwd_batch_norm_forward normalizes per feature", {
  set.seed(1)
...
test_that("batch_norm_forward alias is identical", {
  expect_identical(batch_norm_forward, bnfwd_batch_norm_forward)
})

test_that("bayes_ridge_gibbs returns a well-formed list", {
  set.seed(4)
  X <- matrix(rnorm(100), 20, 5)
...
test_that("bayes_ridge_gibbs accepts custom df0 and S0", {
  set.seed(9)
  X <- matrix(rnorm(80), 20, 4)
...
test_that("brdgr with two matrices counts rows non-empty in both", {
  ...
  expect_equal(res$method, "bridge_observations")
})
...
test_that("bridge_observations alias is identical", {
  expect_identical(bridge_observations, brdgr)
})

test_that("bayesian_ridge_regression returns a well-formed list", {
  set.seed(2)
  X <- matrix(rnorm(100), 20, 5)
...
test_that("bayesian_ridge_regression accepts a fixed lambda", {
  set.seed(12)
  X <- matrix(rnorm(60), 15, 4)
  y <- as.numeric(X %*% c(0.5, 0, -0.5, 0) + 0.1 * rnorm(15))
  res <- bayesian_ridge_regression(X, y, lam = 5)
...
test_that("bysid handles degenerate single-row input", {
  ...
  expect_equal(res$method, "bayesian_ideal_points")
})
...
test_that("bayesian_ideal_points alias is identical", {
  expect_identical(bayesian_ideal_points, bysid)
})

test_that("estimate_propensity_scores returns clipped scores", {
  set.seed(1)
...
test_that("estimate_ate returns ATE with CI", {
  set.seed(1)
  df <- data.frame(t = rbinom(200, 1, 0.4), y = rnorm(200), x = rnorm(200))
  res <- estimate_ate(df, "t", "y", "x")
...
test_that("estimate_ate accepts a pre-computed propensity column", {
  set.seed(2)
  df <- data.frame(t = rbinom(120, 1, 0.5), y = rnorm(120), x = rnorm(120))
  df$ps <- estimate_propensity_scores(df, "t", "x")
  res <- estimate_ate(df, "t", "y", "x", propensity_col = "ps")
  expect_true(is.finite(res$ate))
})

test_that("estimate_att returns ATT with CI", {
  set.seed(2)
  df <- data.frame(t = rbinom(200, 1, 0.4), y = rnorm(200), x = rnorm(200))
  res <- estimate_att(df, "t", "y", "x")
  expect_named(res, c("att", "se", "ci_lower", "ci_upper", "n_treated"))
  expect_true(is.finite(res$att))
  expect_true(res$n_treated > 0)
})

test_that("estimate_atc returns ATC with CI", {
  set.seed(3)
  df <- data.frame(t = rbinom(200, 1, 0.4), y = rnorm(200), x = rnorm(200))
  res <- estimate_atc(df, "t", "y", "x")
  expect_named(res, c("atc", "se", "ci_lower", "ci_upper", "n_control"))
  expect_true(is.finite(res$atc))
  expect_true(res$n_control > 0)
})

test_that("estimate_aipw returns doubly-robust ATE (linear)", {
  set.seed(4)
  df <- data.frame(t = rbinom(200, 1, 0.4), y = rnorm(200), x = rnorm(200))
  res <- estimate_aipw(df, "t", "y", "x", outcome_model = "linear")
...
test_that("estimate_cate t-learner returns per-unit effects", {
  set.seed(6)
...
test_that("estimate_cate s-learner returns per-unit effects", {
  set.seed(7)
  df <- data.frame(t = rbinom(200, 1, 0.5), y = rnorm(200), x = rnorm(200))
...
test_that("estimate_late returns Wald estimate without covariates", {
  set.seed(8)
  n <- 300
  z <- rbinom(n, 1, 0.5)
...
test_that("estimate_late with covariates runs (ivreg or fallback)", {
  set.seed(9)
  n <- 300
  z <- rbinom(n, 1, 0.5)
  x <- rnorm(n)
  ...
  res <- estimate_late(df, "t", "y", "z", covariates = "x")
...
test_that("sensitivity_rosenbaum returns a gamma grid data frame", {
  set.seed(10)
...
test_that("estimate_g_computation returns outcome-regression ATE", {
  set.seed(11)
  df <- data.frame(t = rbinom(200, 1, 0.4), y = rnorm(200), x = rnorm(200))
...
test_that("concordance_incomplete handles degenerate small input", {
  res <- concordance_incomplete(matrix(1, 1, 1))
...
test_that("condorcet_winner alias is identical", {
  expect_identical(condorcet_winner, cndrc)
})

test_that("cnn1d_conv1d_forward computes valid cross-correlation", {
  x <- c(1, 2, 3, 4, 5)
  w <- c(1, 0, -1)
  res <- cnn1d_conv1d_forward(x, w, b = 0)
...
test_that("cnn1d_conv1d_forward respects stride and padding", {
  x <- c(1, 2, 3, 4, 5, 6)
  w <- c(0.5, 0.5)
  res <- cnn1d_conv1d_forward(x, w, b = 1, stride = 2L, padding = 1L)
  expect_true(res$output_length >= 1)
  expect_length(res$y, res$output_length)
})

test_that("cnn1d_conv1d_forward errors when input shorter than kernel", {
  expect_error(cnn1d_conv1d_forward(c(1, 2), c(1, 2, 3)))
})

test_that("conv1d_forward alias is identical", {
  expect_identical(conv1d_forward, cnn1d_conv1d_forward)
})

test_that("cnn2d_conv2d_forward computes valid 2D cross-correlation", {
  x <- matrix(1:16, 4, 4)
  w <- matrix(c(1, 0, 0, -1), 2, 2)
  res <- cnn2d_conv2d_forward(x, w, b = 0)
...
test_that("cnn2d_conv2d_forward respects stride and padding", {
  x <- matrix(rnorm(36), 6, 6)
  w <- matrix(rnorm(9), 3, 3)
  res <- cnn2d_conv2d_forward(x, w, b = 0.5, stride = 2L, padding = 1L)
...
test_that("conv2d_forward alias is identical", {
  expect_identical(conv2d_forward, cnn2d_conv2d_forward)
})
