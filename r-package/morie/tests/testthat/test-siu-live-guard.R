...
test_that("a live index sweep is refused under R CMD check only", {
  allowed <- morie:::.siu_live_fetch_allowed
  ...
  withr::with_envvar(
    c("_R_CHECK_PACKAGE_NAME_" = "morie", RMORIE_NETWORK_TESTS = NA),
    withr::with_options(list(morie.siu.allow_fetch = NULL), {
  ...
  withr::with_envvar(
    c("_R_CHECK_PACKAGE_NAME_" = "morie", RMORIE_NETWORK_TESTS = "1"),
    withr::with_options(list(morie.siu.allow_fetch = NULL), {
      expect_true(allowed())
    })
  )
...
test_that("resolving an absent case number costs nothing under check", {
  ...
  withr::with_envvar(
    ...
    withr::with_options(list(morie.siu.allow_fetch = NULL), {
      ...
      got <- morie:::.siu_resolve_drid("99-XXX-999")
