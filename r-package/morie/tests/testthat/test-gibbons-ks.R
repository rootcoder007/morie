...
test_that("the KS statistic matches stats::ks.test", {
  ...
  for (i in 1:6) {
    x <- stats::runif(25)
    got <- morie:::Ksdistfree(x, stats::punif)
  ...
  x <- c(0.1, 0.4, 0.7)
  k <- morie:::Ksdistfree(x, stats::punif)
  ...
  expect_equal(morie:::Ksdistfree(c(0.5), stats::punif)$statistic, 0.5)
  # against a different hypothesised distribution
  nx <- stats::qnorm(c(0.25, 0.5, 0.75))
  expect_equal(morie:::Ksdistfree(nx, stats::pnorm)$statistic,
               as.numeric(suppressWarnings(
                 stats::ks.test(nx, "pnorm")$statistic)))
  expect_error(morie:::Ksdistfree(numeric(0), stats::punif), "non-empty")
})
...
test_that("the exact distribution of D agrees with ks.test's exact tail", {
  ...
  for (n in c(5L, 12L, 25L)) {
    x <- stats::runif(n)
    d <- morie:::Ksdistfree(x, stats::punif)$statistic
    got <- morie:::Ksexact(d, n)
  ...
  # the degenerate ends
  expect_equal(morie:::Ksexact(0, 10L)$cdf, 0)
  expect_equal(morie:::Ksexact(-1, 10L)$cdf, 0)
  expect_equal(morie:::Ksexact(1, 10L)$cdf, 1)
  expect_equal(morie:::Ksexact(2, 10L)$cdf, 1)
  # it is a distribution function: non-decreasing, inside [0, 1]
  ds <- seq(0.05, 0.95, by = 0.05)
  cdfs <- vapply(ds, function(d) morie:::Ksexact(d, 15L)$cdf, 0)
  expect_true(all(diff(cdfs) >= -1e-12))
  expect_true(all(cdfs >= 0 & cdfs <= 1))
  expect_equal(cdfs, 1 - vapply(ds, function(d) morie:::Ksexact(d, 15L)$sf, 0))
  # for n = 1 the distribution is exact and elementary: D_1 is uniform on
  # (1/2, 1), so P(D_1 <= d) = 2d - 1
  expect_equal(morie:::Ksexact(0.75, 1L)$cdf, 0.5, tolerance = 1e-9)
  expect_error(morie:::Ksexact(0.5, 0L), "at least 1")
})
...
test_that("the one-sided tail is the Birnbaum-Tingey formula", {
  ...
  set.seed(33)
  x <- stats::runif(15)
  dp <- morie:::Ksdistfree(x, stats::punif)$dplus
  expect_equal(morie:::Ksplusdist(dp, 15L)$sf,
  ...
  sfs <- vapply(seq(0.05, 0.9, by = 0.05),
                function(c) morie:::Ksplusdist(c, 20L)$sf, 0)
  expect_true(all(diff(sfs) < 0))
  # the degenerate ends
  expect_equal(morie:::Ksplusdist(0, 10L)$sf, 1)
  expect_equal(morie:::Ksplusdist(1, 10L)$sf, 0)
  # the one-sided tail is no larger than the two-sided one
  expect_lte(morie:::Ksplusdist(0.3, 20L)$sf,
             morie:::Ksexact(0.3, 20L)$sf + 1e-12)
  expect_error(morie:::Ksplusdist(0.5, 0L), "at least 1")
})
...
test_that("the confidence band is the EDF displaced by the critical value", {
  ...
  at <- c(0, 0.5, 1)
  a <- morie:::Ksband(x, 0.2, at = at)
  expect_equal(a$at, at)
  expect_equal(a$edf, c(0, 0.5, 1))
  # a wider critical value is a wider band
  expect_true(all(morie:::Ksband(x, 0.4)$width >= b$width))
  expect_error(morie:::Ksband(x, 0), "strictly inside")
  expect_error(morie:::Ksband(x, 1), "strictly inside")
  expect_error(morie:::Ksband(numeric(0), 0.2), "non-empty")
})

test_that("the sample size attains the requested uniform accuracy", {
  skip_heavy()
  for (c in c(0.2, 0.3)) {
    s <- morie:::Ksn(c, alpha = 0.05)
    # the asymptotic figure is (k_alpha / c)^2
    expect_equal(s$n_asymp, as.integer(ceiling((s$k_alpha / c)^2)))
    expect_equal(morie:::.gbKsQ(s$k_alpha), 0.05, tolerance = 1e-6)
    # the returned n really does attain the coverage, which is the
    # property the search is for
    expect_gte(s$coverage, 0.95)
    expect_equal(s$coverage, morie:::Ksexact(c, s$n)$cdf)
    # and one fewer does not
    expect_lt(morie:::Ksexact(c, s$n - 1L)$cdf, 0.95)
  }
  # a tighter accuracy or a smaller alpha needs more observations
  expect_gt(morie:::Ksn(0.1)$n, morie:::Ksn(0.3)$n)
  expect_gte(morie:::Ksn(0.2, alpha = 0.01)$n, morie:::Ksn(0.2, 0.05)$n)
  expect_error(morie:::Ksn(0), "strictly inside")
  expect_error(morie:::Ksn(0.2, alpha = 1), "strictly inside")
})
...
test_that("the Cramer-von Mises criterion is its closed form", {
  ...
  perfect <- (2 * j - 1) / 40
  expect_equal(morie:::Cvmw2(perfect, stats::punif)$statistic, 1 / 240)
  # a badly-fitting sample scores higher than a well-fitting one
  expect_gt(morie:::Cvmw2(stats::runif(20, 0.8, 1), stats::punif)$statistic,
            w$statistic)

  # the comparison helper reports both criteria on one pass
  cmp <- morie:::Kscvmcmp(x, stats::punif)
  expect_equal(cmp$d, morie:::Ksdistfree(x, stats::punif)$statistic)
  ...
  expect_error(morie:::Cvmw2(numeric(0), stats::punif), "non-empty")
  expect_error(morie:::Kscvmcmp(1, stats::punif), "at least 2")
})
...
test_that("the Lilliefors tests use their published tables", {
  ...
  set.seed(36)
  x <- stats::rnorm(20)
  l <- morie:::Lillienorm(x, alpha = 0.05)
  ...
  expect_equal(morie:::Lillienorm(stats::rnorm(30), 0.05)$reject, 0L)
  expect_equal(morie:::Lillienorm(stats::rexp(30, 1), 0.05)$reject, 1L)
  ...
  cs <- vapply(c(0.100, 0.050, 0.010, 0.001),
               function(a) morie:::Lillienorm(x, a)$dcrit, 0)
  expect_equal(cs, c(0.176, 0.192, 0.223, 0.266))
  expect_true(all(diff(cs) > 0))
  # beyond the table the asymptotic tail divisor is used
  big <- morie:::Lillienorm(stats::rnorm(200), 0.05)
  expect_equal(big$n_table, 0L)
  expect_equal(big$dcrit, 0.888 / sqrt(200))
  # the table is entered at the largest row not exceeding n
  expect_equal(morie:::Lillienorm(stats::rnorm(22), 0.05)$n_table, 20L)
  expect_equal(morie:::Lillienorm(stats::rnorm(25), 0.05)$n_table, 25L)

  expect_error(morie:::Lillienorm(c(1, 2, 3)), "at least 4")
  expect_error(morie:::Lillienorm(rep(2, 10)), "zero variance")
  expect_error(morie:::Lillienorm(x, alpha = 0.025),
               "one of 0\\.10, 0\\.05, 0\\.01, 0\\.001")

  # the exponential counterpart, against its own table
  e <- stats::rexp(20, rate = 2)
  le <- morie:::Lillieexp(e, alpha = 0.05)
  expect_equal(le$n_table, 20L)
  expect_equal(le$dcrit, 0.234)
  expect_equal(morie:::Lillieexp(stats::rexp(40, 3), 0.05)$reject, 0L)
  # a uniform sample is not exponential
  expect_equal(morie:::Lillieexp(stats::runif(40, 1, 2), 0.05)$reject, 1L)
})
