...
test_that("the zip reader streams the CSV member back unchanged", {
  ...
  got <- morie:::.morie_read_usgs_soil_zip(zip_path)
  expect_equal(got, df)
  ...
  expect_equal(morie:::.morie_read_usgs_soil_zip(zip_path, nrows = 2), df[1:2, ])
  expect_equal(nrow(morie:::.morie_read_usgs_soil_zip(zip_path, nrows = 1)), 1L)
  # NULL means every row
  expect_equal(nrow(morie:::.morie_read_usgs_soil_zip(zip_path, nrows = NULL)),
               3L)
  ...
  expect_error(morie:::.morie_read_usgs_soil_zip(nocsv), "no CSV member")
})
...
test_that("the API base honours the environment override", {
  ...
  expect_equal(morie:::.morie_morphosource_api(),
...
test_that("the key comes from the argument or the environment, never the URL", {
  ...
  expect_equal(morie:::.morie_morphosource_key("tok-arg"), "tok-arg")
  # otherwise the caller's environment is read
  Sys.setenv(MORPHOSOURCE_API_KEY = "tok-env")
  expect_equal(morie:::.morie_morphosource_key(), "tok-env")
  expect_equal(morie:::.morie_morphosource_key("tok-arg"), "tok-arg")
  ...
  Sys.unsetenv("MORPHOSOURCE_API_KEY")
  expect_null(morie:::.morie_morphosource_key(required = FALSE))
  expect_null(morie:::.morie_morphosource_key("", required = FALSE))
  expect_error(morie:::.morie_morphosource_key(), "API key required")
  # the refusal says where to put the key and does not echo one
  expect_error(morie:::.morie_morphosource_key(),
               "MORPHOSOURCE_API_KEY", fixed = TRUE)
})

test_that("search params follow the documented query contract", {
  f <- morie:::.morie_morphosource_search_params
