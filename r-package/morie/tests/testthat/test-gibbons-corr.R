...
test_that("the exact null distribution of S is the inversion count", {
  ...
  expect_equal(morie:::Taunull(5L, s = 10L)$pmf_s, 1 / factorial(5))
  # S must lie on the support's lattice
  expect_error(morie:::Taunull(5L, s = 1L), "outside the support")
  expect_error(morie:::Taunull(5L, s = 12L), "outside the support")
  expect_error(morie:::Taunull(1L), "at least 2")
})
...
test_that("the tau trend test matches Kendall's tau against the index", {
  ...
  tied <- morie:::Tautrend(c(1, 1, 2, 2, 3))
  expect_lt(tied$P + tied$Q, 5 * 4 / 2)

  expect_error(morie:::Tautrend(c(1, 2)), "at least 3")
  expect_error(morie:::Tautrend(1:5, "sideways"),
...
test_that("Spearman's rho matches stats::cor and its shortcut", {
  ...
  expect_equal(morie:::Spearrho(1:10, 1:10)$statistic, 1)
  expect_equal(morie:::Spearrho(1:10, 10:1)$statistic, -1)
  expect_equal(morie:::Spearrho(1:10, 1:10)$sumd2, 0)
  # monotone but non-linear is still 1, which is the point of a rank
  # correlation
  expect_equal(morie:::Spearrho(1:10, exp(1:10))$statistic, 1)

  # WITH ties the shortcut form is no longer exact, and the module
  # reports the tie flag so a caller knows which to trust
  tx <- c(1, 2, 2, 3, 4, 5, 5)
  ty <- c(2, 1, 3, 3, 5, 4, 6)
  rt <- morie:::Spearrho(tx, ty)
  ...
  expect_error(morie:::Spearrho(1:5, 1:4), "same length")
  expect_error(morie:::Spearrho(1:2, 1:2), "at least 3")
})
    ...
    for (r in c(-0.6, 0, 0.35, 0.9)) {
      ...
      expect_equal(h$p_value,
                   min(1, 2 * stats::pt(abs(h$t), n - 2,
                                        lower.tail = FALSE)))
      expect_equal(morie:::Rhotest(r, n, "greater")$p_value,
                   stats::pt(h$t, n - 2, lower.tail = FALSE))
      expect_equal(morie:::Rhotest(r, n, "less")$p_value,
                   stats::pt(h$t, n - 2))
    }
  }
  # zero correlation is no evidence at all
  expect_equal(morie:::Rhotest(0, 20L)$p_value, 1)
  expect_equal(morie:::Rhotest(0, 20L)$z, 0)
  # a perfect correlation sends the t statistic to infinity rather than
  # dividing by zero
  expect_equal(morie:::Rhotest(1, 20L)$t, Inf)
  expect_equal(morie:::Rhotest(-1, 20L)$t, -Inf)
  expect_equal(morie:::Rhotest(1, 20L)$p_value, 0)
  # the t test agrees with cor.test's own on the ranks
  set.seed(63)
  x <- stats::rnorm(25)
  y <- x + stats::rnorm(25)
  rr <- morie:::Spearrho(x, y)$statistic
  expect_equal(morie:::Rhotest(rr, 25L)$p_value,
               stats::cor.test(rank(x), rank(y))$p.value, tolerance = 1e-8)
  expect_error(morie:::Rhotest(0.5, 2L), "at least 3")
  expect_error(morie:::Rhotest(1.5, 10L), "\\[-1, 1\\]")
  expect_error(morie:::Rhotest(0.5, 10L, "sideways"),
               "two-sided, greater or less")
})
...
test_that("the normal-scores correlation uses the expected order statistics", {
  ...
  nc <- morie:::Normcorr(x, y, nodes = 801)
  expect_equal(nc$n, n)
  ...
  perfect <- morie:::Normcorr(1:10, 1:10, nodes = 801)
  expect_equal(perfect$statistic, 1, tolerance = 1e-6)
  expect_lt(perfect$p_value, 0.01)
  # and perfectly reversed data the minimum
  expect_equal(morie:::Normcorr(1:10, 10:1, nodes = 801)$statistic, -1,
               tolerance = 1e-6)
  # a non-zero null shifts the reference point
  expect_lt(morie:::Normcorr(x, y, rho = 0.9, nodes = 801)$mean_zf,
            atanh(0.9))
  expect_error(morie:::Normcorr(1:5, 1:4), "same length")
  expect_error(morie:::Normcorr(1:3, 1:3), "at least 4")
})
...
test_that("the partial tau is built from the concordance table", {
  ...
  tied <- morie:::Taupartial(c(1, 1, 2, 3), c(1, 2, 3, 4), c(1, 2, 3, 4))
  ...
  deg <- morie:::Taupartial(1:5, 1:5, 1:5)
  expect_true(is.nan(deg$statistic))
  expect_equal(deg$x11, 10L)

  expect_error(morie:::Taupartial(1:5, 1:4, 1:5), "same length")
  expect_error(morie:::Taupartial(1:2, 1:2, 1:2), "at least 3")
})
