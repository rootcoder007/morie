...
test_that("H matches stats::kruskal.test, ties and all", {
  ...
  expect_equal(morie:::Kwh(tied)$statistic,
  ...
  expect_equal(morie:::Kwh(tied, correct = FALSE)$statistic, kt$h_raw)
  expect_equal(morie:::Kwh(tied, correct = FALSE)$correction, 1)
  # untied data needs no correction
  untied <- list(c(1, 2, 3), c(4, 5, 6))
  expect_equal(morie:::Kwh(untied)$correction, 1)
  expect_equal(morie:::Kwh(untied)$statistic, morie:::Kwh(untied)$h_raw)
  ...
  sep <- morie:::Kwh(list(c(1, 2, 3), c(10, 11, 12)))
  ...
  expect_error(morie:::Kwh(list(1:3)), "at least 2 samples")
  expect_error(morie:::Kwh(list(1:3, numeric(0))), "must be non-empty")
})
...
test_that("the two forms of H are algebraically the same", {
  ...
  for (ns in list(c(3L, 4L, 5L), c(6L, 6L), c(2L, 3L, 4L, 5L))) {
    ...
    expect_equal(a$statistic,
                 morie:::Kwh(split(v, grp), correct = FALSE)$h_raw)
  }
  ...
  expect_equal(morie:::Kwalt(c(5, 5), c(2L, 2L))$statistic, 0)
  expect_equal(morie:::Kwalt(c(5, 5), c(2L, 2L))$h_computing, 0)
  # and three equal groups likewise
  expect_equal(morie:::Kwalt(c(15, 15, 15), c(3L, 3L, 3L))$statistic, 0)
  expect_error(morie:::Kwalt(c(1), c(1L)), "at least 2 samples")
  expect_error(morie:::Kwalt(c(1, 2), c(1L)), "matching sizes")
  expect_error(morie:::Kwalt(c(1, 2), c(0L, 1L)), "at least 1")
})
...
test_that("the chi-square approximation flags the small-sample case", {
  ...
  expect_equal(morie:::Kwchi(5, 3L, ns = c(3L, 4L, 5L))$table_k, 1L)
  expect_equal(morie:::Kwchi(5, 3L, ns = c(3L, 4L, 9L))$table_k, 0L)
  expect_equal(morie:::Kwchi(5, 4L, ns = c(3L, 3L, 3L, 3L))$table_k, 0L)
  expect_equal(morie:::Kwchi(5, 3L)$table_k, 0L)
  # a larger statistic is more significant
  expect_lt(morie:::Kwchi(15, 3L)$p_value, morie:::Kwchi(2, 3L)$p_value)
  expect_error(morie:::Kwchi(1, 1L), "at least 2")
})

test_that("multiple comparisons use the Bonferroni-adjusted normal point", {
  rm <- c(5, 10, 18)
  ns <- c(4L, 4L, 4L)
  mc <- morie:::Kwmc(rm, ns, alpha = 0.20)
  nn <- 12
  ...
  expect_lte(length(morie:::Kwmc(rm, ns, alpha = 0.001)$significant),
             length(mc$significant))
  # unequal sizes have no single bound to report
  expect_true(is.nan(morie:::Kwmc(rm, c(3L, 4L, 5L))$bound))
  expect_false(is.nan(morie:::Kwmc(rm, c(3L, 4L, 5L))$bounds[1, 2]))
  # identical rank means differ nowhere
  expect_length(morie:::Kwmc(c(5, 5, 5), ns)$significant, 0L)

  expect_error(morie:::Kwmc(c(1), c(1L)), "at least 2 samples")
  expect_error(morie:::Kwmc(c(1, 2), c(1L)), "matching sizes")
  expect_error(morie:::Kwmc(rm, ns, alpha = 0), "strictly inside")
})
...
test_that("Jonckheere-Terpstra counts the concordant pairs", {
  ...
  expect_equal(morie:::Jtstat(list(c(5, 6), c(3, 4), c(1, 2)))$statistic, 0)
  # the null moments
  ns <- c(2, 2, 2)
  nn <- 6
  ...
  expect_gt(morie:::Jtstat(list(c(5, 6), c(3, 4), c(1, 2)))$p_value, 0.9)
  # the alternatives
  expect_equal(morie:::Jtstat(g, "less")$p_value, stats::pnorm(j$z))
  expect_equal(morie:::Jtstat(g, "two-sided")$p_value,
               min(1, 2 * (1 - stats::pnorm(abs(j$z)))))

  # exact ties count a half each, which keeps B symmetric under
  # reversal about its maximum
  t1 <- morie:::Jtstat(list(c(1, 1), c(1, 1)))$statistic
  expect_equal(t1, 2)
  ...
  expect_equal(morie:::Jtstat(gg)$statistic, b)
  expect_equal(morie:::Jtstat(gg)$k, 3L)
  expect_equal(morie:::Jtstat(gg)$n, 15L)

  expect_error(morie:::Jtstat(list(1:3)), "at least 2 samples")
  expect_error(morie:::Jtstat(list(1:3, numeric(0))), "non-empty")
  expect_error(morie:::Jtstat(g, "sideways"),
...
test_that("the JT moments and the pairwise matrix agree with the statistic", {
  ...
  s <- morie:::Jtsum(g)
  expect_equal(s$statistic, morie:::Jtstat(g)$statistic)
  ...
  expect_error(morie:::Jtmom(c(3L)), "at least 2 samples")
  expect_error(morie:::Jtmom(c(0L, 2L)), "at least 1")
  expect_error(morie:::Jtsum(list(1:3)), "at least 2 samples")
})
