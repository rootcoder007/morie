...
test_that("Friedman's Q matches stats::friedman.test", {
  ...
  gt <- morie:::Friedq(split(tied, row(tied)))
  ...
  expect_equal(morie:::Friedq(split(tied, row(tied)),
  ...
  same <- morie:::Friedq(list(c(1, 2, 3), c(1, 2, 3), c(1, 2, 3)))
  ...
  expect_error(morie:::Friedq(list(c(1, 2))), "at least 2 blocks")
  expect_error(morie:::Friedq(list(1, 2)), "at least 2 treatments")
  expect_error(morie:::Friedq(list(c(1, 2, 3), c(1, 2))),
               "n observations")
})

test_that("the tie correction is reported in its own right", {
  tied <- list(c(1, 1, 2), c(2, 2, 1), c(1, 2, 2))
  t <- morie:::Friedties(tied)
  fr <- morie:::Friedq(tied)
  ...
  expect_equal(morie:::Friedties(clean)$tiesum, 0)
  expect_equal(morie:::Friedties(clean)$factor, 1)
  expect_equal(morie:::Friedties(clean)$statistic,
               morie:::Friedties(clean)$q_raw)
  expect_error(morie:::Friedties(list(c(1, 2))), "at least 2 blocks")
})

test_that("the chi-square approximation reports its own deficiency", {
  for (k in c(3L, 10L, 40L)) {
    for (n in c(3L, 5L)) {
      c1 <- morie:::Friedchi(6, k, n)
  ...
  expect_gt(morie:::Friedchi(6, 100L, 4L)$ratio,
            morie:::Friedchi(6, 3L, 4L)$ratio)
  expect_equal(morie:::Friedchi(6, 1000L, 4L)$ratio, 0.999)

  # the moments in their own right
  for (k in c(4L, 12L)) {
    for (n in c(3L, 6L)) {
      v <- morie:::Friedvar(k, n)
  ...
  expect_error(morie:::Friedchi(1, 1L, 3L), "k >= 2")
  expect_error(morie:::Friedvar(3L, 1L), "n >= 2")
})

test_that("Friedman multiple comparisons use one common bound", {
  rs <- c(6, 12, 24)
  mc <- morie:::Friedmc(rs, k = 8L, alpha = 0.20)
  ...
  strict <- morie:::Friedmc(rs, 8L, alpha = 0.001)
  ...
  expect_length(morie:::Friedmc(c(10, 10, 10), 8L)$significant, 0L)
  # more blocks means a wider bound on the rank-sum scale
  expect_gt(morie:::Friedmc(rs, 32L)$bound, mc$bound)
  expect_error(morie:::Friedmc(c(1), 8L), "at least 2 treatments")
  expect_error(morie:::Friedmc(rs, 1L), "at least 2 blocks")
  expect_error(morie:::Friedmc(rs, 8L, alpha = 1), "strictly inside")
})
...
test_that("Page's L weights the rank sums by treatment order", {
  ...
  expect_equal(morie:::Pagel(rev_b)$statistic, 1 * 9 + 2 * 6 + 3 * 3)
  expect_lt(morie:::Pagel(rev_b)$statistic, p$statistic)
  # an increasing trend is evidence for the ordered alternative
  expect_lt(p$p_value, 0.05)
  expect_gt(morie:::Pagel(rev_b)$p_value, 0.95)
  # the standardisation
  ...
  expect_equal(morie:::Pagel(b, weights = c(1, 1, 1))$statistic,
               sum(p$rank_sums))
  expect_error(morie:::Pagel(b, weights = c(1, 2)), "length n")
  expect_error(morie:::Pagel(list(c(1, 2))), "at least 2 blocks")
})
...
test_that("Page's exact distribution matches an enumeration", {
  ...
  expect_equal(e3$mean, morie:::Pageasymp(0, 3L, 3L)$mean, tolerance = 1e-9)
  expect_equal(e3$var, morie:::Pageasymp(0, 3L, 3L)$var, tolerance = 1e-9)
  # the point and tail probabilities
  top <- max(e3$support)
  q <- morie:::Pageexact(3L, 3L, ell = top)
  ...
  expect_equal(morie:::Pageexact(3L, 3L, ell = top + 10)$sf_l, 0)
  expect_equal(morie:::Pageexact(3L, 3L, ell = 0)$sf_l, 1)

  expect_error(morie:::Pageexact(0L, 3L), "at least 1")
  expect_error(morie:::Pageexact(2L, 9L), "2\\.\\.8")
  expect_error(morie:::Pageexact(2L, 1L), "2\\.\\.8")
})

test_that("Page's normal approximation tracks the exact tail", {
  for (k in c(4L, 10L)) {
    for (n in c(3L, 4L)) {
      a <- morie:::Pageasymp(100, k, n)
      ...
      expect_lt(a$z, morie:::Pageasymp(100, k, n, correct = FALSE)$z)
    }
  }
  ...
  n <- 4L
  mu <- k * n * (n + 1)^2 / 4
  expect_equal(morie:::Pageasymp(mu, k, n, correct = FALSE)$z, 0)
  expect_equal(morie:::Pageasymp(mu, k, n, correct = FALSE)$p_value, 0.5)
  # and the approximation is close to the exact tail for a moderate k
  e <- morie:::Pageexact(6L, 4L)
  target <- e$support[which(cumsum(rev(e$pmf)) > 0.05)[1]]
  target <- rev(e$support)[which(cumsum(rev(e$pmf)) > 0.05)[1]]
  expect_equal(morie:::Pageasymp(target, 6L, 4L)$p_value, 0.05,
               tolerance = 0.4)
  expect_error(morie:::Pageasymp(1, 0L, 3L), "at least 1")
  expect_error(morie:::Pageasymp(1, 2L, 1L), "at least 2")
})
