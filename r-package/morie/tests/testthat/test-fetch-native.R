...
test_that("native XML tree: attributes, nesting, entities, CDATA", {
  ...
  items <- morie:::.morie_xml_find_all(root, "item")
...
test_that("native HTML tolerates void elements + unclosed tags", {
  ...
  body <- morie:::.morie_xml_find_all(root, "body")[[1]]
  ps <- morie:::.morie_xml_find_all(root, "p")
  expect_length(ps, 2)
  expect_equal(ps[[1]]$text, "first")
  lis <- morie:::.morie_xml_find_all(root, "li")
  expect_length(lis, 2)
})
