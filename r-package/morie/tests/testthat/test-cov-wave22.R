...
test_that("morie_fetch zip member matches by substring and errors if absent", {
  ...
  owd <- getwd(); setwd(dirname(csv)); on.exit(setwd(owd), add = TRUE)
  ...
  expect_error(morie_fetch(paste0("file://", zp), format = "zip",
                           zip_member = "no-such-member"), "not found")
})
