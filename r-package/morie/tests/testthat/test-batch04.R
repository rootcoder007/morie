...
test_that("infer_measurement_level classifies logical and 0/1 as binary", {
  expect_equal(infer_measurement_level(c(TRUE, FALSE, TRUE)), "binary")
  expect_equal(infer_measurement_level(c(0, 1, 1, 0)), "binary")
  expect_equal(infer_measurement_level(c("0", "1", "1")), "binary")
})

test_that("infer_measurement_level classifies factors", {
  expect_equal(infer_measurement_level(factor(c("a", "b", "c"))), "nominal")
  expect_equal(infer_measurement_level(factor(c("a", "b", "a"))), "binary")
  expect_equal(
    infer_measurement_level(ordered(c("low", "med", "high"))),
    "ordinal"
  )
})

test_that("infer_measurement_level classifies numeric ratio vs interval", {
  expect_equal(infer_measurement_level(c(1.2, 3.4, 5.6, 7.8)), "ratio")
  expect_equal(infer_measurement_level(c(-1.5, 0.0, 2.3, 4.1)), "interval")
  expect_equal(infer_measurement_level(c(2, 5, 2, 5)), "binary")
})

test_that("infer_measurement_level returns a single valid string", {
  lvl <- infer_measurement_level(rnorm(50))
  expect_type(lvl, "character")
  expect_length(lvl, 1L)
  expect_true(lvl %in% c("binary", "nominal", "ordinal", "interval", "ratio"))
})

test_that("profile_dataset returns expected structure", {
...
test_that("profile_dataset numeric columns carry summary stats", {
  set.seed(1)
  df <- data.frame(
    a = rnorm(40),
    b = rbinom(40, 1, 0.5),
...
test_that("profile_dataset counts missing values", {
  df <- data.frame(x = c(1, NA, 3, NA, 5))
  p <- profile_dataset(df)
  expect_equal(p$columns$x$n_missing, 2L)
})

test_that("profile_dataset errors on non-data.frame input", {
  expect_error(profile_dataset(1:10))
  expect_error(profile_dataset("not a frame"))
})

test_that("suggest_analysis_plan returns character recommendations", {
  s <- suggest_analysis_plan(profile_dataset(iris))
  expect_type(s, "character")
  expect_gte(length(s), 1L)
})

test_that("suggest_analysis_plan triggers binary+numeric suggestion", {
  set.seed(2)
  df <- data.frame(
...
test_that("suggest_analysis_plan flags missing values and ordinal vars", {
  set.seed(3)
  df <- data.frame(
    num = c(rnorm(29), NA),
...
test_that("suggest_analysis_plan errors on bad profile input", {
  expect_error(suggest_analysis_plan(list(foo = 1)))
  expect_error(suggest_analysis_plan(42))
})

test_that("dbscan_clustering returns expected structure", {
...
test_that("dbscan_clustering handles a vector input", {
  skip_if_not_installed("dbscan")
  set.seed(11)
  v <- c(rnorm(40, 0, 0.2), rnorm(40, 10, 0.2))
  res <- dbscan_clustering(v, eps = 0.7, min_samples = 3L)
  expect_equal(res$n, length(v))
  expect_type(res$labels, "integer")
})

test_that("dcc_multivariate_garch returns expected structure", {
  set.seed(20)
...
test_that("dcc_multivariate_garch errors when too small", {
  expect_error(dcc_multivariate_garch(matrix(rnorm(20), ncol = 2)))
  expect_error(dcc_multivariate_garch(matrix(rnorm(100), ncol = 1)))
})
...
test_that("diffusion_forward alias equals diffu_heat_diffusion", {
  T0 <- c(0, 25, 50, 25, 0)
  expect_identical(
    diffusion_forward(T0, n_steps = 10L)$T_final,
    diffu_heat_diffusion(T0, n_steps = 10L)$T_final
  )
})

test_that("diffu_diffusion_forward returns expected structure", {
  set.seed(21)
  x0 <- rnorm(8)
...
test_that("diffu_diffusion_forward accepts supplied noise and rejects bad t", {
  x0 <- rep(1, 5)
  noise <- rep(0, 5)
  res <- diffu_diffusion_forward(x0, t = 1L, noise = noise, num_steps = 10L)
  expect_true(all(is.finite(res$x_t)))
  expect_error(diffu_diffusion_forward(x0, t = 0L, num_steps = 10L))
  expect_error(diffu_diffusion_forward(x0, t = 999L, num_steps = 10L))
})
...
test_that("dimrd alias and single-column degenerate path", {
  ...
  expect_equal(dimensionality_test(x)$n_dims, dimrd(x)$n_dims)
  res1 <- dimrd(matrix(rnorm(20), ncol = 1))
  expect_equal(res1$n_dims, 0L)
})

test_that("deep_learning_genomic returns expected structure", {
  set.seed(6)
...
test_that("deep_learning_genomic loss generally decreases", {
  set.seed(7)
...
test_that("drpfw_dropout_forward returns expected structure in training", {
  set.seed(40)
...
test_that("drpfw_dropout_forward passes through when not training or p=0", {
  x <- array(1:12, dim = c(3, 4))
  res_eval <- drpfw_dropout_forward(x, p = 0.5, training = FALSE)
  expect_equal(res_eval$y, as.array(x))
  expect_equal(res_eval$kept_fraction, 1.0)
  res_p0 <- drpfw_dropout_forward(x, p = 0, training = TRUE)
  expect_equal(res_p0$y, as.array(x))
})

test_that("drpfw_dropout_forward rejects out-of-range p; alias works", {
  x <- array(rnorm(8), dim = c(2, 4))
  expect_error(drpfw_dropout_forward(x, p = -0.1))
  expect_error(drpfw_dropout_forward(x, p = 1))
  res <- dropout_forward(x, p = 0.5, seed = 2L)
  expect_equal(dim(res$y), dim(x))
})

test_that("decision_tree_split returns expected structure", {
...
test_that("decision_tree_split supports entropy criterion and vector x", {
  skip_if_not_installed("rpart")
  set.seed(51)
  v <- rnorm(60)
  y <- factor(ifelse(v > 0, "a", "b"))
  res <- decision_tree_split(v, y, criterion = "entropy", seed = 1L)
...
test_that("dwnmn handles empty input and alias works", {
  ...
  set.seed(62)
  v <- cumsum(rnorm(20))
  expect_equal(dynamic_wnominate(v)$smoothed, dwnmn(v)$smoothed)
})
...
test_that("is_over_legal_limit returns integer 0/1", {
  expect_identical(is_over_legal_limit(0.09), 1L)
  expect_identical(is_over_legal_limit(0.05), 0L)
  expect_identical(is_over_legal_limit(0.05, limit = 0.05), 1L)
  expect_identical(is_over_legal_limit(0.08), 1L)
})

test_that("egarch_model returns expected structure", {
  set.seed(70)
  r <- rnorm(150, 0, 1)
  res <- egarch_model(r)
  expect_type(res, "list")
...
test_that("egarch_model errors on too few observations", {
  expect_error(egarch_model(rnorm(10)))
})
