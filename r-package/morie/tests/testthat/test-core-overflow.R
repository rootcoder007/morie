test_that("the mean does not overflow where base R does not, on either kernel", {
  ...
  expect_identical(morie:::morie_mean_cpp(rep(1e308, 3), shared = FALSE), 1e308)
  expect_identical(morie:::morie_var_cpp(rep(1e120, 3), 1L, shared = FALSE), 0)
  expect_identical(morie:::morie_mean_cpp(x, shared = FALSE), mean(x))
  # the shared kernel once bricklayer carries the fix
  if (utils::packageVersion("rmoriebricklayer") >= "0.5.1") {
    expect_identical(morie:::morie_mean_cpp(rep(1e308, 3), shared = TRUE), 1e308)
    expect_identical(morie:::morie_var_cpp(rep(1e120, 3), 1L, shared = TRUE), 0)
  }
  expect_type(morie:::.morie_shared_core(), "logical")
})
