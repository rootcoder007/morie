...
test_that("inspect_output reports on json / csv / rds inputs", {
  skip_if_not_installed("jsonlite")
  expect_equal(inspect_output("/no/such/file.json")$status, "missing")
  ...
  expect_equal(inspect_output(cf)$n_columns, 2L)

  rf <- tempfile(fileext = ".rds")
  saveRDS(data.frame(z = 1:2), rf)
  on.exit(unlink(rf), add = TRUE)
  expect_equal(inspect_output(rf)$status, "ok")
  rf2 <- tempfile(fileext = ".rds")
  saveRDS(list(p = 1, q = 2), rf2)
  on.exit(unlink(rf2), add = TRUE)
  expect_equal(inspect_output(rf2)$status, "ok")
})

test_that("inspect_output flags unsupported and unreadable files", {
  uf <- tempfile(fileext = ".dat")
  file.create(uf)
  on.exit(unlink(uf), add = TRUE)
  expect_match(inspect_output(uf)$status, "unsupported")

  skip_if_not_installed("jsonlite")
  bf <- tempfile(fileext = ".json")
  writeLines("{not valid json", bf)
  on.exit(unlink(bf), add = TRUE)
  expect_match(inspect_output(bf)$status, "read-error")
})

test_that("verify_statistical_output runs its quality gates", {
  skip_if_not_installed("jsonlite")
  expect_false(verify_statistical_output("/no/such.json")$passed)
  ...
  expect_false(verify_statistical_output(mf)$passed)
})
...
test_that("predpol_temporal_audit audits multi-city temporal disparity", {
  ...
  res2 <- predpol_temporal_audit(period, city, y_pred, group)
