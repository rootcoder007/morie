...
test_that("POST helper reports non-JSON service error pages clearly", {
  ...
  skip_if_not(exists(".morie_http_post", where = asNamespace("morie"),
                     mode = "function"),
  ...
  expect_error(
    morie:::.morie_dataset_http_post_json("https://example.org/api",
                                          body = list(a = 1L)),
...
test_that(".morie_tps_parse_datetime handles ISO, US, POSIXct, garbage", {
  parse <- morie:::.morie_tps_parse_datetime
...
test_that(".tps_hwka_events_to_days parses US-format OCC_DATE samples", {
  ...
  out <- morie:::.tps_hwka_events_to_days(df, max_n = 10L)
  ...
  expect_error(
    morie:::.tps_hwka_events_to_days(data.frame(x = 1), max_n = 10L),
