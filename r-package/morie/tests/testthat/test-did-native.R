# This file takes about 0.6 minutes, and the eight slowest files are 60% of
...
test_that("test-did-native is slow and opt-in", {
...
test_that("native TWFE equals explicit-dummy OLS (FWL identity)", {
  ...
  fit <- morie:::.morie_did_twfe_native(pan$y, cbind(d = pan$d),
...
test_that("DID-M native engine matches a hand-computed estimand", {
  ...
  hd <- data.frame(
    ...
    d = c(0, 1, 0, 1, 0, 0, 0, 0),
    y = c(0, 5, 0, 7, 0, 1, 0, 3))
  expect_equal(morie:::.morie_didm_point(hd, "y", "d", "unit", "time"), 4)
  # Leavers: symmetric construction, DID- = mean stayers1 - mean leavers.
  hl <- data.frame(
    unit = rep(1:4, each = 2), time = rep(1:2, 4),
    d = c(1, 0, 1, 0, 1, 1, 1, 1),
    y = c(0, 1, 0, 3, 0, 5, 0, 7))
  expect_equal(morie:::.morie_didm_point(hl, "y", "d", "unit", "time"), 4)
})
...
test_that("simplex projection and simplex LS behave", {
  p <- morie:::.morie_simplex_proj(c(2, 0.5, -1))
  expect_equal(sum(p), 1)
  expect_true(all(p >= 0))
  # LS with an exactly representable target recovers the weights
  set.seed(30)
  A <- matrix(rnorm(200), 20, 10)
  w0 <- morie:::.morie_simplex_proj(runif(10))
  b <- as.numeric(A %*% w0)
  w <- morie:::.morie_simplex_ls(A, b)
...
test_that("native SDID recovers tau; all three inference methods run", {
  ...
  fit <- morie:::.morie_sdid_native(Y, N_co, T_pre)
  ...
  for (m in c("placebo", "jackknife", "bootstrap")) {
    inf <- morie:::.morie_sdid_inference(Y, N_co, T_pre, method = m,
                                          n_boot = 60L, seed = 32)
