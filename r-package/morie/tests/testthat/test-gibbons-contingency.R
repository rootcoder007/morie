...
test_that("the independence chi-square matches stats::chisq.test", {
  ...
  expect_equal(morie:::Chiindep(two, correct = TRUE)$statistic,
               as.numeric(stats::chisq.test(two, correct = TRUE)$statistic))
  expect_equal(morie:::Chiindep(two, correct = FALSE)$statistic,
               as.numeric(stats::chisq.test(two, correct = FALSE)$statistic))
  # asking for it on a larger table leaves the statistic alone
  expect_equal(morie:::Chiindep(tb, correct = TRUE)$statistic,
               morie:::Chiindep(tb, correct = FALSE)$statistic)
  # the correction can only shrink the statistic
  expect_lt(morie:::Chiindep(two, correct = TRUE)$statistic,
            morie:::Chiindep(two, correct = FALSE)$statistic)

  # a table already at its expected counts has nothing to explain
  perfect <- rbind(c(10, 20), c(20, 40))
  expect_equal(morie:::Chiindep(perfect)$statistic, 0)
  expect_equal(morie:::Chiindep(perfect)$p_value, 1)

  expect_error(morie:::Chiindep(matrix(1:2, nrow = 1)), "at least 2 rows")
  expect_error(morie:::Chiindep(matrix(1:2, ncol = 1)),
               "at least 2 columns")
  expect_error(morie:::Chiindep(matrix(0, 2, 2)), "positive counts")
  expect_error(morie:::Chiindep(rbind(c(0, 5), c(0, 7))),
...
test_that("the k-by-2 proportions test is the same chi-square", {
  ...
  expect_equal(morie:::Chik2(c(10, 20), c(20, 40))$statistic, 0,
               tolerance = 1e-9)
  # a wider spread of proportions gives a larger statistic
  expect_gt(morie:::Chik2(c(2, 38), c(40, 40))$statistic, got$statistic)
  expect_error(morie:::Chik2(c(1), c(10)), "at least 2 groups")
  expect_error(morie:::Chik2(c(1, 2), c(10)), "matching sizes")
  expect_error(morie:::Chik2(c(1, 2), c(0, 10)), "must be positive")
  expect_error(morie:::Chik2(c(0, 0), c(10, 10)), "inside \\(0, 1\\)")
  expect_error(morie:::Chik2(c(10, 10), c(10, 10)), "inside \\(0, 1\\)")
})
...
test_that("the hypergeometric cell probability is stats::dhyper", {
    ...
    for (r2 in c(4, 9)) {
      for (c1 in c(3, 6)) {
        for (a in 0:min(r1, c1)) {
          expect_equal(morie:::.gbHyper(a, r1, r2, c1),
  ...
  expect_equal(morie:::.gbHyper(-1, 5, 5, 3), 0)
  expect_equal(morie:::.gbHyper(99, 5, 5, 3), 0)
  # the support's probabilities sum to one
  r1 <- 6; r2 <- 5; c1 <- 4
  ks <- max(0, c1 - r2):min(r1, c1)
  expect_equal(sum(vapply(ks, function(k) morie:::.gbHyper(k, r1, r2, c1),
                          0)), 1)
})
...
test_that("Fisher's exact test matches stats::fisher.test", {
  ...
  for (tb in tables) {
    ...
    expect_equal(morie:::Fisherex(tb, "greater")$p_value,
                 stats::fisher.test(tb, alternative = "greater")$p.value,
                 tolerance = 1e-9)
    expect_equal(morie:::Fisherex(tb, "less")$p_value,
  ...
  o <- morie:::Fisherex1(tb, "greater")
  expect_equal(o$p_value, f$p_greater)
  expect_equal(morie:::Fisherex1(tb, "less")$p_value, f$p_less)
  expect_equal(o$mean, sum(tb[1, ]) * sum(tb[, 1]) / sum(tb))

  expect_error(morie:::Fisherex(rbind(c(1, 2, 3), c(4, 5, 6))), "2 x 2")
  expect_error(morie:::Fisherex(rbind(c(-1, 2), c(3, 4))),
               "non-negative")
  expect_error(morie:::Fisherex(tb, "sideways"),
               "two-sided, greater or less")
  expect_error(morie:::Fisherex1(tb, "two-sided"), "greater or less")
})

test_that("McNemar's test matches stats::mcnemar.test and binom.test", {
  tb <- rbind(c(20, 12), c(5, 30))
  got <- morie:::Mcnemarq(tb)
  expect_equal(got$statistic,
  ...
  expect_equal(morie:::Mcnemarq(tb, correct = TRUE)$statistic,
  ...
  expect_equal(morie:::Mcnemarq(other)$statistic, got$statistic)
  ...
  expect_lt(morie:::Mcnemarci(tb, alpha = 0.01)$lower, ci$lower)
  # symmetric discordance estimates no difference
  expect_equal(morie:::Mcnemarci(rbind(c(5, 8), c(8, 5)))$estimate, 0)
  expect_error(morie:::Mcnemarci(tb, alpha = 0), "strictly inside")
  expect_error(morie:::Mcnemarci(matrix(0, 2, 2)), "positive counts")
})

test_that("multinomial goodness of fit matches stats::chisq.test", {
  o <- c(20, 30, 25, 25)
  p <- c(0.2, 0.3, 0.25, 0.25)
  got <- morie:::Multgof(o, p)
  want <- stats::chisq.test(o, p = p)
  expect_equal(got$statistic, as.numeric(want$statistic))
  expect_equal(got$df, as.integer(want$parameter))
  expect_equal(got$p_value, want$p.value)
  # counts exactly at expectation have nothing to explain
  expect_equal(morie:::Multgof(c(20, 30, 25, 25) * 2, p)$statistic, 0,
               tolerance = 1e-9)
  ...
  off <- c(25, 25, 25, 25)
  base <- morie:::Multgof(off, p)
  fewer <- morie:::Multgof(off, p, ddof = 1)
  ...
  expect_equal(morie:::Multgof(o, p, ddof = 1)$df, 2L)
  # a badly-fitting set of counts gives a large statistic
  expect_gt(morie:::Multgof(c(80, 5, 5, 10), p)$statistic, got$statistic)

  expect_error(morie:::Multgof(c(1), c(1)), "at least 2 matching")
  expect_error(morie:::Multgof(o, c(0.1, 0.2, 0.3)),
               "at least 2 matching")
  expect_error(morie:::Multgof(o, c(0.2, 0.2, 0.2, 0.2)), "sum to 1")
  expect_error(morie:::Multgof(o, c(0.5, 0.5, 0, 0)),
               "strictly positive")
})

test_that("the linear-by-linear test uses midrank scores by default", {
  tb <- rbind(c(10, 8, 4), c(3, 7, 12))
  got <- morie:::Linbylin(tb)
  cs <- colSums(tb)
  ...
  # linear trend test
  eq <- morie:::Linbylin(tb, scores = c(1, 2, 3))
  ...
  expect_equal(morie:::Linbylin(flat)$z, 0)
  expect_equal(morie:::Linbylin(flat)$p_twosided, 1)

  expect_error(morie:::Linbylin(matrix(1:9, nrow = 3)), "exactly 2 rows")
  expect_error(morie:::Linbylin(matrix(1:2, nrow = 2)), ">= 2")
  expect_error(morie:::Linbylin(tb, scores = c(1, 2)), "length c")
})

test_that("the odds ratio is Woolf's logit interval", {
  tb <- rbind(c(20, 10), c(8, 25))
  got <- morie:::Oddsrat(tb)
  ...
  null <- morie:::Oddsrat(rbind(c(10, 10), c(10, 10)))
  ...
  expect_error(morie:::Oddsrat(zero), "every cell must be positive")
  fixed <- morie:::Oddsrat(zero, cc = 0.5)
  expect_equal(fixed$estimate, 0.5 * 25.5 / (10.5 * 8.5))
  expect_true(is.finite(fixed$lower) && is.finite(fixed$upper))
  expect_error(morie:::Oddsrat(matrix(1:6, nrow = 2)), "2 x 2")
  expect_error(morie:::Oddsrat(tb, alpha = 1), "strictly inside")
})
