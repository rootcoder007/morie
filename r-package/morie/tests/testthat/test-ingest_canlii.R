...
test_that("a missing key is a clear error before any request", {
  ...
  expect_equal(morie:::.morie_canlii_key("abc"), "abc")
  Sys.setenv(CANLII_API_KEY = "fromenv")
  expect_equal(morie:::.morie_canlii_key(NULL), "fromenv")
})
...
test_that("identifiers and dates are validated", {
  ...
  expect_equal(
    morie:::.morie_canlii_date_params(a = NULL, publishedAfter = "2024-01-01"),
...
test_that("language-keyed ids collapse to strings", {
  set.seed(1)
  df <- morie:::.morie_canlii_records_df(list(
