...
test_that("the exact null distribution of U is stats::dwilcox", {
  ...
  expect_equal(morie:::.gbRankCounts(3, 4)[1], 1)
  expect_equal(utils::tail(morie:::.gbRankCounts(3, 4), 1), 1)
})
...
test_that("U matches stats::wilcox.test and its own moments", {
  ...
  expect_equal(morie:::Mwu(c(10, 11), c(1, 2))$statistic, 4)
  expect_equal(morie:::Mwu(c(1, 2), c(10, 11))$statistic, 0)
  # ties count a half each, which is what keeps U symmetric
  expect_equal(morie:::Mwu(c(1, 2), c(2, 3))$statistic, 0.5)
  expect_equal(morie:::Mwu(c(1, 1), c(1, 1))$statistic, 2)
  # and U for (x, y) mirrors U for (y, x) about mn
  expect_equal(morie:::Mwu(x, y)$statistic +
                 morie:::Mwu(y, x)$statistic, 12)
  expect_error(morie:::Mwu(numeric(0), 1), "non-empty")
})
...
test_that("the rank sum is U shifted by m(m+1)/2", {
  ...
  expect_equal(w$statistic, morie:::Mwu(x, y)$statistic + 8 * 9 / 2)
  # and the two exact p-values therefore agree
  expect_equal(w$p_value, morie:::Mwu(x, y)$p_value)
  ...
  expect_equal(morie:::Wrs(c(1, 2), c(10, 20))$statistic, 3)
  expect_equal(morie:::Wrs(c(10, 20), c(1, 2))$statistic, 7)
  # tied observations get midranks
  expect_equal(morie:::Wrs(c(1, 1), c(1, 1))$statistic, 5)
  expect_error(morie:::Wrs(1, numeric(0)), "non-empty")
})
...
test_that("the rank-sum normal approximation carries the tie correction", {
  ...
  for (w in c(80, mean, 150)) {
    ...
    expect_equal(morie:::Wrsz(w, m, n, "greater")$p_value,
                 1 - stats::pnorm(z$z))
    expect_equal(morie:::Wrsz(w, m, n, "less")$p_value,
                 stats::pnorm(z$z))
  }
  # at the null mean there is nothing to detect
  expect_equal(morie:::Wrsz(mean, m, n)$z, 0)
  expect_equal(morie:::Wrsz(mean, m, n)$p_value, 1)
  # the continuity correction moves the statistic toward the null
  expect_lte(abs(morie:::Wrsz(150, m, n, correct = TRUE)$z),
             abs(morie:::Wrsz(150, m, n)$z))

  # ties reduce the variance by the documented amount
  tv <- c(3, 2)
  tied <- morie:::Wrsz(150, m, n, ties = tv)
  ...
  expect_equal(morie:::Wrsz(150, m, n, ties = c(1, 1))$var, v0)
  # and the approximation tracks the exact tail
  expect_equal(morie:::Wrsz(m * (m + 1) / 2 + 95, m, n, "greater")$p_value,
               1 - stats::pwilcox(94, m, n), tolerance = 0.15)
  expect_error(morie:::Wrsz(1, 0L, 1L), "at least 1")
  expect_error(morie:::Wrsz(1, 2L, 2L, "sideways"),
...
test_that("the shift interval is built from the pairwise differences", {
  ...
  expect_error(morie:::Mwuci(x, y, k = 63L), "0\\.\\.mn-1")
  expect_error(morie:::Mwuci(x, y, k = -1L), "0\\.\\.mn-1")
  expect_error(morie:::Mwuci(numeric(0), y, 0L), "non-empty")
  expect_error(morie:::Wrsci(x, y, wcrit = 0), "outside 0\\.\\.mn-1")
})

test_that("the sample-size formula splits the total by the allocation", {
  skip_heavy()
  s <- morie:::Mwun(0.7, c = 0.5, alpha = 0.05, beta = 0.10)
  ...
  expect_gt(morie:::Mwun(0.7, c = 0.2)$n, s$n)
  expect_equal(morie:::Mwun(0.7, c = 0.25)$m,
               as.integer(round(0.25 * morie:::Mwun(0.7, c = 0.25)$n)))
  # a smaller effect, or a smaller beta, needs more
  ns <- vapply(c(0.9, 0.8, 0.7, 0.6, 0.55), function(p) morie:::Mwun(p)$n, 0L)
  expect_true(all(diff(ns) > 0))
  expect_gt(morie:::Mwun(0.7, beta = 0.01)$n, s$n)
  expect_gte(morie:::Mwun(0.7, twosided = TRUE)$n, s$n)
  # symmetric about one half
  expect_equal(morie:::Mwun(0.7)$n, morie:::Mwun(0.3)$n)
  expect_equal(s$z_alpha, stats::qnorm(0.95))
  expect_equal(s$z_beta, stats::qnorm(0.90))

  expect_error(morie:::Mwun(0.5), "must differ from 0\\.5")
  expect_error(morie:::Mwun(0.7, c = 0), "strictly inside")
  expect_error(morie:::Mwun(0.7, c = 1), "strictly inside")
})
