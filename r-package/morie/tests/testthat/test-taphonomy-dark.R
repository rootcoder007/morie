...
test_that("the SMD-to-RR bridge is exp(0.91 d), folded to the >1 side", {
  for (d in c(0.1, 0.5, 1.3)) {
    expect_equal(morie:::.taphonomy_smd_to_rr(d), exp(0.91 * d))
  }
  ...
  expect_equal(morie:::.taphonomy_smd_to_rr(0.7),
               morie:::.taphonomy_smd_to_rr(-0.7))
  expect_equal(morie:::.taphonomy_smd_to_rr(0), 1)
  expect_true(all(vapply(c(-2, -0.3, 0, 0.3, 2),
                         morie:::.taphonomy_smd_to_rr, 0) >= 1))
})

test_that("the verbal scale reports the ENFSI band and the side it favours", {
  f <- morie:::.taphonomy_lr_verbal
...
test_that("closure puts every row on the simplex", {
  ...
  ps <- morie:::.taphonomy_close(rbind(c(0, 1)), 1)
  expect_equal(unname(ps[1, ]), c(1, 2) / 3)
  # closure is scale invariant: a composition is its ratios
  expect_equal(morie:::.taphonomy_close(X * 7, 0), cl)
  expect_error(morie:::.taphonomy_close(rbind(c(-1, 2)), 0), "non-negative")
  expect_error(morie:::.taphonomy_close(rbind(c("a", "b")), 0),
               "numeric composition")
})
