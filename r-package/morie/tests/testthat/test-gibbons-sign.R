...
test_that("the exact sign-test p-value is stats::binom.test", {
  ...
  for (n in c(6L, 11L, 20L)) {
    for (k in 0:n) {
      ...
      expect_equal(morie:::Signp(k, n, "less")$p_value, p$p_lower)
      expect_equal(morie:::Signp(k, n, "greater")$p_value, p$p_upper)
  ...
  expect_equal(morie:::Signp(4L, 10L)$p_lower + morie:::Signp(4L, 10L)$p_upper,
               1 + stats::dbinom(4, 10, 0.5))
  # the extremes are the smallest attainable p-values
  expect_equal(morie:::Signp(0L, 8L, "less")$p_value, 0.5^8)
  expect_equal(morie:::Signp(8L, 8L, "greater")$p_value, 0.5^8)
  expect_equal(morie:::Signp(0L, 8L)$p_value, 2 * 0.5^8)
  # a statistic at the null mean cannot be evidence against it
  expect_equal(morie:::Signp(5L, 10L)$p_value, 1)
  expect_error(morie:::Signp(0L, 0L), "at least 1")
  expect_error(morie:::Signp(9L, 8L), "0\\.\\.n")
  expect_error(morie:::Signp(-1L, 8L), "0\\.\\.n")
  expect_error(morie:::Signp(4L, 8L, "sideways"),
...
test_that("the normal approximation is the continuity-corrected z", {
  ...
  for (k in c(10L, 18L, 20L, 25L, 33L)) {
    ...
    expect_equal(morie:::Signz(k, n, "greater")$p_value,
                 1 - stats::pnorm(z$z))
    expect_equal(morie:::Signz(k, n, "less")$p_value, stats::pnorm(z$z))
    # the correction always moves the statistic toward the null
    expect_true(abs(z$z) <= abs(morie:::Signz(k, n, correct = FALSE)$z))
  }
  # without the correction it is the plain standardisation
  expect_equal(morie:::Signz(25L, n, correct = FALSE)$z,
               (25 - 20) / sqrt(10))
  # at the null mean the correction has nothing to do
  expect_equal(morie:::Signz(20L, n)$z, 0)
  expect_equal(morie:::Signz(20L, n)$p_value, 1)
  # the approximation tracks the exact test for a large sample
  expect_equal(morie:::Signz(33L, 40L)$p_value,
               morie:::Signp(33L, 40L)$p_value, tolerance = 0.05)
  expect_equal(morie:::Signz(25L, n)$mean, 20)
  expect_equal(morie:::Signz(25L, n)$var, 10)
  expect_error(morie:::Signz(1L, 0L), "at least 1")
  expect_error(morie:::Signz(41L, 40L), "0\\.\\.n")
  expect_error(morie:::Signz(4L, 40L, "sideways"),
               "two-sided, greater or less")
})
...
test_that("the three zero-handling conventions differ as documented", {
  ...
  d <- morie:::Signzero(x, method = "discard")
  ...
  h <- morie:::Signzero(x, method = "half")
  ...
  cn <- morie:::Signzero(x, method = "conservative")
  ...
  y <- c(5, 6, 7, 8, 0, 0, -1)
  expect_equal(morie:::Signzero(y, method = "conservative")$statistic, 4)
  # with no zeros every convention agrees
  z <- c(1, 2, -3, -4, 5)
  for (m in c("discard", "half", "conservative")) {
    r <- morie:::Signzero(z, method = m)
    expect_equal(r$statistic, 3)
    expect_equal(r$n, 5L)
  }
  expect_error(morie:::Signzero(x, method = "keep"),
               "discard, half or conservative")
  expect_error(morie:::Signzero(numeric(0)), "non-empty")
})

test_that("sign-test power rises with n and with the departure from 1/2", {
  skip_heavy()
  p <- morie:::Signpow(20L, 0.75, alpha = 0.05)
  ...
  pw <- vapply(c(10L, 20L, 40L, 80L), function(n) {
    morie:::Signpow(n, 0.75)$power_exact
  }, 0)
  expect_true(all(diff(pw) > 0))
  th <- vapply(c(0.55, 0.65, 0.75, 0.9), function(t) {
    morie:::Signpow(40L, t)$power_exact
  }, 0)
  expect_true(all(diff(th) > 0))
  # at the null the exact power is the attained level, not the nominal one
  expect_equal(morie:::Signpow(20L, 0.5)$power_exact,
               morie:::Signpow(20L, 0.5)$alpha_exact)
  # a larger alpha buys power
  expect_true(morie:::Signpow(20L, 0.75, alpha = 0.10)$power_exact >=
                p$power_exact)
  # the exact branch can be skipped
  no <- morie:::Signpow(20L, 0.75, exact = FALSE)
  expect_true(is.nan(no$power_exact))
  expect_equal(no$power, p$power)
  expect_error(morie:::Signpow(0L, 0.75), "at least 1")
  expect_error(morie:::Signpow(20L, 0), "strictly inside")
  expect_error(morie:::Signpow(20L, 1), "strictly inside")
  expect_error(morie:::Signpow(20L, 0.75, alpha = 0), "strictly inside")
})
...
test_that("simulated power counts the rejections in the supplied samples", {
  ...
  r <- morie:::Signsimpow(m, m0 = 0, kcrit = 4L)
  ...
  # miss gives one
  expect_equal(morie:::Signsimpow(m, 0, 6L)$power, 0)
  expect_equal(morie:::Signsimpow(m, 0, 0L)$power, 1)
  # power is monotone decreasing in the critical value
  pw <- vapply(0:6, function(k) morie:::Signsimpow(m, 0, k)$power, 0)
  expect_true(all(diff(pw) <= 0))
  # raising the hypothesised median can only lower the counts
  expect_true(morie:::Signsimpow(m, 3, 4L)$kmean <= r$kmean)
  expect_error(morie:::Signsimpow(list(), 0, 1L), "non-empty")
})

test_that("the sample-size formulas invert the normal power statement", {
  skip_heavy()
  for (theta in c(0.6, 0.7, 0.25)) {
    one <- morie:::Signn(theta, alpha = 0.05, beta = 0.10)
    za <- stats::qnorm(0.95)
    zb <- stats::qnorm(0.90)
    ...
    two <- morie:::Signnasy(theta, alpha = 0.05, beta = 0.10)
  ...
  ns <- vapply(c(0.9, 0.8, 0.7, 0.6, 0.55), function(t) morie:::Signn(t)$n, 0L)
  expect_true(all(diff(ns) > 0))
  expect_true(morie:::Signn(0.7, beta = 0.01)$n > morie:::Signn(0.7)$n)
  expect_true(morie:::Signn(0.7, alpha = 0.01)$n > morie:::Signn(0.7)$n)
  # the requirement is symmetric about the null
  expect_equal(morie:::Signn(0.7)$n, morie:::Signn(0.3)$n)
  expect_equal(morie:::Signnasy(0.7)$n, morie:::Signnasy(0.3)$n)
  expect_error(morie:::Signn(0.5), "must differ from 0\\.5")
  expect_error(morie:::Signnasy(0.5), "must differ from 0\\.5")
  expect_error(morie:::Signn(0), "strictly inside")
  expect_error(morie:::Signnasy(1), "strictly inside")
})
...
test_that("the median interval inverts the sign test at its stated level", {
  ...
  ci <- morie:::Signmedci(x, alpha = 0.05)
  ...
  wide <- morie:::Signmedci(x, alpha = 0.01)
  ...
  small <- morie:::Signmedci(c(1, 2, 3), alpha = 0.05)
  ...
  # is under 0.05
  expect_equal(morie:::Signmedci(c(1, 2, 3, 4, 5, 6))$r, 1L)
  expect_equal(morie:::Signmedci(c(1, 2, 3, 4, 5, 6))$coverage, 1 - 2 * 0.5^6)
  expect_error(morie:::Signmedci(1), "at least 2")
  expect_error(morie:::Signmedci(x, alpha = 0), "strictly inside")
})
