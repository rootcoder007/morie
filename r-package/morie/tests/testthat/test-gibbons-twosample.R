...
test_that("the runs pmf is its closed form and a distribution", {
  ...
  for (m in 2:5) {
    for (n in 2:5) {
      expect_equal(morie:::.gbRunsPmf(m, n), enumerate_runs(m, n))
    }
  }
  # the two names for the same function agree
  expect_equal(morie:::.gbRunsPmf2(4, 5), morie:::.gbRunsPmf(4, 5))
  # two runs is the fully separated arrangement, of which there are two
  expect_equal(morie:::.gbRunsPmf(3, 3)[1], 2 / choose(6, 3))
})
...
test_that("the Wald-Wolfowitz runs statistic counts the alternations", {
  ...
  alt <- morie:::Wwruns(c(1, 3, 5), c(2, 4, 6))
  ...
  expect_equal(r$m, 8L)
  expect_equal(r$n, 9L)
  expect_error(morie:::Wwruns(numeric(0), 1), "non-empty")
})
...
test_that("the exact runs test reports both tails", {
  ...
  x <- stats::rnorm(6)
  y <- stats::rnorm(7)
  e <- morie:::Wwexact(x, y, tail = "left")
  pmf <- morie:::.gbRunsPmf(6, 7)
  support <- 2:13
  ...
  expect_equal(morie:::Wwexact(x, y, "right")$p_value, e$p_right)
  expect_equal(morie:::Wwexact(x, y, "two-sided")$p_value,
  ...
  expect_equal(e$statistic, morie:::Wwruns(x, y)$statistic)
  # the extremes
  expect_equal(morie:::Wwexact(c(1, 2), c(3, 4), "left")$p_value,
               morie:::.gbRunsPmf(2, 2)[1])
  expect_equal(morie:::Wwexact(c(1, 2), c(3, 4), "right")$p_value, 1)
  expect_error(morie:::Wwexact(x, y, "sideways"),
...
test_that("the tie bounds bracket the attainable run counts", {
  ...
  clean <- morie:::Wwties(c(1, 2, 3), c(4, 5, 6))
  ...
  tied <- morie:::Wwties(c(1, 2, 2), c(2, 3, 4))
...
test_that("the two-sample KS statistic matches stats::ks.test", {
  ...
  expect_equal(morie:::Ks2(c(1, 2), c(10, 20))$statistic, 1)
  expect_equal(morie:::Ks2(c(1, 2), c(10, 20))$p_value,
               suppressWarnings(stats::ks.test(c(1, 2),
                                               c(10, 20))$p.value),
               tolerance = 1e-9)
  # identical samples give D = 0 and no evidence
  expect_equal(morie:::Ks2(c(1, 2, 3), c(1, 2, 3))$statistic, 0)
  expect_equal(morie:::Ks2(c(1, 2, 3), c(1, 2, 3))$p_value, 1)
  expect_error(morie:::Ks2(numeric(0), 1), "non-empty")
})
...
test_that("the exact Smirnov distributions match stats::psmirnov", {
  ...
  sfs <- vapply(seq(0.2, 1, by = 0.1),
                function(d) morie:::Smirnov2(d, 8L, 8L)$sf, 0)
  expect_true(all(diff(sfs) <= 0))
  # D = 1 is complete separation, whose probability is the two ways of
  # splitting the pooled order
  expect_equal(morie:::Smirnov2(1, 5L, 5L)$sf, 2 / choose(10, 5))
  expect_error(morie:::Smirnov2(0, 4L, 4L), "\\(0, 1\\]")
  expect_error(morie:::Smirnov2(1.5, 4L, 4L), "\\(0, 1\\]")
  expect_error(morie:::Smirnov1(0, 4L, 4L), "\\(0, 1\\]")
})
  ...
  for (m in c(20L, 50L)) {
    for (n in c(30L, 80L)) {
      for (d in c(0.1, 0.25, 0.4)) {
        a <- morie:::Ks2asymp(d, m, n)
        expect_equal(a$neff, m * n / (m + n))
        expect_equal(a$k, sqrt(a$neff) * d)
        expect_equal(a$p_value, min(1, max(0, morie:::.gbKsQ(a$k))))
        expect_true(a$p_value >= 0 && a$p_value <= 1)
      }
    }
  }
  # zero deviation is no evidence at all
  expect_equal(morie:::Ks2asymp(0, 20L, 20L)$p_value, 1)
  # and it agrees with ks.test's own asymptotic p-value
  set.seed(44)
  x <- stats::rnorm(200)
  y <- stats::rnorm(250, 0.3)
  d <- morie:::Ks2(x, y)$statistic
  expect_equal(morie:::Ks2asymp(d, 200L, 250L)$p_value,
               suppressWarnings(stats::ks.test(x, y,
                                               exact = FALSE))$p.value,
               tolerance = 1e-6)
  # the asymptotic tail approaches the exact one as the samples grow
  expect_equal(morie:::Ks2asymp(0.3, 60L, 60L)$p_value,
               morie:::Smirnov2(0.3, 60L, 60L)$sf, tolerance = 0.02)
  expect_error(morie:::Ks2asymp(0.5, 0L, 5L), "at least 1")
  expect_error(morie:::Ks2asymp(-1, 5L, 5L), "non-negative")
})
