...
test_that("the EDF count is binomial (Theorem 2.3.1)", {
  ...
  expect_equal(morie:::Edfbinom(5, 0, 0)$cdf, 1)
  expect_equal(morie:::Edfbinom(5, 1, 5)$pmf, 1)
  expect_equal(morie:::Edfbinom(5, 0)$var, 0)
  expect_equal(morie:::Edfbinom(1, 0.5, 1)$pmf, 0.5)

  expect_error(morie:::Edfbinom(0, 0.5), "at least 1")
  expect_error(morie:::Edfbinom(5, 1.2), "\\[0, 1\\]")
  expect_error(morie:::Edfbinom(5, -0.1), "\\[0, 1\\]")
  expect_error(morie:::Edfbinom(5, 0.5, 6), "0\\.\\.n")
  expect_error(morie:::Edfbinom(5, 0.5, -1), "0\\.\\.n")
})
...
test_that("the EDF is the step function stats::ecdf builds", {
  ...
  expect_equal(morie:::Edfstep(x, 1)$edf, 2 / 8)
  expect_equal(morie:::Edfstep(x, 0.999)$edf, 0)
  expect_equal(morie:::Edfstep(x, 100)$edf, 1)
  # a single observation gives the unit step
  expect_equal(morie:::Edfstep(7, c(6, 7, 8))$edf, c(0, 1, 1))
  expect_error(morie:::Edfstep(numeric(0), 1), "non-empty")
})
...
test_that("the r-th order statistic CDF is the binomial tail (eq. 2.4.1)", {
  ...
  expect_equal(morie:::Ostatcdf(qnorm(0.4), 3, n, stats::pnorm)$cdf,
               morie:::Ostatcdf(0, 3, n, 0.4)$cdf)
  # the minimum and the maximum are the two closed forms
  expect_equal(morie:::Ostatcdf(0, 1L, n, p)$cdf, 1 - (1 - p)^n)
  expect_equal(morie:::Ostatcdf(0, n, n, p)$cdf, p^n)
  # the r-th order statistic is stochastically larger as r grows
  cdfs <- vapply(seq_len(n), function(r) morie:::Ostatcdf(0, r, n, p)$cdf, 0)
  expect_true(all(diff(cdfs) < 0))

  expect_error(morie:::Ostatcdf(0, 0L, n, p), "1 <= r <= n")
  expect_error(morie:::Ostatcdf(0, 11L, n, p), "1 <= r <= n")
  expect_error(morie:::Ostatcdf(0, 3L, n, 1.5), "must lie in \\[0, 1\\]")
})
...
test_that("the order-statistic density is the beta density under uniform", {
  ...
  for (r in seq_len(n)) {
    for (x in c(0.1, 0.5, 0.9)) {
      ...
      got <- morie:::Ostatpdf(x, r, n, cdf = x, pdf = 1)
  ...
  got <- morie:::Ostatpdf(0.3, 2L, n, cdf = function(z) z^2,
                           pdf = function(z) 2 * z)
  ...
  m <- morie:::Ostatpdf(0, 3L, 5L, stats::pnorm, stats::dnorm)
  expect_equal(m$pdf, 30 * 0.5^2 * 0.5^2 * stats::dnorm(0))
  expect_error(morie:::Ostatpdf(0.5, 9L, n, 0.5, 1), "1 <= r <= n")
})
...
test_that("the probability-integral transform is Beta(r, n - r + 1)", {
  ...
  expect_equal(morie:::Ostatbeta(0.5, 4L, 7L)$mean, 0.5)
  # the endpoints of the support
  expect_equal(morie:::Ostatbeta(0, 1L, n)$cdf, 0)
  expect_equal(morie:::Ostatbeta(1, n, n)$cdf, 1)
  expect_error(morie:::Ostatbeta(0.5, 8L, n), "1 <= r <= n")
  expect_error(morie:::Ostatbeta(1.5, 1L, n), "must lie in \\[0, 1\\]")
})

test_that("the sample-quantile variance is the asymptotic p(1-p)/(n f^2)", {
  skip_heavy()
  a <- morie:::Ostatasymp(0.5, 100L, 0, stats::dnorm(0))
  ...
  expect_equal(morie:::Ostatasymp(0.5, 64L, 0.5, 1)$var, 1 / 256)
  # and the variance falls like 1/n
  expect_equal(morie:::Ostatasymp(0.5, 200L, 0, 1)$var,
               morie:::Ostatasymp(0.5, 100L, 0, 1)$var / 2)
  # a flatter density at the quantile makes it harder to pin down
  expect_true(morie:::Ostatasymp(0.5, 50L, 0, 0.1)$var >
                morie:::Ostatasymp(0.5, 50L, 0, 0.4)$var)
  expect_error(morie:::Ostatasymp(0, 10L, 0, 1), "strictly inside")
  expect_error(morie:::Ostatasymp(1, 10L, 0, 1), "strictly inside")
  expect_error(morie:::Ostatasymp(0.5, 0L, 0, 1), "at least 1")
  expect_error(morie:::Ostatasymp(0.5, 10L, 0, 0), "strictly positive")
})
...
test_that("uniform order-statistic moments are the beta moments", {
  ...
  for (r in seq_len(n)) {
    ...
    for (k in 1:4) {
      expect_equal(morie:::Ostatmom(r, n, k)$moment,
                   prod((r + 0:(k - 1)) / (n + 1 + 0:(k - 1))))
      expect_equal(morie:::Ostatmom(r, n, k)$moment,
                   beta(r + k, n - r + 1) / beta(r, n - r + 1))
    }
    # and the variance is the second moment less the squared first
    expect_equal(m$var, morie:::Ostatmom(r, n, 2L)$moment - m$mean^2)
  }
  # the order statistics divide (0, 1) into n + 1 equal expected gaps
  means <- vapply(seq_len(n), function(r) morie:::Ostatmom(r, n)$mean, 0)
  expect_equal(diff(means), rep(1 / (n + 1), n - 1))
  expect_error(morie:::Ostatmom(10L, n), "1 <= r <= n")
  expect_error(morie:::Ostatmom(1L, n, 0L), "at least 1")
})
...
test_that("order statistics are positively correlated, decaying with the gap", {
  ...
  expect_equal(morie:::Ostatcov(3L, 3L, n)$corr, 1)
  expect_equal(morie:::Ostatcov(3L, 3L, n)$cov,
               morie:::Ostatmom(3L, n)$var)
  # the correlation decays as the two ranks move apart
  cors <- vapply(2:n, function(s) morie:::Ostatcov(1L, s, n)$corr, 0)
  expect_true(all(diff(cors) < 0))
  # the minimum and the maximum are the least dependent pair
  expect_equal(morie:::Ostatcov(1L, n, n)$cov, 1 / ((n + 1)^2 * (n + 2)))
  expect_equal(morie:::Ostatcov(1L, n, n)$cov,
               min(vapply(2:n, function(s) morie:::Ostatcov(1L, s, n)$cov, 0)))
  # and the uniform order statistics are mirror-symmetric about 1/2, so
  # rank r and rank n - r + 1 have the same variance
  for (r in seq_len(n)) {
    expect_equal(morie:::Ostatcov(r, r, n)$var_r,
                 morie:::Ostatcov(n - r + 1L, n - r + 1L, n)$var_r)
  }
  expect_error(morie:::Ostatcov(5L, 3L, n), "1 <= r <= s <= n")
  expect_error(morie:::Ostatcov(1L, 11L, n), "1 <= r <= s <= n")
})
...
test_that("the joint density of two order statistics integrates to one", {
  ...
  s <- 4L
  dens <- function(x, y) {
    morie:::Ostatjoint(x, y, r, s, n, function(z) z, function(z) 1)$pdf
  }
  inner <- function(y) {
  ...
  expect_error(morie:::Ostatjoint(0.2, 0.5, 2L, 2L, n, function(z) z,
...
test_that("the joint density of the whole ordered sample is n! prod f", {
  ...
  un <- morie:::Ostatjall(c(0.5, 0.2, 1.1), f)
  expect_equal(un$pdf, 0)
  expect_equal(un$ordered, 0L)
  # so are ties, since the ordered region is open
  expect_equal(morie:::Ostatjall(c(0.2, 0.2), f)$pdf, 0)
  # the uniform case is the constant n! on the ordered simplex, whose
  # volume is 1/n!
  expect_equal(morie:::Ostatjall(c(0.1, 0.4, 0.9), function(z) 1)$pdf,
               factorial(3))
  expect_equal(morie:::Ostatjall(0.5, function(z) 1)$pdf, 1)
  expect_error(morie:::Ostatjall(numeric(0), f), "non-empty")
})
...
test_that("the sample quantile is the ceiling order statistic", {
  ...
  expect_equal(morie:::Sampquant(x, 0.5)$estimate, 3)
  # a quantile past the last rank is held at the maximum rather than NA
  expect_equal(morie:::Sampquant(x, 0.999)$r, 5L)
  expect_equal(morie:::Sampquant(x, 0.999)$estimate, 5)
  expect_equal(morie:::Sampquant(x, 1e-9)$estimate, 1)
  expect_error(morie:::Sampquant(numeric(0), 0.5), "non-empty")
  expect_error(morie:::Sampquant(x, 0), "strictly inside")
  expect_error(morie:::Sampquant(x, 1), "strictly inside")
})
...
test_that("the exceedance distribution matches an exhaustive enumeration", {
  ...
  e <- morie:::Exceed(2L, 5L, 4L)
  for (j in 0:5) {
    expect_equal(morie:::Exceed(2L, 5L, 4L, j)$pmf_j, e$pmf[j + 1L])
    expect_equal(morie:::Exceed(2L, 5L, 4L, j)$cdf_j, sum(e$pmf[1:(j + 1L)]))
  }
  expect_equal(morie:::Exceed(2L, 5L, 4L, 5L)$cdf_j, 1)
  ...
  means <- vapply(1:4, function(i) morie:::Exceed(i, 5L, 4L)$mean, 0)
  expect_true(all(diff(means) > 0))

  expect_error(morie:::Exceed(0L, 3L, 4L), "1 <= i <= n")
  expect_error(morie:::Exceed(5L, 3L, 4L), "1 <= i <= n")
  expect_error(morie:::Exceed(1L, 0L, 4L), "at least 1")
  expect_error(morie:::Exceed(1L, 3L, 4L, 4L), "0\\.\\.m")
})
...
test_that("a distribution-free quantile interval has binomial coverage", {
  ...
  full <- morie:::Quantci(x, 0.5, 1L, 20L)
  expect_equal(full$coverage, 1 - 2 * 0.5^20)
  # and widening the interval can only raise the coverage
  cov5 <- morie:::Quantci(x, 0.5, 5L, 16L)$coverage
  cov7 <- morie:::Quantci(x, 0.5, 7L, 14L)$coverage
  expect_true(cov5 > cov7)
  expect_error(morie:::Quantci(x[1], 0.5, 1L, 2L), "at least 2")
  expect_error(morie:::Quantci(x, 0.5, 5L, 5L), "1 <= r < s <= n")
  expect_error(morie:::Quantci(x, 0.5, 1L, 21L), "1 <= r < s <= n")
  expect_error(morie:::Quantci(x, 1, 1L, 5L), "strictly inside")
})
...
test_that("the quantile test is the binomial sign test on the threshold", {
  ...
  k <- sum(x <= q0)
  expect_equal(k, 4L)
  lo <- morie:::Quanttest(x, q0, alternative = "less")
  hi <- morie:::Quanttest(x, q0, alternative = "greater")
  two <- morie:::Quanttest(x, q0)
  ...
  expect_equal(morie:::Quanttest(x, 11)$p_value, 1)
  # a threshold outside the sample is as extreme as the test can report
  expect_equal(morie:::Quanttest(x, 0, alternative = "less")$p_value, 0.5^n)
  expect_equal(morie:::Quanttest(x, 100, alternative = "greater")$p_value,
               0.5^n)
  # testing a quantile other than the median shifts the null binomial
  q3 <- morie:::Quanttest(x, 15, p = 0.75)
  ...
  expect_error(morie:::Quanttest(numeric(0), 1), "non-empty")
  expect_error(morie:::Quanttest(x, 1, p = 0), "strictly inside")
  expect_error(morie:::Quanttest(x, 1, alternative = "bigger"),
