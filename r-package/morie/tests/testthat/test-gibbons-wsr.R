...
test_that("signed ranks are the midranks of the absolute differences", {
  ...
  expect_length(morie:::Absrank(c(1, -2, 3))$ties, 0L)
  expect_equal(morie:::Absrank(c(1, -2, 3))$ranks, c(1, 2, 3))
  # the signed ranks sum to T+ minus T-
  expect_equal(sum(a$signed), sum(a$ranks[d > 0]) - sum(a$ranks[d < 0]))
  expect_error(morie:::Absrank(numeric(0)), "non-empty")
})

test_that("the exact null distribution of T+ is stats::dsignrank", {
  for (n in 1:12) {
    total <- n * (n + 1) / 2
    pmf <- morie:::.gbWsrNull(n)
...
test_that("T+ matches stats::wilcox.test", {
  ...
  expect_equal(morie:::Wsr(x, m0 = 10)$statistic,
  ...
  z <- morie:::Wsr(c(1, 2, 0, 0, -3))
  expect_equal(z$nzero, 2L)
  expect_equal(z$n, 3L)
  # an all-positive sample gives the maximum statistic
  expect_equal(morie:::Wsr(c(1, 2, 3))$statistic, 6)
  expect_equal(morie:::Wsr(c(-1, -2, -3))$statistic, 0)
  ...
  expect_error(morie:::Wsr(c(0, 0)), "no non-zero differences")
})
...
test_that("the null moments and the tie correction are their formulas", {
  ...
  total <- n * (n + 1) / 2
  pmf <- morie:::.gbWsrNull(n)
  expect_equal(morie:::Wsrmom(n)$mean, sum((0:total) * pmf))
  expect_equal(morie:::Wsrmom(n)$var,
  ...
  expect_equal(morie:::Wsrties(c(1, 2, 3))$correction, 0)
  expect_equal(morie:::Wsrties(c(1, 2, 3))$var, 3 * 4 * 7 / 24)
  # zeros are excluded before the ranks are formed
  expect_equal(morie:::Wsrties(c(1, 2, 0, 3))$n, 3L)
  expect_equal(morie:::Wsrties(c(1, 2, 0, 3))$nzero, 1L)
  expect_error(morie:::Wsrmom(0), "at least 1")
  expect_error(morie:::Wsrties(0), "no non-zero differences")
})
...
test_that("the signed-rank normal approximation is the standardised T+", {
  ...
  for (t in c(40, 105, 150)) {
    ...
    expect_equal(morie:::Wsrz(t, n, "greater")$p_value,
                 1 - stats::pnorm(z$z))
    expect_equal(morie:::Wsrz(t, n, "less")$p_value, stats::pnorm(z$z))
  }
  # at the null mean there is no evidence either way
  expect_equal(morie:::Wsrz(mean, n)$z, 0)
  expect_equal(morie:::Wsrz(mean, n)$p_value, 1)
  # the continuity correction always moves the statistic toward the null
  for (t in c(40, 150)) {
    expect_lte(abs(morie:::Wsrz(t, n, correct = TRUE)$z),
               abs(morie:::Wsrz(t, n, correct = FALSE)$z))
  }
  expect_equal(morie:::Wsrz(mean, n, correct = TRUE)$z, 0)
  # and the approximation tracks the exact tail for a moderate n
  expect_equal(morie:::Wsrz(150, 20L, "greater")$p_value,
               1 - stats::psignrank(149, 20), tolerance = 0.1)
  expect_error(morie:::Wsrz(1, 0L), "at least 1")
  expect_error(morie:::Wsrz(1, 5L, "sideways"),
               "two-sided, greater or less")
})

test_that("signed-rank power and sample size invert one another", {
  p <- morie:::Wsrpow(30L, p1 = 0.7, p2 = 0.65)
  ...
  pw <- vapply(c(10L, 20L, 40L, 80L),
               function(n) morie:::Wsrpow(n, 0.7, 0.65)$power, 0)
  expect_true(all(diff(pw) > 0))
  p2s <- vapply(c(0.55, 0.6, 0.7, 0.8),
                function(q) morie:::Wsrpow(30L, 0.7, q)$power, 0)
  expect_true(all(diff(p2s) > 0))
  # a larger alpha buys power
  expect_gt(morie:::Wsrpow(30L, 0.7, 0.65, alpha = 0.10)$power, p$power)
  # at the null the power is the level
  expect_equal(morie:::Wsrpow(30L, 0.5, 0.5)$power, 0.05, tolerance = 1e-9)

  # the sample-size formula
  s <- morie:::Wsrn(0.65, alpha = 0.05, beta = 0.05)
  ...
  expect_gte(morie:::Wsrn(0.65, twosided = TRUE)$n, s$n)
  # a smaller effect needs a larger sample, symmetric about 1/2
  ns <- vapply(c(0.9, 0.8, 0.7, 0.6, 0.55),
               function(q) morie:::Wsrn(q)$n, 0L)
  expect_true(all(diff(ns) > 0))
  expect_equal(morie:::Wsrn(0.65)$n, morie:::Wsrn(0.35)$n)

  expect_error(morie:::Wsrpow(1L, 0.7, 0.6), "at least 2")
  expect_error(morie:::Wsrpow(10L, 0.7, 0.6, alpha = 0), "strictly inside")
  expect_error(morie:::Wsrn(0.5), "must differ from 0\\.5")
  expect_error(morie:::Wsrn(0), "strictly inside")
})
...
test_that("simulated signed-rank power counts its rejections", {
  ...
  r <- morie:::Wsrsimpow(m, m0 = 0, tcrit = 6)
  ...
  expect_equal(morie:::Wsrsimpow(m, 0, 7)$power, 0)
  expect_equal(morie:::Wsrsimpow(m, 0, 0)$power, 1)
  # power is monotone decreasing in the critical value
  pw <- vapply(0:7, function(t) morie:::Wsrsimpow(m, 0, t)$power, 0)
  expect_true(all(diff(pw) <= 0))
  # a list of unequal-length samples is accepted
  l <- list(c(1, 2), c(-1, -2, -3), c(5, 6, 7, 8))
  expect_equal(morie:::Wsrsimpow(l, 0, 3)$nsim, 3L)
  # an all-zero sample has no non-zero differences, so T+ is zero
  expect_equal(morie:::Wsrsimpow(list(c(0, 0)), 0, 1)$power, 0)
  expect_error(morie:::Wsrsimpow(list(), 0, 1), "non-empty")
})
...
test_that("the Walsh-average interval is the Hodges-Lehmann one", {
  ...
  wide <- morie:::Wsrci(x, tcrit = 5L)
  expect_lte(wide$lower, ci$lower)
  expect_gte(wide$upper, ci$upper)
  expect_gt(wide$coverage, ci$coverage)

  expect_error(morie:::Wsrci(1, 1L), "at least 2")
  expect_error(morie:::Wsrci(x, -1L), "non-negative")
  expect_error(morie:::Wsrci(x, 200L), "too large")
  expect_error(morie:::Hlwsrlink(1), "at least 2")
})
...
test_that("the symmetry test standardises T+ about its null mean", {
  ...
  x <- stats::rnorm(20)
  s <- morie:::Wsrsym(x)
  expect_equal(s$statistic, morie:::Wsr(x)$statistic)
  ...
  right <- morie:::Wsrsym(c(rep(1, 8), -20, -30, -40))
  ...
  left <- morie:::Wsrsym(c(rep(-1, 8), 20, 30, 40))
  ...
  expect_equal(morie:::Wsrsym(c(1, 2, 3), centre = 2)$n, 2L)
  expect_error(morie:::Wsrsym(c(1, 0)), "at least 2 non-zero")
})
