...
test_that("the tail exponent is recovered from data built with it", {
  ...
  for (al in c(2.0, 2.5, 3.0, 3.5)) {
    ...
    expect_equal(morie:::.hill_mle(z, 1L), al, tolerance = 0.05)
  }
  ...
  expect_lt(morie:::.hill_mle(z, 1L, approx = TRUE), 2.5)
  expect_equal(morie:::.hill_mle(z, 1L), 3.5, tolerance = 0.05)
  ...
  approx_err <- vapply(c(1L, 3L, 6L, 12L), function(xm) {
    abs(morie:::.hill_mle(big, xm, approx = TRUE) - 2.5)
  }, 0)
  exact_err <- vapply(c(1L, 3L, 6L, 12L), function(xm) {
    abs(morie:::.hill_mle(big, xm) - 2.5)
  }, 0)
...
test_that("the exponent degrades rather than inventing a number", {
  ...
  expect_true(is.na(morie:::.hill_mle(5, 1L)))
  expect_true(is.na(morie:::.hill_mle(c(1, 2), 10L)))
  expect_true(is.na(morie:::.hill_mle(numeric(0), 1L)))
  # a zero cannot be logged, so it is not in the tail
  expect_equal(morie:::.hill_mle(c(0, 0, 3, 4, 5, 9), 1L),
               morie:::.hill_mle(c(3, 4, 5, 9), 1L))
  # nor can an infinity or a missing value be data
  expect_equal(morie:::.hill_mle(c(Inf, NA, 3, 4, 5, 9), 1L),
               morie:::.hill_mle(c(3, 4, 5, 9), 1L))
...
test_that("the concentration measures agree with their definitions", {
  ...
  for (x in list(c(1, 2, 3, 4, 5), c(1, 1, 1, 8), rep(4, 7),
    ...
    expect_equal(morie:::.gini_int(x), want, tolerance = 1e-12)
  }
  # the maximum for n units is 1 - 1/n, not one
  expect_equal(morie:::.gini_int(c(0, 0, 0, 0, 1)), 1 - 1 / 5)
  expect_equal(morie:::.gini_int(rep(2, 10)), 0)
  # a total of zero has no distribution to be unequal about
  expect_true(is.na(morie:::.gini_int(c(0, 0, 0))))
  ...
  for (x in list(c(1, 2, 3, 4, 5), c(1, 1, 1, 8), c(2, 2, 3, 100))) {
    expect_equal(morie:::.gini_int(x), rmoriebricklayer::gini(x),
                 tolerance = 1e-12)
  }
})
