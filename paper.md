...
*A bounded search makes an arbitrary box part of the answer.* The
semivariogram fitters of Sec. 4.5.1 were first written as a scalar
objective handed to a quasi-Newton routine. On the test fixture the
solver stopped after **one** iteration, reported success, and returned
the starting heuristic as the fit. It was invisible because the
heuristic sits near the truth: nugget $0.229$, partial sill $2.058$,
range $6.145$ against a generating $0.3$, $2.0$, $6.0$ reads as a good
recovery. What gave it away was that ordinary least squares, weighted
least squares and restricted maximum likelihood --- three different
criteria --- returned estimates agreeing to all seventeen digits, which
no three distinct estimators do. A crude grid found an objective of
$38.4$ where the reported fit claimed $142.5$.

The deeper error was in the fixture, and it survived two rounds of
tuning before being seen. The field carried a linear trend, and a
trended field has an **unbounded** semivariogram: the exponential model
fits it best in the degenerate limit, partial sill and range both to
infinity with their ratio fixed, which is a straight line. The solver
had not misbehaved --- it was correctly running away, and only the
arbitrary search box $(10\gamma_{\max}, 10h_{\max})$ had stopped it. So
the reference values being tuned against were properties of the box,
not of the estimator. A strongly oscillating field is no better: its
variogram has a hole effect and no monotone model fits that either. The
fixture is now a semivariogram table built from the model itself with a
two per cent deterministic perturbation, which has a known answer and
lies inside the model's domain. Against a generating $(0.3, 2.0, 6.0)$
the fitted values are $(0.3007, 2.0061, 6.0715)$ for OLS and
$(0.2924, 2.0085, 5.9856)$ for WLS.

*An identity can be checked against the wrong expectation.* Sec. 7.2.2
closes with $E[(Z_c(s) - Z(s))^2] = 2\sigma^2_{sk}$, which is the
sharpest available test that a conditional simulation is doing what the
book says. Averaging over 6,000 simulations from a single realized
field gave a ratio of $0.9099$ --- close enough to look like a bug in
the implementation, far enough to matter. The expectation is over
...
causal forest was for a long time not pinned that way, because R drew
subsamples with the Mersenne Twister and the Python forest with a
permuted congruential generator, so the two saw different subsamples by
construction and the parity test could only assert that they recovered
the same estimand. The native generator described under *Exactness*
removes that obstacle: any procedure routed through it draws the same
numbers in both languages, so a resampling estimator can now be pinned
to bits rather than to an estimand.

*Two runs of the same simulation should be the same simulation.* A
statistics library that cannot generate its own random numbers cannot
be checked across languages, and morie could not: what it had was a
32-bit linear congruential generator,
$s \leftarrow (1664525\,s + 1013904223) \bmod 2^{32}$, copied between
modules, alongside a helper that derived a seed by SHA-256 and handed it
to `set.seed()` with NumPy on the other side --- an arrangement whose
own documentation could promise agreement only "to Monte-Carlo
tolerance". The LCG's period is exhausted by a few billion draws, and by
Marsaglia's theorem its tuples lie on a small number of lattice
hyperplanes, which is precisely the failure mode that matters for
simulating spatial fields, since those are built from exactly such
tuples.

Both arms now implement Philox4x32-10 [@salmon2011] and Wichura's
AS 241 [@wichura1988]. Philox is counter-based --- the $n$-th output is
a keyed bijection of $n$ --- so there is no evolving state to keep in
step, any offset is reachable, and the arithmetic is integer-only,
which is what lets two languages agree bit for bit. R has no unsigned
32-bit type, so each word is carried as a double and the
$32\times32\rightarrow64$ product is assembled from 16-bit partial
products, every intermediate staying below $2^{53}$. Normals come from
the inverse CDF rather than Box--Muller: one uniform per normal keeps
the streams aligned, and it avoids `log` and `cos`, whose last-bit
behaviour differs between platforms' libm.

Correctness is established against published Known Answer Tests rather
than asserted. Both implementations reproduce all three Philox KAT
vectors from the reference distribution bit-exactly, including the
$\pi$-digits vector; the R implementation of AS 241 matches base R's own
`qnorm` to $0.00\mathrm{e}{+}00$, which is the strongest check available
since `qnorm` *is* AS 241. The two arms then produce identical streams:
seven uniforms and seven normals at seed 12345 agree to all seventeen
digits. The consequence is that a *simulated field*, not merely its
distribution, is shared --- a Cholesky draw returns
$1.60099564270895$ in both languages, and the conditional simulation of
Sec. 7.2.2 returns $0.611244720302244$ in both.

One limit is documented rather than hidden. Near the extreme upper tail
the accuracy is bounded by representing $1-p$, not by the algorithm: at
$p = 10^{-8}$, $1-p$ rounds to $0.99999999$ and the tail recovered
...
Dumping per-bin pair **counts** rather than values localised it: the
totals matched exactly and only 34 of 165,600 pairs had moved between
angular sectors. `atan2` is not correctly rounded and platforms' libm
differ in the last bit; on a regular lattice 2,304 pairs lie *exactly*
on the $\pi/4$ sector boundary and 4,244 within one ulp of it, so the
sector assignment --- and the reported statistic --- was decided by
rounding. Folding the lag onto $[0,\pi)$ *after* `atan2` rather than
orienting it before made the estimator depend on the order the points
were listed in as well. Both are fixed; the diagnostic worth keeping is
that a count difference localises to a predicate, where a value
...
<div id="ref-salmon2011" class="csl-entry">

Salmon, John K., Mark A. Moraes, Ron O. Dror, and David E. Shaw. 2011.
“Parallel Random Numbers: As Easy as 1, 2, 3.” In *Proceedings of the
2011 International Conference for High Performance Computing, Networking,
Storage and Analysis (SC ’11)*, 1–12. ACM.
...
<div id="ref-wichura1988" class="csl-entry">

Wichura, Michael J. 1988. “Algorithm AS 241: The Percentage Points of
the Normal Distribution.” *Journal of the Royal Statistical Society.
Series C (Applied Statistics)* 37 (3): 477–484.
