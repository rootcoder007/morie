"""moirais.tps_hawkes_advanced — non-stationary Hawkes with non-exponential kernels.
...
Companion to ``moirais.tps_stochastic`` — that module contains the
...
def hawkes_advanced_fit(df: pd.DataFrame, *,
    ...
    A sibling to ``moirais.tps_stochastic.hawkes_temporal_fit`` which is
