"""`moirais verify-earth-engine` — one-command EE auth smoke check.

Companion to `moirais verify-pollution` (Workstream 6). This CLI
...
def _probe_credentials() -> tuple[bool, str]:
    sa = os.environ.get("MOIRAIS_EE_SERVICE_ACCOUNT")
    key = os.environ.get("MOIRAIS_EE_KEY_PATH") or os.environ.get(
        "GOOGLE_APPLICATION_CREDENTIALS"
    )
    project = os.environ.get("MOIRAIS_EE_PROJECT") or os.environ.get(
        "GOOGLE_CLOUD_PROJECT"
    )
    if sa and key and project:
        if not os.path.exists(key):
            return False, f"MOIRAIS_EE_KEY_PATH points at missing file: {key}"
    ...
    return False, (
        "no GCP project configured. Set MOIRAIS_EE_PROJECT or "
        "GOOGLE_CLOUD_PROJECT. See howto/earth_engine_auth.md."
    )


def _probe_initialize() -> tuple[bool, str]:
    from moirais.earth import MissingCredentialsError, _ensure_ee_initialized
...
def _emit(results: list[dict[str, Any]], *, as_json: bool) -> None:
    ...
    print("=" * 66)
    print("  moirais verify-earth-engine")
    print("=" * 66)
    for r in results:
...
def register_subparser(subparsers) -> None:
    """Wire the subcommand into moirais.runner's argparse tree."""
