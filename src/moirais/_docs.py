"""Sphinx documentation helpers for package-facing MOIRAIS pages."""
...
MODULE_DOC_SPECS: dict[str, ModuleDocSpec] = {
    ...
    "power-design": ModuleDocSpec(
        "Power Design", "investigation", "surveillance/investigation/05_power_design.R", "moirais.run_power_design_module"
    ),
    "logistic-models": ModuleDocSpec(
        "Logistic Models",
        "investigation",
        "surveillance/investigation/06_logistic.R",
        "moirais.run_weighted_logistic_analysis",
    ),
    "model-comparison": ModuleDocSpec(
        "Model Comparison",
        "investigation",
        "surveillance/investigation/06_model_comparison.R",
        "moirais.compare_nested_logistic_models",
    ),
    ...
    "propensity-scores": ModuleDocSpec(
        ...
        "moirais.run_propensity_ipw_analysis",
    ),
    ...
    "treatment-effects": ModuleDocSpec(
        ...
        "moirais.run_treatment_effects_analysis",
    ),
    ...
    "ebac-selection-adjustment-ipw": ModuleDocSpec(
        ...
        "moirais.run_ebac_selection_ipw_analysis",
    ),
    ...
    for module_name, doc_spec, module_spec in specs:
        lines.extend(
            [
                ...
                "**Entrypoints**",
                "",
                f"- CLI: `moirais run-module {module_name} --cpads-csv <path> --output-dir <dir>`",
                f'- Python: `moirais.run_module("{module_name}", cpads_csv=..., output_dir=...)`',
                f'- R: `run_moirais_module("{module_name}", cpads_csv = ..., output_dir = ...)`',
            ]
        )
...
def write_artifact_inventory_page(docs_path: str | Path, repo_root: str | Path | None = None) -> Path:
    ...
    lines = [
        ...
        "",
        "This page lists the currently published output files for each MOIRAIS module.",
