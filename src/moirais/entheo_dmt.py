"""moirais.entheo_dmt — loaders + analyses for the Timmermann 2023
...
This module exposes two layers:
  ...
  primitives in moirais.fn.* once those are wired up for EEG.
...
def dataset_overview() -> RichResult:
    ...
    return RichResult(
        ...
        interpretation=(
            ...
            "are stubbed — wire to moirais.fn Rangayyan-style "
...
# ── Layer 2: analyses (wired to moirais.fn Rangayyan primitives) ───
...
def spectral_band_power(signal: np.ndarray, *,
    ...
    Wraps :func:`moirais.fn.psdwl.psdwl` to estimate the PSD, then
...
def lz_complexity(signal: np.ndarray, *,
    ...
    Wraps :func:`moirais.fn.lzcmp.lempel_ziv_complexity`.  The
    ...
    # moirais.fn.lzcmp returns ESRes with .estimate (raw LZ count) and
