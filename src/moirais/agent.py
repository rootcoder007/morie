"""Agentic loop for Perseus — MOIRAIS's resident AI with tool calling."""
...
def tool_get_cheatsheet(name: str) -> str:
    """Get the cheatsheet for an moirais function (usage, signature, examples)."""
    try:
        mod = __import__(f"moirais.fn.{name}", fromlist=["cheatsheet"])
...
def tool_search_codebase(pattern: str, *, file_glob: str = "*.py", max_results: int = 20, sandbox: Path | None = None) -> str:
    """Search the moirais codebase for a pattern (grep-like)."""
    import re

    sandbox = sandbox or _find_project_root()
    moirais_src = sandbox / "libexec" / "config" / "tools" / "py-package" / "moirais"
    if not moirais_src.is_dir():
        moirais_src = sandbox
    ...
    matches: list[str] = []
    for path in sorted(moirais_src.rglob(file_glob)):
...
def tool_describe_data(code: str = "") -> str:
    """Describe a dataset: load from moirais.data or describe a DataFrame expression."""
    stdout_buf = io.StringIO()
    try:
        exec_globals: dict[str, Any] = {}
        exec(
            "import pandas as pd; import numpy as np; "
            "from moirais.data import load_dataset, DATASET_CATALOG; "
            + code,
            exec_globals,
        )
...
def tool_run_moirais_function(name: str, kwargs: dict | None = None) -> str:
    """Run any registered moirais fn/ function by name."""
    try:
        from .fn._registry import REGISTRY
    except ImportError:
        return "Registry not available"
    ...
    try:
        mod = __import__(f"moirais.fn.{name}", fromlist=[name])
        fn = getattr(mod, name)
    except Exception as exc:
        return f"Error importing moirais.fn.{name}: {exc}"
...
def tool_recommend_analysis(question: str) -> str:
    """Given a research question, recommend specific moirais functions and analysis workflow."""
    q = question.lower()
    ...
    lines.append("  2. run_moirais_function(name, kwargs) to execute")
...
def tool_category_tree() -> str:
    ...
    lines = [f"=== MOIRAIS FUNCTION TAXONOMY -- {len(REGISTRY)} functions, {len(cats)} categories ===\n"]
...
def tool_run_pipeline(steps: str) -> str:
    """Run a sequence of moirais functions as a pipeline. Results from step N available as $N in later steps.
    ...
    for i, step in enumerate(step_list):
        ...
        try:
            mod = __import__(f"moirais.fn.{fn_name}", fromlist=[fn_name])
...
_CORE_TOOLS: list[dict] = [
    {
        ...
        "function": {
            ...
            "description": (
                "Search the moirais fn/ registry (5710+ functions) by keyword. "
                "Returns names, categories, descriptions. Use before run_moirais_function."
            ),
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "run_moirais_function",
            "description": ("Run any moirais fn/ function by short name. Use search_functions first to find names."),
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
    ...
    },
    {
        ...
        "function": {
            "name": "get_cheatsheet",
            "description": "Get the usage cheatsheet for an moirais function (signature, purpose, examples).",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Search the moirais source code for a pattern (regex). Useful for finding implementations.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                "Given a research question or analysis goal, recommend specific moirais functions "
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "Quick overview of everything available in moirais."
            ),
    ...
    },
    {
        ...
        "function": {
            "name": "run_pipeline",
            "description": (
                "Run a sequence of moirais functions as a pipeline. Each step's result is available as $N in later steps. "
...
_TOOL_DISPATCH: dict[str, Any] = {
    ...
    "run_moirais_function": lambda **kw: tool_run_moirais_function(**kw),
...
_SYSTEM_PROMPT_FULL = (
    "You are Perseus, the MOIRAIS demigod — an autonomous scientific computing agent with "
    ...
    "- run_moirais_function(name, kwargs): call any function by short name\n"
    ...
    "get_cheatsheet -> run_moirais_function or run_suite -> verify -> report.\n"
    "Never guess function names. Never fabricate results. Show real tool output."
)

_SYSTEM_PROMPT_COMPACT = (
    "You are Perseus, MOIRAIS's scientific computing demigod. ALWAYS use tools.\n"
    ...
    "run_moirais_function(name, kwargs) to execute.\n"
...
class PerseusAgent:
    ...
    def _build_tool_definitions(self) -> list[dict]:
        if _is_small_model(self._model):
            core_names = {
                "search_functions", "run_moirais_function", "get_cheatsheet", "execute_code",
...
_TEXT_TOOL_PROMPT = (
    "You are Perseus, MOIRAIS's scientific computing demigod. "
    ...
    "- run_moirais_function(name, kwargs): Run any moirais function by short name\n"
...
class FreeAPIAgent:
    ...
    def _send(self, messages: list[dict[str, str]], model: str | None = None) -> str:
        ...
        if model is None:
            model = os.environ.get("moiraisfam", "mistral-nemo:custom")  # noqa: SIM112
...
def create_agent(
    ...
    **kwargs: Any,
    ...
    - MOIRAIS_PI_HOST: Pi hostname (used to construct Ollama URL)
    """
    ...
    if base_url is None:
        ...
        if base_url == _DEFAULT_BASE_URL:
            pi_host = os.environ.get("MOIRAIS_PI_HOST")
            if pi_host:
...
def warmup(model: str | None = None, base_url: str | None = None) -> bool:
    ...
    once when moirais starts — after that, tool-calling queries take 2-10s
    instead of 30-120s.
    """
    try:
        import httpx
