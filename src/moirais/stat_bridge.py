"""
Bridge between Go TIDE and Python moirais statistical functions.

Three modes:
  python -m moirais.stat_bridge registry-json   -> JSON dump for Go tree
  python -m moirais.stat_bridge help             -> formatted help text
  python -m moirais.stat_bridge exec "cmd args"  -> run command, print result
"""
...
def help_text() -> str:
    ...
    lines = ["MOIRAIS Statistical Functions", "=" * 40, ""]
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python -m moirais.stat_bridge <registry-json|help|exec 'cmd'>")
        sys.exit(1)
    ...
    if mode == "registry-json":
        ...
        try:
            if len(sys.argv) >= 4 and sys.argv[2] == "--config":
                from moirais._migration import build_migration_inventory, load_mapping_from_json
                ...
                from moirais._migration import MIGRATION_INVENTORY
        ...
        try:
            if len(sys.argv) >= 4 and sys.argv[2] == "--config":
                from moirais._parity import build_parity_matrix, load_parity_config
                ...
                from moirais._parity import PARITY_MATRIX
        ...
        if len(sys.argv) < 4:
            print("Usage: python -m moirais.stat_bridge compare-dirs <source> <target>")
            sys.exit(1)
        source, target = sys.argv[2], sys.argv[3]
        try:
            from moirais._migration import compare_trees
        ...
        if len(sys.argv) < 4:
            print("Usage: python -m moirais.stat_bridge function-parity <source> <target>")
            sys.exit(1)
        source, target = sys.argv[2], sys.argv[3]
        try:
            from moirais._parity import compare_function_parity, summarize_function_parity
        ...
        try:
            from moirais._parity import TIDE_PARITY, summarize_tide_parity
