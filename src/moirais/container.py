"""Docker container management for MOIRAIS.

Provides Python wrappers around Docker and Docker Compose CLI commands for
building, running, inspecting, and managing MOIRAIS analysis containers.  All
...
* Running MOIRAIS modules inside containers with volume-mounted data.
...
MOIRAIS_IMAGE_NAME = "moirais"
MOIRAIS_IMAGE_TAG = "latest"
MOIRAIS_CONTAINER_PREFIX = "moirais-run"
DEFAULT_WORKDIR = "/moirais"
DEFAULT_DATA_MOUNT = "/moirais/data"
DEFAULT_OUTPUT_MOUNT = "/moirais/output"
...
@dataclass
class ContainerInfo:
    """Metadata for a running or stopped MOIRAIS container."""
...
@dataclass
class ImageInfo:
    """Metadata for an MOIRAIS Docker image."""
...
class HealthCheckResult:
    ...
    def all_ok(self) -> bool:
        ...
        return self.python_ok and self.r_ok and self.moirais_importable and self.data_accessible
...
def build_image(
    ...
    *,
    image_name: str = MOIRAIS_IMAGE_NAME,
    tag: str = MOIRAIS_IMAGE_TAG,
    ...
    """Build the MOIRAIS Docker image.

    Wraps ``docker build`` with sensible defaults for the MOIRAIS project.
    ...
    image_name : str
        Image repository name (default ``moirais``).
    tag : str
...
def build_multistage(
    ...
    *,
    ...
    final_tag: str = MOIRAIS_IMAGE_TAG,
    timeout: int = 1800,
) -> list[DockerResult]:
...
def list_images(
    *,
    filter_moirais: bool = True,
) -> list[ImageInfo]:
    """List Docker images, optionally filtered to MOIRAIS images.

    Parameters
    ----------
    filter_moirais : bool
        If True, only return images with repository matching MOIRAIS_IMAGE_NAME.
...
def cleanup_images(
    *,
    ...
    """
    images = list_images(filter_moirais=True)
    results: list[DockerResult] = []
...
def push_image(
    image: str = f"{MOIRAIS_IMAGE_NAME}:{MOIRAIS_IMAGE_TAG}",
    *,
...
def run_container(
    *,
    ...
    data_dir : str | Path | None
        Host directory to mount at ``/moirais/data``.
    output_dir : str | Path | None
        Host directory to mount at ``/moirais/output``.
    config_dir : str | Path | None
        Host directory to mount at ``/moirais/config``.
    command : str | None
    ...
    if name is None:
        ...
        name = f"{MOIRAIS_CONTAINER_PREFIX}-{ts}"
    ...
    if config_dir is not None:
        ...
        cmd.extend(["-v", f"{config_path}:/moirais/config:ro"])
...
def list_containers(
    *,
    ...
    filter_moirais : bool
        Only show containers with names starting with the MOIRAIS prefix.
    ...
    if all_states:
        cmd.append("-a")
    if filter_moirais:
        cmd.extend(["--filter", f"name={MOIRAIS_CONTAINER_PREFIX}"])
...
def cleanup_containers(
    *,
    ...
    list[DockerResult]
    """
    containers = list_containers(all_states=True, filter_moirais=True)
    results: list[DockerResult] = []
...
def inspect_container(
    ...
    *,
    ...
    container : str
        Container name or ID.
    list_files : bool
        List files in the MOIRAIS working directory.
    check_versions : bool
...
def health_check(
    ...
    *,
    ...
    Verifies that Python, R, and the ``moirais`` package are importable and
    ...
    # moirais importable
    moirais_check = exec_in_container(
        container,
        'python -c "import moirais; print(moirais.__version__)"',
    )
    moirais_importable = moirais_check.success
    moirais_version = moirais_check.stdout.strip() if moirais_check.success else "not importable"
    checks.append({"name": "moirais", "ok": moirais_importable, "detail": moirais_version})
    ...
    return HealthCheckResult(
        ...
        r_ok=r_ok,
        r_version=r_version,
        moirais_importable=moirais_importable,
        moirais_version=moirais_version,
...
def verify_pipeline(
    ...
    *,
    ...
    """Run MOIRAIS modules inside a container and verify outputs.
    ...
    if modules is None:
        mod_list = exec_in_container(
            container,
            "python -m moirais.runner list-modules",
        )
        if mod_list.success:
            modules = [
    ...
    for mod in modules:
        cmd_parts = ["python", "-m", "moirais.runner", "run-module", mod, "--output-dir", output_dir]
        if cpads_csv:
...
def run_ci_simulation(
    *,
    ...
    return run_container(
        image=image,
        name=f"moirais-ci-{int(time.time())}",
        data_dir=data_dir,
...
def snapshot_container(
    ...
    *,
    ...
    if tag is None:
        ts = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
        tag = f"{MOIRAIS_IMAGE_NAME}:snapshot-{ts}"
    else:
        if ":" not in tag:
            tag = f"{MOIRAIS_IMAGE_NAME}:{tag}"
...
def create_network(
    name: str = "moirais-net",
    *,
    driver: str = "bridge",
) -> DockerResult:
    """Create a Docker network for MOIRAIS services.
...
def remove_network(name: str = "moirais-net") -> DockerResult:
...
def generate_dockerfile(
    *,
    ...
    """Generate a Dockerfile for the current MOIRAIS environment.

    Creates a multi-stage Dockerfile that installs Python, optionally R
    and Quarto, and the moirais package with requested extras.
    ...
    dockerfile = textwrap.dedent(f"""\
        # =============================================================================
        # MOIRAIS Dockerfile — auto-generated
        ...
        EXPOSE 8888
        CMD ["python", "-m", "moirais.runner", "--help"]
    """)
...
def generate_compose_file(
    *,
    ...
    """Generate a docker-compose.yml for MOIRAIS services.
    ...
    compose = textwrap.dedent(f"""\
        # MOIRAIS Docker Compose — auto-generated
        version: "3.8"

        services:
          moirais:
            build: .
            image: {MOIRAIS_IMAGE_NAME}:{MOIRAIS_IMAGE_TAG}
            volumes:
              - ./data:/moirais/data:ro
              - ./output:/moirais/output
            environment:
              - MOIRAIS_CONTAINER=1
              - MOIRAIS_OLLAMA_HOST=ollama
            networks:
              - moirais-net
            depends_on:
              - ollama
...
def render_health_check(result: HealthCheckResult) -> None:
    ...
    try:
        ...
        console = Console()
        table = Table(
            title="MOIRAIS Container Health Check",
            box=box.ROUNDED,
