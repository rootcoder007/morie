"""moirais.earth — geospatial pollution-exposure data ingestion.

Pulls satellite + ground-station air-quality data into pandas DataFrames
that plug into the rest of MOIRAIS (causal-inference library, Canadian
...
Design goals:
  ...
  - MOIRAIS-native outputs: everything is a DataFrame ready for
    ``moirais.causal`` / ``moirais.fn.dml`` / ``moirais.fn.aipw`` / etc.
"""
...
def earth_cache_dir() -> pathlib.Path:
    ...
    base = os.environ.get(
        "MOIRAIS_EARTH_CACHE",
        os.path.expanduser("~/.cache/moirais/earth"),
    )
    p = pathlib.Path(base)
...
Google Earth Engine requires authentication. Two options:
  ...
  (1) Service account (headless, recommended for MOIRAIS):
      ...
      e. Set MOIRAIS_EE_SERVICE_ACCOUNT and MOIRAIS_EE_KEY_PATH
...
def _ensure_ee_initialized():
    ...
    sa_email = os.environ.get("MOIRAIS_EE_SERVICE_ACCOUNT")
    key_path = os.environ.get("MOIRAIS_EE_KEY_PATH") or os.environ.get("GOOGLE_APPLICATION_CREDENTIALS")
    project = os.environ.get("MOIRAIS_EE_PROJECT") or os.environ.get("GOOGLE_CLOUD_PROJECT")
...
def fetch_earth_engine(
    ...
    Example (port of your Toronto NO2 app to MOIRAIS):
...
or an ArcGIS Online / Enterprise account.
  ...
  Auth (pick one):
    ...
    # Via env var
    export MOIRAIS_ARCGIS_URL=https://your-org.maps.arcgis.com
    export MOIRAIS_ARCGIS_USER=...
    export MOIRAIS_ARCGIS_PASS=...
