"""MOIRAIS environment self-diagnostics.

``moirais doctor`` checks that all required components are installed and
...
def _check_freeapi() -> tuple[bool, str]:
    ...
    try:
        ...
        return False, "moirais.fam not available"
    except Exception as e:
        return False, f"error: {e}"


def _check_datasets() -> tuple[bool, str]:
    """Check built-in MOIRAIS datasets database."""
    try:
        from .data import moirais_db, list_datasets

        db_path = moirais_db()
        if not db_path.exists():
            return False, "moirais.db not found — reinstall moirais"
...
def _render_plain(results: dict[str, Any]) -> None:
    print("MOIRAIS Doctor — environment diagnostics")
    print("=" * 50)
...
def _render_rich(results: dict[str, Any]) -> None:
    ...
    console = Console()
    table = Table(
        title="MOIRAIS Doctor — environment diagnostics",
