"""moirais.run_crime_analysis — one-command end-to-end pipeline.

Usage::

    from moirais import run_crime_analysis
...
def render_figures() -> dict[str, str]:
    ...
    import subprocess
    result = {}
    r_script = PROJECT / "r-package/moirais/R/viz_ggplot.R"
    if r_script.exists():
        try:
            cmd = ["Rscript", "-e",
                   f'source("{r_script}"); moirais_render_all()']
...
def full(sample_rows: int | None = 50_000,
    ...
    """
    t0 = time.time()
    print("MOIRAIS — full crime-analysis pipeline")
    print("=" * 60)
