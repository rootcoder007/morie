"""JIT-ready acceleration layer for moirais hot paths.
...
When numba is NOT importable (the current state of moirais's
...
To actually get the JIT speedup, run moirais on a 3.10–3.14 venv
where `pip install numba` succeeds. Track:

    >>> from moirais._jit import is_jit_available
    >>> is_jit_available()
    True   # ← only when numba imported

This module is *opt-in* — `moirais` proper doesn't import it eagerly.
fn/ files that want acceleration import via:

    from moirais._jit import normal_pdf
