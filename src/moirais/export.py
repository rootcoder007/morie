...
def df_to_html(
    ...
    *,
    ...
    if title:
        parts.append(f'<h4 class="moirais-table-title">{title}</h4>')
    parts.append(html)
    if responsive:
        parts.append("</div>")
    ...
    style = """
<style>
.moirais-table { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
  font-size: 13px; border-collapse: collapse; width: 100%; }
.moirais-table th { background: #f8f9fa; border-bottom: 2px solid #dee2e6; padding: 8px 12px; text-align: left; }
.moirais-table td { padding: 6px 12px; border-bottom: 1px solid #eee; }
.table-striped tbody tr:nth-child(odd) { background: #f8f9fa; }
.table-hover tbody tr:hover { background: #e9ecef; }
.moirais-table-title { margin-bottom: 8px; color: #333; }
</style>
"""
    return style + "\n".join(parts)
...
def generate_citation(
    ...
    *,
    ...
    doi: str = "",
    url: str = "https://github.com/hadesllm/moirais",
    output_format: str = "bibtex",
) -> str:
    """
    Generate a citation entry for the analysis or the MOIRAIS package.
...
def create_reproducibility_manifest(
    ...
    *,
    ...
    for pkg in ["pandas", "numpy", "scipy", "sklearn", "statsmodels", "matplotlib", "moirais"]:
        try:
            mod = __import__(pkg)
...
def latex_preamble(
    *,
    ...
    """
    Generate a LaTeX preamble with required packages for MOIRAIS tables.
