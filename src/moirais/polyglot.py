...
class PolyglotEngine:
    ...
    def __init__(
        ...
        self._py_ns: dict = {"__name__": "__moirais_repl__", "__builtins__": __builtins__}
    ...
    def _preload_python(self) -> None:
        ...
        try:
            ...
            for key, entry in REGISTRY.items():
                try:
                    mod = __import__(f"moirais.fn.{key}", fromlist=[entry.func_name])
    ...
    def _start_sql(self) -> bool:
        ...
        try:
            ...
            db_path = os.environ.get("MOIRAIS_DB", ":memory:")
            if db_path == ":memory:":
                try:
                    from moirais.data import _builtin_db_path
    ...
    def _exec_r(self, code: str) -> ExecResult:
        ...
        sentinel = f"__MOIRAIS_{id(code)}__"
        try:
    ...
    def _exec_latex(self, code: str) -> ExecResult:
        ...
        tmpdir = tempfile.mkdtemp(prefix="moirais_tex_")
    ...
    def _exec_psql(self, code: str) -> ExecResult:
        db_url = os.environ.get("MOIRAIS_PGURL", os.environ.get("DATABASE_URL", ""))
        if db_url:
    ...
    def _exec_java(self, code: str) -> ExecResult:
        ...
        tmpdir = tempfile.mkdtemp(prefix="moirais_java_")
...
def run_headless_repl(
    ...
    print(f"MOIRAIS Polyglot REPL — {len(langs)} languages: {', '.join(langs)}")
    if polyglot:
    ...
    try:
        ...
        print(f"moirais.fn loaded ({len(REGISTRY)} functions)")
    except ImportError:
        pass
