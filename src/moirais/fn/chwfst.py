...
def chow_forecast_test(y, X, split, cdf=None):
    ...
    n = len(y)
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Chow forecast test for parameter constancy"})
    x_sorted = np.sort(y)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Chow forecast test for parameter constancy"})
