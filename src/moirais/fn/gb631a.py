...
def gibbons_ks2_asymp(x, y, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Asymptotic distribution of two-sample KS statistic"})
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Asymptotic distribution of two-sample KS statistic"})
