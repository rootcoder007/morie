...
def ca_chapter_8_unnumbered_308(x):
    """
    ANOVA expression involving 'effect' (auto-extracted; see reference for full context).

    Formula: pwr.anova.test(k = 3, n = 100, f = 0.1,
    ...
    result : RichResult
        ...
        See ``moirais.fn.describe('ca8u308')`` for the full guide.
    ...
    return RichResult(
        title="ANOVA expression involving 'effect' (auto-extracted; see reference for full context).",
        summary_lines=[
            ("Estimate", result),
            ("Standard error", se),
            ("n", n),
...
def cheatsheet():
    return "ca8u308: ANOVA expression involving 'effect' (auto-extracted; see reference for full context)."
