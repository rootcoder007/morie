...
def ca_chapter_10_unnumbered_339(x):
    """
    CausalInference expression involving 'subclasses' (auto-extracted; see reference for full context).

    Formula: data = df_name, method = "subclass" , subclass=10)
    ...
    result : RichResult
        ...
        See ``moirais.fn.describe('ca10u339')`` for the full guide.
    ...
    return RichResult(
        title="CausalInference expression involving 'subclasses' (auto-extracted; see reference for full context).",
        summary_lines=[
            ("Estimate", result),
            ("Standard error", se),
            ("n", n),
