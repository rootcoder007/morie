# describe('cpbbs') — bbs_changepoint

## WHAT IT DOES

Binary segmentation for change-point detection. 'Errors using inadequate data are much less than those using no data at all. — Charles Babbage'
...
`print(result)` render. See `moirais.fn.describe('cpbbs')` for guidance.

## WORKED EXAMPLE

```python
from moirais.fn.cpbbs import *
import numpy as np
# See the function signature in fn/cpbbs.py for argument names.
```
