...
def ca_chapter_8_unnumbered_304(x):
    """
    CentralTendency expression involving 'difference' (auto-extracted; see reference for full context).

    Formula: pwr.t.test(n = NULL, d = NULL, sig.level = 0.05,
    ...
    result : RichResult
        ...
        See ``moirais.fn.describe('ca8u304')`` for the full guide.
    ...
    return RichResult(
        title="CentralTendency expression involving 'difference' (auto-extracted; see reference for full context).",
        summary_lines=[
            ("Estimate", result),
            ("Standard error", se),
            ("n", n),
