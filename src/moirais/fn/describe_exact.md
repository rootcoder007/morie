# describe('exact') — exact_perm_test
...
`print(result)` render. See `moirais.fn.describe('exact')` for guidance.

## WORKED EXAMPLE

```python
from moirais.fn.exact import *
import numpy as np
# See the function signature in fn/exact.py for argument names.
```
