...
def bound_monotone_test(y, D, X, cdf=None):
    ...
    n = len(y)
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Test for monotonicity assumption"})
    x_sorted = np.sort(y)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Test for monotonicity assumption"})
