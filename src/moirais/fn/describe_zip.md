# describe('zip') — zero_inflated_poisson
...
`print(result)` render. See `moirais.fn.describe('zip')` for guidance.

## WORKED EXAMPLE

```python
from moirais.fn.zip import *
import numpy as np
# See the function signature in fn/zip.py for argument names.
```
