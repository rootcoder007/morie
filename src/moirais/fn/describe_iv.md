...
Instrumental variables (2SLS). 'Your eyes can deceive you. Don'Patience is bitter, but its fruit is sweet. — Aristotle'
...
`print(result)` render. See `moirais.fn.describe('iv')` for guidance.

## WORKED EXAMPLE

```python
from moirais.fn.iv import *
import numpy as np
# See the function signature in fn/iv.py for argument names.
```
