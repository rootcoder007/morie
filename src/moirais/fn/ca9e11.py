...
def ca_chapter_9_equation_11(x, cdf=None):
    ...
    result : RichResult
        ...
        See ``moirais.fn.describe('ca9e11')`` for the full guide.
    ...
    if n < 2:
        return hypothesis_test_result(
            ...
            extra_summary=[("n", n)],
            extra_payload={"n": n, "method": "GeneralStatistics expression involving 'smaller' (auto-extracted; see reference for full context).", "p_value": float("nan")},
        )
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return hypothesis_test_result(
        ...
        extra_summary=[("n", n)],
        extra_payload={"n": n, "method": "GeneralStatistics expression involving 'smaller' (auto-extracted; see reference for full context).", "p_value": float(p_value)},
    )
