# describe('cv') — coefficient_of_variation
...
`print(result)` render. See `moirais.fn.describe('cv')` for guidance.

## WORKED EXAMPLE

```python
from moirais.fn.cv import *
import numpy as np
# See the function signature in fn/cv.py for argument names.
```
