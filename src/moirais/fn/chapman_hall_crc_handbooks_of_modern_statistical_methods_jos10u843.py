...
__all__ = ["chapman_hall_crc_handbooks_of_modern_statistical_methods_jos_chapter_10_unnumbered_843"]


def chapman_hall_crc_handbooks_of_modern_statistical_methods_jos_chapter_10_unnumbered_843(x, cdf=None):
    """
    GeneralStatistics expression (auto-extracted; see ref).

    Formula: [EQ] ˆµ(·) = M(Y ∼ [X, W]; O), ˆτ(x) = ˆµ([x, 1]) − ˆµ([x, 0]). (21.5)
    ...
    result : RichResult
        ...
        See ``moirais.fn.describe('chapman_hall_crc_handbooks_of_modern_statistical_methods_jos10u843')`` for the full guide.
...
def cheatsheet():
    return "chapman_hall_crc_handbooks_of_modern_statistical_methods_jos10u843: GeneralStatistics expression (auto-extracted; see ref)."
