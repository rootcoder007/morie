...
def guide_on_data_analysis_chapter_1_equation_63(x, cdf=None):
    ...
    result : RichResult
        ...
        See ``moirais.fn.describe('guide_on_data_analysis1e63')`` for the full guide.
    ...
    if n < 2:
        return hypothesis_test_result(
            test_name="GeneralStatistics equation extracted from guide on data analysis.",
            statistic=float("nan"),
            pvalue=float("nan"),
            warnings=["n<2: insufficient data."],
            extra_summary=[("n", n)],
            extra_payload={"n": n, "method": "GeneralStatistics equation extracted from guide on data analysis.", "p_value": float("nan")},
        )
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return hypothesis_test_result(
        ...
        extra_summary=[("n", n)],
        extra_payload={"n": n, "method": "GeneralStatistics equation extracted from guide on data analysis.", "p_value": float(p_value)},
    )
