# describe('bldal') — bland_altman

## WHAT IT DOES

Bland-Altman agreement analysis. 'Our greatest glory is not in never falling, but in rising every time we fall. — Confucius'
...
`print(result)` render. See `moirais.fn.describe('bldal')` for guidance.

## WORKED EXAMPLE

```python
from moirais.fn.bldal import *
import numpy as np
# See the function signature in fn/bldal.py for argument names.
```
