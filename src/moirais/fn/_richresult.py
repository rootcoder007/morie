"""Rich-output result containers for moirais.fn.
...
def hypothesis_test_result(
    *,
    ...
    `callable_name` (optional) — short name of the moirais.fn callable
...
def _detect_caller_name() -> str | None:
    """Walk back the call stack to find the moirais.fn callable that
    ...
    try:
        ...
        if frame is None:
            return None
        # If the caller is in moirais/fn/<name>.py, fn name should be
        # the same as the module file name.
        mod = inspect.getmodule(frame)
        if mod is None:
            return None
        modname = mod.__name__
        if "moirais.fn." not in modname:
            return None
        # Strip "moirais.fn." prefix
        short = modname.split("moirais.fn.")[-1]
...
def _describe_pointer(callable_name: str) -> str:
    ...
    return (f"Learn more: `from moirais.fn import describe; "
