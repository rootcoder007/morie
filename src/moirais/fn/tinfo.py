...
def test_information(y, theta, a, b, cdf=None):
    ...
    n = len(y)
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Test information function (sum of item infos)"})
    x_sorted = np.sort(y)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Test information function (sum of item infos)"})
