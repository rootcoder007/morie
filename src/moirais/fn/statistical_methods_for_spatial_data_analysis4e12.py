...
def statistical_methods_for_spatial_data_analysis_chapter_4_equation_12(x, cdf=None):
    ...
    result : RichResult
        ...
        See ``moirais.fn.describe('statistical_methods_for_spatial_data_analysis4e12')`` for the full guide.
    ...
    if n < 2:
        return hypothesis_test_result(
            test_name="Spatial equation extracted from Schabenberger & Gotway (2005) Statistical Methods for Spatial Data Analysis.",
            statistic=float("nan"),
            pvalue=float("nan"),
            warnings=["n<2: insufficient data."],
            extra_summary=[("n", n)],
            extra_payload={"n": n, "method": "Spatial equation extracted from Schabenberger & Gotway (2005) Statistical Methods for Spatial Data Analysis.", "p_value": float("nan")},
        )
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return hypothesis_test_result(
        ...
        extra_summary=[("n", n)],
        extra_payload={"n": n, "method": "Spatial equation extracted from Schabenberger & Gotway (2005) Statistical Methods for Spatial Data Analysis.", "p_value": float(p_value)},
    )
