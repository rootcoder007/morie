# describe('nb_') — naive_bayes_fit

## WHAT IT DOES

Gaussian Naive Bayes (pure numpy). 'Our greatest glory is not in never falling, but in rising every time we fall. — Confucius'
...
`print(result)` render. See `moirais.fn.describe('nb_')` for guidance.

## WORKED EXAMPLE

```python
from moirais.fn.nb_ import *
import numpy as np
# See the function signature in fn/nb_.py for argument names.
```
