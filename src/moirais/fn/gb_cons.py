...
def gibbons_consistency(T, H1, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Consistency definition: test statistic power -> 1 as n -> inf"})
    x_sorted = np.sort(T)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Consistency definition: test statistic power -> 1 as n -> inf"})
