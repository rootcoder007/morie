"""MOIRAIS self-test — comprehensive smoke test of all subsystems.

Verifies that every major component of the MOIRAIS IDE is functional
...
def _test_new_modules():
    ...
    new_mods = [
        ...
        "moirais.survival",
        "moirais.missing",
        "moirais.effect_sizes",
        "moirais.multiple_testing",
        "moirais.diagnostics",
        "moirais.sensitivity",
        "moirais.bootstrap_methods",
        "moirais.did",
        "moirais.rdd",
        "moirais.iv",
        "moirais.matching",
...
def _test_chat_session():
    ...
    from moirais.chat import ChatSession
...
def _test_tui_screens():
    ...
    try:
        from moirais.tui import _TEXTUAL_AVAILABLE, MOIRAISApp
    except ImportError:
    ...
    async def _verify():
        ...
        async with app.run_test() as pilot:
            assert pilot.app.title == "MOIRAIS"
            # Doctor screen.
            await pilot.press("d")
...
def _test_progress_tracker():
    ...
    from moirais.progress import PipelineTracker

    with patch("moirais.progress.run_module") as mock_run:
...
def _test_inspector():
    ...
    from moirais.inspector import inspect_output
...
def _test_verify():
    ...
    from moirais.inspector import verify_statistical_output
...
def _test_llm_detection():
    ...
    from moirais.llm import detect_available_provider
...
def _test_datasets():
    ...
    from moirais.data import moirais_db, list_datasets

    db_path = moirais_db()
    if not db_path.exists():
        raise RuntimeError("Built-in moirais.db not found")
    ds = list_datasets()
...
def run_selftest() -> int:
    ...
    if use_rich:
        console = Console()
        table = Table(
            title="MOIRAIS Self-Test Results",
