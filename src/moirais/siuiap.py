"""moirais.siuiap — Structured Intervention Unit Implementation Advisory Panel (federal).

NOTE: this is the FEDERAL Structured Intervention Unit Implementation
Advisory Panel module. It is distinct from `moirais.siu`, which is the
...
prefix collision with `moirais.siu/`.
...
In addition to SIU IAP panel reports, this module also indexes:
  ...
  - **Doob 2020 Federal Court affidavit** (T-539-20): expert evidence
    ...
    the source for the analyses replicated in `moirais.doob_trends`.
...
exists to make the federal context citable from moirais analyses.
...
AFFIDAVITS = {
    "doob_t_539_20_2020": {
        ...
        "notes": ("Source for Tables 1-3 + Figures 1-4 replicated in "
                   "moirais.doob_trends. CCRSO 2018 + StatsCan CANSIM "
                   "+ Pastore-Maguire (US)."),
    },
}
