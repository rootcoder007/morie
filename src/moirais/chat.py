"""Interactive chat REPL for MOIRAIS — Claude Code-like terminal experience.
...
Usage::
    ...
    moirais chat                        # default assistant
    moirais chat --agent moirais-architect # agent-specific persona
...
def load_agent_prompt(agent_name: str) -> str | None:
    ...
    ----------
    agent_name : str
        Agent filename stem (e.g., ``"moirais-architect"``).
...
class ChatSession:
    ...
    system_prompt : str | None
        Custom system prompt.  When ``None``, the default MOIRAIS agent
    ...
    def _default_system_prompt() -> str:
        context = build_moirais_context()
        context_block = _format_context_block(context)
        return _MOIRAIS_SYSTEM_PROMPT_TEMPLATE.format(context_block=context_block)
    ...
    def _register_default_commands(self) -> None:
        ...
        def _cmd_history(_args: list[str]) -> str:
            ...
            for msg in self.history:
                prefix = "You" if msg.role == "user" else "MOIRAIS"
...
def run_chat_repl(*, agent: str | None = None) -> int:
    ...
    if not sys.stdin.isatty():
        print("Error: moirais chat requires an interactive terminal.", file=sys.stderr)
        return 1
    ...
    console.print(
        Panel(
            f"[bold cyan]MOIRAIS Chat{agent_label}[/bold cyan]\n"
    ...
    histfile = os.path.expanduser("~/.moirais_chat_history")
    try:
    ...
    try:
        while True:
            try:
                prompt = f"[bold green]moirais{agent_label}>[/bold green] "
