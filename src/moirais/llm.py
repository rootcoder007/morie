"""Ollama-first LLM integration layer for the MOIRAIS package.
...
moiraisfam : str
    Override the OllamaFreeAPI model (moirais free api model).  Default: ``mistral-nemo:custom``.
GEMINI_API_KEY : str
...
_MOIRAIS_SYSTEM_PROMPT_TEMPLATE = """\
You are the MOIRAIS agent for methods for observational inference and robust analysis of interventions in sociolegal studies.

MOIRAIS is a Python+R terminal IDE for Canadian public health data analysis, \
causal inference, and reproducible research. Install: pip install moirais
...
CLI: moirais list-modules, moirais run-module <name>, moirais pipeline --all -y
CLI: moirais list-datasets, moirais doctor, moirais selftest, moirais ask "question"

32 built-in datasets: CPADS, CCS, CSADS, CSUS, HealthInfobase, CIHI.
Load: load('cpads') in REPL or from moirais.data import load_dataset
...
Debug: press d for Doctor, b for logs, moirais selftest for smoke tests.

Give practical answers with specific MOIRAIS commands. Be explicit about \
...
def _ollama_model() -> str:
    ...
    Priority:
    1. MOIRAIS_OLLAMA_MODEL env var (explicit override)
    ...
    # 1. Env var override
    env = os.environ.get("MOIRAIS_OLLAMA_MODEL", "").strip()
    if env:
...
def _probe_ollama(timeout: float = _PROBE_TIMEOUT) -> bool:
    ...
    Uses :class:`moirais.loc.LocalOllama` for the probe.
...
def _freeapi_model() -> str:
    ...
    return os.environ.get("moiraisfam", DEFAULT_FREEAPI_MODEL).strip()
...
def detect_available_provider() -> str:
    ...
    6. **local**   -- no live provider; MOIRAIS will return static help text.
...
def detect_provider_and_model() -> tuple[str, str]:
    ...
    if provider == _PROVIDER_GEMINI:
        model = os.environ.get("MOIRAIS_GEMINI_MODEL", DEFAULT_GEMINI_MODEL).strip()
        return provider, f"Gemini:{model}"

    if provider == _PROVIDER_API:
        model = os.environ.get("MOIRAIS_API_MODEL", DEFAULT_API_MODEL).strip()
        return provider, f"API:{model}"

    if provider == _PROVIDER_OPENAI:
        model = os.environ.get("MOIRAIS_OPENAI_MODEL", DEFAULT_OPENAI_MODEL).strip()
...
def build_moirais_context(repo_root: str | Path | None = None) -> dict[str, Any]:
    """Build an LLM-friendly context dictionary from the MOIRAIS package state.
    ...
    repo_root : str | Path | None
        Path to the MOIRAIS repository root.  When ``None`` the function
    ...
    if repo_root is None:
        # Attempt to resolve from file location: llm.py -> moirais/ -> py-package/ -> moirais-root/
        try:
...
def _retrieve_relevant_source(query: str, max_chars: int = 1500) -> str:
    ...
    Searches moirais module functions for keyword matches against the query,
    ...
    for mod_name in _MOIRAIS_MODULES:
        try:
        ...
        for name in dir(mod):
            ...
            if not obj_mod.startswith("moirais"):
                continue
...
def _collect_function_signatures() -> list[dict[str, str]]:
    """Introspect key moirais modules for function names + one-line descriptions."""
    import importlib

    sigs: list[dict[str, str]] = []
    for mod_name in _MOIRAIS_MODULES[:14]:
        try:
            mod = importlib.import_module(mod_name)
...
def _current_dataset_schema() -> dict[str, str] | None:
    """If a DataFrame is loaded in MOIRAISApp, return its column schema."""
    try:
...
def _format_context_block(context: dict[str, Any] | None) -> str:
    ...
    if modules:
        ...
        parts.append(f"Available MOIRAIS modules: {names}")
...
def _build_messages(
    ...
    if system_prompt is None:
        ...
        system_prompt = _MOIRAIS_SYSTEM_PROMPT_TEMPLATE.format(context_block=context_block)
...
def _freeapi_stream(
    messages: list[dict[str, str]],
    model: str | None = None,
    chunk_timeout: float = float(os.environ.get("MOIRAIS_LLM_CHUNK_TIMEOUT", "90")),
) -> Iterator[str]:
...
MOIRAIS is running in local-only mode (no LLM provider detected).

Available capabilities without an LLM:
  - moirais list-modules          List all analysis modules
  - moirais run-module <name>     Run a specific module against CPADS data
  - moirais pipeline --all -y     Run the full analysis pipeline
  - moirais assistant <question>  Ask the built-in rule-based agent
...
For more information, see: https://github.com/hadesllm/moirais
"""
...
def _local_fallback(prompt: str) -> str:
    ...
    if "ipw" in normalized or "propensity" in normalized or "causal" in normalized:
        sections.append(
            ...
            "Use `moirais run-module <name>` to execute."
        )
...
def ask(
    ...
    *,
    ...
    context : dict[str, Any] | None
        Optional context dictionary (e.g., from :func:`build_moirais_context`).
    ...
    system_prompt : str | None
        Override the entire system prompt.  When ``None``, the standard MOIRAIS
