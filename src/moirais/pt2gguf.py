...
Bridges autoresearch's train.py GPT architecture to MOIRAIS's inference engine.
Supports optional TurboQuant weight compression during conversion.

Usage:
    python -m moirais.pt2gguf --checkpoint model_baseline.pt --output moirais_model.gguf
    python -m moirais.pt2gguf --checkpoint model_baseline.pt --output moirais_3bit.gguf --turbo-bits 3
"""
...
def _turbo_compress_tensor(tensor_np, bits):
    from moirais.quant import turboquant_mse
...
def convert(checkpoint_path, output_path, tokenizer_dir=None, turbo_bits=0):
    ...
    kv_string("general.name", "moirais-autoresearch-gpt")
