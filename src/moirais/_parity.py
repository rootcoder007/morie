...
Three modes:
  ...
  2. MOIRAIS default: ``build_parity_matrix(epiml_root)`` — built-in epiml→moirais mapping.
  3. TIDE: ``build_tide_parity()`` — Python TUI vs Go TIDE feature parity.

Example (general-purpose)::

    from moirais._parity import compare_function_parity
...
MOIRAIS_SCRIPT_MAP = {
    "03_data_wrangling.R": {
        "target_moirais_api": "moirais run-module data-wrangling",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "04_descriptive_stats.R": {
        "target_moirais_api": "moirais run-module descriptive-statistics",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "04_distributions.R": {
        "target_moirais_api": "moirais run-module distribution-tests",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "05_frequentist.R": {
        "target_moirais_api": "moirais run-module frequentist-inference",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "05_bayesian.R": {
        "target_moirais_api": "moirais run-module bayesian-inference",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "05_power_design.R": {
        "target_moirais_api": "moirais run-module power-design",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "06_logistic.R": {
        "target_moirais_api": "moirais run-module logistic-models",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "06_model_comparison.R": {
        "target_moirais_api": "moirais run-module model-comparison",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "06_regression.R": {
        "target_moirais_api": "moirais run-module regression-models",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "07_propensity.R": {
        "target_moirais_api": "moirais run-module propensity-scores",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "07_causal_estimators.R": {
        "target_moirais_api": "moirais run-module causal-estimators",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "07_treatment_effects.R": {
        "target_moirais_api": "moirais run-module treatment-effects",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "07_dag.R": {
        "target_moirais_api": "moirais run-module dag-specification",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "07_meta_synthesis.R": {
        "target_moirais_api": "moirais run-module meta-synthesis",
        "target_docs_page": "modules/investigation",
        "parity_status": "scaffolded but statistically incomplete",
    },
    "08_figures.R": {
        "target_moirais_api": "moirais run-module figures",
        "target_docs_page": "modules/investigation",
        "parity_status": "scaffolded but statistically incomplete",
    },
    "08_tables.R": {
        "target_moirais_api": "moirais run-module tables",
        "target_docs_page": "modules/investigation",
        "parity_status": "scaffolded but statistically incomplete",
    },
    "09_report.R": {
        "target_moirais_api": "moirais run-module final-report",
        "target_docs_page": "modules/investigation",
        "parity_status": "already present",
    },
    "07_ebac.R": {
        "target_moirais_api": "moirais run-module ebac-core",
        "target_docs_page": "modules/ebac",
        "parity_status": "already present",
    },
    "07_ebac_ipw.R": {
        "target_moirais_api": "moirais run-module ebac-selection-adjustment-ipw",
        "target_docs_page": "modules/ebac",
        "parity_status": "already present",
    },
    "07_ebac_gender_smote_sensitivity.R": {
        "target_moirais_api": "moirais run-module ebac-gender-smote-sensitivity",
        "target_docs_page": "modules/ebac",
        "parity_status": "already present",
    },
    "07_ebac_integrations.R": {
        "target_moirais_api": "moirais run-module ebac-integrations",
        "target_docs_page": "modules/ebac",
        "parity_status": "already present",
    },
}

LEGACY_SCRIPT_MAP = MOIRAIS_SCRIPT_MAP
...
MOIRAIS_SURFACE_MAP = {
    ...
    "epiml_paths": "migrated as moirais_paths",
...
PACKAGE_SURFACE_MAP = MOIRAIS_SURFACE_MAP


def _current_r_exports(repo_root: Path, namespace_path: Path | None = None) -> set[str]:
    if namespace_path is None:
        namespace_path = repo_root / "r-package" / "moirais" / "NAMESPACE"
...
def build_parity_matrix(
    ...
    *,
    ...
    built-in MOIRAIS mappings.  Pass custom dicts to compare any
    two project trees.
    ...
    *file_globs* overrides the default ``["*.R"]`` pattern for
    ...
    used_script_map = script_map if script_map is not None else MOIRAIS_SCRIPT_MAP
    used_surface_map = surface_map if surface_map is not None else MOIRAIS_SURFACE_MAP
        ...
        rows.append(
            {
                ...
                "current_moirais_status": status,
                "target_moirais_api": spec["target_moirais_api"],
    ...
    if old_namespace.exists():
        for line in old_namespace.read_text(encoding="utf-8").splitlines():
            ...
            present = "moirais_paths" in r_exports if export_name == "epiml_paths" else export_name in r_exports
            ...
            rows.append(
                {
                    ...
                    "current_moirais_status": used_surface_map.get(export_name, status),
                    "target_moirais_api": export_name.replace("epiml_", "moirais_"),
        ...
        rows.append(
            {
                ...
                "current_moirais_status": status,
                "target_moirais_api": "module-compatible output writer",
...
def load_parity_config(config_path: str | Path) -> tuple[dict, dict]:
    ...
    Expected format::
        ...
        {
          "script_map": {"old.R": {"target_moirais_api": "...", ...}},
