"""Perseus — the MOIRAIS resident AI agent.

Delegates to the provider-chain in :mod:`moirais.llm`.  The LLM module handles
...
PERSEUS_SYSTEM_PROMPT = """You are Perseus, the MOIRAIS agent for methods for observational inference and robust analysis of interventions in sociolegal studies.
...
def _local_fallback_response(question: str, context: str | None = None) -> str:
    ...
    sections = [
        "MOIRAIS local agent mode is active because no live LLM provider was detected.",
    ]

    if "ipw" in normalized or "propensity" in normalized:
        sections.append(
            "MOIRAIS includes propensity-score and eBAC IPW workflows in both the package interfaces "
...
def ask_percy(
    ...
    *,
    ...
    moirais_context = build_moirais_context()

    try:
        full_prompt = build_prompt(question, context=context)
        output = llm_ask(
            full_prompt,
            context=moirais_context,
            stream=stream,
            model=model,
            provider=provider,
