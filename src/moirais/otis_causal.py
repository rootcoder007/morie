"""moirais.otis_causal — IPW, AIPW, and DML estimators for OTIS.

Companion to ``moirais.otis.otdml`` (which already implements
...
DML PLR:
    ...
    residuals (already implemented in :func:`moirais.otis.otdml`).
...
def make_pair_alert_to_volatility_naive(df: pd.DataFrame
    ...
    This was the original moirais formulation before we audited
...
def otis_dml(df: pd.DataFrame, *,
    ...
    Reproduces :func:`moirais.otis.otdml` but exposes the same
...
def _ensure_a01(df: pd.DataFrame | None,
    ...
    Resolution order:
      ...
      3. live CKAN download via
         :func:`moirais.otis_datasets.download_otis_dataset` (when
         ``auto_download=True``).
