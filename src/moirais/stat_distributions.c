...
/*
 * stat_distributions.c — MOIRAIS Statistical Distribution Functions
 *
 * References:
...
double moirais_lgamma(double x) {
    ...
    if (x < 0.5) {
        return log(M_PI / sin(M_PI * x)) - moirais_lgamma(1.0 - x);
    }
...
double moirais_gamma(double x) {
    return exp(moirais_lgamma(x));
}

double moirais_lbeta(double a, double b) {
    return moirais_lgamma(a) + moirais_lgamma(b) - moirais_lgamma(a + b);
}

double moirais_beta(double a, double b) {
    return exp(moirais_lbeta(a, b));
}
...
static double gammainc_series(double a, double x) {
    ...
    return sum * exp(-x + a * log(x) - moirais_lgamma(a));
}
...
static double gammainc_cf(double a, double x) {
    ...
    return 1.0 - result * exp(-x + a * log(x) - moirais_lgamma(a));
}
...
double moirais_gammainc_lower(double a, double x) {
    return moirais_gammainc_regularized(a, x) * moirais_gamma(a);
}

double moirais_gammainc_upper(double a, double x) {
    return (1.0 - moirais_gammainc_regularized(a, x)) * moirais_gamma(a);
}
...
double moirais_betainc(double a, double b, double x) {
    ...
    double front = exp(moirais_lgamma(a + b) - moirais_lgamma(a) - moirais_lgamma(b)
                       + a * log(x) + b * log(1.0 - x));
...
double moirais_norm_cdf(double x, double mu, double sigma) {
    if (sigma <= 0.0) return 0.0;
    double z = (x - mu) / sigma;
    return 0.5 * (1.0 + moirais_erf(z / M_SQRT2));
}

static double norm_cdf_std(double x) {
    return 0.5 * (1.0 + moirais_erf(x / M_SQRT2));
}
...
double moirais_t_pdf(double x, double df) {
    ...
    double coeff = exp(moirais_lgamma(half_df + 0.5) - moirais_lgamma(half_df))
                   / sqrt(df * M_PI);
...
double moirais_t_cdf(double x, double df) {
    ...
    double t2 = x * x;
    double v = df / (df + t2);
    double ib = moirais_betainc(0.5 * df, 0.5, v);
    if (x >= 0.0) {
...
double moirais_chisq_pdf(double x, double df) {
    ...
    double half_df = 0.5 * df;
    return exp((half_df - 1.0) * log(x) - 0.5 * x - half_df * log(2.0) - moirais_lgamma(half_df));
}

double moirais_chisq_cdf(double x, double df) {
    if (x < 0.0 || df <= 0.0) return 0.0;
    return moirais_gammainc_regularized(0.5 * df, 0.5 * x);
}
...
double moirais_binom_pmf(int k, int n, double p) {
    ...
    double log_coeff = moirais_lgamma((double)(n + 1))
                       - moirais_lgamma((double)(k + 1))
                       - moirais_lgamma((double)(n - k + 1));
...
double moirais_binom_cdf(int k, int n, double p) {
    ...
    if (k >= n) return 1.0;
    return moirais_betainc((double)(n - k), (double)(k + 1), 1.0 - p);
}
...
double moirais_poisson_pmf(int k, double lambda) {
    ...
    return exp((double)k * log(lambda) - lambda - moirais_lgamma((double)(k + 1)));
}

double moirais_poisson_cdf(int k, double lambda) {
    if (k < 0 || lambda < 0.0) return 0.0;
    if (lambda == 0.0) return 1.0;
    return 1.0 - moirais_gammainc_regularized((double)(k + 1), lambda);
}
