"""`moirais verify-pollution` CLI handler (Workstream 6 of pollution-health plan).

One command wrapping `moirais.envhealth` into a reproducible tool that
...
def handle_verify_pollution(args: argparse.Namespace) -> int:
    """Dispatch for ``moirais verify-pollution``.
    ...
    report: dict[str, Any] = {
        "command": "moirais verify-pollution",
        "pollutant": pollutant,
        "outcome": outcome,
        "region": args.region,
        "years": args.years,
...
def _emit(report: dict[str, Any], *, as_json: bool) -> None:
    ...
    print("=" * 66)
    print(f"  moirais verify-pollution — {report['pollutant'].upper()}"
          f" → {report['outcome']}")
...
def register_subparser(subparsers) -> None:
    """Called from moirais.runner.build_parser() to register this command."""
