"""ctypes bridge for MOIRAIS engine C kernels.
...
def _load_lib() -> ctypes.CDLL | None:
    ...
    try:
        ...
        lib.moirais_rmsnorm.restype = ctypes.c_int
        lib.moirais_rmsnorm.argtypes = [c_float_p, c_float_p, c_float_p, ctypes.c_int, ctypes.c_float]

        lib.moirais_rope.restype = ctypes.c_int
        lib.moirais_rope.argtypes = [c_float_p, c_float_p, ctypes.c_int, ctypes.c_int, ctypes.c_float]

        lib.moirais_matvec.restype = ctypes.c_int
        lib.moirais_matvec.argtypes = [c_float_p, c_float_p, c_float_p, ctypes.c_int, ctypes.c_int]

        lib.moirais_silu_inplace.restype = ctypes.c_int
        lib.moirais_silu_inplace.argtypes = [c_float_p, ctypes.c_int]

        lib.moirais_softmax_inplace.restype = ctypes.c_int
        lib.moirais_softmax_inplace.argtypes = [c_float_p, ctypes.c_int]

        lib.moirais_elemwise_mul.restype = ctypes.c_int
        lib.moirais_elemwise_mul.argtypes = [c_float_p, c_float_p, c_float_p, ctypes.c_int]

        lib.moirais_dot.restype = ctypes.c_int
        lib.moirais_dot.argtypes = [c_float_p, c_float_p, ctypes.c_int, c_float_p]

        lib.moirais_argmax.restype = ctypes.c_int
        lib.moirais_argmax.argtypes = [c_float_p, ctypes.c_int, c_int_p]
...
def rmsnorm(x: NDArray, weight: NDArray, eps: float = 1e-5) -> NDArray:
    ...
    if lib is not None:
        out = np.empty_like(x)
        rc = lib.moirais_rmsnorm(_ptr(x), _ptr(weight), _ptr(out), len(x), ctypes.c_float(eps))
        if rc == MOIRAIS_OK:
            return out
...
def rope(q: NDArray, k: NDArray, position: int, head_dim: int, freq_base: float = 10000.0) -> tuple[NDArray, NDArray]:
    ...
    if lib is not None:
        rc = lib.moirais_rope(_ptr(q), _ptr(k), head_dim, position, ctypes.c_float(freq_base))
        if rc == MOIRAIS_OK:
            return q, k
...
def matvec(A: NDArray, x: NDArray) -> NDArray:
    ...
    if lib is not None:
        ...
        rc = lib.moirais_matvec(_ptr(A), _ptr(x), _ptr(out), rows, cols)
        if rc == MOIRAIS_OK:
            return out
...
def silu_inplace(x: NDArray) -> NDArray:
    ...
    if lib is not None:
        rc = lib.moirais_silu_inplace(_ptr(x), len(x))
        if rc == MOIRAIS_OK:
            return x
...
def softmax(x: NDArray) -> NDArray:
    ...
    if lib is not None:
        rc = lib.moirais_softmax_inplace(_ptr(x), len(x))
        if rc == MOIRAIS_OK:
            return x
...
def argmax(x: NDArray) -> int:
    ...
    if lib is not None:
        ...
        rc = lib.moirais_argmax(_ptr(x), len(x), ctypes.byref(result))
        if rc == MOIRAIS_OK:
            return result.value
