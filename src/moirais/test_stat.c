...
/*
 * test_stat.c — MOIRAIS C Library Test Suite
 *
 * Compile & run:
...
static void test_gamma(void) {
    ASSERT_NEAR(moirais_gamma(1.0), 1.0, 1e-10, "gamma(1)");
    ASSERT_NEAR(moirais_gamma(2.0), 1.0, 1e-10, "gamma(2)");
    ASSERT_NEAR(moirais_gamma(3.0), 2.0, 1e-10, "gamma(3)");
    ASSERT_NEAR(moirais_gamma(4.0), 6.0, 1e-10, "gamma(4)");
    ASSERT_NEAR(moirais_gamma(5.0), 24.0, 1e-8, "gamma(5)");
    ASSERT_NEAR(moirais_gamma(0.5), sqrt(M_PI), 1e-10, "gamma(0.5)");
}

static void test_beta(void) {
    ASSERT_NEAR(moirais_beta(1.0, 1.0), 1.0, 1e-10, "beta(1,1)");
    ASSERT_NEAR(moirais_beta(2.0, 2.0), 1.0 / 6.0, 1e-10, "beta(2,2)");
    ASSERT_NEAR(moirais_beta(0.5, 0.5), M_PI, 1e-10, "beta(0.5,0.5)");
}

static void test_erf(void) {
    ASSERT_NEAR(moirais_erf(0.0), 0.0, 1e-10, "erf(0)");
    ASSERT_NEAR(moirais_erf(1.0), 0.8427007929, 1e-6, "erf(1)");
    ASSERT_NEAR(moirais_erf(-1.0), -0.8427007929, 1e-6, "erf(-1)");
    ASSERT_NEAR(moirais_erfc(0.0), 1.0, 1e-10, "erfc(0)");
}

static void test_norm(void) {
    ASSERT_NEAR(moirais_norm_pdf(0.0, 0.0, 1.0), 0.3989422804, 1e-8, "norm_pdf(0)");
    ASSERT_NEAR(moirais_norm_cdf(0.0, 0.0, 1.0), 0.5, 1e-8, "norm_cdf(0)");
    ASSERT_NEAR(moirais_norm_cdf(1.96, 0.0, 1.0), 0.975002, 1e-4, "norm_cdf(1.96)");
    ASSERT_NEAR(moirais_norm_cdf(-1.96, 0.0, 1.0), 0.024998, 1e-4, "norm_cdf(-1.96)");
    ASSERT_NEAR(moirais_norm_quantile(0.5, 0.0, 1.0), 0.0, 1e-6, "norm_quantile(0.5)");
    ASSERT_NEAR(moirais_norm_quantile(0.975, 0.0, 1.0), 1.96, 1e-3, "norm_quantile(0.975)");
    ASSERT_NEAR(moirais_norm_quantile(0.025, 0.0, 1.0), -1.96, 1e-3, "norm_quantile(0.025)");
}

static void test_t(void) {
    ASSERT_NEAR(moirais_t_cdf(0.0, 10.0), 0.5, 1e-8, "t_cdf(0, df=10)");
    ASSERT_TRUE(moirais_t_cdf(2.228, 10.0) > 0.97, "t_cdf(2.228, df=10) > 0.97");
    ASSERT_TRUE(moirais_t_cdf(2.228, 10.0) < 0.98, "t_cdf(2.228, df=10) < 0.98");
    ASSERT_TRUE(moirais_t_pdf(0.0, 10.0) > 0.0, "t_pdf(0, df=10) > 0");
}

static void test_chisq(void) {
    ASSERT_NEAR(moirais_chisq_cdf(0.0, 5.0), 0.0, 1e-8, "chisq_cdf(0, df=5)");
    ASSERT_TRUE(moirais_chisq_cdf(11.07, 5.0) > 0.94, "chisq_cdf(11.07, df=5) > 0.94");
    ASSERT_TRUE(moirais_chisq_pdf(5.0, 5.0) > 0.0, "chisq_pdf(5, df=5) > 0");
}

static void test_f(void) {
    ASSERT_NEAR(moirais_f_cdf(0.0, 5.0, 10.0), 0.0, 1e-8, "f_cdf(0)");
    ASSERT_TRUE(moirais_f_cdf(3.33, 5.0, 10.0) > 0.94, "f_cdf(3.33, 5, 10) > 0.94");
    ASSERT_TRUE(moirais_f_pdf(1.0, 5.0, 10.0) > 0.0, "f_pdf(1) > 0");
}

static void test_binom(void) {
    ASSERT_NEAR(moirais_binom_pmf(0, 10, 0.5), 0.000976563, 1e-6, "binom_pmf(0,10,0.5)");
    ASSERT_NEAR(moirais_binom_pmf(5, 10, 0.5), 0.24609375, 1e-6, "binom_pmf(5,10,0.5)");
    ASSERT_NEAR(moirais_binom_cdf(10, 10, 0.5), 1.0, 1e-8, "binom_cdf(10,10,0.5)");
    ASSERT_TRUE(moirais_binom_cdf(5, 10, 0.5) > 0.6, "binom_cdf(5,10,0.5) > 0.6");
}

static void test_poisson(void) {
    ASSERT_NEAR(moirais_poisson_pmf(0, 5.0), exp(-5.0), 1e-10, "poisson_pmf(0,5)");
    ASSERT_NEAR(moirais_poisson_pmf(5, 5.0), pow(5.0, 5) * exp(-5.0) / 120.0, 1e-8, "poisson_pmf(5,5)");
    ASSERT_TRUE(moirais_poisson_cdf(10, 5.0) > 0.95, "poisson_cdf(10,5) > 0.95");
}

static void test_betainc(void) {
    ASSERT_NEAR(moirais_betainc(1.0, 1.0, 0.5), 0.5, 1e-8, "betainc(1,1,0.5)");
    ASSERT_NEAR(moirais_betainc(1.0, 1.0, 0.0), 0.0, 1e-8, "betainc(1,1,0)");
    ASSERT_NEAR(moirais_betainc(1.0, 1.0, 1.0), 1.0, 1e-8, "betainc(1,1,1)");
    ASSERT_NEAR(moirais_betainc(2.0, 2.0, 0.5), 0.5, 1e-6, "betainc(2,2,0.5)");
}

static void test_gammainc(void) {
    ASSERT_NEAR(moirais_gammainc_regularized(1.0, 1.0), 1.0 - exp(-1.0), 1e-8, "gammainc(1,1)");
    ASSERT_TRUE(moirais_gammainc_regularized(5.0, 10.0) > 0.9, "gammainc(5,10) > 0.9");
}
...
static void test_fft_invalid(void) {
    float re[3] = {1, 2, 3};
    float im[3] = {0, 0, 0};
    ASSERT_TRUE(moirais_fft(re, im, 3) == DSP_ERR_SIZE, "fft non-pow2 rejected");
    ASSERT_TRUE(moirais_fft(NULL, im, 4) == DSP_ERR_NULL, "fft null rejected");
}
...
static void test_convolve(void) {
    ...
    int rc = moirais_convolve(x, 3, h, 2, out, 4);
...
static void test_convolve_fft(void) {
    ...
    moirais_convolve(x, 4, h, 2, out_td, 5);
    moirais_convolve_fft(x, 4, h, 2, out_fft, 5);
...
static void test_autocorrelation(void) {
    ...
    int rc = moirais_autocorrelation(x, 8, r, 4);
...
static void test_fir_filter(void) {
    ...
    int rc = moirais_fir_filter(x, 5, b, 3, out);
...
static void test_iir_filter(void) {
    ...
    int rc = moirais_iir_filter(x, 8, b, 1, a, 2, out);
...
static void test_zcr_rms(void) {
    ...
    float zcr = moirais_zero_crossing_rate(sine, 64);
    ASSERT_TRUE(zcr > 0.1f, "zcr sine > 0.1");
    ASSERT_TRUE(zcr < 0.5f, "zcr sine < 0.5");

    float rms = moirais_rms_energy(sine, 64);
...
static void test_mat_mul(void) {
    ...
    int rc = moirais_mat_mul(A, 2, 3, B, 3, 2, C);
...
static void test_mat_transpose(void) {
    ...
    int rc = moirais_mat_transpose(A, 2, 3, AT);
...
static void test_lu_solve(void) {
    ...
    int rc = moirais_lu_decompose(A, 3, LU, piv, NULL);
    ASSERT_TRUE(rc == MAT_OK, "lu decompose ok");

    rc = moirais_lu_solve(LU, piv, 3, b, x);
...
static void test_det(void) {
    ...
    ASSERT_NEAR(moirais_mat_det(I3, 3), 1.0, 1e-10, "det(I3)");

    double A[] = {1, 2, 3, 4};
    ASSERT_NEAR(moirais_mat_det(A, 2), -2.0, 1e-10, "det 2x2");
}
...
static void test_inverse(void) {
    ...
    int rc = moirais_mat_inverse(A, 2, Ainv);
    ASSERT_TRUE(rc == MAT_OK, "inverse ok");

    moirais_mat_mul(A, 2, 2, Ainv, 2, 2, I2);
...
static void test_cholesky_solve(void) {
    ...
    moirais_cholesky(A, 2, L);
    int rc = moirais_cholesky_solve(L, 2, b, x);
    ASSERT_TRUE(rc == MAT_OK, "chol solve ok");

    double Ax[2];
    moirais_mat_mul(A, 2, 2, x, 2, 1, Ax);
...
static void test_qr(void) {
    ...
    int rc = moirais_qr_decompose(A, 3, 3, Q, R);
    ASSERT_TRUE(rc == MAT_OK, "qr ok");

    double QR[9];
    moirais_mat_mul(Q, 3, 3, R, 3, 3, QR);
    for (int i = 0; i < 9; i++) {
    ...
    double QtQ[9];
    double Qt[9];
    moirais_mat_transpose(Q, 3, 3, Qt);
    moirais_mat_mul(Qt, 3, 3, Q, 3, 3, QtQ);
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
...
static void test_eigen_symmetric(void) {
    ...
    int rc = moirais_eigen_symmetric(A, 2, evals, evecs, 100);
...
static void test_svd(void) {
    ...
    int rc = moirais_svd(A, 3, 2, U, S, Vt, 200);
    ...
    moirais_mat_mul(US, 3, 2, Vt, 2, 2, USVt);
    for (int i = 0; i < 6; i++) {
...
static void test_singular_matrix(void) {
    ...
    double LU[4];
    int piv[2];
    int rc = moirais_lu_decompose(A, 2, LU, piv, NULL);
...
static void test_null_checks(void) {
    ASSERT_TRUE(moirais_mat_mul(NULL, 2, 2, NULL, 2, 2, NULL) == MAT_ERR_NULL, "matmul null");
    ASSERT_TRUE(moirais_fft(NULL, NULL, 4) == DSP_ERR_NULL, "fft null");
    ASSERT_TRUE(moirais_window_hamming(NULL, 4) == DSP_ERR_NULL, "window null");
}

/* ---- Main --------------------------------------------------------------- */

int main(void) {
    printf("MOIRAIS C Library Test Suite\n");
    printf("========================\n\n");
