/*
 * engine_kernels.c — MOIRAIS Inference Engine C Kernels
 *
...
#define CHECK_NULL(ptr) do { if (!(ptr)) return MOIRAIS_ERR_NULL; } while (0)
#define CHECK_SIZE(n)   do { if ((n) <= 0 || (n) > MAX_DIM) return MOIRAIS_ERR_SIZE; } while (0)
...
int moirais_rope(
    ...
    CHECK_NULL(k);
    CHECK_SIZE(head_dim);
    if (head_dim % 2 != 0) return MOIRAIS_ERR_SIZE;
