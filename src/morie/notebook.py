...
_NOTEBOOK_DIR_ENVVAR = "MORIE_NOTEBOOK_DIR"
...
_TEMPLATES["analysis"] = textwrap.dedent("""\
    ...
    This notebook presents a statistical analysis using the MORIE framework.
...
_TEMPLATES["causal"] = textwrap.dedent("""\
    ...
    ```{{python}}
    # ps = morie.compute_propensity_scores(df, treatment, covariates)
    ```

    ### IPW Analysis

    ```{{python}}
    # weights = morie.calculate_ipw_weights(ps)
    ```

    ### Double Machine Learning

    ```{{python}}
    # ate_result = morie.estimate_ate(df, outcome, treatment, covariates)
    ```
...
def inject_results(
    ...
    *,
    output_path: str | Path | None = None,
    marker: str = "MORIE_RESULT",
) -> Path:
    """Inject computed results into a notebook.

    Replaces placeholders of the form ``{{MORIE_RESULT:key}}`` with values
    ...
    marker : str
        Placeholder prefix (default ``MORIE_RESULT``).
