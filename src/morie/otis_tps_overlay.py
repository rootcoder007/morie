...
def _tps_incidents_by_year(df_tps: pd.DataFrame) -> pd.Series:
    yc = "OCC_YEAR" if "OCC_YEAR" in df_tps.columns else ("REPORT_YEAR" if "REPORT_YEAR" in df_tps.columns else None)
    if yc is None:
        return pd.Series(dtype=int)
...
def yoy_correlation(tps_categories: list[str] | None = None, *, sample_rows: int | None = 50_000) -> RichResult:
    ...
    for cat in cats:
        try:
            ...
            if np.isfinite(r) and n > 3 and abs(r) < 1:
                ...
                p = float("nan")
            rows.append(
                [cat, int(common.size), round(r, 4), round(float(p), 6), f"{int(common.min())}–{int(common.max())}"]
            )
    ...
    return RichResult(
        ...
        summary_lines=[
            ...
            ("OTIS years", f"{int(seg.index.min())}–{int(seg.index.max())}"),
        ...
        tables=[
            {
                ...
                "headers": ["TPS category", "Common years", "Pearson r", "p-value", "Year range"],
                "rows": rows,
            }
        ],
        interpretation=(
...
def per_region_rollup(*, tps_total_by_year: pd.Series | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=[
            ...
            ("Regions", ", ".join(map(str, by_region.columns))),
            ("Total placements", int(by_region.values.sum())),
            ("Toronto region total", int(by_region.get("Toronto", pd.Series(dtype=int)).sum())),
        ],
        tables=[
            {
                "title": "Counts by year × region:",
                "headers": ["Year"] + list(map(str, by_region.columns)),
                "rows": [[int(y)] + list(map(int, row)) for y, row in by_region.iterrows()],
            }
        ],
...
def analyze_all(out_dir: Path | None = None, *, sample_rows: int | None = 30_000) -> dict[str, RichResult]:
    ...
    for name, fn in [
        ...
        ("yoy_correlation", lambda: yoy_correlation(sample_rows=sample_rows)),
    ]:
        try:
            r = fn()
            results[name] = r
            ...
            results[name] = RichResult(title=f"overlay {name} (failed)", warnings=[f"{type(e).__name__}: {e}"])
    return results
