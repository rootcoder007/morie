...
and tokenizes with :class:`morie.tokenizer.Tokenizer`.
...
class MORIEEngine:
    """MORIE's own LLM inference engine with TurboQuant KV-cache compression.
    ...
    def __init__(
        ...
        self._is_morie_gpt = self._arch == "morie_gpt"
    ...
    def forward(self, token_id: int, position: int):
        ...
        for layer in range(n_layers):
            if self._is_morie_gpt:
                x, x0 = self._transformer_block_morie(
                    x,
                    x0,
                    layer,
                    position,
    ...
    def _transformer_block_morie(
        ...
        *,
        ...
        normed = _rmsnorm_bare(x, eps=eps)
        ffn_out = self._ffn_morie(normed, layer)
        x = x + ffn_out
    ...
    def _ffn_morie(self, x, layer: int):
    ...
    def __repr__(self) -> str:
        cfg = self.config
        return (
            f"MORIEEngine(arch={cfg.get('architecture')}, "
