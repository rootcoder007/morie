...
def _to_format(
    ...
    *,
    ...
    if fmt == "html":
        ...
        parts = []
        if title:
            parts.append(f'<caption class="morie-caption">{title}</caption>')
        parts.append(html)
        if footnotes:
            fn_html = footnotes.replace("\n", "<br>")
            parts.append(f'<div class="morie-footnotes">{fn_html}</div>')
        ...
        if spanning_headers:
            ...
            parts = [html]
            if footnotes:
                parts.append(f'<div class="morie-footnotes">{footnotes.replace(chr(10), "<br>")}</div>')
