...
TORONTO_DISTRICTS: list[tuple[str, str, dict]] = [
    ...
    ("North York", "#fff2b3", dict(lat_min=43.745, lon_min=-79.475, lon_max=-79.275)),
    ("Scarborough", "#cfe2f3", dict(lon_min=-79.275)),
    ("York", "#e8b6cf", dict(lat_max=43.745, lon_min=-79.475, lon_max=-79.430)),
    ("Old Toronto", "#f7c69b", dict(lat_max=43.745, lon_min=-79.430, lon_max=-79.310)),
    ("East York", "#d2c2a0", dict(lat_max=43.745, lat_min=43.685, lon_min=-79.355, lon_max=-79.300)),
]
...
def _district_for_centroid(lat: float, lon: float) -> str:
    ...
    for name, _, box in TORONTO_DISTRICTS:
        ...
        for k, v in box.items():
            if k == "lon_max" and lon > v:
                ok = False
            if k == "lon_min" and lon < v:
                ok = False
            if k == "lat_max" and lat > v:
                ok = False
            if k == "lat_min" and lat < v:
                ok = False
        if ok:
            return name
    return "Old Toronto"
...
def pretty_label(s: str) -> str:
    ...
    return " · ".join([" ".join(out[:-1]).strip(), out[-1]]) if (out[-1].isdigit() and len(out) > 1) else " ".join(out)
...
def _polygon_pieces(df) -> list[tuple[np.ndarray, dict]]:
    ...
    for _, row in df.iterrows():
        ...
        for ring in rings:
            ...
            if lats.min() < 43.55 or lats.max() > 43.90 or lons.min() < -79.65 or lons.max() > -79.10:
                continue
            xk, yk = project_xy(lats, lons)
            out.append((np.column_stack([xk, yk]), {k: row.get(k) for k in row.index}))
    return out


def render_choropleth(
    *,
    rate_col: str = "ASSAULT_RATE_2024",
    title: str | None = None,
    cmap: str | None = None,
    ...
    if rate_col not in df.columns:
        raise KeyError(f"{rate_col!r} not in NeighbourhoodCrimeRates ({len(df.columns)} cols).")
    ...
    pc = PolyCollection(polys, array=np.asarray(vals), edgecolor=border_color, linewidth=border_lw, cmap=cmap)
    ...
    _setup_axes(ax, title=title or f"Toronto · {pretty_label(rate_col)} · 158 wards")
    out = Path(outfile) if outfile else FIG_DIR / f"choropleth_{rate_col.lower()}.png"
...
def render_quad(
    ...
    *,
    ...
    axA = axes[0, 0]
    if len(pt_x) >= 50:
        kde = gaussian_kde(np.vstack([pt_x, pt_y]), bw_method="silverman")
        ...
        fig.colorbar(cs, ax=axA, shrink=0.7, label=f"{category} density (per sq km)")
    pc_outline = PolyCollection(polys, facecolors="none", edgecolors=border_color, linewidth=border_lw)
    ...
    rate_col = next((c for c in rate_col_candidates if c in poly_df.columns), None)
    ...
    if rate_col is not None:
        ...
        pcB = PolyCollection(
            polys, array=np.asarray(rate_vals), edgecolor=border_color, linewidth=border_lw, cmap=seq_cmap
        )
        axB.add_collection(pcB)
        fig.colorbar(pcB, ax=axB, shrink=0.7, label=pretty_label(rate_col))
    else:
        axB.text(0.5, 0.5, f"no per-100k {category} {year} column", ha="center", va="center", transform=axB.transAxes)
        pcB = PolyCollection(polys, facecolors="white", edgecolor=border_color, linewidth=border_lw)
        axB.add_collection(pcB)
    axB.autoscale_view()

    # ---- (c) LISA HH/LL/HL/LH ----------------------------------------
    axC = axes[1, 0]
    rr_lisa = local_morans_i(df, ds_name=category, k_neighbours=5, top_n=200)
    quad_color = {"HH": "#d7191c", "HL": "#fdae61", "LH": "#abd9e9", "LL": "#2c7bb6"}
    hood_q: dict[int, str] = {}
    ...
    pcC = PolyCollection(polys, facecolors=poly_face, edgecolor=border_color, linewidth=border_lw)
    ...
    for tbl in rr_gi.tables or []:
        rows = tbl["rows"] if isinstance(tbl, dict) else getattr(tbl, "rows", [])
        headers = tbl["headers"] if isinstance(tbl, dict) else getattr(tbl, "headers", [])
    ...
    for letter, ax, title in zip(panel_letters, axes.ravel(), titles, strict=False):
        ax.set_aspect("equal", adjustable="box")
        ax.set_xticks([])
        ax.set_yticks([])
        ax.text(0.02, 0.97, letter, transform=ax.transAxes, fontsize=14, fontweight="bold", va="top")
        ax.set_title(title, fontsize=11)
    fig.suptitle(f"Toronto · {category} · 4-panel spatial analysis · 158 wards", fontsize=12, y=0.995)
    out = Path(outfile) if outfile else FIG_DIR / f"quad_{category.lower()}_{year}.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    fig.tight_layout()
    fig.savefig(out, dpi=140, bbox_inches="tight")
    plt.close(fig)
...
def render_dbscan(
    ...
    *,
    ...
    fig, ax = plt.subplots(figsize=(fig_w, fig_h))
    noise_mask = labels == -1
    ax.scatter(xk[noise_mask], yk[noise_mask], s=1.5, c="#888", alpha=0.25, label=f"noise ({int(noise_mask.sum())})")
    ...
    for col, idx in zip(palette, order, strict=False):
        ...
        m = labels == cid
        ax.scatter(xk[m], yk[m], s=4, color=col, label=f"cluster {cid} (n={int(m.sum())})")
    ax.legend(loc="lower right", fontsize=7, frameon=True, facecolor="white", framealpha=0.85, ncols=2)
    n_clusters = int((cluster_ids).size)
    n_noise = int(noise_mask.sum())
    _setup_axes(
        ax,
        title=(
            f"Toronto {category} -- DBSCAN (eps={eps_km}km, "
            f"min={min_samples}) -- {n_clusters} clusters, "
            f"{n_noise:,} noise"
        ),
    )
    out = Path(outfile) if outfile else FIG_DIR / f"map_dbscan_{category.lower()}.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    fig.tight_layout()
    fig.savefig(out, dpi=140, bbox_inches="tight")
    plt.close(fig)
...
def _draw_compass(ax, *, x_frac: float = 0.93, y_frac: float = 0.83, size_frac: float = 0.10) -> None:
...
def _draw_scalebar(ax, *, x_frac: float = 0.05, y_frac: float = 0.06, length_km: float = 10.0) -> None:
    ...
    ax.plot([dx0, dx0 + length_km], [dy0, dy0], color="#1a1a1a", linewidth=2.0, zorder=8)
    ax.text(
        dx0 + length_km / 2,
        dy0 + 0.4,
        f"{int(length_km)} km",
        ...
        color="#1a1a1a",
        path_effects=[
            __import__("matplotlib.patheffects", fromlist=["pe"]).withStroke(linewidth=2, foreground="white")
        ],
    )


def render_district_proportional(
    metric: str = "ASSAULT_2024",
    *,
    title: str | None = None,
    outfile: str | Path | None = None,
    fig_h: float = 7.0,
    ...
    pc = PolyCollection(polys, facecolors=district_face, edgecolor=border_color, linewidth=border_lw)
    ax.add_collection(pc)

    # Proportional-symbol circles at projected centroids
    vals = np.asarray([float(a.get(metric)) if a.get(metric) is not None else 0.0 for a in poly_attrs])
    ...
    for poly, v in zip(polys, vals, strict=False):
        ...
        cx, cy = poly.mean(axis=0)
        r = _radius(float(v))
        ax.add_patch(Circle((cx, cy), r, facecolor="#1a1a1a", edgecolor="none", alpha=0.92, zorder=4))

    ax.set_aspect("equal", adjustable="box")
    ax.autoscale_view()
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_title(title or f"Toronto · {pretty_label(metric)} · district map · district map", fontsize=12)
    ...
    leg.text(0.50, 0.97, "Map Divisions", ha="center", va="top", fontsize=11, fontweight="bold")
    ...
    leg.text(0.50, 0.55, f"Number of {pretty_label(metric)}", ha="center", va="top", fontsize=11, fontweight="bold")
    tiers = np.linspace(vmin, vmax, 5)
    for i, t in enumerate(tiers):
        y = 0.50 - i * 0.082
        r_pts = _radius(float(t))
        leg.add_patch(Circle((0.18, y), r_pts * 0.012, facecolor="#1a1a1a", edgecolor="none", transform=leg.transAxes))
        leg.text(0.34, y, f"{t:.0f}", va="center", fontsize=9)

    out = Path(outfile) if outfile else FIG_DIR / f"district_prop_{metric.lower()}.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out, dpi=140, bbox_inches="tight")
    plt.close(fig)
    return out
...
def render_satscan_panel(
    ...
    *,
    ...
    def _llr(n_in, n_out, mu_in, mu_out):
        ...
        return a + b - (n_in + n_out) * math.log((n_in + n_out) / (mu_in + mu_out))
    ...
    bbox_area = ((pt_x.max() - pt_x.min()) * (pt_y.max() - pt_y.min())) or 1.0
    ...
    for ci, c in enumerate(cents):
        for r in radii_km:
            ...
            for t0, t1 in twins:
                ...
                if n_in < 5:
                    continue
                mu_in = n_total * (math.pi * r**2 / bbox_area) * ((t1 - t0 + 1) / span_yrs)
    ...
    for cand in candidates:
        ...
        c = cents[ci]
        if any(((c[0] - cents[c2[1]][0]) ** 2 + (c[1] - cents[c2[1]][1]) ** 2) < max(r, c2[2]) ** 2 for c2 in chosen):
            continue
        chosen.append(cand)
    ...
    pc_base = PolyCollection(polys, facecolors="white", edgecolor=border_color, linewidth=border_lw)
    ...
    for cand in chosen:
        ...
        for pi_, p in enumerate(polys):
            ...
            if (cx - c[0]) ** 2 + (cy - c[1]) ** 2 <= r**2:
                yellow_idx.add(pi_)
    pc_yellow = PolyCollection(
        [polys[i] for i in yellow_idx], facecolors="#fff7a8", edgecolor=border_color, linewidth=border_lw + 0.2
    )
    ...
    info.text(
        0.5,
        0.955,
        f"{year_window_yrs}-yr window · {yr_min}–{yr_max} · {n_mc} MC permutations",
        ha="center",
        va="top",
        fontsize=8,
        color="#444",
    ...
    for k, cand in enumerate(chosen, start=1):
        ...
        ax.add_patch(Circle(c, r, facecolor="#f4b6c2", edgecolor="#9a1d3a", linewidth=1.0, alpha=0.32, zorder=2))
        # Red dots = significant locations
        ax.scatter(pt_x[sel], pt_y[sel], s=14, c="#d7191c", edgecolor="#7a0000", linewidths=0.3, zorder=4)
        ...
        # MC p-value
        p_val = (1 + sum(1 for v in null_max_llr if v >= llr)) / (1 + n_mc)
        ...
        body = (
            ...
            f"P-value       : " + ("< 0.001" if p_val < 0.001 else f"{p_val:.3f}")
        )
        info.text(0.10, y_top - 0.012, body, va="top", ha="left", family="monospace", fontsize=9.0)
    ...
    handles = [
        plt.Line2D(
            ...
            markersize=7,
            label=f"Sig. locations (n={sig_loc_count})",
        ),
        plt.Line2D(
            [],
            [],
    ...
    ax.legend(handles=handles, loc="lower right", fontsize=8, framealpha=0.92, ncols=1)
    ax.set_aspect("equal", adjustable="box")
    ax.autoscale_view()
    ax.set_xticks([])
    ax.set_yticks([])
    _draw_compass(ax)
    _draw_scalebar(ax, length_km=10)
    ax.set_title(f"{category} · Space-time significant clusters", fontsize=11)
    out = Path(outfile) if outfile else FIG_DIR / f"satscan_d_{category.lower()}.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out, dpi=140, bbox_inches="tight")
    plt.close(fig)
    return out
...
def render_yearly_grid(
    *,
    prefix: str = "ASSAULT_RATE",
    ...
    fig, axes = plt.subplots(nrows, ncols, figsize=(3.5 * ncols, 2.4 * nrows), squeeze=False)
    ...
    for k in range(n, nrows * ncols):
        ...
        axes[r][c].axis("off")
    if pc is not None:
        fig.colorbar(pc, ax=axes.ravel().tolist(), shrink=0.6, label=f"{prefix}_<year>")
    fig.suptitle(f"Toronto {prefix} · yearly small-multiples", y=1.0)
    out = Path(outfile) if outfile else FIG_DIR / f"choropleth_{prefix.lower()}_yearly.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out, dpi=140, bbox_inches="tight")
    plt.close(fig)
    return out
