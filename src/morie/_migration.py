...
Two modes:
  ...
  2. MORIE default: ``build_migration_inventory()`` uses built-in MORIE mappings.

Example (general)::

    from morie._migration import compare_trees, scan_directory
    inv = scan_directory("/path/to/legacy")
    df = compare_trees("/path/to/legacy", "/path/to/modern")

Example (MORIE-specific)::

    from morie._migration import build_migration_inventory
...
MORIE_IMPLEMENTATION_OWNER = {v: "R" for v in MORIE_LEGACY_TO_MODULE.values()}

LEGACY_MODULE_DOCS = MORIE_MODULE_DOCS
LEGACY_TO_MODULE = MORIE_LEGACY_TO_MODULE
IMPLEMENTATION_OWNER = MORIE_IMPLEMENTATION_OWNER
...
def build_migration_inventory(
    ...
    *,
    ...
    """Build a migration inventory (MORIE default or custom mapping)."""
    ...
    rows: list[dict[str, Any]] = []
    for script, module in sorted(MORIE_LEGACY_TO_MODULE.items()):
        rows.append(
            {
                "legacy_script": script,
                "module_name": module,
                "implementation_owner": MORIE_IMPLEMENTATION_OWNER.get(module, "unknown"),
                "docs_page": MORIE_MODULE_DOCS.get(script, ""),
