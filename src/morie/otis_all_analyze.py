...
def _summary_lines(df: pd.DataFrame, ds_id: str) -> list[tuple[str, Any]]:
    ...
    if yc and df.shape[0]:
        ...
        if years.size:
            out.append(("Years covered", f"{int(years.min())}–{int(years.max())}"))
    if meta.primary_metric in df.columns:
        col = pd.to_numeric(df[meta.primary_metric], errors="coerce")
        out.append((f"Total {meta.primary_metric}", int(col.sum())))
    return out


def _crosstab(
    df: pd.DataFrame, row: str, col: str, value: str, aggfunc: str = "sum", top_rows: int = 20
) -> dict | None:
    ...
    return {
        ...
        "rows": [[str(idx)] + [int(v) if pd.notna(v) else 0 for v in row_vals] for idx, row_vals in pivot.iterrows()],
    }


def _year_trend(df: pd.DataFrame, value: str, year_col: str | None = None) -> dict | None:
...
def analyze_b01(df: pd.DataFrame | None = None) -> RichResult:
    ...
    summary.append(("Unique individuals", int(df["UniqueIndividual_ID"].nunique())))
    summary.append(("Mean consecutive days", float(df["NumberConsecutiveDays_Segregation"].mean())))
    summary.append(("Median consecutive days", int(df["NumberConsecutiveDays_Segregation"].median())))
    summary.append(("Max consecutive days", int(df["NumberConsecutiveDays_Segregation"].max())))
    ...
    return RichResult(
        ...
        tables=[
            ...
            {"title": "Alert flags on placements:", "headers": ["Alert", "Count", "Percent"], "rows": alert_rows},
            _year_trend(df, "Number_Of_Placements") or {"title": "(no year trend)", "headers": [], "rows": []},
        ],
        payload={"n_rows": int(df.shape[0]), "n_individuals": int(df["UniqueIndividual_ID"].nunique())},
    )
...
def analyze_b02(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        tables=[
            ...
            _crosstab(df, "Gender", "Region_MostRecentPlacement", "TotalAggregatedDays_Segregation")
            or {"title": "(none)", "headers": [], "rows": []},
        ],
    )
...
def analyze_b03(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Alert_Type", "Alert_Presence", "Number_SegregationPlacements") or {},
            _crosstab(df, "Region_AtTimeOfPlacement", "Alert_Type", "Number_SegregationPlacements") or {},
        ],
    )
...
def analyze_b04(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Region_AtTimeOfPlacement", "Measure", "NumberConsecutiveDays_Segregation", aggfunc="max")
            or {},
        ],
    )
...
def analyze_b05(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Consecutive_Duration", "EndFiscalYear", "Number_SegregationPlacements") or {},
        ],
    )
...
def analyze_b06(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Reason", "EndFiscalYear", "Number_SegregationPlacements") or {},
            _crosstab(df, "Reason", "Gender", "Number_SegregationPlacements") or {},
        ],
    )
...
def analyze_b07(df: pd.DataFrame | None = None) -> RichResult:
    ...
    for _, r in df.iterrows():
        ...
        rows.append([_to_int(r["EndFiscalYear"]), str(r["Alert_Type"]), str(r["Gender"]), with_a, wo_a, f"{rate:.1f}%"])
    return RichResult(
        title="b07 -- Segregation placements with/without alert × gender",
        summary_lines=summary,
        tables=[
            {
                "title": "By alert × gender × year:",
                "headers": ["Year", "Alert_Type", "Gender", "With_Alert", "Without_Alert", "% with alert"],
                "rows": rows,
            }
        ],
    )
...
def analyze_b09(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "NumberPlacements_Segregation", "Gender", "NumberIndividuals_Segregation") or {},
        ],
    )
...
def analyze_c01(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        tables=[
            {
                "title": "Cohort sizes + ratios:",
                "headers": ["Year", "Gender", "Custody", "RC", "Seg", "RC/custody", "Seg/custody"],
                "rows": rate_rows,
            }
        ],
    )
...
def analyze_c03(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        tables=[
            {
                "title": "By race (totals across years/genders):",
                "headers": ["Race", "Custody", "RC", "Seg", "RC/custody", "Seg/custody"],
                "rows": rows,
            }
        ],
        interpretation=(
...
def analyze_c04(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Race", "Region_MostRecentPlacement", "NumberIndividuals_RestrictiveConfinement") or {},
        ],
    )
...
def analyze_c05(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Religion", "Region_MostRecentPlacement", "NumberIndividuals_RestrictiveConfinement") or {},
        ],
    )
...
def analyze_c06(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Age_Category", "Region_MostRecentPlacement", "NumberIndividuals_RestrictiveConfinement")
            or {},
        ],
    )
...
def analyze_c07(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Alert_Type", "Gender", "NumberIndividuals_RestrictiveConfinement") or {},
            _crosstab(df, "Alert_Type", "EndFiscalYear", "NumberIndividuals_Segregation") or {},
        ],
    )
...
def analyze_c08(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Religion", "Gender", "NumberIndividuals_RestrictiveConfinement") or {},
        ],
    )
...
def analyze_c09(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Age_Category", "Gender", "NumberIndividuals_RestrictiveConfinement") or {},
        ],
    )
...
def analyze_c11(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        summary_lines=summary,
        tables=[
            _crosstab(df, "Aggregate_Duration", "EndFiscalYear", "NumberIndividuals_RestrictiveConfinement") or {},
        ],
    )
...
def analyze_c12(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        tables=[
            _crosstab(
                df, "Region_MostRecentPlacement", "Measure", "TotalAggregatedDays_RestrictiveConfinement", aggfunc="max"
            )
            or {},
        ],
    )
...
def analyze_d01(df: pd.DataFrame | None = None) -> RichResult:
    ...
    summary.append(("Distinct individuals", int(df["UniqueIndividual_ID"].nunique())))
    # Real CSV uses MedicalCauseofDeath / MeansofDeath (lowercase o);
    # tolerate either casing.
    cause_col = "MedicalCauseofDeath" if "MedicalCauseofDeath" in df.columns else "MedicalCauseOfDeath"
    means_col = "MeansofDeath" if "MeansofDeath" in df.columns else "MeansOfDeath"
    return RichResult(
        ...
        tables=[
            {
                ...
                "rows": [[k, int(v)] for k, v in df["Region_AtTimeOfDeath"].value_counts().items()],
            },
            {
                "title": "By housing unit type:",
                "headers": ["HousingUnit", "Deaths"],
                "rows": [[k, int(v)] for k, v in df["HousingUnit_Type"].value_counts().items()],
            },
            {
                "title": "By medical cause:",
                "headers": ["Cause", "Deaths"],
                "rows": [[k, int(v)] for k, v in df[cause_col].value_counts().items()],
            },
            {
                "title": "By means of death:",
                "headers": ["Means", "Deaths"],
                "rows": [[k, int(v)] for k, v in df[means_col].value_counts().items()],
            },
        ],
    )
...
def analyze_d06(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        tables=[
            _crosstab(df, "MedicalCauseOfDeath", "Alert_Type", "Number_CustodialDeaths") or {},
        ],
    )
...
def analyze_d07(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return RichResult(
        ...
        tables=[
            _crosstab(df, "HousingUnit_Type", "Alert_Type", "Number_CustodialDeaths") or {},
        ],
    )
...
def analyze_a01(df: pd.DataFrame | None = None) -> RichResult:
    ...
    summary = [
        ...
        ("ATE", round(fit["ate"], 4)),
        ("ATE 95% CI", (round(fit["ate_ci95"][0], 4), round(fit["ate_ci95"][1], 4))),
        ("ATTE", round(fit["atte"], 4)),
        ("ATTE 95% CI", (round(fit["atte_ci95"][0], 4), round(fit["atte_ci95"][1], 4))),
        ("Standard error type", fit["se_kind"]),
    ]
    return RichResult(
        title=("OTIS a01 -- high alert complexity (ac ≥ 2) -> regional volatility (vm count)"),
        summary_lines=summary,
        interpretation=(
...
def _dual_irm_dml_on(
    ...
    *,
    ...
    for arm in ("ruhela", "naive"):
        ...
        payloads[arm] = {k: v for k, v in res.items() if k not in {"data"}}
    ...
    if include_match_first:
        ...
        payloads["ruhela_match_first"] = {k: v for k, v in res_mf.items() if k not in {"data"}}
    ...
    if include_per_year:
        ...
        try:
            ...
            payloads["ruhela_per_year"] = {
                str(k): {kk: vv for kk, vv in r.items() if kk not in {"data"}}
                for k, r in yr_results.items()
    ...
    summary = [
        ...
        ("Ruhela ATE", f"{payloads['ruhela']['ate']:+.4f} (SE {payloads['ruhela']['ate_se']:.4f})"),
        ("Naive ATE", f"{payloads['naive']['ate']:+.4f} (SE {payloads['naive']['ate_se']:.4f})"),
    ]
    if include_match_first:
        summary.append(
            (
                "Ruhela ATE (match_first PSM)",
    ...
    if include_per_year and per_year_rows:
        summary.append(
            ("Per-year Ruhela ATEs", ", ".join(f"FY{r[0]}={r[2]}" for r in per_year_rows if isinstance(r[0], int)))
        )

    tables = [
        {
            "title": (
                f"Side-by-side dual on {ds_id}: T = high-alert-complexity "
                f"(arm-specific), Y = vm; cluster = {cluster_col}"
            ),
            "headers": ["Arm", "T", "Y", "n", "p_treat", "ATE", "ATE 95% CI", "ATE p", "ATTE", "ATTE 95% CI"],
    ...
    return RichResult(
        ...
        payload={"arms": payloads, "cluster_col": cluster_col, "ds_id": ds_id},
    )


def _ruhela_formulations_on(
    df: pd.DataFrame,
    *,
    ds_id: str,
    source_label: str,
    ...
    def _row(idx: str, est, label: str = None, estimand: str = "ATE") -> list:
    ...
    try:
        ipw = oc.otis_ipw(
            data_r, treatment=T_r, outcome=Y_r, covariates=cov_r, propensity_calibration=propensity_calibration
        )
        rows.append(_row("1. IPW (Hájek)", ipw))
        payloads["ensemble"].append(
            {
                "estimator": "IPW",
                "ate": float(ipw.ate),
                "se": float(ipw.ate_se),
    ...
    # 3. g-computation
    try:
        gc = oc.otis_gcomputation(data_r, treatment=T_r, outcome=Y_r, covariates=cov_r, n_bootstrap=200)
    ...
    # 4. PSM 1:1 NN
    try:
        psm = oc.otis_psm(data_r, treatment=T_r, outcome=Y_r, covariates=cov_r, k=1)
    ...
    # 5. PSM subclass
    try:
        psm_sc = oc.otis_psm_subclass(data_r, treatment=T_r, outcome=Y_r, covariates=cov_r, n_strata=5)
        rows.append(_row("5. PSM subclass (5)", psm_sc))
        payloads["ensemble"].append(
            {
                "estimator": "PSM-subclass",
    ...
    try:
        dml = oc.otis_irm_dml(
            ...
            outcome=Y_r,
            covariates=cov_r,
            cluster_cols=[cluster_col] if cluster_col in data_r.columns else None,
        )
        rows.append(
            [
                "6. IRM-DML",
        ...
        if include_atc and dml.get("atc") is not None:
            rows.append(
                [
                    ...
                    (f"[{dml['atc_ci95'][0]:+.3f}, {dml['atc_ci95'][1]:+.3f}]" if dml.get("atc_ci95") else "--"),
                    (f"{dml['atc_pval']:.2e}" if dml.get("atc_pval") is not None else "--"),
                    "--",
                ]
            )
        irm_results = dml
        payloads["ensemble"].append(
            {
                ...
                "atc": (float(dml["atc"]) if dml.get("atc") is not None else None),
                "n": int(dml["n"]),
            }
        )
    except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        rows.append(
            [
                "7. PSM->IRM-DML (match_first)",
                "ATE",
                f"{dml_mf['ate']:+.4f}",
                f"{dml_mf['ate_se']:.4f}",
                f"[{dml_mf['ate_ci95'][0]:+.3f}, {dml_mf['ate_ci95'][1]:+.3f}]",
                f"{dml_mf['ate_pval']:.2e}",
                f"matched_n={dml_mf['n']}",
            ]
        )
        payloads["match_first"] = {k: v for k, v in dml_mf.items() if k != "data"}
    except Exception as e:  # noqa: BLE001
    ...
    if include_atc:
        try:
            atc = oc.otis_atc(data_r, treatment=T_r, outcome=Y_r, covariates=cov_r)
            ...
            rows.append(["8. ATC (AIPW)", "--", "err", str(e)[:30], "--", "--", "--"])

    # 9. PLR (Partially Linear DML)
    if include_plr:
        try:
            plr = oc.otis_plr(data_r, treatment=T_r, outcome=Y_r, covariates=cov_r)
            rows.append(_row("9. PLR (Chernozhukov 2018)", plr, estimand="θ"))
            payloads["ensemble"].append(
                {
                    "estimator": "PLR",
                    "theta": float(plr.ate),
    ...
    if include_superlearner:
        try:
            sl = oc.otis_aipw_superlearner(
                data_r, treatment=T_r, outcome=Y_r, covariates=cov_r, propensity_calibration=propensity_calibration
            )
            rows.append(_row("10. SuperLearner AIPW", sl))
            payloads["ensemble"].append(
                {
                    "estimator": "SuperLearner-AIPW",
                    "ate": float(sl.ate),
                    "se": float(sl.ate_se),
                    "p": float(sl.ate_pval),
                    "n": int(sl.n),
            ...
            rows.append(["10. SuperLearner AIPW", "--", "err", str(e)[:30], "--", "--", "--"])
    ...
    if include_multi_se and irm_results:
        ...
        flavours = [
            ("pooled (iid)", None),
            ("cluster: EndFiscalYear", ["EndFiscalYear"] if "EndFiscalYear" in data_r.columns else None),
            (
                "cluster: UniqueIndividual_ID",
                ["UniqueIndividual_ID"] if "UniqueIndividual_ID" in data_r.columns else None,
            ),
            (
                "multi-way: yr × id",
                ["EndFiscalYear", "UniqueIndividual_ID"]
                if ("EndFiscalYear" in data_r.columns and "UniqueIndividual_ID" in data_r.columns)
                else None,
            ),
        ]
        for label, cl in flavours:
            if cl is None and label != "pooled (iid)":
                ...
                continue
            try:
                fit = oc.otis_irm_dml(data_r, treatment=T_r, outcome=Y_r, covariates=cov_r, cluster_cols=cl)
                ...
                multi_se_rows.append([label, "--", "err", str(e)[:30], "--"])
    ...
    if naive_arm is not None:
        ...
        for label_n, fn in [
            ...
            try:
                r = fn(data_n, treatment=T_n, outcome=Y_n, covariates=cov_n)
                naive_rows.append(
                    [
                        label_n,
                        f"{r.ate:+.4f}",
    ...
    tables = [
        {
            "title": (f"Ruhela formulations (full DLRM, alias DLRM) on {ds_id} -- {formulation_label}:"),
            "headers": ["Estimator", "Estimand", "Estimate", "SE", "95% CI", "p-value", "Notes"],
            "rows": rows,
        }
    ]
    if include_multi_se and multi_se_rows:
        tables.append(
            {
                "title": (
                    "IRM-DML SE comparison: pooled vs clustered on "
    ...
    if naive_rows:
        tables.append(
            {
                "title": (
                    "Naive-arm sensitivity (any-flag, vm-binary): concordance with Ruhela in sign, smaller magnitude"
                ),
...
def analyze_a01_ruhela_formulations(
    ...
    *,
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS a01 -- Ruhela formulations (full DLRM) on the canonical formulation (T = ac ≥ 2 -> Y = vm count)"),
        interpretation=(
...
def analyze_a01_dual(*args, **kwargs):
    ...
    _w.warn("analyze_a01_dual is deprecated; use analyze_a01_ruhela_formulations", DeprecationWarning, stacklevel=2)
    return analyze_a01_ruhela_formulations(*args, **kwargs)


def analyze_b01_ruhela_formulations(
    df: pd.DataFrame | None = None,
    *,
    cluster_col: str = "EndFiscalYear",
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS b01 -- Ruhela formulations (full DLRM) (segregation-detailed complement)"),
        interpretation=(
...
def analyze_b02_ruhela_formulations(
    df: pd.DataFrame | None = None,
    *,
    cluster_col: str = "EndFiscalYear",
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS b02 -- Ruhela formulations: T=Female -> Y=Total seg days within FY"),
        interpretation=(
...
def analyze_ruhela_per_year(
    ...
    *,
    ...
    for yr, year_results in sorted(by_year.items()):
        ...
        for est_label in ("ipw", "aipw", "gcomp", "psm_nn", "psm_subclass", "atc", "plr", "superlearner"):
            est = year_results.get(est_label, {})
            if "error" in est:
                long_rows.append([yr, est_label.upper(), "err", "--", "--", "--", est.get("error", "")[:30]])
                continue
        ...
        if "error" in irm:
            long_rows.append([yr, "IRM-DML", "err", "--", "--", "--", irm.get("error", "")[:30]])
        else:
        ...
        if "error" in mf:
            long_rows.append([yr, "PSM->IRM-DML", "err", "--", "--", "--", mf.get("error", "")[:30]])
        else:
        ...
        (
            ...
            len(
                [
                    y
                    for y in by_year
                    if not (isinstance(by_year[y], dict) and "error" in by_year[y] and len(by_year[y]) == 1)
                ]
            ),
        ),
    ...
    return RichResult(
        title=(f"OTIS {ds_id} -- per-fiscal-year full DLRM-on-Ruhela-formulations suite (10 estimators × N years)"),
        summary_lines=summary,
        tables=[
            {
                "title": (
                    "Per-year × estimator: ATE / SE / 95% CI / p / n. "
                    "Triangulation across IPW + AIPW + g-comp + PSM "
                    "(NN+subclass) + IRM-DML (ATE+ATTE+ATC) + "
                    "match_first + ATC-AIPW + PLR + SuperLearner."
                ),
                "headers": ["FY", "Estimator", "Estimate", "SE", "95% CI", "p", "Notes"],
                "rows": long_rows,
            }
        ],
        interpretation=(
        ...
        payload={"by_year": by_year, "ds_id": ds_id, "treatment": treatment, "outcome": outcome},
    )


def analyze_a01_ruhela_per_year(
    df: pd.DataFrame | None = None,
) -> RichResult:
...
def analyze_b01_ruhela_per_year(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return analyze_ruhela_per_year(
        data, ds_id="b01", treatment=T, outcome=Y, covariates=cov, cluster_col="EndFiscalYear"
    )
...
def _ruhela_aggregate_on(
    ...
    *,
    ...
    _all_cols = list(dict.fromkeys([treatment, outcome, year_col, *cov, *cluster_cols]))
    ...
    for fam_label, family in (
        ...
        try:
            ...
            t_coef_keys = [k for k in model.params.index if treatment in k]
            if not t_coef_keys:
            ...
            rows.append([fam_label, treatment, "fit failed", str(type(e).__name__), str(e)[:30], "--", "--"])
    ...
    if cluster_group is not None:
        ...
        gee_specs = [
            ("Poisson", "Exch", sm.families.Poisson(), sm.cov_struct.Exchangeable()),
            ("Poisson", "Indep", sm.families.Poisson(), sm.cov_struct.Independence()),
            ("NB", "Exch", sm.families.NegativeBinomial(alpha=1.0), sm.cov_struct.Exchangeable()),
            ("NB", "Indep", sm.families.NegativeBinomial(alpha=1.0), sm.cov_struct.Independence()),
        ]
        for fam_lbl, corr_lbl, family, cov_struct in gee_specs:
            row_label = f"GEE-{fam_lbl} (cluster:{cluster_group}, {corr_lbl})"
            try:
                gee_model = smf.gee(
                    formula,
                ...
                t_coef_keys = [k for k in gee_model.params.index if treatment in k]
                if not t_coef_keys:
    ...
    overdispersion = round(aic_pois - aic_nb, 1) if np.isfinite(aic_pois) and np.isfinite(aic_nb) else "n/a"
    ...
    summary = [
        ...
        ("Cluster group (GEE marginal model)", cluster_group or "--"),
    ...
    return RichResult(
        ...
        tables=[
            {
                "title": (
                    "Aggregate Ruhela formulation -- Poisson + NB IRR "
                    + ("+ GEE clustered " if cluster_group else "")
                    + "for treatment effect on count outcome:"
                ),
                "headers": ["Family", "Coefficient", "IRR", "95% CI", "p-value", "AIC", "Notes"],
                "rows": rows,
            }
        ],
        interpretation=interpretation
...
def analyze_b03_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["Alert_Presence", "Number_SegregationPlacements"]).copy()
    work["T_alert"] = (
        work["Alert_Presence"].astype(str).str.strip().str.lower().isin(["yes", "y", "true", "1"]).astype(int)
    )
    return _ruhela_aggregate_on(
        work,
        ds_id="b03",
        source_label=("b03_segregation_placements_alerts_and_hold_flags_by_institution.csv"),
        title=("OTIS b03 -- Aggregate Ruhela formulation: Alert presence -> Number of segregation placements"),
        interpretation=(
...
def analyze_b07_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    # Pivot to long
    long_with = work[["EndFiscalYear", "Alert_Type", "Gender", "Number_Segregation_Placements_With_Alert"]].rename(
        columns={"Number_Segregation_Placements_With_Alert": "n_placements"}
    )
    long_with["T_alert"] = 1
    long_without = work[
        ["EndFiscalYear", "Alert_Type", "Gender", "Number_Segregation_Placements_Without_Alert"]
    ].rename(columns={"Number_Segregation_Placements_Without_Alert": "n_placements"})
    long_without["T_alert"] = 0
    long_df = pd.concat([long_with, long_without], ignore_index=True)
    return _ruhela_aggregate_on(
        long_df,
        ds_id="b07",
        source_label=("b07_segregation_placements_alerts_and_hold_flags_by_gender.csv"),
        title=(
            "OTIS b07 -- Aggregate Ruhela formulation: "
...
def analyze_c01_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["EndFiscalYear", "Gender", "NumberIndividuals_RestrictiveConfinement"]).copy()
    work["T_female"] = _female_indicator(work["Gender"])
    return _ruhela_aggregate_on(
        work,
        ds_id="c01",
        source_label=("c01_individuals_in_segregation_and_restrictive_confinement_total_individuals.csv"),
        title=("OTIS c01 -- Aggregate Ruhela formulation: Female indicator -> Number of individuals in RC"),
        interpretation=(
...
def analyze_c07_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(
        subset=["EndFiscalYear", "Alert_Type", "Gender", "NumberIndividuals_RestrictiveConfinement"]
    ).copy()
    # Treat any Alert_Type other than "No Alert" / "None" as alert-present
    no_alert_strs = {"no alert", "none", "no_alert", "no"}
    work["T_alert_present"] = (~work["Alert_Type"].astype(str).str.strip().str.lower().isin(no_alert_strs)).astype(int)
    # If no rows remain in either arm, skip
    if work["T_alert_present"].sum() == 0 or (1 - work["T_alert_present"]).sum() == 0:
        return RichResult(
            title="c07 aggregate Ruhela",
            warnings=["c07 has no no-alert reference rows -- treatment indicator is degenerate"],
        )
    return _ruhela_aggregate_on(
        work,
        ds_id="c07",
        source_label=("c07_individuals_in_segregation_and_restrictive_confinement_alerts_and_hold_flags.csv"),
        title=("OTIS c07 -- Aggregate Ruhela formulation: Alert presence × Gender -> Individuals in RC"),
        interpretation=(
...
def analyze_b06_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work["T_disciplinary"] = (work["Reason"].astype(str).str.lower().str.contains("disciplinary", na=False)).astype(int)
    return _ruhela_aggregate_on(
        work,
        ds_id="b06",
        source_label=("b06_segregation_placements_reason_for_placement_by_institution.csv"),
        title=("OTIS b06 -- Aggregate Ruhela formulation: Disciplinary reason -> Number of seg placements"),
        interpretation=(
...
def analyze_c03_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["EndFiscalYear", "Race", "Gender", "NumberIndividuals_RestrictiveConfinement"]).copy()
    work["T_indigenous"] = (work["Race"].astype(str).str.lower() == "indigenous").astype(int)
    return _ruhela_aggregate_on(
        work,
        ds_id="c03",
        source_label=("c03_individuals_in_segregation_and_restrictive_confinement_race_by_gender.csv"),
        title=("OTIS c03 -- Aggregate Ruhela formulation: Indigenous race -> Individuals in RC"),
        interpretation=(
...
def analyze_c04_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(
        subset=["EndFiscalYear", "Race", "Region_MostRecentPlacement", "NumberIndividuals_RestrictiveConfinement"]
    ).copy()
    work["T_indigenous"] = (work["Race"].astype(str).str.lower() == "indigenous").astype(int)
    return _ruhela_aggregate_on(
        work,
        ds_id="c04",
        source_label=("c04_individuals_in_segregation_and_restrictive_confinement_race_by_region.csv"),
        title=("OTIS c04 -- Aggregate Ruhela formulation: Indigenous race -> Individuals in RC, by region"),
        interpretation=(
...
def analyze_c06_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_aggregate_on(
        work,
        ds_id="c06",
        source_label=("c06_individuals_in_segregation_and_restrictive_confinement_age_category_by_region.csv"),
        title=("OTIS c06 -- Aggregate Ruhela formulation: Age 50+ -> Individuals in RC, by region"),
        interpretation=(
...
def analyze_c09_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(
        subset=["EndFiscalYear", "Age_Category", "Gender", "NumberIndividuals_RestrictiveConfinement"]
    ).copy()
    work["T_50plus"] = _age_50plus_indicator(work["Age_Category"])
    return _ruhela_aggregate_on(
        work,
        ds_id="c09",
        source_label=("c09_individuals_in_segregation_and_restrictive_confinement_age_category_by_gender.csv"),
        title=("OTIS c09 -- Aggregate Ruhela formulation: Age 50+ -> Individuals in RC, by gender"),
        interpretation=(
            "Aggregate Ruhela formulation on c09 testing age-50+ overrepresentation in RC controlling for gender."
        ),
        treatment="T_50plus",
        outcome="NumberIndividuals_RestrictiveConfinement",
        covariates=["Gender"],
...
def analyze_a01_ruhela_alt_gender(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        cell_data,
        ds_id="a01-altG",
        source_label=("a01_restrictive_confinement_detailed_dataset.csv (alt-T: Gender)"),
        title=("OTIS a01 -- Alt-T Ruhela formulation: T=Female -> Y=vm count (regional volatility)"),
        interpretation=(
...
def analyze_a01_ruhela_alt_age(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        cell_data,
        ds_id="a01-altA",
        source_label=("a01_restrictive_confinement_detailed_dataset.csv (alt-T: Age 50+)"),
        title=("OTIS a01 -- Alt-T Ruhela formulation: T=Age 50+ -> Y=vm count"),
        interpretation=(
...
def analyze_a01_ruhela_alt_toronto(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        cell_data,
        ds_id="a01-altT",
        source_label=("a01_restrictive_confinement_detailed_dataset.csv (alt-T: Toronto region)"),
        title=("OTIS a01 -- Alt-T Ruhela formulation: T=Toronto region -> Y=vm count"),
        interpretation=(
...
def analyze_b01_ruhela_alt_gender(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS b01 -- Alt-T Ruhela formulation: T=Female -> Y=vm count (regional volatility)"),
        interpretation=(
...
def analyze_b01_ruhela_alt_age(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS b01 -- Alt-T Ruhela formulation: T=Age 50+ -> Y=vm count"),
        interpretation=(
...
def analyze_b01_ruhela_alt_toronto(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS b01 -- Alt-T Ruhela formulation: T=Toronto region -> Y=vm count"),
        interpretation=(
...
def analyze_a01_ruhela_subgroup_female(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        sub,
        ds_id="a01-subF",
        source_label=("a01_restrictive_confinement_detailed_dataset.csv (subgroup: Female)"),
        title=("OTIS a01 -- Subgroup Ruhela formulation: Female-only cell frame, T_high_ac -> vm count"),
        interpretation=(
        ...
        treatment=T,
        outcome=Y,
        covariates=cov_no_gender,
        cluster_col="EndFiscalYear",
    )


def analyze_a01_ruhela_subgroup_male(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        sub,
        ds_id="a01-subM",
        source_label=("a01_restrictive_confinement_detailed_dataset.csv (subgroup: Male)"),
        title=("OTIS a01 -- Subgroup Ruhela formulation: Male-only cell frame, T_high_ac -> vm count"),
        interpretation=(
            "Subgroup analysis: canonical Ruhela formulation restricted "
...
def analyze_b01_ruhela_subgroup_female(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS b01 -- Subgroup Ruhela formulation: Female-only cell frame, T_high_ac -> vm count"),
        interpretation=(
...
def analyze_b01_ruhela_subgroup_male(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS b01 -- Subgroup Ruhela formulation: Male-only cell frame, T_high_ac -> vm count"),
        interpretation=(
...
def analyze_b02_ruhela_alt_region(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work["T_toronto"] = _region_toronto_indicator(work["Region_MostRecentPlacement"])
    return _ruhela_formulations_on(
        work,
        ds_id="b02-altR",
        source_label=("b02_segregation_detailed_total_days.csv (alt-T: Toronto region)"),
        title=("OTIS b02 -- Alt-T Ruhela formulation: T=Toronto region -> Y=Total seg days within FY"),
        interpretation=(
...
def analyze_b02_ruhela_alt_age(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_formulations_on(
        ...
        title=("OTIS b02 -- Alt-T Ruhela formulation: T=Age 50+ -> Y=Total seg days within FY"),
        interpretation=(
        ...
        treatment="T_50plus",
        outcome="TotalAggregatedDays_Segregation",
        covariates=["Gender", "Region_MostRecentPlacement", "EndFiscalYear"],
        cluster_col="EndFiscalYear",
    )
...
def analyze_c05_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(
        subset=["EndFiscalYear", "Religion", "Region_MostRecentPlacement", "NumberIndividuals_RestrictiveConfinement"]
    ).copy()
    excluded = {"christian", "no religion", "unknown or not reported"}
    work["T_minority_religion"] = (~work["Religion"].astype(str).str.strip().str.lower().isin(excluded)).astype(int)
    return _ruhela_aggregate_on(
        work,
        ds_id="c05",
        source_label=("c05_individuals_in_segregation_and_restrictive_confinement_religion_by_region.csv"),
        title=("OTIS c05 -- Aggregate Ruhela formulation: Non-Christian/non-majority religion -> Individuals in RC"),
        interpretation=(
...
def analyze_c08_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["EndFiscalYear", "Religion", "Gender", "NumberIndividuals_RestrictiveConfinement"]).copy()
    excluded = {"christian", "no religion", "unknown or not reported"}
    work["T_minority_religion"] = (~work["Religion"].astype(str).str.strip().str.lower().isin(excluded)).astype(int)
    return _ruhela_aggregate_on(
        work,
        ds_id="c08",
        source_label=("c08_individuals_in_segregation_and_restrictive_confinement_religion_by_gender.csv"),
        title=(
            "OTIS c08 -- Aggregate Ruhela formulation: "
...
def analyze_b04_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(
        subset=["EndFiscalYear", "Region_AtTimeOfPlacement", "Gender", "Measure", "NumberConsecutiveDays_Segregation"]
    ).copy()
    work = work[work["Measure"].astype(str).str.strip() == "Median"].copy()
    if work.empty:
        return RichResult(
    ...
    return _ruhela_aggregate_on(
        work,
        ds_id="b04",
        source_label=("b04_segregation_placements_consecutive_durations_by_region.csv"),
        title=("OTIS b04 -- Aggregate Ruhela formulation: Female -> median consecutive seg duration"),
        interpretation=(
...
def analyze_b08_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_aggregate_on(
        work,
        ds_id="b08",
        source_label=("b08_segregation_placements_consecutive_durations_by_institution.csv"),
        title=("OTIS b08 -- Aggregate Ruhela formulation: Female -> median seg duration, GEE-clustered on institution"),
        interpretation=(
...
def analyze_b09_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(
        subset=["EndFiscalYear", "NumberPlacements_Segregation", "Gender", "NumberIndividuals_Segregation"]
    ).copy()
    work["T_female"] = _female_indicator(work["Gender"])
    return _ruhela_aggregate_on(
        work,
        ds_id="b09",
        source_label=("b09_individuals_in_segregation_number_of_times_in_segregation.csv"),
        title=("OTIS b09 -- Aggregate Ruhela formulation: Female -> Number of individuals in segregation"),
        interpretation=(
...
def analyze_c02_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    **Institution as a GEE cluster group** (marginal model with
    ...
    return _ruhela_aggregate_on(
        work,
        ds_id="c02",
        source_label=("c02_individuals_in_segregation_and_restrictive_confinement_by_institution.csv"),
        title=("OTIS c02 -- Aggregate Ruhela formulation: Female -> Individuals in RC, GEE-clustered on institution"),
        interpretation=(
...
def analyze_c10_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    **GEE cluster group** (25 levels) for cluster-robust marginal
    ...
    return _ruhela_aggregate_on(
        work,
        ds_id="c10",
        source_label=("c10_segregation_and_restrictive_confinement_aggregate_durations_by_institution.csv"),
        title=("OTIS c10 -- Aggregate Ruhela formulation: Female -> median RC days, GEE-clustered on institution"),
        interpretation=(
...
def analyze_c12_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    return _ruhela_aggregate_on(
        work,
        ds_id="c12",
        source_label=("c12_segregation_and_restrictive_confinement_aggregate_durations_by_region.csv"),
        title=("OTIS c12 -- Aggregate Ruhela formulation: Female -> median RC days, by region"),
        interpretation=(
...
def analyze_c11_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["EndFiscalYear", "Aggregate_Duration", "NumberIndividuals_RestrictiveConfinement"]).copy()
    long_bins = {
        "16 to 20 days",
        "21 to 25 days",
        "26 to 30 days",
    ...
    work["T_long_duration"] = work["Aggregate_Duration"].astype(str).isin(long_bins).astype(int)
    return _ruhela_aggregate_on(
        work,
        ds_id="c11",
        source_label=("c11_individuals_in_segregation_and_restrictive_confinement_aggregate_lengths.csv"),
        title=("OTIS c11 -- Aggregate Ruhela formulation: Long-duration bin (≥16 days) -> Individuals in RC"),
        interpretation=(
...
def analyze_c01_ruhela_aggregate_region_cluster(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return _ruhela_aggregate_on(
        ...
        ds_id="c01-RC",
        source_label=(
            "c01_individuals_in_segregation_and_restrictive_confinement_total_individuals.csv (year-clustered variant)"
        ),
        title=("OTIS c01 -- Aggregate Ruhela formulation, year-clustered: Female -> Individuals in RC"),
        interpretation=(
...
def analyze_c04_ruhela_aggregate_region_cluster(df: pd.DataFrame | None = None) -> RichResult:
    ...
    return _ruhela_aggregate_on(
        ...
        ds_id="c04-RC",
        source_label=(
            "c04_individuals_in_segregation_and_restrictive_confinement_race_by_region.csv (region-clustered variant)"
        ),
        title=("OTIS c04 -- Aggregate Ruhela formulation, region-clustered: Indigenous -> Individuals in RC"),
        interpretation=(
...
def analyze_d02_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["Year", "Gender", "Number_CustodialDeaths"]).copy()
    work["T_female"] = _female_indicator(work["Gender"])
    return _ruhela_aggregate_on(
        work,
        ds_id="d02",
        source_label="d02_deaths_in_custody_gender.csv",
        title=("OTIS d02 -- Aggregate Ruhela formulation: Female -> Number of custodial deaths"),
        interpretation=(
...
def analyze_d03_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["Year", "Race", "Number_CustodialDeaths"]).copy()
    work["T_indigenous"] = (work["Race"].astype(str).str.lower() == "indigenous").astype(int)
    return _ruhela_aggregate_on(
        work,
        ds_id="d03",
        source_label="d03_deaths_in_custody_race.csv",
        title=("OTIS d03 -- Aggregate Ruhela formulation: Indigenous -> Number of custodial deaths"),
        interpretation=(
...
def analyze_d04_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["Year", "Religion", "Number_CustodialDeaths"]).copy()
    excluded = {"christian", "no religion", "unknown or not reported"}
    work["T_minority_religion"] = (~work["Religion"].astype(str).str.strip().str.lower().isin(excluded)).astype(int)
    return _ruhela_aggregate_on(
        work,
        ...
        title=("OTIS d04 -- Aggregate Ruhela formulation: Non-majority religion -> Number of custodial deaths"),
        interpretation=(
...
def analyze_d05_ruhela_aggregate(
    df: pd.DataFrame | None = None,
) -> RichResult:
    ...
    work = df.dropna(subset=["Year", "Age_Category", "Number_CustodialDeaths"]).copy()
    work["T_50plus"] = _age_50plus_indicator(work["Age_Category"])
    return _ruhela_aggregate_on(
        work,
        ds_id="d05",
        source_label="d05_deaths_in_custody_age_category.csv",
        title=("OTIS d05 -- Aggregate Ruhela formulation: Age 50+ -> Number of custodial deaths"),
        interpretation=(
...
def analyze_ruhela_grid() -> RichResult:
    ...
    for ds_id, fn in aggregates_to_run:
        try:
            ...
            rows.append([ds_id, "--", "load_err", str(type(e).__name__), str(e)[:30], "--", "--"])
            continue
        if r.warnings:
            rows.append([ds_id, "--", "warn", (r.warnings or [""])[0][:50], "--", "--", "--"])
            continue
        ...
        primary = _find("GEE-NB") and _find("GEE-NB (cluster") or _find("GEE-NB")
        ...
        if primary is None:
            primary = next((row for row in r.tables[0]["rows"] if row[0] == "NB" and row[2] != "fit failed"), None)
            primary_type = "NB GLM" if primary else None
        if primary is None:
            primary = next((row for row in r.tables[0]["rows"] if row[0] == "Poisson" and row[2] != "fit failed"), None)
    ...
    return RichResult(
        ...
        summary_lines=[
            ...
            ("MRM chi² datasets (c, d)", "see analyze_c_chi2 / analyze_d_chi2"),
            ("Primary estimator priority", "GEE-NB > GEE-Poisson > NB GLM > Poisson GLM"),
        ],
        tables=[
            {
                "title": ("Aggregate Ruhela formulations -- primary IRR per dataset (GEE cluster-robust > NB GLM):"),
                "headers": ["DS", "Type", "Formulation", "IRR", "95% CI", "p", "Notes"],
                "rows": rows,
            }
        ],
        interpretation=(
...
def analyze_ruhela_master(*, include_per_row: bool = False) -> RichResult:
    ...
    if grid.tables:
        sections.append(
            {
                "title": ("§1 Aggregate Ruhela formulations -- primary IRR per dataset:"),
    ...
    if include_per_row:
        for ds_id, fn in [
            ...
            try:
                ...
                per_row_rows.append([ds_id, "load_err", str(type(e).__name__), str(e)[:30], "--", "--"])
                continue
            if r.warnings:
                per_row_rows.append([ds_id, "warn", (r.warnings or [""])[0][:50], "--", "--", "--"])
                continue
            if not (r.tables and r.tables[0].get("rows")):
                per_row_rows.append([ds_id, "no rows", "--", "--", "--", "--"])
                continue
            irm_row = next(
                (row for row in r.tables[0]["rows"] if "IRM-DML" in str(row[0]) and "ATE" in str(row[1])), None
            )
            if irm_row:
                per_row_rows.append(
                    [
    ...
    if per_row_rows:
        sections.append(
            {
                "title": ("§2 Per-row Ruhela formulations (canonical T_high_ac -> vm_count) -- IRM-DML ATE:"),
    ...
    try:
        ...
        if doob_rows:
            sections.append(
                {
                    "title": ("§3 MRM chi-square family -- Pearson χ² + Cramer's V on aggregate contingency tables:"),
                    "headers": ["Series", "Slice/measure", "χ²", "dof", "p", "Cramer's V"],
                    "rows": doob_rows,
                }
            )
    except Exception:  # noqa: BLE001
    ...
    try:
        ...
        for r in _sd.TABLE19_MANDELA_CLASSIFICATION:
            sd_rows.append(["SD-2021-Feb T19", r["category"], f"{r['percent']:.1f}%", r["n"], "--", "--"])
        ...
        if hasattr(_sd, "TABLE11_CHISQ"):
            sd_rows.append(
                [
                    ...
                    f"χ²={_sd.TABLE11_CHISQ['chi2']}, df={_sd.TABLE11_CHISQ['df']}",
        ...
        if hasattr(_sd, "TABLE22_CHISQ"):
            sd_rows.append(
                [
                    "SD-2021-Feb T22",
                    "Region × Mandela χ²",
                    f"χ²={_sd.TABLE22_CHISQ['chi2']}, df={_sd.TABLE22_CHISQ['df']}",
        ...
        sd_rows.append(["SDI-2021-May", "n_iedm_stays_reviewed", "--", h["n_iedm_stays_reviewed"], "--", "--"])
        sd_rows.append(
            [
                "SDI-2021-May",
                "% IEDM stay-in (Table 9)",
        ...
        sd_rows.append(
            ["SDI-2021-May", "long-stay no-IEDM record (T15)", "--", h["n_long_stay_no_iedm_record_min76d"], "--", "--"]
        )
        sd_rows.append(
            [
                "SDI-2021-May",
        ...
        if hasattr(_sd, "TABLE5_MAY2021_CHISQ"):
            sd_rows.append(
                [
                    "SDI-2021-May T5",
                    "Race × #IEDM-reviews χ²",
                    f"χ²={_sd.TABLE5_MAY2021_CHISQ['chi2']}, df={_sd.TABLE5_MAY2021_CHISQ['df']}",
        ...
        if hasattr(_sd, "TABLE10_MAY2021_CHISQ"):
            sd_rows.append(
                [
                    ...
                    f"χ²={_sd.TABLE10_MAY2021_CHISQ['chi2']}, df={_sd.TABLE10_MAY2021_CHISQ['df']}",
        ...
        if sd_rows:
            sections.append(
                {
                    "title": (
                        "§4 RF-extended (federal): Sprott-Doob CRIMSL "
                        "+ Sprott-Doob-Iftene Schulich Law SIU "
                        "analyses (national-aggregate companion):"
                    ),
                    "headers": ["Source", "Item", "Percent/Stat", "N", "Solitary/1k", "Torture/1k"],
                    "rows": sd_rows,
                }
            )
    except Exception:  # noqa: BLE001
    ...
    try:
        ...
        if doob_nat_rows:
            sections.append(
                {
                    "title": (
                        "§5 RF-extended (federal): Doob T-539-20 "
    ...
    try:
        ...
        if b05_man.tables and b05_man.tables[0].get("rows"):
            for r in b05_man.tables[0]["rows"]:
                man_rows.append(["b05 placement", str(r[0]), r[2], r[4], r[5]])
        # Provincial c11 (individuals) Segregation per year
        if c11_man.payload.get("rows"):
            for r in c11_man.payload["rows"]:
                if r[1] == "Segregation":
                    man_rows.append([f"c11 individuals ({r[1]})", str(r[0]), r[3], r[5], r[6]])
        ...
        if man_rows:
            sections.append(
                {
                    "title": (
                        "§6 Mandela-RF -- Ontario provincial Mandela "
                        "classification (OTIS b05/c11) + federal "
                        "cross-comparison:"
                    ),
                    "headers": ["Source", "Year/Slice", "Solitary %", "Torture %", "N or note"],
                    "rows": man_rows,
                }
            )
    except Exception:  # noqa: BLE001
    ...
    return RichResult(
        title=("OTIS Ruhela formulations -- paper-ready master report (provincial + federal-aggregate companion)"),
        summary_lines=[
            ...
            ("Federal panel context", "morie.siuiap (Sapers chair, Doob, Sprott)"),
        ],
        tables=sections,
        interpretation=(
        ...
        payload={"include_per_row": include_per_row, "n_sections": len(sections)},
    )
...
def analyze_b05_mandela_classification(
    ...
    df = df if df is not None else load_otis_dataset("b05")
    df = df.copy()
    df["mandela_class"] = df["Consecutive_Duration"].astype(str).apply(_classify_otis_bins)

    # Per-year totals
    rows = []
    for year, ydf in df.groupby("EndFiscalYear", sort=True):
        n_solitary = int(
            ydf.loc[ydf["mandela_class"] == "Solitary Confinement (≤15d)", "Number_SegregationPlacements"].sum()
        )
        n_torture = int(ydf.loc[ydf["mandela_class"] == "Torture (≥16d)", "Number_SegregationPlacements"].sum())
        total = n_solitary + n_torture
        if total == 0:
            continue
        sol_pct = 100.0 * n_solitary / total
        tor_pct = 100.0 * n_torture / total
        rows.append([int(year), n_solitary, f"{sol_pct:.1f}%", n_torture, f"{tor_pct:.1f}%", total])
    ...
    if rows:
        ...
        prov_torture_pct = float(latest_year_row[4].rstrip("%"))
    else:
        prov_torture_pct = float("nan")

    return RichResult(
        title=(
            "Mandela-RF on OTIS b05 -- per-placement Mandela "
        ...
        summary_lines=[
            ...
            (
                ...
                f"Solitary {sd_federal_solitary_pct}%, Torture {sd_federal_torture_pct}%",
            ),
            (
        ...
        tables=[
            {
                "title": ("OTIS b05 Mandela-class by fiscal year (placements as the unit):"),
                "headers": ["Fiscal year", "Solitary N", "Solitary %", "Torture N", "Torture %", "Total"],
                "rows": rows,
            }
        ],
        interpretation=(
...
def analyze_c11_mandela_classification(
    ...
    for year, ydf in df.groupby("EndFiscalYear", sort=True):
        for kind_col, kind_label in [
            ...
            ("NumberIndividuals_RestrictiveConfinement", "Restrictive confinement"),
        ]:
            n_solitary = int(ydf.loc[ydf["mandela_class"] == "Solitary Confinement (≤15d)", kind_col].sum())
            n_torture = int(ydf.loc[ydf["mandela_class"] == "Torture (≥16d)", kind_col].sum())
            total = n_solitary + n_torture
            if total == 0:
                continue
            sol_pct = 100.0 * n_solitary / total
            tor_pct = 100.0 * n_torture / total
            rows.append([int(year), kind_label, n_solitary, f"{sol_pct:.1f}%", n_torture, f"{tor_pct:.1f}%", total])

    return RichResult(
        title=(
            "Mandela-RF on OTIS c11 -- per-individual Mandela "
        ...
        summary_lines=[
            ...
            (
                "Two views",
                "Segregation (closer match to federal SIU); Restrictive Confinement (broader Ontario superset)",
            ),
        ...
        tables=[
            {
                "title": ("OTIS c11 Mandela-class by year × confinement type (individuals as the unit):"),
                "headers": ["Year", "Type", "Solitary N", "Solitary %", "Torture N", "Torture %", "Total"],
                "rows": rows,
            }
        ],
        interpretation=(
...
def analyze_otis_mandela_provincial_vs_federal() -> RichResult:
    ...
    fed_solitary_pct = next(
        r["percent"] for r in _sd.TABLE19_MANDELA_CLASSIFICATION if r["category"] == "Solitary Confinement"
    )
    fed_torture_pct = next(r["percent"] for r in _sd.TABLE19_MANDELA_CLASSIFICATION if r["category"] == "Torture")
    fed_n = sum(r["n"] for r in _sd.TABLE19_MANDELA_CLASSIFICATION)
    ...
    for r in c11_r.payload["rows"]:
        if r[1] == "Segregation":
            rows.append(["Ontario Provincial Segregation", str(r[0]), "Individuals", r[3], r[5], r[6]])
    # Provincial RC rows
    for r in c11_r.payload["rows"]:
        if r[1] == "Restrictive confinement":
            rows.append(["Ontario Provincial RC (broader)", str(r[0]), "Individuals", r[3], r[5], r[6]])

    # Compute the largest provincial-vs-federal torture-rate gap
    seg_rows = [r for r in c11_r.payload["rows"] if r[1] == "Segregation"]
    if seg_rows:
        max_torture_pct = max(float(r[5].rstrip("%")) for r in seg_rows)
        max_torture_year = next(r[0] for r in seg_rows if abs(float(r[5].rstrip("%")) - max_torture_pct) < 0.01)
    ...
    return RichResult(
        title=("Mandela-RF cross-comparison -- Ontario provincial (OTIS) vs federal SIUs (Sprott-Doob T19)"),
        summary_lines=[
            ("Federal SIU reference", f"Solitary {fed_solitary_pct}%, Torture {fed_torture_pct}% (N={fed_n})"),
            ("Provincial peak torture-rate (Segregation)", f"{max_torture_pct:.1f}% in {max_torture_year}"),
            (
                "Provincial-vs-federal gap (peak)",
                f"{gap_pp:+.1f} percentage points ({'higher' if gap_pp > 0 else 'lower'} provincially)",
            ),
            (
                "Caveat",
        ...
        tables=[
            {
                ...
                "headers": ["Source", "Period", "Unit", "Solitary %", "Torture %", "N"],
                "rows": rows,
            }
        ],
        interpretation=(
        ...
        payload={
            "federal": {"solitary_pct": fed_solitary_pct, "torture_pct": fed_torture_pct, "n": fed_n},
...
def analyze_b01_dual(*args, **kwargs):
    ...
    _w.warn("analyze_b01_dual is deprecated; use analyze_b01_ruhela_formulations", DeprecationWarning, stacklevel=2)
    return analyze_b01_ruhela_formulations(*args, **kwargs)
...
def _contingency_chi2(df: pd.DataFrame, *, row: str, col: str, value: str) -> tuple[pd.DataFrame, dict]:
    ...
    table = g.groupby([row, col])[value].sum().unstack(fill_value=0)
    ...
    *,
    ...
    for ds, r_col, c_col in slices:
        try:
            ...
            table, stats = _contingency_chi2(df, row=r_col, col=c_col, value=contingency_value)
        except Exception as e:  # noqa: BLE001
            stats = {
                "chi2": float("nan"),
                "cramer_v": float("nan"),
        ...
        rows.append(
            [
                ...
                f"{r_col} × {c_col}",
                int(stats.get("n", 0)),
                (round(stats["chi2"], 3) if np.isfinite(stats.get("chi2", float("nan"))) else "n/a"),
                int(stats.get("dof", 0)),
                (f"{stats['pvalue']:.2e}" if np.isfinite(stats.get("pvalue", float("nan"))) else "n/a"),
                (round(stats["cramer_v"], 4) if np.isfinite(stats.get("cramer_v", float("nan"))) else "n/a"),
    ...
    return RichResult(
        title=("OTIS c-series -- χ² + Cramer's V family on demographic contingency tables"),
        summary_lines=[
        ...
        tables=[
            {
                "title": (f"Contingency χ² and Cramer's V on c-series slices (value = {contingency_value}):"),
                "headers": ["Dataset", "Slice", "n", "χ²", "dof", "p", "Cramer's V", "min cell"],
                "rows": rows,
            }
        ],
        interpretation=(
        ...
        payload={"slices": payloads, "contingency_value": contingency_value},
    )
...
def analyze_d_aggregate() -> RichResult:
    ...
    yearly = d01.dropna(subset=["Year"]).groupby("Year").size().rename("n_deaths")
    yearly_rows = []
    ...
    if len(yearly) >= 2:
        ...
        if n_first > 0:
            ...
            rr_text = f"{rr:.3f} ({last_y}/{first_y}; n={n_last}/{n_first}; 95% CI [{ci[0]:.3f}, {ci[1]:.3f}])"
            payloads["yoy_rate_ratio"] = {
                "first_year": first_y,
                "last_year": last_y,
                "n_first": n_first,
                "n_last": n_last,
    ...
    table_d06, stats_d06 = _contingency_chi2(
        d06, row="Alert_Type", col="MedicalCauseOfDeath", value="Number_CustodialDeaths"
    )
    payloads["d06_chi2"] = stats_d06

    # 4. d07 Alert × Housing_Type
    d07 = load_otis_dataset("d07")
    table_d07, stats_d07 = _contingency_chi2(d07, row="Alert_Type", col="Housing_Type", value="Number_CustodialDeaths")
    ...
    for ds, slice_label, st in [
        ...
        chi_rows.append(
            [
                ...
                slice_label,
                int(st.get("n", 0)),
                (round(st["chi2"], 3) if np.isfinite(st.get("chi2", float("nan"))) else "n/a"),
                int(st.get("dof", 0)),
                (f"{st['pvalue']:.2e}" if np.isfinite(st.get("pvalue", float("nan"))) else "n/a"),
                (round(st["cramer_v"], 4) if np.isfinite(st.get("cramer_v", float("nan"))) else "n/a"),
                int(st.get("min_cell", 0)),
            ]
        )

    return RichResult(
        title=("OTIS d-series -- yearly death counts + Alert × Cause/Housing χ²"),
        summary_lines=[
            ("d01 total deaths", int(yearly.sum())),
            ("d01 year range", f"{int(yearly.index.min())}–{int(yearly.index.max())}" if not yearly.empty else "n/a"),
            ("Year-over-year RR", rr_text),
            (
                "d06 (Alert × MedicalCause) Cramer's V",
                round(stats_d06["cramer_v"], 4) if np.isfinite(stats_d06.get("cramer_v", float("nan"))) else "n/a",
            ),
            (
                "d07 (Alert × Housing_Type) Cramer's V",
                round(stats_d07["cramer_v"], 4) if np.isfinite(stats_d07.get("cramer_v", float("nan"))) else "n/a",
            ),
        ],
        tables=[
            {
            ...
            {
                "title": ("d06 / d07 Alert-flag × outcome contingency χ² + Cramer's V:"),
                "headers": ["Dataset", "Slice", "n", "χ²", "dof", "p", "Cramer's V", "min cell"],
                "rows": chi_rows,
            },
        ],
...
def analyze_a01_with_csi_context(
    df: pd.DataFrame | None = None,
    *,
    variant: str = "total",
    ...
    for cat in tps_csi.CSI_CATEGORIES:
        ...
        try:
            ...
            if not p.exists():
                ...
                continue
            tps_df = pd.read_csv(p, usecols=lambda c: c in {"OCC_YEAR", "HOOD_158"})
            csi_dfs[cat] = tps_df
    ...
    csi_result = tps_csi.analyze_csi_from_tps_dataframes(csi_dfs, variant=variant)
    ...
    if rebase_to_year is not None and not by_year_full.empty:
        try:
            by_year_full = tps_csi.csi_per_year(
                {int(r["year"]): {c: 0 for c in tps_csi.CSI_CATEGORIES} for _, r in by_year_full.iterrows()},
                variant=variant,
                ...
                if "OCC_YEAR" in tps_df.columns:
                    for y, n in tps_df.groupby("OCC_YEAR").size().items():
                        try:
                            ...
                            continue
            by_year_full = tps_csi.csi_per_year(counts_by_year, variant=variant, rebase_to_year=rebase_to_year)
        except (ValueError, KeyError) as e:
            csi_load_errors.append(f"rebase_to_year={rebase_to_year} failed: {e}")

    # Restrict to OTIS overlap (2023-2025)
    otis_years = {2023, 2024, 2025}
    overlap = by_year_full[by_year_full["year"].isin(otis_years)] if "year" in by_year_full.columns else by_year_full

    csi_table_rows = []
    for _, r in overlap.iterrows():
        idx_val = round(float(r["csi_index"]), 2) if "csi_index" in r.index and pd.notna(r["csi_index"]) else "--"
    ...
    summary = list(otis_result.summary_lines or []) + [
        ...
        (
            "-- Years with CSI data",
            sorted(by_year_full["year"].astype(int).tolist()) if "year" in by_year_full.columns else "n/a",
        ),
    ]
    interpretation = (
    ...
    return RichResult(
        ...
        tables=[
            {
                "title": (f"Toronto CSI by year (variant={variant}, rebased to {rebase_to_year}=100):"),
                "headers": ["year", "weighted_sum", "incidents", "csi_per_capita", "csi_index"],
                "rows": csi_table_rows,
            }
        ],
        interpretation=interpretation,
        payload={
            "otis": otis_result.payload,
            "csi_overlap_years": csi_table_rows,
            "csi_full_payload": csi_result.payload,
            "variant": variant,
            "rebase_to_year": rebase_to_year,
        },
        warnings=(list(otis_result.warnings or []) + csi_load_errors if csi_load_errors else otis_result.warnings),
    )
