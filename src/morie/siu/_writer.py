...
def write_csv(rows: Iterable[dict], path: Path | str, *, exclude_narrative: bool = True) -> int:
    ...
    cols = SIU_COLUMNS if not exclude_narrative else [c for c in SIU_COLUMNS if c != "narrative_full"]
...
def write_jsonl(
    rows: Iterable[dict], path: Path | str, *, only_keys: tuple = ("case_number", "drid", "narrative_full")
) -> int:
