"""
Central command registry for MORIE StatScreen and REPL.
...
def _make_stat_handler(mod_name: str, func_name: str) -> Callable:
    ...
    def handler(parts: list[str], log: Any, store: Callable) -> None:
        ...
        mod = importlib.import_module(f"morie.{mod_name}")
...
def _make_repl_handler(mod_name: str, func_name: str) -> Callable:
    ...
    try:
        ...
        _mod = importlib.import_module(f"morie.{mod_name}")
        _fn = getattr(_mod, func_name)
        handler.__doc__ = _fn.__doc__
        handler.__name__ = func_name
    except Exception:
        handler.__doc__ = f"{func_name} from morie.{mod_name}"
        handler.__name__ = func_name
...
def _auto_register(
    ...
    """Auto-register all public functions/classes from morie.<mod_name>.
    ...
    for name in sorted(dir(mod)):
        ...
        if obj_module != f"morie.{mod_name}":
            continue
...
def _register_fn_shortcuts() -> int:
    """Register morie.fn short-name aliases for individual function files.
