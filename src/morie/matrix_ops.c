...
/*
 * matrix_ops.c — MORIE Matrix Operations
 *
 * References:
...
double morie_mat_det(const double *A, int n) {
    ...
    int sign;
    int rc = morie_lu_decompose(A, n, LU, piv, &sign);
    if (rc != MAT_OK) {
        free(LU); free(piv);
        return 0.0;
    }
...
int morie_mat_inverse(const double *A, int n, double *Ainv) {
    ...
    int rc = morie_lu_decompose(A, n, LU, piv, NULL);
    if (rc != MAT_OK) {
        free(LU); free(piv); free(col); free(e);
        return rc;
    }
...
int morie_svd(
    ...
    int rc = morie_eigen_symmetric(AtA, n, evals, evecs, max_iter);
    if (rc != MAT_OK) {
        free(AtA); free(evals); free(evecs);
        return rc;
    }
