...
def gibbons_fligner_wolfe_test(groups, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Fligner-Wolfe test against simple tree alternative using sum of U statistics"})
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Fligner-Wolfe test against simple tree alternative using sum of U statistics"})
