...
_DBTAPS = {
    # Full double precision, as PyWavelets 1.x tabulates them; they agree
...
def orthfilt(order=4):
    ...
    return RichResult(
        payload={
            # decomposition filters in the convolution convention of
            # PyWavelets and MATLAB wfilters: the time-reversed synthesis
            # filters (the transform here correlates with h, which is the
            # same operation)
            "dec_lo": list(reversed(h)),
            "dec_hi": list(reversed(g)),
            "rec_lo": list(rl),
            "rec_hi": list(rh),
            "order": k,
            "length": L,
