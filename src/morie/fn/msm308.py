...
def mvsml_convolutional_nn_eq_14_14(ediction, performance, of, the, model, no):
    ...
    """
    ediction = np.atleast_1d(np.asarray(ediction, dtype=float))
    n = len(ediction)
    result = float(np.mean(ediction))
    se = float(np.std(ediction, ddof=1) / np.sqrt(n)) if n > 1 else np.nan
    return RichResult(payload={"estimate": result, "se": se, "n": n, "method": "Numbered display equation (14.14) from MVSML chapter 14."})


def cheatsheet():
    return "msm308: Numbered display equation (14.14) from MVSML chapter 14."
