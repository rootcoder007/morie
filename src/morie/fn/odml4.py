...
"""DML ATE by gender for OTIS correctional data."""
...
def otis_dml_gender(df: pd.DataFrame, cdf=None, *, outcome: str = "Y", treatment: str = "D", gender_col: str = "gender", n_folds: int = 3, seed: int = 123) -> pd.DataFrame:
    """Estimate DML ATE separately within each gender group.
    ...
    ----------
    df : DataFrame
        Data with outcome, treatment, gender, and numeric covariates.
    outcome, treatment, gender_col : str
        Column names.
    n_folds : int
    ...
    -------
    DataFrame
        Columns: gender, ate, se, pval, ci_lower, ci_upper, n.
    """
    results = []
    for grp_name, grp in df.groupby(gender_col):
