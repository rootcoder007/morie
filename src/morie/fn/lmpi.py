...
def lmpi(X, y, x0, level=0.95, mean=False):
    ...
    xty = [sum(rows[i][a] * yv[i] for i in range(n)) for a in range(k)]
    xtxinv = [[float(v) for v in row] for row in C.inv(xtx)]
    beta = [sum(xtxinv[a][b] * xty[b] for b in range(k)) for a in range(k)]
    rss = 0.0
    for i in range(n):
        fit_i = 0.0
        for j in range(k):
