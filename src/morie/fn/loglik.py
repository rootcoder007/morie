...
def loglik(x: Union[Sequence[float], np.ndarray], pdf: Callable[[float], float]):
    ...
    if np.any(densities <= 0):
        ...
        raise ValueError(f"pdf returned zero/negative density at index {zero_idx[0]} (value={a[zero_idx[0]]}).")
    ...
    return RichResult(
        ...
        interpretation=(
            "Higher (less negative) is better fit. Compare via AIC (`akike`), BIC (`bayic`), or LR test (`lrtst`)."
        ),
...
def cheatsheet() -> str:
    return "loglik: loglik(x, pdf) -> Sum of log-pdfs evaluated at each observation."
