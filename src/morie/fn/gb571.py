...
def gibbons_wilcoxon_signed_rank(differences, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Wilcoxon signed-rank statistic T+ = sum R_i * I(D_i > 0)"})
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Wilcoxon signed-rank statistic T+ = sum R_i * I(D_i > 0)"})
