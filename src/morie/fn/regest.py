...
def regression_estimator(y, x, weights):
    ...
    if n < 1:
        return RichResult(
            payload={"estimate": np.nan, "n": 0, "method": "Regression estimator (linear adjustment to known X-mean)"}
        )
    ...
    return RichResult(
        payload={
            ...
            "n": n,
            "method": "Regression estimator (linear adjustment to known X-mean)",
        }
    )
