...
Kendall states this for untied rankings, where it coincides with the
tie-corrected tau-b.  With ties the two differ, and (P - Q)/(P + Q) is
no longer bounded by 1 in the way a correlation should be, so ``tau``
here is tau-b -- the form base R ``cor(method = "kendall")`` and scipy
``kendalltau`` both return --

    tau_b = (P - Q) / sqrt( (n0 - n1) (n0 - n2) )

    n0 = n(n-1)/2,  n1 = sum_g u_g(u_g-1)/2 over t ties,
                    n2 = sum_g v_g(v_g-1)/2 over x ties,

while the untied ratio Kendall (1938) writes is returned separately as
``tau_a``.  With no ties the two are equal.
...
def linear_trend(t, x):
    ...
    -------
    RichResult
        keys: ``tau`` (tau-b), ``tau_a`` (the untied ratio
        ``(P-Q)/(P+Q)``), ``S`` (Mann-Kendall concordance excess),
        ``var_S``, ``z``, ``p_value``, ``n_concordant``,
    ...
    S = conc - disc
    tau_a = float(S) / float(conc + disc) if (conc + disc) else float("nan")
    n0 = n * (n - 1.0) / 2.0
    den = math.sqrt((n0 - _tie_pairs(t)) * (n0 - _tie_pairs(xs)))
    tau = float(S) / den if den > 0.0 else float("nan")
