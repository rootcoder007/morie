...
def boot_multinomial_weights(n, B=200, rng=2, exhaustive=False):
    ...
    replication b, position i takes the point vdc(b, p_i) where p_i is
    the i-th prime -- a Halton design, one base per position.  A single
    stream shared by all positions is NOT usable here: consecutive van
    der Corput points stratify so evenly that every replication draws
    each index exactly once, every resample is a permutation of the
    data, and the bootstrap variance collapses to zero.  ``rng`` shifts
    the starting prime rather than being a seed.  With
