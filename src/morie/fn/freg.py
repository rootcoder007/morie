...
delta = 0.5 (c_{-1} - c_{+1}) / (c_{-1} - 2 c_0 + c_{+1}).  It is zero
only when the criterion happens to be symmetric about the integer
minimiser, which a real signal will not be, so the refined estimate
generally differs from the integer lag by a fraction of a sample even
when the two curves are an exact whole-sample shift of one another.
The INTEGER lag is the quantity that is exact in that case, and the
criterion there is exactly zero; both are anchored as such.  Callers
who want only whole-sample alignment should read the shift field, not
the estimate field.
"""
...
def function_register(y1, y2, max_lag=None):
    ...
    Returns
    -------
    estimate : the refined shift delta, in samples, such that
        y2(t + delta) best matches y1(t).  It is NEGATIVE when y2
        leads y1 and positive when y2 lags it -- the sign follows
        from the criterion being evaluated as y1[i] - y2[i + delta].
