...
def manwhi(x: Union[Sequence, np.ndarray], y: Union[Sequence, np.ndarray], alternative: str = "two-sided"):
    ...
    if n1 + n2 < 20:
        warnings.append(
            f"small total n ({n1 + n2}); exact distribution may be more reliable than the asymptotic Normal approx."
        )
...
def cheatsheet() -> str:
    return "manwhi: manwhi(x, y, alternative) -> Mann-Whitney U test for independent samples."
