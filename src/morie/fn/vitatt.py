...
def vit_self_attention(q, k, v, mask=None):
    ...
    mask : array-like or None
        N-by-N matrix; a zero entry blocks that key for that query, a
        non-zero entry keeps it.  ``None`` keeps everything.
    ...
    V = core.mat(v)
    n = len(Q)
    if n == 0 or len(K) == 0 or len(V) == 0:
        raise ValueError("vit_self_attention: q, k and v must be non-empty")
    if len(K) != n or len(V) != n:
        raise ValueError("vit_self_attention: q, k and v must have the same number of rows")
    dh = _rect(Q, "q")
    if _rect(K, "k") != dh:
    ...
    if mask is not None:
        ...
        if len(M) != n or len(M[0]) != n:
            raise ValueError("vit_self_attention: mask must be N-by-N")
