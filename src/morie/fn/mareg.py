...
def _wls(X, y, w, n, p):
    XtWX = [[sum(w[i] * X[i][r] * X[i][s] for i in range(n))
             for s in range(p)] for r in range(p)]
    XtWy = [sum(w[i] * X[i][r] * y[i] for i in range(n)) for r in range(p)]
    beta = core.ridgesolve(XtWX, XtWy, 1e-12)
    cols = []
    for j in range(p):
        e = [1.0 if r == j else 0.0 for r in range(p)]
        cols.append(core.ridgesolve(XtWX, e, 1e-12))
