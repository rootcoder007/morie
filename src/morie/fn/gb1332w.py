...
def gibbons_wrs_efficacy(f, lam, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Efficacy of Wilcoxon rank-sum test for two-sample location"})
    x_sorted = np.sort(f)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Efficacy of Wilcoxon rank-sum test for two-sample location"})
