...
def farrington_signal(counts, baseline_years=5, reference_window=3,
    ...
    result : dict
        ...
        alarm, phi, phi_raw, trend_coef, score, nbaseline, n, method.
    ...
    def _fit(om):
        ...
        dof = nb - p
        raw = sum(om[i] * (yb[i] - mu[i]) ** 2 / mu[i] for i in range(nb)) / dof
        phi = raw if raw > 1.0 else 1.0
        return beta, mu, inv, phi, raw
    ...
    se0 = mu0 * math.sqrt(raw * q0 if q0 > 0.0 else 0.0)
    tau = phi + se0 * se0 / mu0
