...
DETERMINISM.  The subsets are enumerated in lexicographic order rather
than drawn at random, so both language arms examine the same
...
def mve(X, h=None, n_starts=100000):
    ...
    best = None
    for J in R.combos(n, p + 1, int(n_starts)):
        mu, C = R.meancov(Xm, J)
        dd = R.mahal2(Xm, mu, C)
        if dd is None:
            continue
