...
def gibbons_t_efficacy(N, sigma, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Efficacy of Student's t test for location"})
    x_sorted = np.sort(N)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Efficacy of Student's t test for location"})
