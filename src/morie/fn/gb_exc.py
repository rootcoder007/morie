...
def gibbons_exceedance_stat(x, y, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Exceedance statistic: number of Y's exceeding the largest X or specific X order stat"})
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Exceedance statistic: number of Y's exceeding the largest X or specific X order stat"})
