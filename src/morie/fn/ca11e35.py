"""Weighted mean effect size ybar = sum(w y)/sum(w).
...
def ca_chapter_11_equation_35(ys, ws):
    """Weighted mean effect size ybar = sum(w y)/sum(w)

    Formula: ybar = sum(w_i y_i) / sum(w_i)

    Returns
    -------
    result : RichResult
        dict subclass; headline key 'mean' plus the full payload.
