...
*Biometrics* 57(4), 1138-1147, doi:10.1111/j.0006-341X.2001.01138.x.
That bibliographic record is verified against Crossref; note the
published title ends "spatial field trend", singular.

CITATION LIMIT, stated rather than papered over.  The Smith, Cullis and
Thompson paper is fully closed access -- Unpaywall reports no open
location, and the Rothamsted repository copy is staff-restricted -- so
its text could NOT be read.  The specific claims that this paper writes
the variance matrix as Lambda Lambda' + Psi, that it imposes the
zero-upper-triangle identifiability constraint, and that it gives a free
parameter count are therefore UNVERIFIED and are not asserted here.
What is implemented is the k-factor factor-analytic structure as this
function's own specification states it,

    Sigma = Lambda Lambda' + Psi,

with Lambda the n_env-by-k loadings, Psi the diagonal of environment
specific variances, and the conventional lower-triangular constraint on
Lambda that makes the decomposition identifiable, which gives
n_env*k - k(k-1)/2 + n_env free parameters by direct count.  Nothing
beyond that statement is assumed, and no equation is attributed to a
