...
def mirt_2d_compensatory(y, theta, a, d, c=0.0, D=1.0):
    ...
    for i in range(n):
        ...
        for j in range(m):
            z += av[j] * th[i][j]
        # k.sigmoid is the shared numerically stable logistic: writing
        # 1/(1 + exp(-t)) directly overflows at large negative t, where
        # R silently returns 0 and Python raises.
        pi = cc + (1.0 - cc) * k.sigmoid(Dm * z)
        p.append(pi)
