...
def gibbons_continuity_corr(T, mu, sigma, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Continuity correction for normal approximation to discrete statistics"})
    x_sorted = np.sort(T)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Continuity correction for normal approximation to discrete statistics"})
