...
def evt_heffernan_tawn(X, u):
    ...
    for i in range(41):
        ...
        for j in range(41):
            ...
            # a tolerance, not a bare <: the likelihood is flat in b near
            # the optimum, so a last-bit difference between the two
            # language arms flips a comparison and walks them onto
            # answers 1e-8 apart.  Ties break toward the lower coordinate
            # in both arms.
            if f < best[0] - _TIE * (1.0 + abs(best[0])):
                best = (f, a, b)
    a, b = best[1], best[2]
    da = (a_hi - a_lo) / 40.0
    db = (b_hi - b_lo) / 40.0
    ...
    for _ in range(60):
        ...
        for _ in range(40):
            c = hi - gr * (hi - lo)
            d = lo + gr * (hi - lo)
            fc = _ht_nll(xv, yv, c, b)[0]
            fd = _ht_nll(xv, yv, d, b)[0]
            if fc < fd + _TIE * (1.0 + abs(fd)):
                hi = d
            else:
                lo = c
        a = 0.5 * (lo + hi)
        ...
        for _ in range(40):
            c = hi - gr * (hi - lo)
            d = lo + gr * (hi - lo)
            fc = _ht_nll(xv, yv, a, c)[0]
            fd = _ht_nll(xv, yv, a, d)[0]
            if fc < fd + _TIE * (1.0 + abs(fd)):
                hi = d
            else:
                lo = c
