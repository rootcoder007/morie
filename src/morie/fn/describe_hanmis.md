# describe('hanmis') — handle_missing

## WHAT IT DOES

Missing data handler. 'Our greatest glory is not in never falling, but in rising every time we fall. — Confucius'
...
`print(result)` render. See `morie.fn.describe('')` for guidance.
...
import numpy as np
# See the function signature in fn/.py for argument names.
```
