...
def dinocenter(g_t, C=None, m=0.9, tau_t=0.04):
    """DINO teacher centring and sharpening.

    Centring alone pushes the teacher toward the uniform distribution and sharpening alone pushes it toward a one-hot; either on its own collapses, and it is the two together that hold the output away from both degenerate fixed points. The centre used for the current batch is the incoming one and the update is returned separately, which is the order the paper's pseudocode uses -- applying the freshly updated centre to the same batch is a different and subtly self-referential algorithm.
    ...
    C : array-like, optional
        Current centre of length K; zeros if omitted.
    m : float
        Centre EMA rate.
    tau_t : float
        Teacher temperature.
