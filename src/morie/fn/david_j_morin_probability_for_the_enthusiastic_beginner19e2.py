"""Deprecated alias for :func:`morie.fn.lsqfit`.

The book-coordinate name is kept so existing code keeps working.  It warns
once and forwards to the method-named function.
"""

import warnings

from .lsqfit import lsqfit as _impl

__all__ = ["david_j_morin_probability_for_the_enthusiastic_beginner_chapter_19_equation_2"]


def david_j_morin_probability_for_the_enthusiastic_beginner_chapter_19_equation_2(x=None, y=None):
    """Deprecated; use :func:`morie.fn.lsqfit` instead."""
    warnings.warn(
        "david_j_morin_probability_for_the_enthusiastic_beginner_chapter_19_equation_2() is the book-coordinate name for lsqfit(); "
        "it will be removed. Use morie.fn.lsqfit() instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    return _impl(x, y)


def cheatsheet() -> str:
    return "david_j_morin_probability_for_the_enthusiastic_beginner19e2: david_j_morin_probability_for_the_enthusiastic_beginner_chapter_19_equation_2(x, y) -> Deprecated; use :func:`morie.fn.lsqfit` instead."
