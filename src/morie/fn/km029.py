...
def kamath_ch2_sbo_loss(x, S):
    """L = -(1/|S|) sum over the span positions S of log P.

    ``x`` holds the model's probability of the TRUE token at every
    position; the index set selects which positions the loss reads.
    The book conditions on the span boundary states f(x_s-1, x_e+1, p); operationally the caller's model has already produced the per-position probabilities under that conditioning. The optional third argument p is accepted and recorded. A probability of 0 at a scored position gives infinite
    loss, which is the mathematics and is returned, not clipped.

    References: Kamath, Keenan, Somers and Sorenson (2024), *Large
    Language Models: A Deep Dive*, Springer, Ch 2, Eq 2.29, printed
    p. 54.

    Examples
    --------
    >>> import math
    >>> out = kamath_ch2_sbo_loss([0.5, 1.0, 0.25], [0, 2])
    >>> abs(out["estimate"] - (math.log(2) + math.log(4)) / 2) < 1e-12
    True
    """
    p = _validate_probs(x, "x")
    idx = np.atleast_1d(np.asarray(S)).astype(int)
    if len(idx) == 0:
    ...
    return RichResult(payload={
        ...
        "method": "span boundary objective (SBO) loss (Kamath Eq 2.29)"})


def cheatsheet():
    return "km029: -mean log P over the span positions S"
