...
def shuffle_model(epsilon0, n, delta):
    ...
    logd = math.log(1.0 / d)
    e1 = 2.0 * math.exp(2.0 * e0) * (math.exp(e0) - 1.0) / nn
    general = e1 * math.sqrt(2.0 * nn * logd) + nn * e1 * (math.exp(e1) - 1.0)

    refined = math.exp(2.0 * e0) * (math.exp(e0) - 1.0) * math.sqrt(8.0 * logd / nn) \
        + 6.0 * math.exp(4.0 * e0) * (math.exp(e0) - 1.0) ** 2 / nn
