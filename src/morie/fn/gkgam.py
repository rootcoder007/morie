...
def gkgam(x: Union[Sequence, np.ndarray], y: Union[Sequence, np.ndarray]):
    ...
    return RichResult(
        ...
        summary_lines=[
            ...
            (
                "Strength",
                "negligible" if abs(g) < 0.1 else "weak" if abs(g) < 0.3 else "moderate" if abs(g) < 0.5 else "strong",
            ),
        ],
        warnings=[]
        if nt < (nc + nd)
        ...
        interpretation=(
            f"gamma={g:+.3f}; positive = concordant, negative = discordant. "
...
def cheatsheet() -> str:
    return "gkgam: gkgam(x, y) -> Goodman-Kruskal gamma - ordinal association ignoring ties."
