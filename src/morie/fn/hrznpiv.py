...
def horowitz_npiv_model(x, y, w, bandwidth=None, grid=25, tol=1e-8):
    ...
    bandwidth : float, optional
        Kernel bandwidth on the [0, 1] scale; default ``n ** (-1/6)``.
    grid : int, default 25
        Number of quadrature points on [0, 1].
    tol : float, default 1e-8
        Relative eigenvalue cutoff for the truncated inverse.

    Returns
    -------
    RichResult
        keys: ``g_hat``, ``grid_points``, ``r_hat``, ``eigenvalues``,
        ``trace_T``, ``n_terms``, ``identified``, ``bandwidth``, ``n``,
        ``m``, ``method``.
    ...
    h = float(bandwidth) if bandwidth is not None else n ** (-1.0 / 6.0)
    if h <= 0:
