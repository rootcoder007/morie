"""Deprecated alias for :func:`morie.fn.sdcoinsum`.

The book-coordinate name is kept so existing code keeps working.  It warns
once and forwards to the method-named function.
"""

import warnings

from .sdcoinsum import sdcoinsum as _impl

__all__ = ["david_j_morin_probability_for_the_enthusiastic_beginner_chapter_3_equation_48"]


def david_j_morin_probability_for_the_enthusiastic_beginner_chapter_3_equation_48(n):
    """Deprecated; use :func:`morie.fn.sdcoinsum` instead."""
    warnings.warn(
        "david_j_morin_probability_for_the_enthusiastic_beginner_chapter_3_equation_48() is the book-coordinate name for sdcoinsum(); "
        "it will be removed. Use morie.fn.sdcoinsum() instead.",
        DeprecationWarning,
        stacklevel=2,
    )
    return _impl(n)


def cheatsheet() -> str:
    return "david_j_morin_probability_for_the_enthusiastic_beginner3e48: david_j_morin_probability_for_the_enthusiastic_beginner_chapter_3_equation_48(n) -> Deprecated; use :func:`morie.fn.sdcoinsum` instead."
