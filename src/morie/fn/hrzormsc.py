...
def horowitz_ordered_max_score(x, y):
    ...
    return RichResult(payload={"estimate": float(estimate), "se": float(se), "ci_lower": float(ci_lower), "ci_upper": float(ci_upper), "n": n, "method": "Ordered-response maximum-score estimator"})
