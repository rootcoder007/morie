...
def fauzi_wilcoxon_moments(n, bandwidth, theta, cdf, density):
    """
    Mean and variance of smoothed Wilcoxon statistic

    Formula: E[W_tilde]=n(n+1)/2*{G(theta)+O(h^2)}; Var[W_tilde]=n(n+1)^2*{int F^2(u+2theta)f(u)du-G^2+O(h^2)}
...
def cheatsheet():
    return "fzwm: Mean and variance of smoothed Wilcoxon statistic"
