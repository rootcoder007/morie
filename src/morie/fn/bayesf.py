...
def bayesf(loglik_h1: float, loglik_h0: float, k_h1: int, k_h0: int, n: int):
    ...
    if bf > 100:
        strength = "decisive in favor of H1"
    elif bf > 30:
        strength = "very strong in favor of H1"
    elif bf > 10:
        strength = "strong in favor of H1"
    elif bf > 3:
        strength = "substantial in favor of H1"
    elif bf > 1:
        strength = "weak in favor of H1"
    elif bf > 0.33:
        strength = "weak in favor of H0"
    elif bf > 0.1:
        strength = "substantial in favor of H0"
    elif bf > 0.033:
        strength = "strong in favor of H0"
    elif bf > 0.01:
        strength = "very strong in favor of H0"
    else:
        strength = "decisive in favor of H0"
    return RichResult(
        title="Bayes factor (BIC approximation)",
        summary_lines=[
            ("BF_10", bf),
            ("log10(BF_10)", math.log10(bf) if bf > 0 else float("-inf")),
