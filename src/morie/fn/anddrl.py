...
def anddrl(x: Union[Sequence, np.ndarray]):
    ...
    return RichResult(
        ...
        tables=[
            {
                "title": "Critical values (right-tail):",
                "headers": ["Significance level", "Critical value", "At this level"],
                "rows": crit_rows,
            }
        ],
        warnings=[] if a.size <= 5000 else [f"n={a.size} very large; tiny deviations yield large A^2."],
