...
def gibbons_median_ties(x, y, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Handling ties at combined median in two-sample median test"})
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Handling ties at combined median in two-sample median test"})
