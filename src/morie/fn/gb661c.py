...
def gibbons_mw_ci(x, y, alpha, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Confidence interval for location shift from Mann-Whitney test"})
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Confidence interval for location shift from Mann-Whitney test"})
