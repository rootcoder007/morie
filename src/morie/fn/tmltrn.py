...
def tmle_transportability(y, D, X, S):
    ...
    pb = S.glmbin(W, Sv)
    p = [S.clip(S.expit(C.dot(W[i], pb)), 0.025, 0.975) for i in range(n)]
    gb = S.glmbin([W[i] for i in src], [Dv[i] for i in src])
    g = [S.clip(S.expit(C.dot(W[i], gb)), 0.025, 0.975) for i in range(n)]
    qb, _, _, _ = S.ols([[Dv[i]] + list(W[i]) for i in src], [yv[i] for i in src])
