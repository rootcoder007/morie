...
def _lchoose(n, k):
    """log C(n, k) as a sum of logs -- exact-order arithmetic in both arms."""
    tot = 0.0
    for j in range(1, int(k) + 1):
        tot += float(np.log((n - k + j) / j))
    return tot
...
def _nll(x, n, lgc, mu, sigma, nodes, wts):
    ...
    for i in range(len(x)):
        ...
        for q in range(len(nodes)):
            ...
            acc += wts[q] * float(np.exp(x[i] * lp + (n[i] - x[i]) * lq))
        tot += lgc[i] + (float(np.log(acc / _SQRTPI)) if acc > 0.0 else -700.0)
    return -tot
...
def ma_glmm_ipd_proportion(xi, ni, quad=21, level=0.95):
    ...
    lgc = [_lchoose(n[i], x[i]) for i in range(len(x))]
