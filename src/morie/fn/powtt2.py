...
def powtt2(d: float, n1: int, n2: int, alpha: float = 0.05):
    ...
    return RichResult(
        ...
        warnings=[]
        if pw >= 0.8
        else [f"power={pw:.2f} < 0.80 conventional minimum; consider larger n or use `npowtt` to size for power=0.80."],
        interpretation=(
            f"At d={d}, n1={n1}, n2={n2}, alpha={alpha}: probability of detecting effect = {pw * 100:.1f}%."
        ),
