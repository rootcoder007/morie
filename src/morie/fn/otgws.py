...
def ot_gromov_sinkhorn(Cx, Cy, a, b, epsilon, max_iter=20, inner_iter=200):
    ...
    stationary point.  When the two spaces share a symmetry and the plan
    is started at ``a b'``, which is itself symmetric, the iteration cannot
    break the tie and settles on the symmetric average of the two optima
    rather than on either of them; that is a property of the scheme, not a
    defect of this code, and it is why the cost is reported alongside the
    plan.
