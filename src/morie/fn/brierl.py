...
def brierl(p_pred: Union[Sequence, np.ndarray], y_true: Union[Sequence, np.ndarray]):
    ...
    return RichResult(
        ...
        warnings=[] if 0 <= base_rate <= 1 else ["y_true has values outside [0,1] - check encoding."],
        interpretation=(
        ...
        payload={"value": bs, "statistic": bs, "skill_score": skill, "baseline": bs_baseline},
    )
