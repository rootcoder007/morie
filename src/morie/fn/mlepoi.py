...
def mlepoi(counts: Union[Sequence, np.ndarray]):
    ...
    log_lik = (
        float(
            np.sum(
                a * np.log(lam)
                - lam
                - np.log(np.array([np.math.factorial(int(x)) if x < 21 else 0 for x in a], dtype=float))
            )
        )
        if lam > 0 and np.all(a == np.round(a)) and a.max() < 21
        else float("nan")
    )
    warnings = []
    if not np.isnan(overdisp) and overdisp > 1.5:
        warnings.append(
            f"overdispersion: var/mean = {overdisp:.2f} > 1.5; "
    ...
    return RichResult(
        ...
        summary_lines=[
            ...
            ("n", int(a.size)),
            ("Min observed", int(a.min())),
            ("Max observed", int(a.max())),
        ],
        warnings=warnings,
