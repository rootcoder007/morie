...
def gibbons_terry_hoeffding(x, y, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Terry-Hoeffding (normal scores) test for location using expected normal order statistics"})
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Terry-Hoeffding (normal scores) test for location using expected normal order statistics"})
