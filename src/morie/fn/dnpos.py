...
"""Density of ideal points for positive-weight respondents."""
...
def density_positive_weights(positions, weights, n_points: int = 100) -> DescriptiveResult:
    """Kernel density estimate for respondents with positive A-M weights.
    ...
    pos_sub = pos[mask]
    if len(pos_sub) < 2:
        return DescriptiveResult(name="density_positive_weights", value=0, extra={"grid": [], "density": [], "n": 0})
    ...
    return DescriptiveResult(
        name="density_positive_weights",
        value=int(mask.sum()),
        extra={"grid": grid.tolist(), "density": density.tolist(), "n": int(mask.sum()), "bandwidth": float(bw)},
    )


dnpos = density_positive_weights


def cheatsheet() -> str:
    return "density_positive_weights({}) -> Density of ideal points for positive-weight respondents."
