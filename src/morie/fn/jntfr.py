...
F the Gamma(1/theta, 1/theta) distribution function evaluated through
the Wilson-Hilferty approximation.  The rule is fixed, so both language
...
def _nodes(theta):
    """Midpoint probability grid pushed through the Gamma(1/th, 1/th) quantile."""
    k = 1.0 / theta
    xs = []
    ws = []
    for i in range(_NQ):
        u = (i + 0.5) / _NQ
        # Wilson-Hilferty inverse for the gamma quantile
        z = core.qnorm(u)
