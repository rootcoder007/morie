...
fixed-length coordinate golden-section search with a guarded
comparison, so that a flat optimum cannot send the two language arms
...
def _golden(f, lo, hi, iters=40):
    ...
    for _ in range(iters):
        # Guarded comparison: near the optimum the objective is flat and the
        # two language arms differ by an ulp or two, which would otherwise
        # send them into different sub-intervals.  Anything inside the guard
        # is treated as a tie and resolved the same way in both arms.
        if fc - fd > 1e-10 * (1.0 + abs(fc) + abs(fd)):
            hi, d, fd = d, c, fc
            c = hi - g * (hi - lo)
            fc = f(c)
        else:
