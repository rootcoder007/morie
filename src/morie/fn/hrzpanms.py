...
def horowitz_panel_max_score(x, y, n_periods):
    ...
    if n < 1:
        return RichResult(
            payload={"estimate": np.nan, "n": 0, "method": "Maximum-score estimator for panel data with fixed effects"}
        )
    ...
    return RichResult(
        payload={
            ...
            "n": n,
            "method": "Maximum-score estimator for panel data with fixed effects",
        }
    )
