...
def _k_model2(r, v, lam):
    ...
    # tiny hard-core radius produces, 1 - exp(-t) loses most of its
    # significant digits to cancellation, and (3.6.9) is a ratio of two such
    # differences, so the error does not divide out.
    num = 2.0 * U * (-math.expm1(-lam * gam)) - 2.0 * gam * (-math.expm1(-lam * U))
