...
def aurroc(y_true: Union[Sequence, np.ndarray], score: Union[Sequence, np.ndarray]):
    ...
    if auc >= 0.9:
        ...
        bench = "no better than chance"
    warnings = []
    pos = int(np.sum(y == 1))
    neg = int(np.sum(y == 0))
    if min(pos, neg) < 10:
        warnings.append(
            f"class imbalance: {pos} positives, {neg} negatives - AUROC unstable; consider stratified bootstrap CI."
        )
    return RichResult(
