"""Q_within for the analog-to-the-ANOVA.

Book-as-spec implementation; see reference for context.
"""

import math as _math  # noqa: F401

from . import _ca_crim
from ._richresult import RichResult

__all__ = ["ca_chapter_11_equation_45"]


def ca_chapter_11_equation_45(ys_by_group, ws_by_group):
    """Q_within for the analog-to-the-ANOVA

    Formula: Q_within = sum_j sum_i w_ij (y_ij - ybar_j)^2

    Returns
    -------
    result : RichResult
        dict subclass; headline key 'q_within' plus the full payload.

    References
    ----------
    Weisburd, Wilson, Wooditch & Britt (2022). Advanced Statistics in Criminology and Criminal Justice, 5th ed. Springer. doi:10.1007/978-3-030-67738-1,
    ch.11 eq.11.45
    """
    payload = dict(_ca_crim.q_within_between(ys_by_group, ws_by_group))
    value = payload['q_within']
    summary = [(k, v) for k, v in payload.items()
               if isinstance(v, (int, float))][:4]
    payload = dict(payload)
    payload.setdefault("value", value)
    payload["method"] = "Weisburd et al. (2022) eq. (11.45)"
    return RichResult(
        title='Q_within for the analog-to-the-ANOVA',
        summary_lines=summary,
        payload=payload,
    )


def cheatsheet():
    return 'ca11e45: Q_within = sum_j sum_i w_ij (y_ij - ybar_j)^2 [Weisburd et al. 2022, eq. 11.45]'
