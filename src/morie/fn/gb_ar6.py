...
def gibbons_are_unif(distribution, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "ARE of Wilcoxon rank-sum vs t-test under uniform distribution: 1.0"})
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "ARE of Wilcoxon rank-sum vs t-test under uniform distribution: 1.0"})
