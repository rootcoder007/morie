    ...
    for _ in range(_LEVELS):
        ...
        for i in range(_NGRID):
            ...
            for j in range(_NGRID):
                v = a2 + j * s2
                f = negll(u, v)
                # Strict-improvement threshold, not `<`. The two language
                # arms evaluate this likelihood to ~1e-15 of each other;
                # on a flat ridge a bare `<` lets that last bit decide the
                # argmin and the arms walk to adjacent lattice points. A
                # 1e-12 margin means only a real improvement moves the
                # incumbent, and both arms scan in the same order, so both
                # keep the same point.
                if f < best[0] - 1e-12:
                    best = (f, u, v)
