...
def gibbons_ks_dist_free(x, F0, n, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "KS statistics D_n, D+_n, D-_n are distribution-free under H0"})
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "KS statistics D_n, D+_n, D-_n are distribution-free under H0"})
