...
def _wls_fit(X, y, w, n, p):
    A = [[sum(w[i] * X[i][r] * X[i][s] for i in range(n)) for s in range(p)]
         for r in range(p)]
    bvec = [sum(w[i] * X[i][r] * y[i] for i in range(n)) for r in range(p)]
    beta = core.ridgesolve(A, bvec, 1e-12)
    cols = []
    for j in range(p):
...
def ma_network_lme(yi, vi, design):
    ...
    X, treats, T = _net_design(design)
    if len(X) != n:
        raise ValueError("design must have one row per study")
    p = T - 1
    w0 = [1.0 / t for t in v]
    b0, _ = _wls_fit(X, y, w0, n, p)
