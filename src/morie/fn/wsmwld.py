...
def wasserman_wald(data, f, theta0, cdf=None):
    ...
    n = len(data)
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Wald test (theta_hat - theta_0)/se ~ N(0,1)"})
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Wald test (theta_hat - theta_0)/se ~ N(0,1)"})
