...
def variogram_fit(coords, values, model="exponential", n_bins=15,
    ...
    The range is confined to the profiling grid, ``[hmax/20, 2*hmax]``
    where ``hmax`` is the largest binned lag. A weakly structured sample
    can push the optimum onto an endpoint, in which case the fitted range
    means only "at least this large" (or "at most"); ``range_at_bound``
    flags exactly that, and ``grid_lo`` / ``grid_hi`` say where the wall
    is. Do not read a boundary range as an estimate.
    ...
    return RichResult(
        payload={
            ...
            "fitted": fitted,
            "wss": wss,
            "range_at_bound": bool(range_at_bound),
            "grid_lo": lo_a,
            "grid_hi": hi_a,
            "n_bins_used": len(use),
