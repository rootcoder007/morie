...
def gibbons_kw_mult_comp(groups, alpha, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Multiple comparisons following Kruskal-Wallis test: pairwise rank-sum tests"})
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Multiple comparisons following Kruskal-Wallis test: pairwise rank-sum tests"})
