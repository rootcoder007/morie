...
def schabenberger_moran_i_residuals(residuals, w, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Moran's I test on OLS residuals for spatial autocorrelation"})
    x_sorted = np.sort(w)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Moran's I test on OLS residuals for spatial autocorrelation"})
