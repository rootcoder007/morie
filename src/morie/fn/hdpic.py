...
def highest_density_credible_interval(samples, alpha=0.05):
    ...
    # spans ceil((1 - alpha) n) order statistics -- that is
    # ceil((1 - alpha) n) - 1 index steps.  Taking floor((1 - alpha) n)
    # STEPS instead, as a careless reading of the estimator invites, makes
    # the window cover one point too many and it then comes out wider than
    # the equal-tailed interval at the same alpha, which is the wrong way
    # round for an HPD region.
    want = int(math.ceil((1.0 - a) * n - 1e-12))
    if want < 2:
        want = 2
    if want > n:
        want = n
