...
def cosine_lr_schedule(x, lr_max: float = 1e-3, lr_min: float = 0.0, total_steps: int = 1000, warmup_steps: int = 0):
    ...
    lr[~warm] = lr_min + 0.5 * (lr_max - lr_min) * (1.0 + np.cos(np.pi * dec[~warm]))
    if scalar:
        lr_out = float(lr[0])
    ...
    return RichResult(
        ...
        summary_lines=[("lr_max", lr_max), ("lr_min", lr_min), ("total", total_steps), ("warmup", warmup_steps)],
        payload={
            "value": lr_out,
            "tensor": tensor,
            "step": t,
