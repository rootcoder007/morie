...
def diffd(y_treated_pre: float, y_treated_post: float, y_control_pre: float, y_control_post: float):
    ...
    return RichResult(
        ...
        interpretation=(
            f"DD = ({treated_change:+.4g}) - ({control_change:+.4g}) "
...
def cheatsheet() -> str:
    return "diffd: diffd(y_treated_pre, y_treated_post, y_control_pre, y_control_post) -> Canonical Difference-in-Differences estimator (2x2)."
