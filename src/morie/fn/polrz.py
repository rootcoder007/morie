...
def polarization_index(x, group=None):
    ...
    n = int(x.size)
    if n < 2:
        return RichResult(payload={"estimate": np.nan, "n": n, "method": "polarization_index"})
    if group is None:
        med = np.median(x)
        g = (x >= med).astype(int)
    else:
    ...
    pooled = np.sqrt(((nR - 1) * sR**2 + (nD - 1) * sD**2) / max(nR + nD - 2, 1))
    ...
    return RichResult(
        ...
        summary_lines=[
            ("|mean(R) - mean(D)|", abs(mR - mD)),
            ("pooled SD", float(pooled)),
            ("Polarization P", float(pol) if np.isfinite(pol) else pol),
            ("n_R / n_D", f"{nR}/{nD}"),
        ],
        interpretation=(
        ...
        payload={
            "estimate": float(pol) if np.isfinite(pol) else np.nan,
            "mean_R": mR,
            "mean_D": mD,
            "sd_R": sR,
            "sd_D": sD,
