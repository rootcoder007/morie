...
def gibbons_page_test(data, k, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Page's test for ordered alternatives in two-way layout"})
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Page's test for ordered alternatives in two-way layout"})
