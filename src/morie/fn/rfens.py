...
def random_forest_ensemble(
    x, y, *, n_estimators=100, max_depth=None, task="auto", seed=0, deterministic_seed: int | None = None
):
    ...
    if task == "classification":
        clf = RandomForestClassifier(
            n_estimators=n_estimators, max_depth=max_depth, random_state=rs, oob_score=True, bootstrap=True
        )
        clf.fit(X, y)
