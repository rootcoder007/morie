...
def ghosal_moment_prior(x, cdf=None):
    ...
    if n < 2:
        return RichResult(payload={"statistic": np.nan, "p_value": np.nan, "n": n, "method": "Prior construction via moments: specify E[int f dG] for test functions f"})
    x_sorted = np.sort(x)
    if cdf is None:
    ...
    return RichResult(payload={"statistic": float(statistic), "p_value": float(p_value), "n": n, "method": "Prior construction via moments: specify E[int f dG] for test functions f"})
