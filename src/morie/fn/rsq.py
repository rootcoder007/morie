...
def rsq(y_true: Union[Sequence[float], np.ndarray], y_pred: Union[Sequence[float], np.ndarray]):
    """R^2 = 1 - SS_res / SS_tot."""
    ...
    if r2 >= 0.9:
        ...
        bench = "worse than baseline"
    warnings = []
    if r2 < 0:
        warnings.append(
            f"R^2 = {r2:.3f} < 0: model is WORSE than predicting the mean. Check for parameter issues or data leakage."
        )
    return RichResult(
