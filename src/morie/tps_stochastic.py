...
def _date_series(df: pd.DataFrame, *, min_year: int = 2014) -> pd.Series:
...
def hawkes_temporal_fit(df: pd.DataFrame, *, ds_name: str = "?", max_n: int = 5000) -> RichResult:
    ...
    dt = _date_series(df)
    if dt.size < 100:
        return RichResult(title=f"Hawkes -- {ds_name}", warnings=[f"only {dt.size} timestamps"])
    if dt.size > max_n:
    ...
    try:
        ...
        ax[0].plot(df_t.index, df_t.values, lw=1, color="#3584e4", label="monthly count")
        ax[0].axhline(mu * 30, color="#e66100", ls="--", label=f"μ·30days = {mu * 30:.1f}")
    ...
    return RichResult(
        ...
        interpretation=(
            ...
            + (
                ...
                if kappa < 1
                else "κ ≥ 1 ⇒ EXPLOSIVE -- fit may be unstable; treat with care."
            )
        ),
        payload={
            "mu": float(mu),
...
def sarima_forecast(
    ...
    *,
    ...
    dt = _date_series(df)
    if dt.size < 36:
        return RichResult(title=f"SARIMA -- {ds_name}", warnings=[f"need ≥36 months, got {dt.size}"])
    monthly = dt.dt.to_period("M").value_counts().sort_index()
    monthly.index = monthly.index.to_timestamp()
    if monthly.size < 36:
        return RichResult(title=f"SARIMA -- {ds_name}", warnings=[f"only {monthly.size} months"])
    ...
    try:
        ...
        return RichResult(title=f"SARIMA -- {ds_name}", warnings=[f"fit failed: {e!r}"])
    # Metrics
    ...
    try:
        ...
        ax.plot(test.index, fc, color="#e66100", lw=2, label=f"SARIMA forecast (h={h})")
        ax.axvline(train.index[-1], color="#5e5c64", ls="--", alpha=0.5)
        ax.set_title(f"{ds_name} -- SARIMA{order}×{seasonal}, MAPE={mape:.1f}%")
        ...
        tables=[
            {
                ...
                "rows": [
                    [str(d.date()), int(a), round(float(f), 1), round(float(a - f), 1)]
...
def langevin_simulate(
    df: pd.DataFrame, *, ds_name: str = "?", n_paths: int = 100, T_days: int = 365, dt: float = 1.0, seed: int = 42
) -> RichResult:
    ...
    if dt_ser.size < 60:
        return RichResult(title=f"Langevin OU -- {ds_name}", warnings=[f"only {dt_ser.size} timestamps"])
    ...
    for k in range(1, n_steps):
        ...
        paths[:, k] = paths[:, k - 1] + theta * (mu - paths[:, k - 1]) * dt + sigma * sqrtdt * z
    ...
    try:
        ...
        future = pd.date_range(daily.index[-1], periods=n_steps, freq="D")
        ...
        ax.fill_between(future, lo, hi, color="#e66100", alpha=0.2, label="5–95% envelope")
        ax.axvline(daily.index[-1], color="#5e5c64", ls="--", alpha=0.5)
        ax.set_title(f"{ds_name} -- Langevin OU simulation (θ={theta:.3f}, μ={mu:.1f}, σ={sigma:.1f})")
    ...
    return RichResult(
        ...
        summary_lines=[
            ...
            ("Half-life of shocks (days)", round(math.log(2) / max(theta, 1e-6), 2)),
            ("Simulated paths", n_paths),
            ("Forecast horizon (days)", T_days),
            ("Final day p5 / median / p95", f"{quantiles[0]:.1f} / {quantiles[1]:.1f} / {quantiles[2]:.1f}"),
        ],
        interpretation=(
...
def fokker_planck_grid(df: pd.DataFrame, *, ds_name: str = "?", n_grid: int = 64, n_steps: int = 200) -> RichResult:
    ...
    dt_ser = _date_series(df)
    if dt_ser.size < 60:
        return RichResult(title=f"Fokker-Planck -- {ds_name}", warnings=[f"only {dt_ser.size} timestamps"])
    daily = dt_ser.dt.to_period("D").value_counts().sort_index()
    x = daily.values.astype(float)
    dx_obs = np.diff(x)
    x_lag = x[:-1]
    ...
    try:
        ...
        ax.plot(grid, p, color="#e66100", lw=2, label=f"FP-evolved (t={n_steps * dt:.1f} days)")
        ax.axvline(mu, color="#5e5c64", ls="--", alpha=0.6, label=f"μ = {mu:.1f}")
    ...
    return RichResult(
        ...
        summary_lines=[
            ...
            ("Stationary peak (grid x)", round(float(grid[p.argmax()]), 2)),
...
def analyze(name: str = "Assault", *, sample_rows: int | None = 50_000) -> dict[str, RichResult]:
