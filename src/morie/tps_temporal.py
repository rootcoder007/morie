...
def year_over_year_trend(df: pd.DataFrame, *, year_col: str = "OCC_YEAR", ds_name: str = "?") -> RichResult:
    """Linear regression of incident counts vs year."""
    if year_col not in df.columns:
        return RichResult(title=f"YoY trend -- {ds_name}", warnings=[f"{year_col} missing"])
    counts = df.groupby(year_col).size().sort_index()
    valid = (
        counts[counts.index.notna() & (counts.index >= 1990) & (counts.index <= 2030)]
        if hasattr(counts.index, "notna")
        else counts
    )
    valid = valid.dropna()
    valid = valid[(valid.index >= 1990) & (valid.index <= 2030)]
    if valid.size < 3:
        return RichResult(title=f"YoY trend -- {ds_name}", warnings=[f"only {valid.size} usable years"])
    ...
    return RichResult(
        ...
        tables=[
            {
                ...
                "rows": [[int(yr), int(c), round(float(f), 1)] for yr, c, f in zip(years, y, y_hat)],
            }
        ],
        interpretation=(
        ...
        payload={"slope": slope, "intercept": intercept, "r2": float(r2), "direction": direction},
    )


def seasonal_pattern(df: pd.DataFrame, *, ds_name: str = "?") -> RichResult:
    ...
    def _cycle(col: str, expected_levels: int, label: str) -> dict | None:
        ...
        try:
            obs = counts.values
            exp = np.full_like(obs, fill_value=obs.sum() / obs.size, dtype=float)
    ...
    for c in [month, dow, hour]:
        if c:
            summary.append((f"{c['label']} chi² uniformity p", round(c["p"], 6)))
            tables.append(
                {
                    "title": f"{c['label']}:",
...
def changepoint_detection(df: pd.DataFrame, *, year_col: str = "OCC_YEAR", ds_name: str = "?") -> RichResult:
    """Simple Pettitt-style change-point on yearly counts."""
    if year_col not in df.columns:
        return RichResult(title=f"Change-point -- {ds_name}", warnings=[f"{year_col} missing"])
    counts = df.groupby(year_col).size().sort_index()
    counts = counts[(counts.index >= 1990) & (counts.index <= 2030)]
    if counts.size < 6:
        return RichResult(title=f"Change-point -- {ds_name}", warnings=[f"need ≥6 years, got {counts.size}"])
    ...
    return RichResult(
        ...
        summary_lines=[
            ...
            ("Δ mean", round(post - pre, 1) if not np.isnan(post) else "n/a"),
        ],
        interpretation=(
...
def arima_forecast(df: pd.DataFrame, *, h: int = 12, ds_name: str = "?") -> RichResult:
    ...
    if "OCC_DATE" in df.columns:
        ...
        return RichResult(title=f"ARIMA -- {ds_name}", warnings=["no OCC_DATE / REPORT_DATE"])
    monthly = dt.dt.to_period("M").value_counts().sort_index()
    monthly.index = monthly.index.to_timestamp()
    if monthly.size < 24:
        return RichResult(title=f"ARIMA -- {ds_name}", warnings=[f"need ≥24 months, got {monthly.size}"])
    try:
        model = sm.tsa.ARIMA(monthly.values.astype(float), order=(1, 1, 1))
        fit = model.fit()
        fc = fit.forecast(steps=h)
    except Exception as e:
        return RichResult(title=f"ARIMA -- {ds_name}", warnings=[f"fit failed: {e!r}"])
    return RichResult(
        title=f"ARIMA(1,1,1) {h}-month forecast -- {ds_name}",
        summary_lines=[
            ("Training months", int(monthly.size)),
            ("Last training month", monthly.index[-1].strftime("%Y-%m")),
        ...
        payload={"forecast": list(map(float, fc)), "aic": float(fit.aic), "bic": float(fit.bic)},
    )
