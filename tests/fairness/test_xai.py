...
def test_permutation_importance_finds_the_used_feature():
    res = xai_permutation_importance(lambda A: A[:, 0], _X(), feature_names=NAMES, seed=1)
...
def test_permutation_importance_flags_protected_feature():
    res = xai_permutation_importance(
        lambda A: A[:, 0], _X(), feature_names=["race", "b", "c"], protected=["race"], seed=1
    )
...
def test_permutation_importance_unused_protected_no_warning():
    ...
    res = xai_permutation_importance(
        lambda A: A[:, 0], _X(), feature_names=["a", "b", "race"], protected=["race"], seed=1
    )
...
def test_partial_dependence_linear_model():
    res = xai_partial_dependence(lambda A: 2.0 * A[:, 0], _X(), "a", feature_names=NAMES, grid_size=12)
...
def test_ale_linear_model_range():
    X = _X()
    res = xai_ale(lambda A: 2.0 * A[:, 0], X, "a", feature_names=NAMES, n_bins=8)
    feat_range = float(X[:, 0].max() - X[:, 0].min())
    assert res.payload["value"] == pytest.approx(2.0 * feat_range, rel=0.05)
...
def test_ceteris_paribus_linear_model():
    X = _X()
    res = xai_ceteris_paribus(lambda A: 3.0 * A[:, 1], X[0], "b", X_ref=X, feature_names=NAMES, grid_size=12)
...
def test_shap_additive_model_sums_to_prediction_gap():
    X = _X(seed=2, n=200)
    res = xai_shap_values(lambda A: A.sum(axis=1), X[0], X, feature_names=NAMES, n_samples=20, seed=3)
...
def test_shap_additive_values_equal_centred_features():
    X = _X(seed=4, n=300)
    x = X[0]
    res = xai_shap_values(lambda A: A.sum(axis=1), x, X, feature_names=NAMES, n_samples=20, seed=5)
...
def test_xai_unknown_feature_name_raises():
    with pytest.raises(ValueError):
        xai_partial_dependence(lambda A: A[:, 0], _X(), "nope", feature_names=NAMES)
