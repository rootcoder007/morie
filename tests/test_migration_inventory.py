...
def test_legacy_to_module_map_covers_all_scripts():
    ...
    for script in LEGACY_MODULE_DOCS:
        assert script in LEGACY_TO_MODULE, f"Legacy script '{script}' has docs mapping but no module mapping"
...
def test_all_21_modules_mapped():
    ...
    assert len(LEGACY_TO_MODULE) >= 21, f"Only {len(LEGACY_TO_MODULE)} mappings, expected 21+"
