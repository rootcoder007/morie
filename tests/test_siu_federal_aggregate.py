...
class TestSprottDoobTableConstants:
    ...
    def test_table19_percentages_sum_to_100(self):
        ...
        total_pct = sum(r["percent"] for r in TABLE19_MANDELA_CLASSIFICATION)
    ...
    def test_table1_iedm_population_n_265(self):
        ...
        for category in ("gender", "age_group", "race", "mental_health"):
            ...
            assert n_sum == 265, f"{category}: expected 265, got {n_sum}"
    ...
    def test_pacific_torture_22_6x_ontario(self):
        ...
        pac = next(r["torture_rate"] for r in TABLE23_REGIONAL_TORTURE_RATES if r["region"] == "Pacific")
        ont = next(r["torture_rate"] for r in TABLE23_REGIONAL_TORTURE_RATES if r["region"] == "Ontario")
        ratio = pac / ont
...
class TestDoobTrends:
    def test_table1_releases_5_year_avg_consistent(self):
        ...
        violent = sum(r["revoke_violent"] for r in CCRSO_TABLE1_RELEASES)
...
class TestMandelaRF:
    ...
    def test_c11_segregation_torture_at_least_5_percent(self):
        ...
        r = analyze_c11_mandela_classification()
        seg_rows = [row for row in r.tables[0]["rows"] if row[1] == "Segregation"]
        assert seg_rows
        for row in seg_rows:
