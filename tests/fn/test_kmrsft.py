"""Tests for kmrsft.kamath_rejection_sampling_finetune."""

from morie.fn import _array_core as np

from morie.fn.kmrsft import kamath_rejection_sampling_finetune


def test_kmrsft_basic():
    """Test basic functionality."""
    prompts = np.random.default_rng(43).normal(0.0, 1.0, (40, 3))
    samples = np.random.default_rng(43).normal(0.0, 1.0, (40, 3))
    rewards = np.random.default_rng(43).normal(0.0, 1.0, (40, 3))
    k = 5
    result = kamath_rejection_sampling_finetune(prompts, samples, rewards, k)
    assert isinstance(result, dict)
    assert "estimate" in result or "retained" in result


def test_kmrsft_edge():
    """Test edge cases."""
    prompts = np.random.default_rng(43).normal(0.0, 1.0, (40, 3))
    samples = np.random.default_rng(43).normal(0.0, 1.0, (40, 3))
    rewards = np.random.default_rng(43).normal(0.0, 1.0, (40, 3))
    k = 5
    result = kamath_rejection_sampling_finetune(prompts, samples, rewards, k)
    assert isinstance(result, dict)
