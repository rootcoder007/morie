...
class TestEmgRmsThreshold:
    def test_basic(self):
        ...
        x = np.concatenate(
            [rng.standard_normal(200) * 0.01, rng.standard_normal(100) * 3.0, rng.standard_normal(200) * 0.01]
        )
    ...
    def test_detects_burst(self):
        rng = np.random.default_rng(42)
        x = np.concatenate(
            [rng.standard_normal(200) * 0.01, rng.standard_normal(100) * 5.0, rng.standard_normal(200) * 0.01]
        )
