"""Verification tests for hedderich8e68.

Hedderich, eq (8.68)-(8.69) -- the deviance residual and its influence form. Every expected value is recomputed in the test body
...
def test_pearson_and_deviance_residuals_for_grouped_binomial_data():
    ...
    n = [10.0, 10.0, 5.0]
    pi = [0.4, 0.6, 0.5]
    res = hedderich_chapter_8_equation_68(y, pi, n=n)
    for i in range(3):
        mu = n[i] * pi[i]
...
def test_influence_is_the_hosmer_lemeshow_deviance_change():
    ...
    pi = [0.4, 0.6]
    hat = [0.2, 0.5]
    res = hedderich_chapter_8_equation_68(y, pi, n=n, hat=hat)
    for i in range(2):
        mu = n[i] * pi[i]
...
def test_influence_exceeds_the_plain_squared_deviance():
    y = [3.0, 7.0]
    res = hedderich_chapter_8_equation_68(y, [0.4, 0.6], n=[10.0, 10.0], hat=[0.2, 0.5])
    for i in range(2):
...
def test_rejects_a_fitted_probability_on_the_boundary():
    with pytest.raises(ValueError):
        hedderich_chapter_8_equation_68([1.0], [1.0])
