...
class TestIDW:
    def test_exact_at_known_point(self):
        ...
        result = inverse_distance_weighting(known_coords, known_values, predict_coords, power=2.0)
    ...
    def test_negative_power_raises(self):
        ...
        with pytest.raises(ValueError, match="power must be"):
            inverse_distance_weighting(np.array([[0, 0]]), np.array([1.0]), np.array([[1, 1]]), power=-1.0)
