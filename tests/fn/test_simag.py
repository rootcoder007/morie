...
class TestSimag:
    ...
    def test_simag_rastrigin(self):
        ...
        x_min = simag(f, np.array([0.0, 0.0]), bounds, T_init=10.0, cooling_rate=0.95, max_iter=2000, seed=42)
        assert f(x_min) < 100
    ...
    def test_simag_full_output(self):
        ...
        bounds = [(-1, 1), (-1, 1)]
        x_min, info = simag(f, np.array([0.5, 0.5]), bounds, max_iter=100, full_output=True, seed=42)
    ...
    def test_simag_temperature_decay(self):
        ...
        x_fast = simag(f, np.array([0.0]), bounds, cooling_rate=0.8, max_iter=500, seed=42)
