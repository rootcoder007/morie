...
from morie.fn.bookadvanced_elementsofstatisticallearning3u564 import (
...
def test_bookadvanced_elementsofstatisticallearning3u564_basic():
    ...
    result = bookadvanced_elementsofstatisticallearning_chapter_3_unnumbered_564(x)
    assert isinstance(result, dict)
    assert "estimate" in result or "statistic" in result


def test_bookadvanced_elementsofstatisticallearning3u564_edge():
    """Test edge cases."""
    x = np.random.default_rng(42).normal(0, 1, 100)
    result = bookadvanced_elementsofstatisticallearning_chapter_3_unnumbered_564(x)
    assert isinstance(result, dict)
