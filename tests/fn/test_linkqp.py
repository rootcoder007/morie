"""Tests for linkqp (Stocking-Lord characteristic-curve linking).

Replaces the generated stub, which imported ``linking_stocking_lord``.
"""

from morie.fn.linkqp import linkqp
...
def test_a_known_rescaling_is_recovered():
    src = _items()
    A, B = 1.6, -0.4
    res = linkqp(src, _rescale(src, A, B))
...
def test_the_symmetric_variant_runs_and_agrees_closely():
    ...
    one = linkqp(src, tgt)
    sym = linkqp(src, tgt, symmetric=True)
