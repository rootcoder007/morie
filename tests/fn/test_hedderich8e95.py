"""Verification tests for hedderich8e95.

Hedderich, eq (8.95) -- the loglinear likelihood-ratio statistic. Every expected value is recomputed in the test body
...
def test_degrees_of_freedom_follow_the_table_shape():
    ...
    assert hedderich_chapter_8_equation_95(table)["df"] == 4


def test_supplying_expected_counts_requires_the_degrees_of_freedom():
    table = [[10.0, 20.0], [30.0, 40.0]]
    with pytest.raises(ValueError):
        hedderich_chapter_8_equation_95(table, expected=[[12.0, 18.0], [28.0, 42.0]])
