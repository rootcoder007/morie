...
def test_flat_series_has_no_trend():
    # every value tied: S = 0 and the Mann-Kendall null variance collapses
    # to 0, so the deviate and its p-value are undefined rather than 0/1.
    res = linear_trend(T, [3.0] * 20)
    assert res["S"] == 0
    assert res["var_S"] == 0.0
    assert res["z"] != res["z"]  # NaN
    assert res["p_value"] != res["p_value"]  # NaN
