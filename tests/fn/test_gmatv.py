...
def test_gvr2_matches_book_worked_example():
    """G_VR2 vs the printed table. Source Method 2, p.51."""
...
def test_gvr1_uses_the_marker_divisor_not_the_line_divisor():
    """G_VR1 = XX'/m, per the printed formula AND the printed R code.
...
def test_gvr3_uses_allele_frequency_scaling_not_sample_sd():
    """G_VR3 scales by sqrt(2p(1-p)), per the printed formula.
    ...
    the worklist.
    """
    got = np.asarray(grm_vanraden(X, method="G_VR3")["estimate"], dtype=float)
    p = X.mean(axis=0) / 2.0
...
@pytest.mark.parametrize("method", [1, 2, 3, "G_VR1", "G_VR2", "G_VR3"])
...
def test_identity_trace_over_n_tracks_average_relationship():
    ...
    For G_VR2 and G_VR3, which are centred on the allele frequencies, this
    sits near 1 for a population in Hardy-Weinberg equilibrium. G_VR1 is
    uncentred so it carries no such normalisation -- asserting the same
    bound on it would be wrong, and that asymmetry is the point.
    """
    for method in ("G_VR2", "G_VR3"):
        G = np.asarray(grm_vanraden(X, method=method)["estimate"], dtype=float)
