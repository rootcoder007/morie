...
def gate_df():
    ...
    effect = np.where(group == "young", 1.0, np.where(group == "middle", 2.0, 3.0))
...
class TestEstimateATT:
    ...
    def test_att_positive_for_positive_effect(self, causal_df):
        ...
        assert abs(result["att"] - 2.0) < 0.5, f"ATT estimate {result['att']:.3f} too far from true value 2.0"
