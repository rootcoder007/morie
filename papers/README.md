...
| **MRM empirical** | [`morie-empirical-paper/main.tex`](morie-empirical-paper/main.tex) | _(in preparation)_ |
...
All five papers carry a `morie version stamp` comment block (right after `\Plainkeywords{}`) declaring the morie public-API version the paper documents. The current stamp is **morie v0.5.0**. The papers will be re-stamped on each major API revision.

## Citing

If you use morie or its findings, cite both software papers (R and Python) plus the framework paper. See the main repo's [`CITATION.cff`](https://github.com/hadesllm/morie/blob/main/CITATION.cff) for machine-readable metadata.
