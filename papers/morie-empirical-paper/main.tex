...
\section[Causal estimates: alert complexity on placement volatility]{Causal estimates: alert complexity on the volatility measure of placements}
\label{sec:dml}
...
carried from prior analyses (MA-thesis values pending re-fit against the maintained TPS Assault CSV at \code{moirais-dev/dev/sphinx/project/data/datasets/TPS/Assault/CSV/Assault_Open_Data_*.csv}).
...
\paragraph*{Verification status.}
verified 2026-05-11 against \texttt{b09\_individuals\_in\_segregation\_number\_of\_times\_in\_segregation.csv} (Hill $\hat\alpha = 2.08$, Gini $= 0.585$, top-5\% concentration $= 25.9\%$) and \texttt{b01\_segregation\_detailed\_dataset.csv} (Cram\'er's $V = 0.189$ for mental-health $\times$ suicide-risk, region $\chi^2 = 285{,}917$). Outputs in \texttt{results/06-goffmanian-churn-b09.txt} and \texttt{results/07-mortification-region-b01.txt}.
