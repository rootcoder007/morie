...
\proglang{R} package is released under GPL-2.0-only to match the
R-ecosystem and CRAN convention; the companion
\proglang{Python} package \citep{Ruhela2026MoriePy} is
dual-licensed under \code{MIT OR Apache-2.0} (the Rust-ecosystem
convention) so downstream Python users are not forced into
copyleft.  The package supports a
...
\begin{Code}
R> library("morie")
R> data <- otis_load_a01_rcdd(year = 2025)
R> head(data)
\end{Code}
...
+   d     = "D",
+   x     = c("age_band", "gender", "regional_cluster"),
+   data  = data)
R> print(result)
\end{Code}
...
\code{RichResult} (Section~\ref{sec:taxonomy}) that prints a
...
The spatial group provides \code{moran\_i()}, \code{geary\_c()},
and the kernel-density and ripley-K wrappers used by the MRM
spatial-temporal modules.

\subsection{Statistical physics of crime}

This group provides Hawkes process maximum-likelihood fits
(\code{fit\_hawkes()}; exponential, gamma, Weibull and Lomax kernels under both constant and sinusoidal baselines),
...
The psychometrics group provides classical-test-theory estimators
(\code{cronbach\_alpha()}, \code{mcdonald\_omega()},
\code{kuder\_richardson()}) and item-response-theory estimators
(1PL, 2PL, 3PL Rasch family).

\subsection{Cryptography}

The cryptography group provides reference implementations of
...
\code{mrm\_*} prefixed function: \code{mrm\_otis\_load()},
\code{mrm\_siu\_scrape()}, \code{mrm\_tps\_load()},
\code{mrm\_classify\_mandela()}, \code{mrm\_chi2\_doob()}, and
\code{mrm\_hawkes\_tps()}. The functions handle source-specific
...
shipped in \code{inst/extdata/describe/}.  The \code{morie\_describe()}
...
Version 0.1.15 ships twelve new exported callables that compose
...
\paragraph{Animated demo.}  A user can confirm the package is
actually doing work, in the same way DoubleML and the Optuna
training UI do, by running \texttt{python -m morie.demo} after
installing the Python sibling.  The R-only equivalent prints
per-step progress to the console via the \pkg{cli} package.
...
\proglang{Python} parity at the v0.3.0 release (see the package
\code{NEWS} file).
...
deferrals carry over from the v0.2.1 tracker: an opt-in
...
The \proglang{R} package is released on CRAN and via r-universe
under GPL-2.0-only (matching the R-ecosystem convention); the
companion \proglang{Python} package is released on PyPI under
\code{MIT OR Apache-2.0} (the Rust-ecosystem convention) so that
downstream users in either language are not forced to adopt the
copyleft of the other.  The Linux kernel module
(\texttt{kernel-module/morie.c}) ships under GPL-2.0-only (the
kernel ABI requirement); papers and documentation are
\code{CC-BY-4.0}.  The full per-component breakdown is in
