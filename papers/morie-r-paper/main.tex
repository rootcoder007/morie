...
The package is distributed via r-universe at
\href{https://rootcoder007.r-universe.dev/morie}{rootcoder007.r-universe.dev/morie}
and from source on GitHub; it is not on CRAN.
...
When I started analysing Ontario's restrictive-confinement
records I had three problems, not one: the data came from five
different agencies with five different schemas, the methods I
needed lived in five different \proglang{R} packages, and the
result-printing conventions disagreed between them.
\pkg{morie}\footnote{\textit{morie} is pronounced ``MOY-rye'',
like the Greek \textit{Moirai}, the Fates.} is the
\proglang{R} package I built to make those problems disappear
from the analyst's notebook. It exposes a unified estimator
interface --- the average treatment effect (ATE), the average
treatment effect on the treated (ATT), the conditional and
group average treatment effects (CATE, GATE), the local
...
\pkg{DoubleML} classes \citep{Bach2024DoubleML}, which
themselves rely on \pkg{mlr3} \citep{Lang2019mlr3} for the
...
psychometric estimators that complement the causal-inference
core. \pkg{morie} and its \proglang{Python} companion
...
over a network, must publish its source --- a deliberate
choice to keep carceral- and oversight-evidence methodology in
the public record.

\pkg{morie} builds upon a robust ecosystem of foundational
packages: \pkg{DoubleML} \citep{Bach2024DoubleML} for canonical
double-machine-learning estimators, \pkg{MatchIt} for matching,
\pkg{survey} for design-based standard errors, \pkg{hdm}
\citep{ChernozhukovHansenSpindler2016hdm} for high-dimensional
post-selection inference, \pkg{grf} for forest-based
heterogeneous treatment effects, and \pkg{AIPW} for augmented
inverse-probability weighting. Rather than reimplementing
these established methods, \pkg{morie} unifies them under a
single interface and pairs them with MRM modules tailored for
Canadian open carceral and police-oversight data.
...
the \proglang{R} realisation makes the traversal cheap.
...
thirteen empirical-workflow callables added in v0.1.15.
...
The released \pkg{morie} package is not on CRAN; it is distributed
via r-universe. Nightly Linux, macOS, and Windows binaries are built
by r-universe \citep{Ooms2021runiverse} from the GitHub source tree
...
\noindent The \code{morie\_estimate\_irm()} wrapper composes an
...
plain \proglang{R} list with the canonical fields \code{ate},
\code{se}, \code{ci\_lower}, \code{ci\_upper}, \code{n}, and
\code{method}; the full \pkg{DoubleML} object is preserved under
\code{result\$doubleml} for users who want the sample splits,
psi-score arrays, or bootstrap distribution.
...
estimator family: \code{morie\_estimate\_att()},
\code{morie\_estimate\_atc()}, \code{morie\_estimate\_aipw()},
\code{morie\_estimate\_cate()}, \code{morie\_estimate\_gate()},
\code{morie\_estimate\_late()}, and \code{morie\_estimate\_irm()}.
The first four are independently implemented in pure \proglang{R}
using the identifying assumptions described in
\citet{Ruhela2026MRM}; the latter three wrap \pkg{DoubleML} as
detailed in Section~\ref{sec:wrapper}.

\subsection{Survey sampling and weights}

The survey group provides \code{morie\_calculate\_ipw\_weights()},
\code{morie\_calculate\_ebac()} (the Widmark blood-alcohol-content
...
filters; retained as short legacy aliases);
\code{morie\_sgolay\_smooth()} (Savitzky--Golay smoothing);
\code{morie\_hurst\_r()} (Hurst exponent by R/S analysis);
\code{hfd()} (Higuchi fractal dimension; legacy alias); and
\code{morie\_pcg\_filter()} (phonocardiogram band-pass).
...
estimators are planned for a future release; effect-size measures
ship in the current release as \code{morie\_omega\_squared()},
\code{morie\_cohens\_d()}, \code{morie\_hedges\_g()}, and
\code{morie\_eta\_squared()}, together with the spatial IRT
estimator \code{morie\_irt\_spatial()}.

\subsection{Cryptography}

A cryptography subset covering ML-KEM, ML-DSA, NTRU, McEliece,
elliptic-curve and hybrid post-quantum primitives is planned for
a future release; the intent is educational reference
implementations only, not production-audited primitives.

\subsection{Investigation utilities}

The investigation-utility group provides
\code{morie\_compare\_nested\_logistic\_models()} (likelihood-ratio
test across nested specifications),
\code{morie\_run\_treatment\_effects\_analysis()} (end-to-end
ATE/ATT/CATE/GATE estimation pipeline),
\code{morie\_run\_weighted\_logistic\_analysis()} (survey-aware
logistic regression via \pkg{survey}), and
\code{morie\_inspect\_output()} (audit-trail inspection of result
objects).
...
\code{morie\_chi\_square\_test()} for the MRM $\chi^2$ family
(renamed in v0.9.5.4 from the earlier ``Doob $\chi^2$ family''),
\code{mrm\_anova\_oneway()} for one-way ANOVA, and
...
\pkg{morie}'s public API is functional: each
\code{morie\_estimate\_*} call takes a data frame and column-name
arguments and returns a plain \proglang{R} list. The
class machinery sits on the \pkg{DoubleML} side and is reachable
through the \code{result\$doubleml} handle for users who want
sample-split objects, psi-score arrays, or bootstrap
distributions.

\subsection[Return-type conventions]{Return-type conventions}\label{sec:richresult}

Every estimator in \pkg{morie} returns a plain \proglang{R}
named list. The canonical fields are estimator-specific --- for
example, \code{morie\_estimate\_irm()} returns \code{ate},
\code{se}, \code{ci\_lower}, \code{ci\_upper}, \code{n}, and
\code{method}. The only S3 print method currently shipped is
\code{print.morie\_hawkes\_fit} for Hawkes fits; other estimators
print via the default named-list dispatch. The cross-language
story is parity of field names and unit conventions, not parity of
a class hierarchy: the \proglang{Python} sibling's
\code{RichResult} dataclass \citep{Ruhela2026MoriePy} carries the
same canonical fields under the same names. A multi-section S3
print method that mirrors the \proglang{Python} side's narrative
output is planned for a future release.
...
relevant narrative; the R equivalent is not yet exported. Future
usage will follow this pattern:
...
object to a \code{morie\_result} that prints a multi-section
...
R> res_ipw  <- morie_estimate_ate (df, treatment="D",
+                                     outcome="placement_transitions",
+                                     covariates=covs)
R> res_aipw <- morie_estimate_aipw(df, treatment="D",
+                                     outcome="placement_transitions",
+                                     covariates=covs,
+                                     outcome_model="logistic")
R> res_irm  <- morie_estimate_irm (df, treatment="D",
+                                     outcome="placement_transitions",
...
\subsection{Example 2: replication of the MRM chi-square record from Sprott--Doob}

The second example replicates the published $\chi^{2}$ statistics
from \citet{SprottDoob2021} on the federal $\SIU$ aggregate
record. The family was renamed from ``Doob chi-square'' to
``MRM chi-square'' in v0.9.5.4; the Sprott--Doob author citation
that introduced these federal records is preserved. \pkg{morie}
exposes the family as \code{morie\_chi\_square\_test()}, and base
\proglang{R}'s \code{chisq.test()} reproduces every published
statistic to within $0.01$:
...
The current release ships thirteen new exported callables that compose
...
\paragraph{OTIS suite.}  Five callables operate on the Offender
...
\code{morie\_run\_treatment\_effects\_analysis()} with \code{verbose = TRUE}.
...
\midrule
\code{schabenberger} & 20 & \citet{SchabenbergerGotway2005} & \code{sptau()} / \code{morie\_spatial\_autocorrelation()} \\
\code{kosorok} & 20 & \citet{Kosorok2008} & \code{morie\_kosorok\_empirical\_process()} \\
\code{ml\_foundations} & 20 & \citet{MohriRostamizadehTalwalkar2018,HastieTibshiraniFriedman2009} & \code{morie\_linear\_regression\_ols()} \\
\code{ghosal} & 20 & \citet{GhosalVanDerVaart2017} & \code{morie\_ghosal\_dirichlet\_posterior()} \\
\code{horowitz} & 20 & \citet{Horowitz2009} & \code{morie\_horowitz\_kernel\_regression()} \\
\code{montesinos} & 20 & \citet{MontesinosLopez2022} & \code{morie\_gblup\_full()} \\
\code{armstrong} & 20 & \citet{Armstrong2014SpatialVoting} & \code{morie\_ideal\_point\_recovery()} \\
\code{deep\_learning} & 20 & \citet{GoodfellowBengioCourville2016} & \code{morie\_forward\_pass\_dense()} \\
\code{missing\_stats} & 20 & QMC / copulas / EVT & \code{morie\_bootstrap\_ci()} \\
\code{llm\_arch} & 20 & \citet{Vaswani2017Attention,Dao2022FlashAttention,Su2021RoPE,ZhangSennrich2019RMSNorm} & \code{morie\_bpe\_tokenizer()} \\
\code{fauzi} & 15 & \citet{Fauzi2018Kernel} & \code{morie\_fauzi\_kdfe\_properties()} \\
\code{gibbons\_chakraborti} & 20 & \citet{GibbonsChakraborti2014} & \code{morie\_tolerance\_limits()} \\
\code{rangayyan} & 20 & \citet{Rangayyan2015} & \code{morie\_rangayyan\_fir\_filter()} \\
\code{time\_series\_advanced} & 20 & \citet{HyndmanAthanasopoulos2018,Tsay2010FinancialTimeSeries} & \code{morie\_garch\_fit()} \\
\bottomrule
\end{tabular}
\caption{The fourteen textbook-derived suites in \code{morie.fn}.
Total: 275 callables, all with \proglang{R}+\proglang{Python}
parity verified at v0.9.5.5.  The \code{missing\_stats} label
...
under \emph{both} its compact mnemonic and its \code{morie\_*}-
prefixed long form: in the \code{schabenberger} suite, for
instance, both \code{sptau()} and
\code{morie\_spatial\_autocorrelation()} resolve to the same
function. The mnemonic matches the spec \code{name} field of the
originating formula-index JSON and the prefixed long form matches
the \code{full\_name} field (with a \code{morie\_} prefix added
by the v0.9.5 mass-rename), so a user can locate the originating
specification and the cross-language sibling without guessing.  The \proglang{Python} sibling uses the longer form
...
\proglang{R} smoke suite confirms that all 331 \proglang{R}
...
deferrals carry over in the open tracker: an opt-in
...
\pkg{morie} measures bias in risk-assessment and
predictive-policing systems; it does not build them. The
disparity-audit surface (added in v0.8.0) exists because those
systems are already in use, and the question worth asking now
is whether the systems already in use produce disparate
outcomes. Six group-fairness metrics carry that question:
\code{morie\_fairness\_disparate\_impact} (the EEOC four-fifths
rule), \code{morie\_fairness\_demographic\_parity},
\code{morie\_fairness\_equalized\_odds},
\code{morie\_fairness\_average\_odds\_difference},
\code{morie\_fairness\_gini}, and the composite
\code{morie\_fairness\_bias\_amplification} after
\citet{Barman2026PredictivePolicingBias}.  The
\code{morie\_predpol\_calibration\_audit} callable generalises
the Strategic Subjects List district analysis of
\citet{Lacherade2021PredPolChicago}: it ranks areas by predicted
risk against their realised outcome rate and tests whether the
rank disagreement tracks demographic composition, while
\code{morie\_predpol\_temporal\_audit} extends the audit
...
\pkg{morie} pulls Canada's carceral, police, and oversight
data into a single estimator namespace and a single result
idiom. The \pkg{DoubleML} wrappers are thin on purpose ---
most of the value is in the MRM modules and the schema
reconciliation that lets them work together. The honest
limits, as of v0.9.5.5: there is no \code{R6} class layer (an
earlier sketch was removed as needless complexity); there is
no CRAN binary (deliberate, pending a v1.0 freeze); and the
cryptography subset ships on the \proglang{Python} side, with
a native \proglang{R} + Rcpp port planned for v1.0.0. The
\code{describe()} pedagogical narratives now ship on both
sides: \code{morie\_describe()} (and the string-only variant
\code{morie\_describe\_by\_name()}) read the bundled
\code{describe\_corpus.Rds} (36{,}433 entries) and render the
narrative for any morie callable.
...
\url{https://github.com/rootcoder007/morie}.  Contributions follow
the process documented in \code{CONTRIBUTING.md}.

\section*{Acknowledgements}

The author thanks Jeroen Ooms for maintaining the r-universe
infrastructure on which the \pkg{morie} nightly binaries are
built, and the DoubleML team (Philipp Bach, Malte S.\ Kurz,
Victor Chernozhukov, Martin Spindler, Sven Klaassen) for the
BSD-3-Clause double-machine-learning library on which the
package's IRM and PLR wrappers are layered.
