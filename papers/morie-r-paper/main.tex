...
major-crime event series. Fitting itself is offline (handled by
the Python sibling's \code{morie.tps\_hawkes\_advanced} module
under a Numba-JIT'd $O(n)$ recursive likelihood for the exponential
kernel and a parallel $O(n^2)$ JIT path for non-Markovian
kernels); the \proglang{R} side consumes the resulting fitted-parameter
manifest via \code{mrm\_tps\_load\_hawkes\_refit()}:
...
\paragraph{Animated demo.}  A user can confirm the package is actually doing work, in the same
way DoubleML's console output does, by running
\code{morie::run\_treatment\_effects\_analysis(\dots, verbose = TRUE)}
after installing the R sibling. Per-step progress prints to the
console via the \pkg{cli} package; the Python sibling provides an
equivalent animated demo via \texttt{python -m morie.demo}.
