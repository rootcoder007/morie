...
records that the MRM framework integrates.  Version~0.9.5 ships
275 textbook-derived callables in \code{morie.fn} across
fourteen suites with full \proglang{R} parity, and adopts a
licensing model under which the \proglang{Python} and
\proglang{R} packages are both \code{AGPL-3.0-or-later}.
}
...
estimation (\code{morie.tps\_stochastic.hawkes\_temporal\_fit} and the full non-Markovian \code{morie.tps\_hawkes\_advanced.fit\_hawkes\_general}; exponential
...
The psychometrics submodule \code{morie.psymet} provides classical-test-theory
...
\subsection{morie.crypto (forthcoming)}

A cryptography submodule covering ML-KEM, ML-DSA, NTRU, and
hybrid post-quantum primitives is on the v0.7 roadmap; the
intent is educational reference implementations only, not
production-audited primitives. Current development status is
documented in the package's \texttt{ROADMAP.md}.
...
honest deferrals carry over from the v0.3.x tracker: an opt-in
