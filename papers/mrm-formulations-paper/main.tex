...
MORIE\footnote{\textit{morie} is pronounced ``MOY-rye'',
...
\midrule
Federal $\SIU$ 2019--2020 & $\geq 10\%$ & --- & --- & $\approx 28\%$ & $9.9\%$ \citep{SprottDoob2023} \\
...
\proglang{R} sides, so the two language implementations cannot
diverge; the non-Markovian kernels are evaluated either by exact
...
MORIE's accountability mission extends naturally from estimating
...
reference group.  MORIE computes the Disparate Impact Ratio
...
\citet{Lacherade2021PredPolChicago}, MORIE ranks geographic areas by
...
For model-level audits, MORIE reimplements the explainer suite of the
